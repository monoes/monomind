...
```bash
npx monobrain hive-mind init --topology hierarchical-mesh --consensus raft
```

### Spawn Queen
```bash
npx monobrain hive-mind spawn --role queen --name coordinator
```

### Check Consensus Status
```bash
npx monobrain hive-mind consensus --status
```

### View Sessions
```bash
npx monobrain hive-mind sessions --active
```
