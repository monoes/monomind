---
name: "source-command-monograph-monograph-wiki"
description: "Scan all docs and PDFs and build a searchable knowledge graph — optimized for documentation-heavy projects with Codex LLM semantic extraction"
---

# source-command-monograph-monograph-wiki

Use this skill when the user asks to run the migrated source command `monograph-monograph-wiki`.
...
| `--llm` | — | boolean | `false` | Enrich with Codex semantic extraction (requires `ANTHROPIC_API_KEY`) |
...
# Deep enrichment — process 200 sections with Codex
...
With `--llm`, Codex extracts semantic relationships between sections:
