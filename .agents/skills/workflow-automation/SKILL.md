...
### Create Workflow
```bash
npx monobrain workflow create --name "deploy-flow" --template ci
```

### Execute Workflow
```bash
npx monobrain workflow execute --name "deploy-flow" --env production
```

### List Workflows
```bash
npx monobrain workflow list
```

### Export Template
```bash
npx monobrain workflow export --name "deploy-flow" --format yaml
```

### View Status
```bash
npx monobrain workflow status --name "deploy-flow"
```
