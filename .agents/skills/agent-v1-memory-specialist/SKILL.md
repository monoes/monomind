---
name: agent-v1-memory-specialist
description: Agent skill for v1-memory-specialist - invoke with $agent-v1-memory-specialist
---
...
hooks:
pre_execution: |
echo "🧠 v1 Memory Specialist starting memory system unification..."
...
echo "🧠 Memory unification milestone complete"
    ...
    npx agentic-flow@alpha memory store-pattern \
      --session-id "v1-memory-$(date +%s)" \
      --task "Memory Unification: $TASK" \
      --agent "v1-memory-specialist" \
