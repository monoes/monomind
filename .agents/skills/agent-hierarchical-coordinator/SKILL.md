...
hooks:
  pre: |
    ...
    mcp__monobrain__swarm_init hierarchical --maxAgents=10 --strategy=adaptive
    # MANDATORY: Write initial status to coordination namespace
    mcp__monobrain__memory_usage store "swarm$hierarchical$status" "{\"agent\":\"hierarchical-coordinator\",\"status\":\"initializing\",\"timestamp\":$(date +%s),\"topology\":\"hierarchical\"}" --namespace=coordination
    # Set up monitoring
    mcp__monobrain__swarm_monitor --interval=5000 --swarmId="${SWARM_ID}"
  post: |
    ...
    mcp__monobrain__memory_usage store "swarm$hierarchical$complete" "{\"status\":\"complete\",\"agents_used\":$(mcp__monobrain__swarm_status | jq '.agents.total'),\"timestamp\":$(date +%s)}" --namespace=coordination
    # Cleanup resources
    mcp__monobrain__coordination_sync --swarmId="${SWARM_ID}"
---
