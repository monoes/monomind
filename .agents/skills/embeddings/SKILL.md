...
```bash
npx monobrain embeddings init --backend sqlite
```

### Embed Text
```bash
npx monobrain embeddings embed --text "authentication patterns"
```

### Batch Embed
```bash
npx monobrain embeddings batch --file documents.json
```

### Semantic Search
```bash
npx monobrain embeddings search --query "security best practices" --top-k 5
```
...
npx monobrain memory store --key "pattern-1" --value "description" --embed

# Search with embeddings
npx monobrain memory search --query "related patterns" --semantic
```
