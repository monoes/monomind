---
name: "source-command-monograph-monograph-build"
description: "Build the knowledge graph from code, docs (md/txt/rst), and PDFs — full index with optional Codex LLM semantic extraction"
---

# source-command-monograph-monograph-build

Use this skill when the user asks to run the migrated source command `monograph-monograph-build`.
...
| `--llm` | — | boolean | `false` | Enable Codex semantic extraction (requires `ANTHROPIC_API_KEY`) |
...
# With Codex semantic extraction
npx monomind monograph build --llm

# Enrich up to 200 sections with Codex
...
When `--llm` is passed and `ANTHROPIC_API_KEY` is set, Codex extracts typed semantic relationships between sections (e.g. `DESCRIBES`, `CAUSES`, `PART_OF`). These become `INFERRED` edges in the graph, enabling richer semantic search.
