...
### Train Patterns
```bash
npx monobrain neural train --model-type moe --epochs 10
```
...
### View Patterns
```bash
npx monobrain neural patterns --type all
```

### Predict
```bash
npx monobrain neural predict --input "task description"
```

### Optimize
```bash
npx monobrain neural optimize --target latency
```
