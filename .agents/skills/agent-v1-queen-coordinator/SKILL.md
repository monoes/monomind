---
name: agent-v1-queen-coordinator
description: Agent skill for v1-queen-coordinator - invoke with $agent-v1-queen-coordinator
---
...
hooks:
pre_execution: |
echo "👑 v1 Queen Coordinator starting 15-agent swarm orchestration..."

    # Check intelligence status
    npx agentic-flow@alpha hooks intelligence stats --json > $tmp$v1-intel.json 2>$dev$null || echo '{"initialized":false}' > $tmp$v1-intel.json
    echo "🧠 RuVector: $(cat $tmp$v1-intel.json | jq -r '.initialized // false')"
...
- **Agent #15**: Deployment + V1.0.0 release
