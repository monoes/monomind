...
```bash
npx @monobrain/cli memory store --key "[key]" --value "[value]" --namespace patterns
```

**Example:**
```bash
npx @monobrain/cli memory store --key "auth-jwt-pattern" --value "JWT validation with refresh tokens" --namespace patterns
```
...
```bash
npx @monobrain/cli memory search --query "[search terms]" --limit 10
```

**Example:**
```bash
npx @monobrain/cli memory search --query "authentication best practices" --limit 5
```
...
```bash
npx @monobrain/cli memory get --key "[key]" --namespace [namespace]
```

**Example:**
```bash
npx @monobrain/cli memory get --key "auth-jwt-pattern" --namespace patterns
```
...
```bash
npx @monobrain/cli memory list --namespace [namespace]
```

**Example:**
```bash
npx @monobrain/cli memory list --namespace patterns --limit 20
```
...
```bash
npx @monobrain/cli memory delete --key "[key]" --namespace [namespace]
```
...
```bash
npx @monobrain/cli memory init --enable-hnsw
```
...
```bash
npx @monobrain/cli memory export --output memory-backup.json
```
