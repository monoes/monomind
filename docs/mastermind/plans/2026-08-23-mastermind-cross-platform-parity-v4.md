# Mastermind Cross-Platform Parity Implementation Plan (v4 — execution-readiness revision)
...
> 1. OpenClaw has no documented project-local `openclaw.json`; project-scope OpenClaw MCP is now `cli_fallback`. User-scope MCP is emitted only after explicit `--scope user` consent, targeting `~/.openclaw/openclaw.json` (or a user-supplied `OPENCLAW_CONFIG_PATH`).
...
`applyPlan` backs up each mutated file (timestamped, adjacent `.monomind-backup/` dir), routes `managed_block` vs `named_entry` by `replace`, uses the JSONC-tolerant parser for `format: 'jsonc'`, and never rewrites TOML full-file (managed comment block inside, or surgical entry splice preserving comments/order). YAML named-entry mutation is enabled only after the round-trip test above preserves comments and foreign keys; until then the Hermes adapter remains `cli_fallback` and reports the reason.
...
```ts
const CORE_LOCATIONS: Record<PlatformId, { instruction: string; skillRoot?: string; mcp?: { path: string; format: 'json' | 'jsonc' | 'toml' | 'yaml'; entry: readonly string[] } | 'cli_fallback' }> = {
  claude:     { instruction: 'CLAUDE.md',  skillRoot: '.claude/skills',  mcp: { path: '.mcp.json', format: 'json', entry: ['mcpServers', 'monomind'] } },
  gemini:     { instruction: 'GEMINI.md',  skillRoot: '.agents/skills',  mcp: { path: '.gemini/settings.json', format: 'json', entry: ['mcpServers', 'monomind'] } },
  cursor:     { instruction: '.cursor/rules/monomind.mdc', skillRoot: '.agents/skills', mcp: { path: '.cursor/mcp.json', format: 'json', entry: ['mcpServers', 'monomind'] } },
  vscode:     { instruction: '.github/copilot-instructions.md', skillRoot: '.agents/skills', mcp: { path: '.vscode/mcp.json', format: 'json', entry: ['servers', 'monomind'] } },
  copilot:    { instruction: '.github/copilot-instructions.md', skillRoot: '.agents/skills', mcp: 'cli_fallback' },
  opencode:   { instruction: 'AGENTS.md',  skillRoot: '.agents/skills', mcp: { path: 'opencode.json', format: 'jsonc', entry: ['mcp', 'monomind'] } },
  ...
  openclaw:   { instruction: 'AGENTS.md',  skillRoot: '.agents/skills', mcp: 'cli_fallback' }, // user-scope MCP uses ~/.openclaw/openclaw.json only after explicit consent
  droid:      { instruction: 'AGENTS.md',  skillRoot: '.agents/skills', mcp: { path: '.factory/mcp.json', format: 'json', entry: ['mcpServers', 'monomind'] } }, // v2's settings.json was wrong
  ...
  hermes:     { instruction: 'AGENTS.md',  skillRoot: '.agents/skills', mcp: 'cli_fallback' }, // discovery may upgrade only after YAML preservation contract passes
  ...
  kimi:       { instruction: 'AGENTS.md',  skillRoot: '.agents/skills', mcp: { path: '.kimi-code/mcp.json', format: 'json', entry: ['mcpServers', 'monomind'] } }, // AGENTS.md verified in Task 7 fixture
...
Every `cli_fallback` path emits a diagnostic naming `monomind mastermind run --print`, `monomind status --format json`, and `monomind platforms doctor`. OpenClaw additionally states that project-scope MCP is unavailable and that user-scope installation requires `--scope user` or a supplied `OPENCLAW_CONFIG_PATH`.
...
- `runPlatformsDoctor(request): Promise<PlatformDoctorReport>` — implemented in this task before the Task 9 MCP wrapper is registered.
...
Write `__tests__/platform-adapters/doctor.test.ts` in this task. It must prove that JSON output exactly mirrors `PLATFORM_REGISTRY`, reports legacy Codex injection without modifying it, redacts the user home directory, and makes no writes.
