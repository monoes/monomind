# Mastermind Cross-Platform Parity Implementation Plan (v3 — swarm-reviewed)
...
- **Portable skill default:** where a platform's official docs document `.agents/skills` (workspace) or `~/.agents/skills` (user) — Gemini CLI, Cursor, VS Code, Copilot CLI, OpenCode, OpenClaw, Droid, Antigravity, Hermes, Codex, Kimi, Zed — adapters MUST prefer those roots for cross-platform skill delivery. Claude (`~/.claude/skills`, `.claude/skills`), Kiro (`.kiro/skills`), and Aider (no skills) are the exceptions.
...
- Do not claim a native capability where the platform does not provide one, or before a per-platform contract fixture proves it. Surface the capability through MCP or the `monomind` CLI and report the intentional fallback in `monomind platforms doctor`.
- Do not make global (user-scope) changes from project initialization unless the user explicitly passes `--scope user`. Existing user-scope writes performed by project init (statusline scripts, global `~/.claude/CLAUDE.md`, global settings hooks) are migrated to explicit user scope or removed (Tasks 5, 8).
...
| Codex has two incompatible hook schemas — **with a 1000× timeout-unit disagreement.** | `setupCodex` writes `[[hooks]]` with `timeout = 5000` (ms); `codex-generator.ts:111,120` writes `[[hooks.PreToolUse]]` with `timeout = 5` (seconds) — same TOML schema, 1000× apart. | One Codex adapter; timeout unit rendered per platform schema and asserted by test. |
...
| OpenClaw | `openclaw` (alias: `claw`) | Skill precedence: `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.openclaw/skills` (state dir); workspace `AGENTS.md`; `~/.openclaw/openclaw.json` (MCP); plugins; hooks | Full native parity | Never write `.claw/config.md`. Note: OpenClaw docs explicitly state Codex's `$CODEX_HOME/skills` is not an OpenClaw root. [Skills](https://docs.openclaw.ai/tools/skills) |
| Droid | `droid` | `AGENTS.md`, `.factory/skills` (project) / `~/.factory/skills`, **`.factory/mcp.json`** (project) / `~/.factory/mcp.json` (user), `.factory/hooks.json`, plugins | Full native parity (commands/agents `experimental` until fixtures) | Greenfield today (repo has only `DROID.md` legacy) — every native claim gated behind Task 10 fixtures. [Skills](https://docs.factory.ai/harness/skills), [MCP](https://docs.factory.ai/harness/mcp) |
...
Every adapter declares one of `native`, `cli_fallback`, `unsupported`, or `experimental` for each capability. **`experimental` upgrades to `native` only after that platform's Task 10 fixture passes; it never blocks M1 core parity.** An `activation: 'manual-step'` annotation (Kimi plugins) may accompany `native` and is surfaced in doctor.
...
`n` = native, `f` = cli_fallback, `u` = unsupported, `e` = experimental. `reqDisc` = requiresDiscovery.
...
| openclaw | n | n | n | f | f | n | f | n | u | no |
| droid | n | n | n | e | e | n | f | n | u | no |
| antigravity | n | n | n | f | e | n | f | n | u | yes |
...
|---|---|
| `packages/@monomind/cli/src/platform-adapters/types.ts` | Canonical types, capability enum (9 capabilities), install scopes, artifact model, `InstallRequest`, `DiscoveryResult`, diagnostics. |
| `packages/@monomind/cli/src/platform-adapters/registry.ts` | The only platform list: 16 adapters, capability matrix above, legacy aliases (`claw`, `kimicode`), `requiresDiscovery`. |
...
| `packages/@monomind/cli/src/platform-adapters/merge.ts` | Multi-block managed-marker merge/remove (md/toml/yaml), JSON named-entry merge/remove, JSONC-tolerant parse/serialize, legacy bare-marker recognition. |
...
| `write-antigravity.ts`, `geminimd-generator.ts` | Tasks 4/5: fold into the antigravity renderer; user-scope statusline writes move behind `--scope user`; `~/.gemini/skills` writes migrate to Antigravity's real paths. |
| `write-kimicode.ts`, `kimi-generator.ts` | Tasks 4/5/6: fold into the kimi renderer; user-scope statusline writes move behind `--scope user`. |
...
- [ ] **Step 1: Verify the toolchain the plan depends on**
...
- [ ] **Step 2: Record the fixed vocabulary**

Note in the task README: filter string `@monoes/monomindcli`; typecheck = `run build` (tsc); lint = root `pnpm lint` (biome); final gate = `CI=true pnpm test:all` (workspace-wide); focused test = `pnpm --filter @monoes/monomindcli exec vitest run <path>`; package `testTimeout` is 30000ms; tests run under `__tests__/setup/resource-governor.setup.ts`.
...
**Interfaces:**
- `PlatformId` (16 canonical ids), `Capability` (9 values), `SupportLevel`, `PlatformAdapter`, `PLATFORM_REGISTRY`, `PLATFORM_IDS`, `LEGACY_PLATFORM_ALIASES`, `resolvePlatformId()`.
- `PLATFORM_REGISTRY` implements the capability matrix in this document verbatim. The matrix is contractual: the compatibility doc, doctor, and tests are generated from it.
...
- [ ] **Step 3: Replace duplicate platform arrays and keep the build green**
...
Expected: PASS; sixteen adapters; nine capabilities each; matrix snapshot matches this document; `claw`/`kimicode` aliases resolve; build compiles.

- [ ] **Step 5: Commit**

```bash
git add packages/@monomind/cli/src/platform-adapters packages/@monomind/cli/src/commands/platforms.ts packages/@monomind/cli/__tests__/commands/platforms.test.ts packages/@monomind/cli/__tests__/platform-adapters
git commit -m "refactor(platforms): add canonical adapter registry with legacy id aliases"
```
...
- `applyPlan(plan): Promise<ApplyResult>` — writes only after preflight; atomic (pid-suffixed tmp + rename, precedent in `shared.ts`); cross-process lock file for `upgrade --all`.
...
`applyPlan` backs up each mutated file (timestamped, adjacent `.monomind-backup/` dir), routes `managed_block` vs `named_entry` by `replace`, uses the JSONC-tolerant parser for `format: 'jsonc'`, and never rewrites TOML full-file (managed comment block inside, or surgical entry splice preserving comments/order).
...
it.each(PLATFORM_IDS)('%s has instructions, a router path, and a tool path', (platform) => {
  ...
  expect(hasSkill || adapter.capabilities.skills === 'cli_fallback').toBe(true); // v2 forced fake skills on aider — fixed
  ...
  expect(hasMcp || adapter.capabilities.mcp === 'cli_fallback').toBe(true);
});
```
...
```ts
const CORE_LOCATIONS: Record<PlatformId, { instruction: string; skillRoot?: string; mcp?: { path: string; format: 'json' | 'jsonc' | 'toml'; entry: readonly string[] } | 'cli_fallback' }> = {
  claude:     { instruction: 'CLAUDE.md',  skillRoot: '.claude/skills',  mcp: { path: '.mcp.json', format: 'json', entry: ['mcpServers', 'monomind'] } },
  gemini:     { instruction: 'GEMINI.md',  skillRoot: '.gemini/skills',  mcp: { path: '.gemini/settings.json', format: 'json', entry: ['mcpServers', 'monomind'] } },
  cursor:     { instruction: '.cursor/rules/monomind.mdc', skillRoot: '.cursor/skills', mcp: { path: '.cursor/mcp.json', format: 'json', entry: ['mcpServers', 'monomind'] } },
  vscode:     { instruction: '.github/copilot-instructions.md', skillRoot: '.github/skills', mcp: { path: '.vscode/mcp.json', format: 'json', entry: ['servers', 'monomind'] } },
  copilot:    { instruction: '.github/copilot-instructions.md', skillRoot: '.github/skills', mcp: 'cli_fallback' },
  opencode:   { instruction: 'AGENTS.md',  skillRoot: '.opencode/skills', mcp: { path: 'opencode.json', format: 'jsonc', entry: ['mcp', 'monomind'] } },
  aider:      { instruction: 'CONVENTIONS.md', skillRoot: undefined, mcp: 'cli_fallback' }, // conventions registered via `.aider.conf.yml` read: key
  kiro:       { instruction: '.kiro/steering/monomind.md', skillRoot: '.kiro/skills', mcp: { path: '.kiro/mcp.json', format: 'json', entry: ['mcpServers', 'monomind'] } },
  trae:       { instruction: '.trae/rules/monomind.md', skillRoot: undefined /* discovery */, mcp: 'cli_fallback' /* discovery may upgrade */ },
  openclaw:   { instruction: 'AGENTS.md',  skillRoot: '.agents/skills', mcp: { path: 'openclaw.json', format: 'json', entry: ['mcp', 'monomind'] } }, // workspace skills per precedence; user scope uses ~/.openclaw/openclaw.json
  droid:      { instruction: 'AGENTS.md',  skillRoot: '.factory/skills', mcp: { path: '.factory/mcp.json', format: 'json', entry: ['mcpServers', 'monomind'] } }, // v2's settings.json was wrong
  antigravity:{ instruction: 'AGENTS.md',  skillRoot: '.agents/skills', mcp: { path: 'discovery', format: 'json', entry: ['mcpServers', 'monomind'] } }, // 2.0 vs CLI picked by discovery
  hermes:     { instruction: 'AGENTS.md',  skillRoot: '.hermes/skills', mcp: { path: 'discovery', format: 'yaml', entry: ['mcp', 'monomind'] } }, // trust-gated; ~/.hermes/config.yaml
  codex:      { instruction: 'AGENTS.md',  skillRoot: '.agents/skills', mcp: { path: '.codex/config.toml', format: 'toml', entry: ['mcp_servers', 'monomind'] } }, // .codex/skills was wrong
  kimi:       { instruction: 'AGENTS.md',  skillRoot: '.kimi-code/skills', mcp: { path: '.kimi-code/mcp.json', format: 'json', entry: ['mcpServers', 'monomind'] } }, // AGENTS.md verified in Task 7 fixture
  zed:        { instruction: 'AGENTS.md',  skillRoot: '.agents/skills', mcp: { path: '.zed/settings.json', format: 'jsonc', entry: ['context_servers', 'monomind'] } },
};
```
...
Every `cli_fallback` path emits a diagnostic naming `monomind mastermind run --print`, `monomind status --format json`, and `monomind platforms doctor`.
...
Expected: PASS; all sixteen targets have a concise instruction; every non-cli_fallback MCP target renders a named entry in the verified file/entry path; OS-parametrized command rendering covered.
...
// droid/kimi stay 'experimental'/'manual-step' until Task 10 fixtures pass (v2 over-claimed greenfield platforms).
...
- **Timeout units are per-platform renderer data and tested** (Codex TOML uses *seconds*; Cursor/Kimi JSON use *milliseconds* — the repo currently ships `timeout = 5` and `timeout = 5000` into the same Codex TOML schema, a 1000× bug this task eliminates).
...
- `discover(commandRunner, adapter): DiscoveryResult` for `requiresDiscovery` adapters (trae, hermes, antigravity, zed) **and** the MCP file-location probes for openclaw/droid/kimi before their `native` MCP entries are trusted:
...
Shared content via `renderCoreArtifacts`; native enhancements only for `native` capabilities; `experimental` capabilities emit nothing without a successful discovery result; dedicated parsers per format. Kimi's renderer verifies the AGENTS.md instruction surface as part of its fixture (unverified in current docs) — if unverifiable, downgrade the matrix cell to `cli_fallback` and record it.
...
- Create: `__tests__/platform-adapters/migration.test.ts`

**Interfaces:**
- `monomind platforms install --platform <id|alias> [--scope project|user] [--dry-run] [--enable-blocking-hooks]`
- `monomind platforms upgrade --all [--dry-run]`
- `monomind platforms doctor [--platform <id>] [--json]`
- `monomind platforms uninstall --platform <id|alias> [--remove-legacy]`
- `monomind platforms setup …` — **deprecation shim for one full release cycle**: detects legacy artifacts, prints the replacement (`platforms install --scope user` + automatic legacy cleanup), runs the migration detector, exits 0. Then removed per the deprecation-cycle definition.
...
- `--remove-legacy` on uninstall MAY remove Monomind-owned legacy artifacts (marker-verified only); without it they are reported, never deleted.
...
| 4 | `~/.agents/skills/mastermind-*` and `~/.gemini/skills/mastermind-*` shared roots | `installMastermindSkills` | Report; remove only with `--remove-legacy` (marker/frontmatter-verified) |
...
| 14 | Global `~/.claude/CLAUDE.md` append + global settings SessionStart hook written by project init | `write-claude.ts:262-351` | Remove owned block/hook; scope-gate any replacement behind `--scope user` |
...
```bash
pnpm --filter @monoes/monomindcli exec vitest run __tests__/platform-adapters/migration.test.ts __tests__/commands/platforms.test.ts
```
...
it('reports native capabilities and legacy migration advice as structured JSON', async () => {
  const report = await runPlatformsDoctor({ platform: 'codex', json: true, fixture: 'legacy-codex' });
...
Doctor surface decision (v2 shipped three overlapping ones): **`platforms doctor` is the one CLI surface** (with `--json`); top-level `doctor` gains a `platforms` **component** (29th category, one-line-per-adapter summary pointing at `platforms doctor`); `platforms_doctor` is the MCP wrapper. No `doctor --platform` flag.
...
1. **(M1)** One registry (16 platforms × 9 capabilities, matrix identical to this document) is the sole source for `init`, `platforms`, monograph skill installation, monograph build-tool enums, doctor, tests, and generated documentation — verified by a snapshot test plus grep for retired inventories.
...
3. **(M1)** Every target provides a project instruction, router path, portable skill path (`native` file or tested CLI fallback), and MCP (`native` named entry or tested CLI fallback); the fallback commands (`monomind mastermind run --print`, `monomind status --json`, `monomind platforms doctor`) exist, execute, and are covered by tests.
...
6. **(M1)** Install, upgrade, and uninstall are idempotent, preserve user content, and survive sibling-block contention in shared files — verified per-platform fixtures plus the shared-AGENTS.md test.
7. **(M1)** `monomind platforms doctor --json` and the `platforms_doctor` MCP tool report capability levels **identical to `PLATFORM_REGISTRY`** (test-compared), are core-advertised on `tools/list`, and sanitize absolute paths.
...
- **Treat user scope as explicit consent.** The statusline/global-writes audit found project init currently violating this; the migration fixes it rather than preserving it.
...
- **Consistency:** `InstallRequest`, `DiscoveryResult`, `HookDecision`, `ArtifactIntent`, and the capability matrix are each defined exactly once before their consumers; every fallback named in a capability contract has a preceding implementation step; milestones tag every task and gate.
