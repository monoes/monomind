...
Monomind's LanceDB implements this three-way separation via memory namespaces. The `auto-memory` system (writing to `~/.claude/projects/<project-hash>/memory/`) stores user-level facts, while agent-level patterns go to the `patterns` namespace and run-level context is held in the session's active working set.
...
- `packages/@monomind/memory/` — LanceDB namespace model
