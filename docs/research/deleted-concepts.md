...
### Healthcare Clinical (`@monomind/plugin-healthcare-clinical`)
...
### Financial Risk (`@monomind/plugin-financial-risk`)
...
### Legal Contracts (`@monomind/plugin-legal-contracts`)
...
### Code Intelligence (`@monomind/plugin-code-intelligence`)
...
### Test Intelligence (`@monomind/plugin-test-intelligence`)
...
### Performance Optimizer (`@monomind/plugin-perf-optimizer`)
...
- **Why deleted:** Overlapped with hooks `benchmark` worker + `performance` MCP tools + `@monomind/performance` package (3 existing systems)
- **Rebuild as:** Not needed — existing systems cover this

### Neural Coordination (`@monomind/plugin-neural-coordination`)
...
### Cognitive Kernel (`@monomind/plugin-cognitive-kernel`)
...
### Quantum Optimizer (`@monomind/plugin-quantum-optimizer`)
...
### Hyperbolic Reasoning (`@monomind/plugin-hyperbolic-reasoning`)
...
- **Note:** the `@monomind/embeddings` package (which had a basic hyperbolic mode) has since been removed; real embeddings are served by LanceDB
- **Use case:** Ontology navigation, taxonomy-aware search, hierarchical classification
- **Rebuild as:** a hyperbolic-reasoning layer over the LanceDB embedding path

---

## Packages (6 deleted)

### `@monomind/codex` (14 files)
...
|-----|-----------------|--------|
| `monomind.modelPreferences` | Static model routing overrides | Not needed — pre-task dynamic scoring is better |
| `monomind.agentTeams.*` | Custom agent team coordination | Not needed — Claude Code native Task tool handles this |
| `monomind.ddd.*` | DDD bounded context validation | Would need a DDD model defined first |
| `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` | Feature flag | Not needed — env var never checked |
| `MONOMIND_v1_ENABLED` | Feature flag | Not needed — env var never checked |
| `MONOMIND_HOOKS_ENABLED` | Feature flag | Not needed — hooks fire via settings.json definitions |

**Kept and wired:**
- `monomind.neural.enabled` — now a real kill switch for intelligence.init()
- `monomind.adr.autoGenerate` — now auto-creates ADR stubs after architect tasks
...
- **Why deleted:** Zero imports from any package — superseded by `packages/@monomind/*` scoped packages
- **Rebuild as:** Not needed — all functionality exists in @monomind/memory, @monomind/swarm, @monomind/mcp, @monomind/plugins
...
- 7 nested `.monomind/` dirs in packages (stale session artifacts)
