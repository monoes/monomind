 ...
 */
export async function requireRuVector(
  feature: keyof RuVectorFeatures,
): Promise<void>;
```
 ...
 */
export async function routeWithQLearning(
 ...
 * @throws RuVectorNotAvailableError if ruvector not installed
 */
export async function routeWithCoverage(
  request: RouteRequest,
): Promise<RouteResult>;
```
 ...
 * @throws RuVectorNotAvailableError if ruvector not installed
 */
export async function analyzeGraphBoundaries(
  ...
  constructor(feature: string) {
    super(
      ...
      1,
      {
        ...
        documentation: "https://github.com/nokhodian/ruvector",
      },
    );
...
Learn more: https://github.com/nokhodian/ruvector
`.trim();
}
...
export {
  RuVectorNotAvailableError,
  RuVectorFeatureDisabledError,
} from "./errors.js";
...
describe("Route Command", () => {
  it("should show helpful error when ruvector not available", async () => {
    ...
    vi.mock("../../src/ruvector/availability.js", () => ({
      requireRuVector: vi
        .fn()
        .mockRejectedValue(new RuVectorNotAvailableError("qLearningRouter")),
    }));
...
## Performance Optimizations (V1.0.0-alpha.21 - alpha.23)
...
const analysisCache = new Map<
  string,
  { result: DiffAnalysisResult; timestamp: number }
>();
...
const graphCache = new Map<
  string,
  { graph: DependencyGraph; timestamp: number }
>();
...
const analysisResultCache = new Map<
  string,
  { result: GraphAnalysisResult; timestamp: number }
>();
...
const coverageDataCache = new Map<
  string,
  { report: CoverageReport; timestamp: number }
>();
...
## Quick Wins Performance Optimizations (V1.0.0-alpha.24 - alpha.25)
...
Five high-impact, low-effort optimizations were implemented to achieve measurable performance gains across the V1 codebase.
...
const pool = createPooledHttpTransport(
  logger,
  {
    host: "localhost",
    port: 3000,
