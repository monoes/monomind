# ADR-053: AgentDB v1 Controller Activation & Runtime Wiring
...
- 4-tier proof engine fallback: `@ruvector/graph-transformer` (native NAPI-RS) → `ruvector-graph-transformer-wasm` → `@nokhodian/ruvector-verified-wasm` (legacy) → pure JS validation
...
### AgentDB v1 Release History

| Version | Key Change | Blocker Resolved |
|---------|-----------|-----------------|
| alpha.3 | Initial v1 release | ESM-only (CJS broken), empty default export, 4 controllers missing from barrel |
...
Additionally, the `HybridBackend` facade (#1212) does not proxy new v1 methods (`recordFeedback`, `verifyWitnessChain`, `getWitnessChain`), and the hook handler (#1211) ignores stdin on Claude Code 2.x, making all hooks non-functional.

### Root Cause

The v2→v1 migration was a package-level upgrade without a corresponding CLI integration pass. Each controller was implemented and exported in isolation, but no orchestration layer connects them to the MCP tools, hook handlers, or daemon workers that the CLI exposes.

## Decision

Implement a **phased controller activation plan** organized by dependency order, with a central `ControllerRegistry` that manages lifecycle (init, health check, shutdown) for all AgentDB v1 controllers.
...
- **42 exports activated**: Unlocks the full AgentDB v1 capability surface (14 internal + CLI-layer controllers).
...
MutationGuard.initialize()
  ...
  → try: import('@nokhodian/ruvector-verified-wasm')    // Legacy WASM
...
## Implementation Status (v1.1.0-alpha.51)
...
| AgentDB v1 instance | OK | alpha.7 loaded, 13 controllers |
...
| Issue | Title | Status |
|-------|-------|--------|
| #1207 | Dual memory system | **Resolved** — Bridge eliminates duplication, CLI routes through AgentDB v1 |
...
All 13 controllers are instantiated via AgentDB v1, registered in the ControllerRegistry, and accessible through the bridge. Embeddings are generated on every store operation. Semantic search uses cosine similarity over stored embeddings.
