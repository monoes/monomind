...
- 42edc1df — fix(odt-chat): _odtChatCurrentId stuck on old run when new run starts while on another tab — run:start auto-switch and session:start auto-select both had _v2OrgTab==='chat' guards; if a new run began while user was on Live/Agents/etc tab, _odtChatCurrentId kept pointing to the previous completed run; on returning to chat the catch-up re-render replayed the old session instead of the new one; fixed by removing both guards (the #odt-chat feed is in the DOM regardless of active tab; odtChatSelectSession correctly pre-loads events while hidden, and v2RenderOrgChat's catch-up re-render displays them on tab return)

- be5d4508 — fix(chat-view): main chat handleChatViewEvent dropped org events and missed auto-switch when user was on another view — (1) org events without ev.session (org:comms, run:start, org:checkpoint, loop:*) were only appended to #chat-v-feed when currentView==='chat'; navigating away caused permanent event loss since there was no buffering; fixed by removing the guard — #chat-v-feed is in the DOM even when hidden so events accumulate; (2) session:start auto-switch only fired when currentView==='chat'; new sessions starting while user browsed Orgs/Now/etc were never auto-selected; fixed by removing the guard so chatVSelectSession pre-loads the hidden feed — run:start auto-switch and session:start auto-select both had _v2OrgTab==='chat' guards; if a new run began while user was on Live/Agents/etc tab, _odtChatCurrentId kept pointing to the previous completed run; on returning to chat the catch-up re-render replayed the old session instead of the new one; fixed by removing both guards (the #odt-chat feed is in the DOM regardless of active tab; odtChatSelectSession correctly pre-loads events while hidden, and v2RenderOrgChat's catch-up re-render displays them on tab return)

- 0700c217 — fix(odt-chat): fallback path leaves feed empty for all-complete sessions — _odtLoadChatSessions fallback path (/api/mastermind/sessions) only called _odtPopulateChatSel which auto-selects running sessions only; for orgs like monomind-improver where all sessions are 'complete', dropdown was populated but nothing got auto-selected and the feed stayed blank; fixed by (1) adding first-session auto-select after _odtPopulateChatSel in the fallback path to mirror primary-path behaviour, and (2) adding lazy-load for non-_isOrgRun sessions in odtChatSelectSession — fetches /api/mastermind/session/:id which returns full session JSON including events[], then renders; same pattern as org-run lazy-load

- 73c25051 — fix(chat-view+odt-chat): auto-select most recent session and block duplicate events during lazy-load — (1) loadChatViewSessions (main chat view): no previous selection + no running session left dropdown populated but feed empty; added fallback to select sessions[0] (most recent); (2) _odtHandleLiveEvent session branch: SSE events arriving during rep-4 lazy-load were appended to DOM immediately AND replayed by fetch-complete render → duplicates; added !sess._loading guard (matching org-run branch pattern at line 7115); (3) also call chatVSelectSession(prev) when restoring prev selection if chatVCurrentId is null

- 7939171f — fix(odt-chat): session header, excerpt banner, and label for session-based orgs — (1) no session header: odtChatSelectSession never injected cv-session-hdr for non-_isOrgRun sessions (users saw events with no context); added header showing prompt/ID/status/start/event-count; (2) excerpt banner blank for historical sessions: cleared on select but never populated since renderCvExcerptBanner was only called on live SSE; fixed by calling it after rendering events in odtChatSelectSession and in catch-up re-render; (3) "RUN" label/empty state for session-based orgs: added dynamic label in _odtPopulateChatSel (SESSION vs RUN based on _isOrgRun), updated empty state strings; (4) added .cv-session-hdr to querySelectorAll cleanup to prevent stale headers stacking on session switch

- 91c65e01 — fix(odt-chat): re-inject session header in catch-up re-render on tab-return — v2RenderOrgChat's catch-up block cleared .cv-session-hdr elements but never re-injected them for session-based (non-_isOrgRun) orgs; returning to the Chat tab after visiting Live/Agents/etc caused the session metadata header to disappear; fixed by re-injecting cv-session-hdr (prompt/ID/status/timestamp/event-count) in the catch-up re-render using the same pattern as odtChatSelectSession

- 6a0cb8b4 — fix(odt-chat): merge mastermind sessions into org run list so repeat-loop reps are visible — _odtLoadChatSessions primary path returned early after loading org runs, skipping all mastermind sessions; orgs running via --repeat loops emit session:start/complete events (not org:comms run events) so their recent reps were completely invisible in the Chat tab; fixed by fetching /api/mastermind/sessions after the primary path succeeds, filtering by _odtOrgSessionMatch, deduplicating against existing run IDs, merging and re-sorting by ts descending so both types appear in the dropdown

- 7cddef2f — fix(odt-chat): skip redundant lazy-load for pre-loaded merged sessions; add [R]/[S] type indicators — merged sessions from /api/mastermind/sessions already included events[] but _eventsLoaded was never set, causing a second fetch and a spurious "Loading session events…" spinner on every first-select; fixed: (1) set _eventsLoaded=true during merge for complete sessions with events[], (2) prepend [R]/[S] type tag to each option, show ev count, update placeholder to "N entries" and sidebar label to "HISTORY" when both types coexist

- f2488e68 — fix(odt-chat): populate excerpt banner for org runs and refresh agent bar on tab-return — three gaps: (1) odtChatSelectSession cleared excerpt for _isOrgRun, (2) SSE runId branch never called renderCvExcerptBanner, (3) catch-up skipped agent bar refresh and org-run excerpt

- 656bd034 — fix(odt-chat): normalise ev.role→ev.from in org:comms rendering — older JSONL files store sender as ev.role not ev.from; all 5 consumers (_odtAppendEvent, appendChatViewEvent, _cvExcerptText, agentSet derivation, msgCount) only read ev.from → every old-format event displayed as 'boss → ?' hiding the actual agent name; fixed with ev.from||ev.role fallback in all 5 places — three gaps in Chat tab UX for org run sessions: (1) odtChatSelectSession cleared the excerpt banner for all _isOrgRun sessions (else branch ran `ex.innerHTML = ''`) so "LAST ACTIVITY" was always blank for org runs even with events; (2) SSE handler's runId branch never called renderCvExcerptBanner so the banner stayed blank during live runs; (3) catch-up re-render on tab-return skipped _odtPopulateAgentBar and only refreshed excerpt for !_isOrgRun sessions — fixed all three: org run select now calls renderCvExcerptBanner when events exist, SSE handler calls it after appending each event, catch-up re-render calls both _odtPopulateAgentBar and renderCvExcerptBanner for all session types
