...
- [2026-06-14 00:16] 57ebaa19 — fix(doctor): rebuild dist from src to include checkMonoesMemory and checkGitignoreCoverage health checks; both checks existed in src/commands/doctor.ts (allChecks + componentMap entries for memory-pkg and gitignore) but were never compiled into dist — running `monomind doctor` silently skipped both diagnostics
- [2026-06-14 00:17] 67afc90c — fix(doctor): show 'installed' fallback in agentic-flow check when no named exports detected; when agentic-flow is installed but none of ./reasoningbank, ./router, ./transport/quic are in its package.json exports the message rendered as 'v1.x.x ()' — empty parenthetical; now shows 'v1.x.x (installed)'
...
- [2026-06-14 04:30] bffbaaf4 — fix(server): honour ?dir= query param in POST /api/loops/stop; handler used server-level projectDir for all stop requests, ignoring the ?dir= the client sends from stopLoopInProject() in Global Loops view — cross-project loop stopping always wrote .stop to the wrong directory- [2026-06-14 08:04] c7df72cb — fix(dashboard): implement missing v2RenderOrgConfig and v2RenderOrgFiles tab functions; Config/Files tabs were routed in v2RenderOrgTab() dispatcher but their functions were never defined, causing ReferenceError on click; added v2RenderOrgConfig (renders topology/governance/budget/goal from _v2OrgData) and async v2RenderOrgFiles (fetches /api/org/:name, lists foundation/config/issues/members file paths)
- [2026-06-14 09:00] 958a5ae1 — fix(orgs): show run_success_rate_7d and budget_used_pct in health panel; /api/org/:name/health computed and returned these fields but renderHealth() never rendered them — the health tab only showed agents_active and tasks_pending; added SUCCESS (7d run rate + run count) and BUDGET (token budget %) health-cell cards with green/amber/red colour coding
