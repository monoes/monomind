...
const FIXTURES = [
  {
    key: 'fact-auth',
    value: 'Authentication uses JWT with rotating refresh tokens stored in httpOnly cookies',
    q: 'how do we handle login tokens?',
  },
  {
    key: 'fact-dash',
    value:
      'The dashboard server is the single long-lived process and keeps the embedding model warm',
    q: 'which process holds the warm embedding model?',
  },
  {
    key: 'fact-exfat',
    value: 'exFAT volumes create AppleDouble ._ sidecar files that corrupt directory scans',
    q: 'why do directory listings break on external drives?',
  },
  {
    key: 'fact-publish',
    value:
      'The CLI consumes the published @monoes/memory package, so backend changes need an npm publish',
    q: 'when do memory package changes reach the CLI?',
  },
];
...
before(
  async () => {
    ...
    for (const f of FIXTURES) {
      const res = await bridge.bridgeStoreEntry({
        key: f.key,
        value: f.value,
        namespace: 'default',
        dbPath: STORE,
...
describe('feedback loop', () => {
  it('EWMA-applies once per ledger key and lifts blended rank score', {
    timeout: 120_000,
  }, async (t) => {
    if (!semantic) {
      t.skip('blend is semantic-only by design (cognee guard)');
      return;
    }
    const q = 'how do we handle login tokens?';
    const beforeRes = await bridge.bridgeSearchEntries({
      query: q,
      namespace: 'default',
      dbPath: STORE,
      limit: 3,
    ...
    for (let i = 0; i < 5; i++) {
      const res = await bridge.bridgeApplyFeedback({
        entryIds: [target.id],
        score: 1.0,
        ledgerKey: `eval-${i}`,
        dbPath: STORE,
    ...
    const repeat = await bridge.bridgeApplyFeedback({
      entryIds: [target.id],
      score: 1.0,
      ledgerKey: 'eval-0',
      dbPath: STORE,
    ...
    const afterRes = await bridge.bridgeSearchEntries({
      query: q,
      namespace: 'default',
      dbPath: STORE,
      limit: 3,
  ...
  it('usage capture increments frequency and dedup-store reinforces', {
    timeout: 60_000,
  }, async () => {
    const stored = await bridge.bridgeStoreEntry({
      key: 'freq-probe',
      value: 'A distinctive probe fact about zebra caching',
      namespace: 'default',
      dbPath: STORE,
    });
...
describe('knowledge graph', () => {
  it('merges same-name entities, retrieves triplets, rolls back per origin', {
    timeout: 120_000,
  }, async () => {
    const a = await kg.kgIngest({
      ...
      dbPath: STORE,
      nodes: [
        { name: 'Eval Daemon', type: 'Service', description: 'runs the orgs' },
        { name: 'Eval Bus', type: 'Module' },
      ],
      edges: [
        {
          source: 'Eval Daemon',
          target: 'Eval Bus',
          relation: 'emits_to',
          description: 'Eval Daemon emits run events to Eval Bus',
        },
      ],
    });
    assert.equal(a.nodesAdded, 2);
    const b = await kg.kgIngest({
      originRef: 'run:eval2',
      dbPath: STORE,
      nodes: [
        {
          name: 'eval daemon',
          type: 'Service',
          description: 'the long-running organization runtime service',
        },
      ],
    });
  ...
  it('rule distillation dedups paraphrases (rule-recall)', { timeout: 120_000 }, async (t) => {
    ...
    const r1 = await kg.kgIngestRules({
      originRef: 'run:eval3',
      dbPath: STORE,
      rules: [{ rule: 'Always run the build before committing TypeScript changes' }],
    });
    assert.equal(r1.verdicts[0].verdict, 'accepted');
    const r2 = await kg.kgIngestRules({
      originRef: 'run:eval4',
      dbPath: STORE,
      rules: [{ rule: 'You should always build before you commit any TypeScript change' }],
    });
