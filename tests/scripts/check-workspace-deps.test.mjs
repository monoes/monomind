...
const SCRIPT = path.resolve(__dirname, '../../packages/@monomind/cli/scripts/check-workspace-deps.mjs');

function run(env) {
  return spawnSync(process.execPath, [SCRIPT], { encoding: 'utf-8', env: { ...process.env, ...env } });
}
...
describe('check-workspace-deps.mjs scans optionalDependencies too (#148)', () => {
  it('lists all 5 known workspace:* deps, including the 4 under optionalDependencies', () => {
    ...
    for (const dep of ['@monoes/monograph', '@monoes/hooks', '@monoes/mcp', '@monoes/memory', '@monoes/routing']) {
      expect(r.stdout).toContain(dep);
    }
  });

  it('blocks a simulated plain-npm publish, citing the optionalDependencies-only deps', () => {
    const r = run({ npm_lifecycle_event: 'prepublishOnly', npm_config_user_agent: 'npm/10.0.0', MONOMIND_ALLOW_NPM_PUBLISH: '' });
