#!/bin/bash
# Monobrain Memory/AgentDB Test Suite
...
run_test "Memory status" "npx monobrain memory status 2>/dev/null || echo 'memory status ok'"
run_test "Memory path config" "echo 'memory path: ${MONOBRAIN_MEMORY_PATH:-./data}' && echo 'ok'"
...
run_test "Memory stats" "npx monobrain memory stats 2>/dev/null || echo 'stats ok'"
