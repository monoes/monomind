...
Trajectory + outcome logging (`intelligence.ts`); keyword routing (`createKeywordRouter`) with route-outcome correlation measured by `doctor`. Memory has two parallel backends: a JSON pattern store (hooks/intelligence trajectory logging), local SQLite + HF-embeddings (`memory-bridge.ts`, backs CLI `memory store/search` and MCP memory tools), with an HNSW ANN index inside `@monoes/memory`'s `SqlBackend.search()` that switches on above `MONOMIND_HNSW_THRESHOLD` (default 5,000 entries) — not consolidated, see `packages/@monomind/memory/src/`. LanceDB support was fully removed.
