...
### B1 — Resolved: The documentation conflates document indexing with graph extraction
...
**Resolved by `ea8475e16` and `c73d68956`:** the concept guide and site page now document two explicit flows — document to searchable excerpts, and extracted claims to the memory knowledge graph — with extraction shown as a deliberate separate step. "Document KG"/"Second Brain KG" are retired as names in favor of the corrected boundary.
...
### B2 — Resolved: “For an org” does not match KG storage and query scoping
...
**Resolved by `05db18bf5` and `98a26ca66`:** `KgScope` carries the owning org through every entry point; namespaces resolve through `kgNamespaces()` (`kg:nodes:org:<org>` etc.) and origin refs are qualified with `kgQualifyOrigin()`. Reads (`kgSearch`, `kgGlossary`, `kgStats`, `kgListRules`, `kgRollback`) and the coordinator glossary all see only the named scope. Sharing is now explicit via `kgPromote` (`monomind org memory <org> promote <ref>`), independently rollback-able from the org's own copy.
...
### B3 — Resolved: Search names do not guarantee the same retrieval surfaces
...
**Resolved by `46cee4091`:** every response now carries a `RetrievalReport` naming requested/executed/failed/unsupported surfaces, so document-only search states what it did not read. A code-dependency question is now detected and answered by saying code analysis was not searched (naming `monograph_query`/`_impact`/`_neighbors`) rather than being silently misrouted to the memory graph. Org recall issues flat memory, private memory, and the graph concurrently and returns early only when all are empty.
...
### B4 — Resolved: Store and namespace names conceal real ownership
...
**Resolved:** the code half (CLI hardcoded `org:<name>` vs. the configured `memory_namespace`) by `05db18bf5` and `98a26ca66` — both the CLI and runtime now call one `orgMemoryNamespace()` resolver. The documentation half by `ea8475e16` — the guide now documents the real resolved path (`~/.monomind/projects/<dir-name>-<sha256-prefix>/lancedb/memory.db`, SQLite despite the directory name) and org runtime/global brain paths, keeping the legacy directory name explained rather than renamed.
...
### B5 — Resolved: Generic labels and outdated model descriptions encourage wrong assumptions
...
**Resolved by `ab914147a`:** init now reports the Monograph *code graph* rather than a bare "knowledge graph"; `memory_kg_search`'s description states both retrieval modes instead of promising "vector-seeded" unconditionally; and the README/CLI README are corrected to the actual model (`Alibaba-NLP/gte-modernbert-base`) and the actual number of distinct models.
