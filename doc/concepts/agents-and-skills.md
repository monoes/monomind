...
The picker reads the index `.monomind/registry.json` ([`registry-builder.ts → buildUnifiedRegistry`](packages/@monomind/cli/src/agents/registry-builder.ts#buildUnifiedRegistry)). It belongs to the project root — the nearest directory at or above the current one that holds `.claude/agents` or `.monomind`, never above the git root or at `$HOME`. It is rebuilt when it is missing or older than any agent file ([`registry-freshness.ts → ensureRegistry`](packages/@monomind/cli/src/agents/registry-freshness.ts#ensureRegistry)): by any `monomind` command run in the project (the MCP server Claude Code starts included), and synchronously before `monomind pick` and the `pick` MCP tool rank. The prompt hook reads the file as it is. A build that finds no agents never replaces a registry that has some. Duplicate slugs keep the first file found and are reported by `monomind doctor -c registry`.
