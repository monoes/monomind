...
| **Helpers** | When Antigravity is selected (it is in the default `--target all`), the full `.claude/helpers/` tree is mirrored to `.gemini/helpers/` so the statusline resolves the same scripts Claude Code uses. No other platform reads this copy, so `init` without Antigravity does not write it (an existing copy is left in place). `init upgrade` refreshes this copy the same way it refreshes `.claude/helpers`, and `doctor` warns when it is stale. |
| **MCP server + tools** | Same stdio server as every other target (`npx monomind@latest mcp start`) — 71 tools: `monograph_query`, `memory_kg_search`, … |
