...
| `hooks_post-edit` | [`hooks-routing.ts → hooksPostEdit`](packages/@monomind/cli/src/mcp-tools/hooks-routing.ts#hooksPostEdit) | **Edit Outcome Feedback**: Records editing outcome to memory store/feedback loop. <br>• *Input*: `{ filePath, success?, agent? }` <br>• *Output*: `{ recorded, filePath, success, timestamp, feedback }` (`recorded` is true only when the feedback write succeeded) |
