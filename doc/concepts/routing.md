...
2. **CLI & MCP Integration Layer (`@monomind/cli`)**:
   ...
   - Outcome Logger: [`packages/@monomind/cli/src/monovector/route-outcomes.ts`](packages/@monomind/cli/src/monovector/route-outcomes.ts)
...
- **File**: [`packages/@monomind/cli/src/routing/pick-step.ts`](packages/@monomind/cli/src/routing/pick-step.ts#pickRoute), wired in ahead of Tier 1 by [`route-layer-factory.ts → createConfiguredRouteLayer`](packages/@monomind/cli/src/routing/route-layer-factory.ts#createConfiguredRouteLayer).
...
- **One index**: `monomind pick` is the lookup every skill and script selects from. It ranks the agent registry (`.monomind/registry.json`, built from `.claude/agents/`) and one skill list: Claude skills (`source: "platform"`, loaded with their `invoke`) followed by Org-library skills whose name is not already taken (`source: "org"`, read with `monomind org skills show <name>`), catalog entries included under the catalog's own `jev` rule. The mastermind domain skills ask it for their specialists instead of naming fixed agent types.
...
- **Files**: [`packages/@monomind/routing/src/route-layer.ts`](packages/@monomind/routing/src/route-layer.ts#route), [`packages/@monomind/routing/src/encoder.ts`](packages/@monomind/routing/src/encoder.ts), [`packages/@monomind/routing/src/cosine.ts`](packages/@monomind/routing/src/cosine.ts)
...
- **File**: [`packages/@monomind/cli/src/mcp-tools/hooks-routing.ts`](packages/@monomind/cli/src/mcp-tools/hooks-routing.ts#hooksRoute)
...
- **Files**: [`packages/@monomind/routing/src/llm-fallback.ts`](packages/@monomind/routing/src/llm-fallback.ts#classify), [`packages/@monomind/routing/src/capability-index.ts`](packages/@monomind/routing/src/capability-index.ts)
...
1. **Length & Keyword Heuristic**:
   - Evaluated in MCP hooks ([`hooks-routing.ts`](packages/@monomind/cli/src/mcp-tools/hooks-routing.ts#hooksRoute)).
...
2. **Coverage-Aware Effort & Impact**:
   - Evaluated in [`monomind route coverage`](packages/@monomind/cli/src/commands/route.ts#coverageRouteCommand).
...
The keyword router (`createKeywordRouter`, [`monovector/index.ts → createKeywordRouter`](packages/@monomind/cli/src/monovector/index.ts#createKeywordRouter)) is a fixed keyword-substring matcher — it does not learn. What it does track is an append-only outcome ledger, read back to report routing quality over time:

- **Recording**: [`monomind route feedback`](packages/@monomind/cli/src/commands/route.ts#feedbackCommand) writes a reward (`-1.0`–`1.0`) for a task/agent pair. Internally, `KeywordRouter.update()` ([`monovector/index.ts → update`](packages/@monomind/cli/src/monovector/index.ts#update)) joins it to the latest unresolved route record (or creates a manual-feedback record) and appends it to `route-outcomes.jsonl` — no model weights are touched.
- **Statistics**: [`monomind route stats`](packages/@monomind/cli/src/commands/route.ts#statsCommand) reports `KeywordRouterStats` ([`monovector/index.ts → KeywordRouterStats`](packages/@monomind/cli/src/monovector/index.ts#KeywordRouterStats)): `outcomeCount`, `accuracy`, `adherence` (fraction of joined routes where the agent actually used matched the recommendation), `trend` (recent-half accuracy minus prior-half accuracy), and a `byMode: { native, js }` split. Computed by `computeRoutingAccuracy()` / `computeAdherence()` in [`route-outcomes.ts`](packages/@monomind/cli/src/monovector/route-outcomes.ts).
