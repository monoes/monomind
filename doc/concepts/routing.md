...
Agents come only from the project's agent registry, `.monomind/registry.json`, built from `.claude/agents/**/*.md` (plus any extra agent roots — `$MONOMIND_EXTRA_AGENT_PATHS` or a sibling `../agency-agents` — which win slug conflicts) and the user's `~/.claude/agents/**` (origin `user`; a project agent of the same name wins) by `registry-builder.ts`. The registry is built for the project root, rebuilt when it is missing or older than the agent files — by `monomind init`, at every session start, by any `monomind` command run in the project, and awaited before `monomind pick` and the `pick` MCP tool rank — written atomically, and never replaced by an empty one. Outside a project, `monomind pick` builds it in memory and writes nothing. The full list of roots and the frontmatter reference are in [Agents & Skills](./agents-and-skills.md#1-where-things-live).
...
- **Real use: `node scripts/pick-eval.mjs --logs [dir]`** scores the hook logs instead of the synthetic set (default this checkout's `.monomind`; a copied `.monomind` or a project root works too). Every Task/Agent spawn whose prompt is on record (`pick-adherence.jsonl` joined to `route-outcomes.jsonl` by `routeId`, or an older route record with `agentActuallyUsed`) is one example: redacted prompt preview → the agent actually spawned. Notification and one- or two-word prompts are skipped. It reports the follow rate of shown picks, how often the current ranker's top-1/top-3 holds the spawned agent (agents outside the catalog, such as `general-purpose`, are listed, not scored) and the top disagreements; with no spawns it prints route stats only. `--jev` and `--catalog` work as above, `--json` prints the report, and `--export FILE` writes the examples as a `tests/pick-eval` dataset for curating real tasks into the committed set — review the previews for private text before committing any. Logic: [`pick-real.ts`](packages/@monomind/cli/src/decision/pick-real.ts#readRealUse).
- **`monomind doctor -c pick`** ([`doctor-pick-checks.ts → checkPick`](packages/@monomind/cli/src/commands/doctor-pick-checks.ts#checkPick)), opt-in: registry agent count, duplicates and staleness; skill index freshness and counts (platform, org, user); whether a decision model is configured (no network); the keyword eval score when the project carries `tests/pick-eval`; `[PICK]` adherence; subagent success when the pick was followed versus overridden; and, from 10 logged spawns, one real-use line (`real use: N spawns, followed X%, current ranker agrees Y% (top-3)`). It rebuilds stale indexes, so it is not in the default run. `doctor -c registry` reports duplicate slugs and extra agent roots.
