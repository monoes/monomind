...
**Schema Architecture**: Embedded SQLite database operating in **WAL mode** (`PRAGMA journal_mode = WAL`); the database file itself is created `chmod 0600` (owner read/write only). Supports standalone `@monoes/memory` core schema (schema version **3** — `SCHEMA_VERSION` constant, [`sql-schema.ts → SCHEMA_VERSION`](packages/@monomind/memory/src/sql-schema.ts#SCHEMA_VERSION), applied via `PRAGMA user_version`; previously documented here as "v2", now stale) — **5 tables**: `memory_entries`, `memory_embeddings`, `memory_entry_tags`, `agent_reads`, plus the FTS5 full-text virtual table `memory_entries_fts` ([`sql-schema.ts → createFTS5Index`](packages/@monomind/memory/src/sql-schema.ts#createFTS5Index), added for issue #66) — and CLI project memory schema (v3.0.0, 9 tables at [`memory-schema.ts → MEMORY_SCHEMA`](packages/@monomind/cli/src/memory/memory-schema.ts#MEMORY_SCHEMA): `memory_entries`, `patterns`, `pattern_history`, `trajectories`, `trajectory_steps`, `migration_state`, `sessions`, `vector_indexes`, `metadata`).

### Key Memory Stores

| Store Type | Namespace / Location | Implementation File | Feature Highlights |
|---|---|---|---|
| **Episodic & Semantic** | Namespace `default` (or custom) in `memory_entries` | [`memory-crud.ts → storeEntry`](packages/@monomind/cli/src/memory/memory-crud.ts#storeEntry), [`memory-bridge.ts → bridgeStoreEntry`](packages/@monomind/cli/src/memory/memory-bridge.ts#bridgeStoreEntry) | Temporal decay (`decay_rate = 0.01`), access frequency tracking, confidence score, importance weighting (`0.5` default). |
| **Pattern Store** | `patterns` table & `.swarm/sona-patterns.json` | [`sona-optimizer.ts → LearnedPattern`](packages/@monomind/cli/src/memory/sona-optimizer.ts#LearnedPattern), [`memory-schema.ts → MEMORY_SCHEMA`](packages/@monomind/cli/src/memory/memory-schema.ts#MEMORY_SCHEMA) | Learned task routing patterns based on keyword extraction, success/failure counts, and EWC-inspired importance weighting (squared-embedding proxy — not Fisher information) (`.swarm/ewc-fisher.json`). |
| **Second Brain document index** | `memory_entries` (`doc:<hash>:<chunk>`) in namespace `knowledge:<scope>` | [`document-chunking.ts → chunkDocumentInline`](packages/@monomind/cli/src/knowledge/document-chunking.ts#chunkDocumentInline), [`bm25-index.ts → Bm25Index`](packages/@monomind/cli/src/memory/bm25-index.ts#Bm25Index) | Multi-format document ingestion, content-hash chunking, Okapi BM25 lexical indexing. Stores chunks and metadata only — no entity extraction. |
| **Memory knowledge graph** | `kg:nodes`, `kg:edges`, `rules` in `memory_entries` | [`memory-kg.ts → resolveEntity`](packages/@monomind/cli/src/memory/memory-kg.ts#resolveEntity), [`memory-kg.ts → resolveEntity`](packages/@monomind/cli/src/memory/memory-kg.ts#resolveEntity) | Cognee-style concept triplets. Nodes: `n:<normalized-name>`, Edges: `e:<src>\|<rel>\|<dst>`. Deterministic entity keys and rule deduplication threshold (`0.78`). `resolveEntity` also consults a second, additive `kg:names` index keyed by [`mergeKey`](packages/@monomind/cli/src/memory/entity-name-key.ts) — a coarser, case/separator/plain-plural-insensitive fold (e.g. `Node.js` and `nodejs` resolve to the same entity) that still stays exact on distinct symbols, acronyms and identifiers (`C` vs `C++`, `HTTPS` vs `HTTP`, `fetchUsers` vs `fetchUser`); a merge-key hit is only ever a candidate fed into the same `(type, name)` resolution rules, never an automatic cross-type merge. Populated only by explicit ingestion — see [The two graphs](#the-two-graphs). |
...
- **Tokenizer:** Shared `contentTokens` ([`text-tokens.ts → contentTokens`](packages/@monomind/cli/src/memory/text-tokens.ts#contentTokens)) ensuring exact evaluation harness parity.
- **Scaling Thresholds:** Live chunk warning at 50,000 chunks; index review threshold at 1,000,000 chunks ([`bm25-index.ts → LIVE_CHUNK_WARN_THRESHOLD`](packages/@monomind/cli/src/memory/bm25-index.ts#LIVE_CHUNK_WARN_THRESHOLD)).

### 3. Query Router & Surface Fusion
- **Surface Routing:** Evaluates rules across `chunks` (prior=0.5), `kg` (wt=2), `rules` (wt=2), `memory` (wt=2) ([`query-router.ts → routeQuery`](packages/@monomind/cli/src/memory/query-router.ts#routeQuery)).
- **Gates:** 20-character negation pre-match window skips negated query terms ([`query-router.ts → NEGATION_RE`](packages/@monomind/cli/src/memory/query-router.ts#NEGATION_RE)). Top routing surface must score ≥ 2× runner-up or query broadcasts to all surfaces.
...
- **Document OKF:** [`document-pipeline.ts → exportToOKF`](packages/@monomind/cli/src/knowledge/document-pipeline.ts#exportToOKF) exports/imports documents with standard YAML frontmatter headers and `index.md` manifest logs.
- **Memory OKF:** [`memory-transfer.ts → exportCommand`](packages/@monomind/cli/src/commands/memory-transfer.ts#exportCommand) transfers memory key-values across filesystem boundaries (`monomind memory export --format okf`).
...
**Chunks:** `memory_entries` rows keyed `doc:<content-hash>:<chunk-index>` in namespace `knowledge:<scope>` (default scope `shared`), inside the project memory store ([`document-ingest.ts → ingestDocument`](packages/@monomind/cli/src/knowledge/document-ingest.ts#ingestDocument)). The store's on-disk location is **not** `.monomind/memory/` — see [Storage layout](#5-cross-session-persistence).
**Document metadata:** `<project>/.monomind/knowledge/doc-metadata.jsonl` ([`document-index.ts → metadataPath`](packages/@monomind/cli/src/knowledge/document-index.ts#metadataPath)).
...
|---|---|---|
| **What it holds** | Parsed repository structure — files, functions, classes, and their import/call/dependency edges. Monograph can also index repository documentation. | Entities, relations, and distilled rules that something asserted. Entries may well describe code elements — `heuristicExtract` types a node `CodeElement` when its name looks like code ([`memory-kg.ts → heuristicExtract`](packages/@monomind/cli/src/memory/memory-kg.ts#heuristicExtract)). |
| **How it gets there** | `monomind monograph build` parses the repository | Explicit ingestion only: `memory_kg_ingest`, the post-task hook's causal edges ([`hooks-routing.ts → hooksPostTask`](packages/@monomind/cli/src/mcp-tools/hooks-routing.ts#hooksPostTask)), and org runs via `org_learn` or its run-summary heuristic fallback ([`org-memory.ts → learnOrgKnowledge`](packages/@monomind/cli/src/orgrt/org-memory.ts#learnOrgKnowledge), [`storeRunMemory`](packages/@monomind/cli/src/orgrt/org-memory.ts#storeRunMemory)) |
...
| **Rebuild / undo** | Rebuild from source at any time — the repository is the truth | No rebuild: claims accumulate. `memory_kg_rollback` withdraws one `originRef` from every element's origin list and deletes an element only once no origin remains ([`memory-kg.ts → kgRollback`](packages/@monomind/cli/src/memory/memory-kg.ts#kgRollback)) — which is why every ingest requires an `originRef` |
...
- **`knowledge_search` is a retrieval interface, not a store.** Its rule-based router picks among the `chunks`, `kg`, `rules`, and `memory` surfaces and fuses them by reciprocal rank ([`knowledge-tools.ts → knowledgeSearch`](packages/@monomind/cli/src/mcp-tools/knowledge-tools.ts#knowledgeSearch)). Passing `store: 'global'` searches the personal cross-project brain's documents only — the KG, rules, and pattern surfaces are project-scoped and are deliberately excluded. Ask for `surfaces` explicitly when you need a specific one.
- **Code questions are not routed to the memory graph.** Phrases like "what calls X" or "what imports X" used to score for the `kg` surface — the *memory* graph, not Monograph — so the answer came from what someone once asserted rather than from parsed code. They are now recognised as code questions ([`query-router.ts → CODE_QUERY_RE`](packages/@monomind/cli/src/memory/query-router.ts#CODE_QUERY_RE)) and `knowledge_search` reports the code graph as an `unsupported` surface, naming the `monograph_*` tools, instead of answering from remembered claims. Call `monograph_query` / `monograph_impact` directly for real dependencies.
