...
- The sandbox runtime **creates zero-byte, read-only placeholder files in the role's cwd** for the deny entries it keeps relative to the working directory (`.bashrc`, `.gitconfig`, `.npmrc`, `.idea`, `.vscode`, `.claude/hooks`, …). They show up in `git status`, so a `commit` role doing `git add -A` can stage them; ignore them in the workspace if that matters.
