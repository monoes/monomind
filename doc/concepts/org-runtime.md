...
Layer 3's `read`-level subcommand surface (`GIT_READ_CMDS` plus the args-aware refinements, #299): `status`, `log`, `diff`, `show`, `branch`, `tag`, `remote`, `rev-parse`, `ls-files`, `ls-tree`, `blame`, `shortlog`, `describe`, `cat-file`, `for-each-ref`, `rev-list`, `grep`, `worktree`, `merge-base`, `show-ref`, `name-rev`, `cherry` (not `cherry-pick`), `range-diff`, `check-ignore`, `check-attr`, `count-objects`, `var`, `ls-remote`, `config` (reads only), `stash list`/`stash show` (not `stash push`/`pop`/`apply`/`drop`/`clear`), and `reflog`/`reflog show` (not `reflog expire`/`reflog delete`).
