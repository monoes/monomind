...
| `completion_evidence` | `false` | Gate `org_task_done` on machine-checkable evidence: `{ headSha, worktree?, checks: [{ command, exitCode, output }] }`. Every command must exit 0, and `headSha` must be the current head of some local work — the `HEAD` of any worktree of the repository or the tip of any local branch; with `worktree` named, that worktree's `HEAD` exactly. A sha that is the head of nothing is stale and refused ([`completion-gate.ts → checkTaskEvidence`](packages/@monomind/cli/src/orgrt/completion-gate.ts#checkTaskEvidence), heads from [`decisions.ts → localHeads`](packages/@monomind/cli/src/orgrt/decisions.ts#localHeads)). `max_evidence_attempts` (default 3) bounds refusals before the task is escalated |
