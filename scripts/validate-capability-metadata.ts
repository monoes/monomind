...
export function validateCapability(
  ...
  if (!capability.role) {
    ...
    errors.push({
      file: filePath,
      field: 'role',
      message: `Role "${capability.role}" is not valid kebab-case`,
    });
  } else if (capability.role.length < 2 || capability.role.length > 60) {
    errors.push({
      file: filePath,
      field: 'role',
      message: `Role length must be 2-60 chars, got ${capability.role.length}`,
    });
  }
  ...
  if (!capability.goal) {
    ...
    errors.push({
      file: filePath,
      field: 'goal',
      message: `Goal must be at least 20 chars, got ${capability.goal.length}`,
    });
  } else if (capability.goal.length > 200) {
    errors.push({
      file: filePath,
      field: 'goal',
      message: `Goal must be at most 200 chars, got ${capability.goal.length}`,
    });
  }
  ...
  if (!capability.version) {
    ...
    errors.push({
      file: filePath,
      field: 'version',
      message: `Version "${capability.version}" is not valid semver (expected X.Y.Z)`,
    });
  }
  ...
  if (!capability.expertise || !Array.isArray(capability.expertise)) {
    errors.push({
      file: filePath,
      field: 'expertise',
      message: 'Missing required field "expertise" (must be array)',
    });
  } else if (capability.expertise.length < 3) {
    errors.push({
      file: filePath,
      field: 'expertise',
      message: `Expertise must have at least 3 entries, got ${capability.expertise.length}`,
    });
  }
  ...
  if (!capability.task_types || !Array.isArray(capability.task_types)) {
    errors.push({
      file: filePath,
      field: 'task_types',
      message: 'Missing required field "task_types" (must be array)',
    });
  } else if (capability.task_types.length < 1) {
    errors.push({
      file: filePath,
      field: 'task_types',
      message: 'task_types must have at least 1 entry',
    });
  }
  ...
  if (!capability.output_type) {
    errors.push({
      file: filePath,
      field: 'output_type',
      message: 'Missing required field "output_type"',
    });
  } else if (!PASCAL_CASE_REGEX.test(capability.output_type)) {
    errors.push({
      file: filePath,
      field: 'output_type',
      message: `output_type "${capability.output_type}" is not PascalCase`,
    });
  }
