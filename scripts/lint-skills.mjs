...
} catch { /* CLI not built — skip command validation */ }

// Also add known subcommands from the commands/ directory
try {
  const cmdFiles = readdirSync(join(ROOT, 'packages/@monomind/cli/src/commands'))
    .filter(f => f.endsWith('.ts'));
  for (const f of cmdFiles) {
    const name = f.replace(/\.ts$|\.js$/, '').replace(/-commands$/, '').replace(/[-_]/g, '-');
...
} catch { /* src not available */ }

// Always-valid names (MCP tool prefixes, alias commands, etc.)
['mcp', 'memory', 'hooks', 'doctor', 'init', 'org', 'swarm', 'monograph', 'browse',
 'start', 'stop', 'status', 'help', 'version'].forEach(c => VALID_COMMANDS.add(c));

// --- Lint each skill tree ---
function lintTree(treePath, label) {
  if (!existsSync(treePath)) return;
  const skills = readdirSync(treePath).filter(d =>
    statSync(join(treePath, d)).isDirectory() &&
    existsSync(join(treePath, d, 'SKILL.md'))
  );
  ...
  for (const skill of skills) {
    ...
    const alphaLines = lines.filter(l =>
      ALPHA_PATTERN.test(l) && !/never\s+use/i.test(l) && !/do\s+not\s+use/i.test(l)
    );
    if (alphaLines.length > 0) {
      errors.push(`[${label}] ${skill}/SKILL.md: ${alphaLines.length} instance(s) of 'monomind@alpha' (excluding 'Never use' warnings) — use 'monomind@latest'`);
    }
    ...
    if (VALID_COMMANDS.size > 0) {
      ...
      while ((match = CMD_PATTERN.exec(content)) !== null) {
        ...
        if (!VALID_COMMANDS.has(cmd) && !cmd.includes(':')) {
          warnings.push(`[${label}] ${skill}/SKILL.md: references 'monomind ${cmd}' — not a recognized top-level command`);
        }
      }
    }
  }
...
function checkDrift() {
  ...
  const skills1 = new Set(readdirSync(tree1).filter(d =>
    existsSync(join(tree1, d, 'SKILL.md'))));
  const skills2 = new Set(readdirSync(tree2).filter(d =>
    existsSync(join(tree2, d, 'SKILL.md'))));
  ...
  if (only2.length > 0) {
    warnings.push(`Cross-tree drift: ${only2.length} skill(s) only in packages: ${only2.join(', ')}`);
  }
}
