...
if (!existsSync(dbPath)) {
  console.error('monograph.db not found:', dbPath, '— build the graph first');
  process.exit(1);
}
...
try {
  db.prepare(`ALTER TABLE nodes ADD COLUMN properties TEXT`).run();
} catch {
...
const findByFilePath = db.prepare(`SELECT id FROM nodes WHERE file_path = ? AND name = ? LIMIT 1`);
const findByName = db.prepare(`SELECT id FROM nodes WHERE name = ? AND label = ? LIMIT 1`);
...
const insertMany = db.transaction((nodes) => {
  for (const uaNode of nodes) {
    ...
    if (existingId) {
      ...
      const lang = uaNode.filePath ? uaNode.filePath.split('.').pop() || null : null;
      upsertNode.run(
        newId,
        label,
        uaNode.name || uaNode.id,
        normLabel,
