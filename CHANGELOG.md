...
- **`cleanup --force`'s orphan reaper no longer treats bwrap's own sandbox plumbing as a live session.** `selectOrphanedSdkPids` matched `LIVE_SESSION_RE` (`/\bclaude\b|monomind/i`) against the raw cmdline of every ancestor in a candidate orphan's parent chain. Under bwrap — every org role's actual runtime — pid 1 inside the sandbox is the bwrap wrapper itself, and its cmdline is packed with `--ro-bind .../.claude/... --ro-bind .../.monomind/...` bind-mount arguments that substring-match the same regex, so every orphan's ancestor scan hit a spurious match and the reaper reaped nothing regardless of whether the orphan was actually live-parented. bwrap always separates its own options from the wrapped command with a literal ` -- `; the check now only tests the portion after that separator, so a real live session is still protected and bwrap's own bind-mount plumbing no longer shields orphans from the reaper.
