...
- **Removed dashboard code for the v1 org model**: 22 org tabs that could no longer be shown, and 13 `GET /api/org/:name/…` routes that had no caller left (`projects`, `members`, `issues`, `environments`, `workspaces`, `invites`, `my-issues`, `secrets`, `join-requests`, `goals`, `search`, `threads`, `routines`). No live view used them.
- **The Runtime tab shows the budget a running org enforces.** It summed the run log's usage events, which miss turns that are metered but never reach a result, so it could read lower than the ceiling the org was actually closing on. While an org runs, the budget card and a new per-role Budget column now come from the daemon's policy engines, and config saved since the run started is listed as applying from the next run.
