...
- **`monomind pick` is one index for agents and every skill.** Its skill list now holds the Org skill library (bundled, `~/.monomind/org-skills`, project and active catalog entries) after the Claude skills, each ranked entry tagged `source: "platform"` or `"org"`; an Org skill's `invoke` is `monomind org skills show <name>`, and a name in both pools is listed once, as the Claude skill. `mastermind-review`, `-research`, `-release`, `-idea`, `-execute` and `-delegation` now ask `monomind pick` for their specialists instead of naming fixed agent types, five of which (`Analytics Reporter`, `Trend Researcher`, `UX Researcher`, `cicd-engineer`, `Product Manager`) did not exist; each keeps a fallback of agents that do. `mastermind-agent-select`'s category map and fallbacks now name real registry categories and agents.
