...
- **`cleanup --force`'s orphan reaper now correctly reaps orphaned SDK processes under a bwrap sandbox — every org role's actual runtime.** `selectOrphanedSdkPids` matched `LIVE_SESSION_RE` (`/\bclaude\b|monomind/i`) against the raw cmdline of every ancestor in a candidate orphan's parent chain, so it reaped nothing regardless of whether the orphan was actually live-parented, fooled three different ways in turn: bwrap's own `--ro-bind .../.claude/... --ro-bind .../.monomind/...` bind-mount arguments substring-matched the regex (fixed by only testing the portion of an ancestor's cmdline after bwrap's ` -- ` separator between its own options and the wrapped command); a bwrap-wrapped shell script's incidental mentions of a `.claude` path or a socket name did the same (fixed by judging an ancestor by its own executable name(s), not by whether some argument or path elsewhere on the line happens to contain those words); and a wrapped script whose text merely contained the substrings "claude-agent-sdk" and "--output-format" — unrelated to any real invocation — still matched (fixed by requiring a `claude-agent-sdk` path token to sit directly adjacent to an `--output-format` token, an argv-adjacency check instead of a wildcard substring scan). Verified against a real bwrap sandbox: a genuine orphan is reaped ("Reaped N orphaned SDK agent process(es)", pid gone from `/proc`) while a genuine live session, and a wrapped script that only mentions those strings incidentally, are still protected.
