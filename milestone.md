...
**Revision 11 — consolidation after the org was stopped (2026-09-20).** The `monomind-dev` run
(`run-20260919221725-nb4b`) wedged at 03:05 on a session limit and never recovered; it was answered,
briefly resumed, then stopped deliberately. Its in-flight work was finished and merged to local main
by hand. Merged this session, all verified on the **merged** tree rather than per branch:

| item | what shipped | merge |
|---|---|---|
| o-38 | plain `init` no longer silently deletes user files; `.opencode/skills` sweep gated on provenance | `fa50a795b` |
| o-16 | the memory project-root walk can no longer escape into the filesystem | `f82457a9f` |
| i-078 | the "no data leaves your machine" claim made true on **both** published packages | `fa149827c` |
| i-032a | a checked-in `enable-terminal.json` can no longer arm `terminal_execute` | `183f5c768` |
| i-052 | `dashboard-token` and friends now come from one `MONOMIND_NEVER_COMMIT` list | `b143e37c2` |
| o-09 | one mastermind router per SKILL.md, generated from one list | `8a38c4566` |
| i-073 | the dashboard's `?token=` CSRF preflight bypass closed | `08dfa5d09` |

Also done: `dev/i-116…i-302` recorded as merged (**ancestry only — it carried no content**; every fix
was already on main under different SHAs), `monobrowse`→1.0.12 and `monodesign`→1.2.9 so `publish` is
unblocked, and 27 merged branches plus 11 worktrees pruned.

**State: both suites fully green** — root 1249 passed / 0 failed, CLI 4175 passed / 0 failed, build and
typecheck clean. Local main is 105 commits ahead of `origin/main` and **nothing from this session is
pushed**. 2.12.0 is already published to npm and shipped the false privacy claim that i-078 corrects.

**Still open and worth knowing:** `i-032`'s `system_health` half; the `packages/*/__tests__/**` trees are
outside biome's scope entirely, so `biome check` there reports "paths ignored" and reads as a pass; and
the pre-commit secret gate false-positives on a query string that puts the literal param name `token`, an equals sign and a `${...}` interpolation adjacent in source — the value is a variable, not a secret, and this sentence had to be written the long way round to get itself committed.
...
- [x] **#8 — Decide: monoswarm, enforce the claim or stop making it (i-035).** {i-035} ✅ done 2026-09-19 (a22635f9c)
...
- **[4/10]** (quick win) `_checkAuth` (`server.mjs:1770-1782`) accepts `?token=` on any method. Refuse query tokens on state-changing requests, and add a `Sec-Fetch-Site` check. {i-073} ✅ done 2026-09-20 (08dfa5d09)
