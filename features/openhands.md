...
**Role in Monomind:** Sandboxing, agent registry versioning, session replay
...
OpenHands runs every agent action inside an isolated container or WASM sandbox, limiting the blast radius of errors and preventing agents from affecting each other's environments. Monomind adopted this pattern via two mechanisms:
- **Docker sandboxing**: `SandboxConfig.use_gvisor` flag in deployment configuration (enhanced with gVisor's `runsc` runtime)
- **WASM agents**: The `wasm_agent_create` MCP tool launches agents in WASM-isolated contexts via the monomind MCP server

### 2. Semantic Versioned Agent Registry (AgentHub)
OpenHands maintains an `AgentHub` — a registry of agents with semantic version numbers, compatibility matrices, and capability declarations. Monomind's agent registry follows the same pattern: agents are versioned, and the `PromptVersionStore` (from `packages/@monomind/cli/src/agents/prompt-experiment.ts`) manages prompt versions per agent slug with A/B experiment routing.

### 3. EventStream Session Replay
OpenHands captures every agent action as an event in an append-only stream, enabling exact replay of any session for debugging or verification. Monomind's intelligence trajectory system (`hooks_intelligence trajectory-start`, `trajectory-step`, `trajectory-end`) implements the same pattern — each step of a task is logged as a structured event that can be replayed by the RETRIEVE→JUDGE→DISTILL pipeline.
...
- `packages/@monomind/security/` — sandbox configuration
- `packages/@monomind/cli/src/agents/prompt-experiment.ts` — versioned agent registry
- `hook-handler.cjs` intelligence trajectory handlers — event stream capture
- `packages/@monomind/cli/src/mcp-tools/` — WASM agent tools
