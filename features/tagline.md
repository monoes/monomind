...
| Element            | Meaning                                 | Source                                                                                                                                                                                                       |
| ------------------ | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `0 vectors`        | Embeddings stored in AgentDB            | Counts entries in `.monomind/data/auto-memory-store.json`. Also checks `ranked-context.json → entries.length`. Green when > 0. `⚡ HNSW` tag appears only when both `hnsw.index` exists **and** vectors > 0 |
