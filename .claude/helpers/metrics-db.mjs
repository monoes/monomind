...
async function initDatabase() {
  ...
  db.run(`
    CREATE TABLE IF NOT EXISTS v1_progress (
  ...
  const progressCheck = db.exec("SELECT COUNT(*) FROM v1_progress");
  if (progressCheck[0]?.values[0][0] === 0) {
    db.run("INSERT INTO v1_progress (id) VALUES (1)");
  }
...
async function syncMetrics() {
  ...
  const totalStats = countFilesAndLines(v1_DIR);
...
function getMetricsJSON() {
  const progress = db.exec("SELECT * FROM v1_progress WHERE id = 1")[0];
...
function exportToJSON() {
  ...
  // v1-progress.json
  writeFileSync(join(metricsDir, 'v1-progress.json'), JSON.stringify({
    domains: {
      completed: metrics.v1Progress.domains_completed,
      total: metrics.v1Progress.domains_total
    },
    ddd: {
      progress: metrics.v1Progress.ddd_progress,
      modules: metrics.v1Progress.total_modules,
      totalFiles: metrics.v1Progress.total_files,
      totalLines: metrics.v1Progress.total_lines
    },
    swarm: {
      activeAgents: metrics.swarmActivity.estimated_agents,
      totalAgents: 15
    },
    lastUpdated: metrics.v1Progress.last_updated,
    source: 'metrics.db'
  }, null, 2));
