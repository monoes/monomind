...
```bash
# List all results
npx monobrain@latest memory list --namespace {{namespace}}

# Search for specific patterns
npx monobrain@latest memory search -q "{{filter}}" -n {{namespace}}

# Get detailed entries
{{#each results}}
npx monobrain@latest memory get -k "{{this.key}}" -n {{namespace}}
{{/each}}
```
