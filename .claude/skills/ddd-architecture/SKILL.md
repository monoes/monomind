...
description: "Domain-Driven Design architecture for monomind. Implements modular, bounded context architecture with clean separation of concerns and microkernel pattern."
triggers:
  - god object
...
Start with the shared kernel (base entity, value object, aggregate root, domain event classes) before any bounded context. Then implement core domains in dependency order: task-management first (no cross-domain dependencies), followed by session-management and health-monitoring (which may subscribe to task events), and finally event-coordination as the integration layer that wires domain events across contexts.
...
- `core-implementation` - Implementation of DDD domains
- `memory-unification` - AgentDB integration within bounded contexts
- `swarm-coordination` - Swarm coordination as domain plugin
- `performance-optimization` - Performance optimization across domains
