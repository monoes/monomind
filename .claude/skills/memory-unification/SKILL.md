...
class HNSWIndexer {
  constructor(dimensions: number = 1536) {
    this.index = new HNSWIndex({
      dimensions,
      efConstruction: 200, // higher = better build quality, slower construction
      M: 16,               // max bi-directional connections per node in the graph
      ef: 100,             // search beam width — larger = higher recall, slower query
    });
  }
...
const migrateFromSQLite = async () => {
  ...
  // TODO: For large datasets (>10k records), implement:
  // 1. Batch processing (e.g., 500 records per batch)
  // 2. Checkpoint/resume via a migration_progress table
  // 3. Progress reporting via onProgress callback
