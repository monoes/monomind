...
description: "Core module implementation for monomind. Implements DDD domains, clean architecture patterns, dependency injection, and modular TypeScript codebase with comprehensive testing."
triggers:
...
Implements the core TypeScript modules for monomind following Domain-Driven Design principles, clean architecture patterns, and modern TypeScript best practices with comprehensive test coverage.
...
export class TaskStatus extends ValueObject<TaskStatusType> {
  ...
  static fromString(value: string): TaskStatus {
    const valid: TaskStatusType[] = ["pending", "assigned", "in_progress", "completed", "failed"];
    if (!valid.includes(value as TaskStatusType)) {
...
export class Priority extends ValueObject<PriorityLevel> {
  ...
  static fromString(value: string): Priority {
    const valid: PriorityLevel[] = ["low", "medium", "high", "critical"];
    if (!valid.includes(value as PriorityLevel)) {
...
- `ddd-architecture` - DDD architectural design
- `mcp-optimization` - MCP server integration
- `memory-unification` - AgentDB repository integration
- `swarm-coordination` - Swarm domain implementation
