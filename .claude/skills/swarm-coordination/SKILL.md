...
description: "15-agent hierarchical mesh coordination for complex implementations. Orchestrates parallel execution across security, core, and integration domains following 10 ADRs with 14-week timeline."
triggers:
  - spawn 15 agents
...
Orchestrates the complete 15-agent hierarchical mesh swarm for monomind implementation, coordinating parallel execution across domains while maintaining dependencies and timeline adherence.
...
Task("Integration architecture", "Deep monomind integration architecture", "v1-integration-architect")
...
| 10  | Integration Architect | Integration   | Integration  | monomind deep integration   |
...
const phase4 = async () => {
  ...
  await Promise.all([
    ...
    // Replace with actual agent IDs from the roster above
    // e.g. Task("Final polish", "Agent #X final optimization", "agent-name-from-roster")
  ]);
};
```
...
class GitHubCoordination {
  ...
  async createEpicIssues(): Promise<void> {
    const epics = [
      ...
      { title: "Monomind Integration", agents: [10] },
...
class SwarmCommunication {
  // Communication via MCP tool calls and shared .monomind/sessions/ namespace
  // QuicSwarmBus is aspirational — implement when QUIC transport is added
  private bus = {
    broadcast: async (message: SwarmMessage, opts: { targetAgents: number[]; priority: string }) => {
      // Use mcp__monomind__* tools or write to shared session namespace
      throw new Error("Not implemented: use MCP tool calls for inter-agent messaging");
    },
  };
...
- `security-hardening` - Security domain coordination
- `memory-unification` - Memory system coordination
- `agentic-integration` - Integration domain coordination
- `performance-optimization` - Performance domain coordination
