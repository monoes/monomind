 ...
 *
 * @module v1/plugins/agentic-qe/bridges/QEModelRoutingAdapter
 */
...
// Router types (would be imported from @monobrain/cli/ruvector in production)
 ...
 *
 * Aligns TinyDancer routing with V1's ADR-026 Agent Booster routing.
 * Provides optimal model selection for QE tasks.
 */
export class QEModelRoutingAdapter implements IQEModelRoutingAdapter {
  private enhancedRouter: IEnhancedModelRouter | null;
  private logger: QELogger;

  constructor(
    logger: QELogger,
    enhancedRouter?: IEnhancedModelRouter
  ) {
    this.logger = logger;
    this.enhancedRouter = enhancedRouter || null;
  }
  ...
  async routeQETask(task: QETask): Promise<QERouteResult> {
    try {
      ...
      // 4. Try V1 router if available for enhanced routing
      let enhancedResult: EnhancedRouteResult | null = null;
      if (this.enhancedRouter) {
        try {
          enhancedResult = await this.enhancedRouter.route(task.description, {
            filePath: task.targetPath,
          });
        } catch (error) {
          this.logger.warn('V1 router failed, using fallback', error);
        }
      }

      // 5. Build final route result
      const model = this.selectModel(tier, enhancedResult);
      ...
      const result: QERouteResult = {
        ...
        explanation: this.buildExplanation(task, tier, agentBooster, enhancedResult),
      };
  ...
  /**
   * Select model based on tier and V1 router result
   */
  private selectModel(tier: ModelTier, enhancedResult: EnhancedRouteResult | null): ModelSelection {
    // If V1 router provided a result, use its model if tiers align
    if (enhancedResult && enhancedResult.tier === tier) {
      return enhancedResult.model;
    }
  ...
  private buildExplanation(
    ...
    enhancedResult: EnhancedRouteResult | null
  ): string {
    const parts: string[] = [];
    ...
    // router alignment
    if (enhancedResult) {
      if (enhancedResult.tier === tier) {
        parts.push('V1 router agrees with tier selection.');
      } else {
        parts.push(`Router suggested Tier ${enhancedResult.tier} but QE complexity mapping prefers Tier ${tier}.`);
      }
    }
