...
async function logSecurityEvent(event: SecurityEvent): Promise<void> {
  await memory.store({
    namespace: 'aqe/v1/security-audit',
...
const NAMESPACE_PREFIXES = {
  'test-generation': 'aqe/v1/test-patterns',
  'test-execution': 'aqe/v1/test-execution',
  'security-compliance': 'aqe/v1/security-findings', // HIGH security
  'chaos-resilience': 'aqe/v1/chaos-experiments',    // CRITICAL security
} as const;
...
Both plugins MUST use the V1 security module components:
...
```
+----------------+     +----------------+     +----------------+
|  MCP Boundary  | --> | Plugin Domain  | --> | V1 Core Domain |
