...
| v1 Component       | QE Usage              | Namespace                      |
| ------------------ | --------------------- | ------------------------------ |
| AgentDB Adapter    | Test pattern storage  | `aqe/v1/test-patterns`         |
| HNSW Index         | Coverage gap search   | `aqe/v1/coverage-data`         |
| Embeddings Service | Code semantic search  | `aqe/v1/code-knowledge`        |
| ReasoningBank      | Learning trajectories | `aqe/v1/learning-trajectories` |
...
export class QEMemoryBridge {
  ...
  async storeTestPattern(pattern: TestPattern): Promise<string> {
    // Generate embedding using v1 embeddings service
  ...
  async storeCoverageGap(gap: CoverageGap): Promise<string> {
    ...
    return this.memory.store({
      namespace: "aqe/v1/coverage-data",
  ...
  async storeTrajectory(trajectory: LearningTrajectory): Promise<string> {
    ...
    return this.memory.store({
      namespace: "aqe/v1/learning-trajectories",
...
async function generateQEEmbedding(text: string): Promise<Float32Array> {
  const v1Embeddings = await context.get<EmbeddingsService>("embeddings");

  // Use v1's ONNX-based embedding generation (75x faster)
  return v1Embeddings.generate(text, {
    normalize: true,
...
The QE security-compliance context adapts to v1's security module patterns and uses its primitives.

#### Security Compliance Mapping

| QE Security Feature | v1 Security Component | Integration              |
| ------------------- | --------------------- | ------------------------ |
...
export class QESecurityBridge {
  ...
  async executeDAST(
    ...
    // Use v1 SafeExecutor for controlled execution
  ...
  async createAuditEntry(event: AuditEvent): Promise<SignedAuditEntry> {
    ...
    // Sign with v1 token generator
    const signature = this.security.tokenGenerator.sign(JSON.stringify(entry), {
      algorithm: "HS256",
    });
  ...
  /**
   * Detect PII using v1 input validator patterns
   */
...
QE acts as a customer of v1 Core services, generating tests that Core executes.

#### Core Service Usage

| QE Feature          | v1 Core Service    | Interaction                         |
| ------------------- | ------------------ | ----------------------------------- |
...
export class QECoreBridge {
  ...
  /**
   * Spawn test execution agents via v1 Core
   */
  ...
  /**
   * Create test execution task via v1 Task Service
   */
...
QE's Queen Coordinator integrates with v1's Hive Mind for swarm coordination.

#### Coordination Mapping

| QE Coordinator      | v1 Hive Mind     | Integration                         |
| ------------------- | ---------------- | ----------------------------------- |
...
export class QEHiveBridge {
   ...
   */
  async broadcastResult(taskId: string, result: QEResult): Promise<void> {
...
QE tools are registered with v1's MCP server using the standard tool definition format.
...
TinyDancer model routing is aligned with v1's ADR-026 Agent Booster routing.
...
export class QEModelRoutingAdapter {
  ...
  async routeQETask(task: QETask): Promise<QERouteResult> {
    ...
    // Get v1 route decision
    const v1Result = await this.v1Router.route(task.description, {
    ...
    return {
      ...
      recommendedAgents: this.getAgentsForTier(v1Result.tier, task.category),
      costEstimate: this.estimateCost(v1Result, task),
    };
  }
...
```typescript
// QE-specific errors that bridge to v1 error system

class QEIntegrationError extends Error {
  constructor(
    message: string,
    public code: QEErrorCode,
    public v1Error?: Error,
...
// Error translation from v1 to QE domain
function translatev1Error(error: Error): QEIntegrationError {
