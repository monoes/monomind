...
export class ConfigTools implements MCPToolProvider {
  private configs: Map<string, MonobrainConfig>;
      ...
      switch (toolName) {
        ...
        case 'config_save':
          return await this.saveConfigTool(
            ...
            params.config as MonobrainConfig
          );

        case 'config_validate':
          return await this.validateConfigTool(params.config as MonobrainConfig);
   ...
   */
  async loadConfig(path: string): Promise<MonobrainConfig> {
   ...
   */
  async saveConfig(path: string, config: MonobrainConfig): Promise<{ success: boolean }> {
  ...
  private async saveConfigTool(path: string, config: MonobrainConfig): Promise<MCPToolResult> {
    await this.saveConfig(path, config);
    return { success: true };
  }

  private async validateConfigTool(config: MonobrainConfig): Promise<MCPToolResult> {
    const errors: string[] = [];
  ...
  private getDefaultConfig(): MonobrainConfig {
    return {
      swarm: {
