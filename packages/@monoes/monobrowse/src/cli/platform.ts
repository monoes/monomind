...
export function createPlatformCommand(): Command {
  ...
  cmd
    ...
    .description(
      'Connect a platform account via browser — opens Chrome, waits for login, saves session',
    )
    ...
    .action(async (platform: string, opts: { port: string; timeout: string }) => {
      ...
      try {
        browser = await import('@monoes/monobrowse');
      } catch (err) {
      ...
      try {
        ...
        if (!isLoggedIn) {
          ...
          console.log(
            `│  Waiting up to ${Math.round(timeoutMs / 60000)} minutes for login…           │`,
          );
          console.log('└──────────────────────────────────────────────────────┘');
          console.log('');
          ...
          while (Date.now() < deadline) {
            ...
            if (
              !currentUrl.includes(new URL(adapter.loginURL()).hostname) &&
        ...
        const userAgent = (await browser
          .evaluateJs(client, sid, 'navigator.userAgent')
        ...
        if (idx >= 0) sessions[idx] = sessionEntry;
        else sessions.push(sessionEntry);
        ...
        console.log('');
        console.log(`Session will be automatically used by: monomind browse playbook run`);
      } finally {
        client.close();
      }
    });
  ...
  cmd
    ...
    .action(async () => {
      ...
      for (const s of sessions) {
        ...
        console.log(
          `${s.id.padEnd(40)} ${s.platform.padEnd(12)} ${s.username.padEnd(30)} ${lastUsed}`,
        );
      }
    });
