...
import { fetchNewTarget, fetchTargets } from './cdp.js';
...
export async function activateTab(
  ...
  const result = await client.send<{ sessionId: string }>('Target.attachToTarget', {
    targetId,
    flatten: true,
  });
...
export async function switchToFrame(
  ...
  const { frameTree } = await client.send<{
    frameTree: {
      childFrames?: Array<{ frame: { id: string; url: string; securityOrigin: string } }>;
    };
  ...
  const oopif = targets.targetInfos.find((t) => t.type === 'iframe' && t.url === match.frame.url);
  if (oopif) {
