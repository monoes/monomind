...
export const microsoftAdapter: PlatformAdapter = {
  ...
  loginURL: () =>
    'https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=common&response_type=code&scope=openid+profile+email',
  async isLoggedIn(page: PageInterface): Promise<boolean> {
    ...
    return page.evaluate<boolean>(
      "!!(document.querySelector('[data-tid=\"user-email\"]') || document.querySelector('.mectrl_currentAccount') || document.cookie.includes('ESTSAUTH'))",
    );
  },
