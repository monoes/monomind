...
import type { PlaybookDef, NodeDef, Item, RunRecord, StepEvent, RunStatus } from './types.js';
...
export async function runPlaybook(
  ...
  const { handlers = new Map(), onEvent = () => {}, signal, params = {}, isStopRequested } = options;
  ...
  if (isStopRequested) {
    stopPollTimer = setInterval(() => {
      if (isStopRequested(runId)) controller.abort();
    }, 500);
    // Avoid keeping Node.js event loop alive if nothing else is running
    stopPollTimer.unref?.();
  }

  const emit = (partial: Omit<StepEvent, 'runId' | 'playbookId' | 'playbookName' | 'timestamp'>) =>
    onEvent({ runId, playbookId: def.id, playbookName: def.name, timestamp: Date.now(), ...partial });
  ...
  if (sorted.length !== def.nodes.length) {
    throw new Error('Playbook contains a cycle');
  }
