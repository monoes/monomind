...
function printUsage() {
  console.log(`Usage: monodesign ignores <action> [options]

Manage detector ignores in .monodesign config.
...
Scope:
  --shared                              Write .monodesign/config.json (default)
  --local                               Write .monodesign/config.local.json
...
Examples:
  monodesign ignores add-file "src/legacy/**"
  monodesign ignores add-value overused-font Inter --reason "Brand font"
  monodesign ignores add-value design-system-color "*" --file "src/demo.css"
  monodesign ignores remove-value overused-font Inter`);
}
...
function parseValueArgs(args, { allowUnscopedWildcard = false } = {}) {
  ...
  if (!rule || !value) throw new Error('Pass a rule id and value, e.g. monodesign ignores add-value overused-font Inter');
...
function addRule(cwd, args) {
  ...
  if (!rule) throw new Error('Pass a rule id, e.g. monodesign ignores add-rule side-tab');
...
function addFile(cwd, args) {
  ...
  if (!glob) throw new Error('Pass a glob, e.g. monodesign ignores add-file "src/legacy/**"');
  const config = readScopeConfig(cwd, local);
...
function removeRule(cwd, args) {
  return removeFromScopes(cwd, args, (config, rest) => {
    ...
    if (!rule) throw new Error('Pass a rule id, e.g. monodesign ignores remove-rule side-tab');
...
function removeFile(cwd, args) {
  return removeFromScopes(cwd, args, (config, rest) => {
    const glob = String(rest[0] || '').trim();
    if (!glob) throw new Error('Pass a glob, e.g. monodesign ignores remove-file "src/legacy/**"');
...
export async function run(args = [], opts = {}) {
  ...
  if (!action) {
    throw new Error(`Unknown ignores action: ${actionArg}. Run "monodesign ignores --help".`);
  }
