...
import { filterDetectionFindings } from '../cli/lib/monodesign-config.mjs';
...
describe('detectUrl — browser-only fixtures', { skip: browserAvailable ? false : 'puppeteer browser not installed' }, () => {
  ...
  it('typography side-by-side: element-level flag cases get regular overlays', async () => {
    ...
    try {
      ...
      await page.evaluate(() => { window.__MONODESIGN_CONFIG__ = { autoScan: false }; });
      await page.evaluate(browserScript);
      const result = await page.evaluate(() => {
        const groups = window.monodesignScan();
        ...
        return {
          ...
          hasBanner: Boolean(document.querySelector('.monodesign-banner')),
          overlays: document.querySelectorAll('.monodesign-overlay:not(.monodesign-banner)').length,
        };
      });
  ...
  it('browser API: visual contrast fallback resolves readable image backgrounds without overlays', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        const before = document.querySelectorAll('.monodesign-overlay, .monodesign-label, .monodesign-banner').length;
        const analyses = await window.monodesignAnalyzeVisualContrast({ maxCandidates: 20, scrollOffscreen: true });
        const after = document.querySelectorAll('.monodesign-overlay, .monodesign-label, .monodesign-banner').length;
        return {
          before,
          after,
  ...
  it('browser API: visual contrast scan decorates visible findings without scrolling by default', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        const syncScanResult = window.monodesignScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const syncDetectResult = window.monodesignDetect({
          visualContrast: true,
          serialize: true,
        });
        const groups = await window.monodesignScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        ...
        return {
          ...
          overlays: document.querySelectorAll('.monodesign-overlay:not(.monodesign-banner)').length,
          labels: document.querySelectorAll('.monodesign-label').length,
          analyses: window.monodesignGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
          scrollEvents,
          maxScrollY,
          finalScrollY: window.scrollY,
          syncScanIsArray: Array.isArray(syncScanResult),
          syncDetectIsArray: Array.isArray(syncDetectResult),
          hasAsyncApi: typeof window.monodesignScanAsync === 'function' && typeof window.monodesignDetectAsync === 'function',
        };
      });
      ...
      assert.equal(result.syncScanIsArray, true, `monodesignScan should keep a synchronous Array return: ${JSON.stringify(result)}`);
      assert.equal(result.syncDetectIsArray, true, `monodesignDetect should keep a synchronous Array return: ${JSON.stringify(result)}`);
      ...
      const refreshedOverlayResult = await page.evaluate(async () => {
        ...
        const initialGroups = window.monodesignScan({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        ...
        while (
          Date.now() < deadline &&
          !/low contrast/i.test(target?._monodesignOverlay?.textContent || '')
        ) {
          const nextButton = target?._monodesignOverlay?.querySelector('button:last-of-type');
          if (nextButton) nextButton.click();
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const labelVariants = [];
        const overlay = target?._monodesignOverlay;
        for (let i = 0; i < 3; i++) {
        ...
        return {
          ...
          labelText: target?._monodesignOverlay?.textContent || '',
          labelVariants,
          overlayConnected: Boolean(target?._monodesignOverlay?.isConnected),
        };
      });
      ...
      const lazyResult = await page.evaluate(async () => {
        ...
        return {
          ...
          labels: document.querySelectorAll('.monodesign-label').length,
          analyses: window.monodesignGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
          targetHasOverlay: Boolean(target?._monodesignOverlay),
      ...
      const staleOverlayResult = await page.evaluate(async () => {
        ...
        window.scrollTo(0, 0);
        await new Promise(resolve => setTimeout(resolve, 50));
        await window.monodesignScanAsync({
          visualContrast: true,
          visualContrastMaxCandidates: 20,
        });
        const staleCleared = !target?._monodesignOverlay;
        target?.scrollIntoView({ block: 'center', inline: 'nearest', behavior: 'instant' });
        await new Promise(resolve => setTimeout(resolve, 250));
        return {
          staleCleared,
          targetHasOverlay: Boolean(target?._monodesignOverlay),
          targetOverlayConnected: Boolean(target?._monodesignOverlay?.isConnected),
          overlays: document.querySelectorAll('.monodesign-overlay:not(.monodesign-banner)').length,
          analyses: window.monodesignGetLastVisualContrastAnalyses().filter(item => item.status === 'fail').length,
        };
      });
  ...
  it('extension mode remove cancels pending lazy visual contrast work', async () => {
    ...
    try {
      ...
      await page.evaluate(() => {
        ...
        window.__monodesignMessages = [];
        window.addEventListener('message', event => {
          if (event.source !== window || !event.data?.source?.startsWith('monodesign-')) return;
          window.__monodesignMessages.push(event.data);
        });
      });
      await page.evaluate(browserScript);
      const result = await page.evaluate(async () => {
        ...
        while (
          ...
          !window.monodesignGetLastVisualContrastAnalyses()
            .some(item => item.status === 'unresolved' && item.reason === 'text outside viewport')
        ) {
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const unresolvedBeforeRemove = window.monodesignGetLastVisualContrastAnalyses()
        ...
        const resultsAfterRemove = window.__monodesignMessages
          .filter(message => message.source === 'monodesign-results').length;
        return {
          unresolvedBeforeRemove,
          overlayCount: document.querySelectorAll('.monodesign-overlay').length,
          targetHasOverlay: Boolean(target?._monodesignOverlay),
          resultsAfterRemove,
        };
      });
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          ...
          while (
            Date.now() < deadline &&
            !window.__monodesignMessages.some(message => message.source === 'monodesign-error')
          ) {
            await new Promise(resolve => setTimeout(resolve, 25));
          }
          return {
            ready: window.__monodesignMessages.some(message => message.source === 'monodesign-ready'),
            results: window.__monodesignMessages.some(message => message.source === 'monodesign-results'),
            errors: window.__monodesignMessages
              .filter(message => message.source === 'monodesign-error')
  ...
  it('extension mode echoes scan ids on result messages', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        while (
          Date.now() < deadline &&
          !window.__monodesignMessages.some(message =>
            message.source === 'monodesign-results' &&
            message.scanId === 'scan-2'
          )
        ) {
          await new Promise(resolve => setTimeout(resolve, 25));
        }
        const resultMessage = window.__monodesignMessages.find(message =>
          message.source === 'monodesign-results' &&
          message.scanId === 'scan-2'
        );
        return {
  ...
  it('browser API: monodesignDetect is pure, monodesignScan decorates', async () => {
    ...
    try {
      ...
      const pure = await page.evaluate(() => {
        const before = document.querySelectorAll('.monodesign-overlay, .monodesign-label, .monodesign-banner').length;
        const findings = window.monodesignDetect({ decorate: false, serialize: true });
        const after = document.querySelectorAll('.monodesign-overlay, .monodesign-label, .monodesign-banner').length;
      ...
      const decorated = await page.evaluate(() => {
        const groups = window.monodesignScan();
        const overlays = document.querySelectorAll('.monodesign-overlay, .monodesign-label, .monodesign-banner').length;
  ...
  it('browser API: async scan and detect reject instead of throwing synchronously', async () => {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        ...
        try {
          const scan = await window.monodesignScanAsync().then(
            () => ({ state: 'resolved' }),
            error => ({ state: 'rejected', message: error?.message || String(error) }),
          );
          const detect = await window.monodesignDetectAsync().then(
            () => ({ state: 'resolved' }),
            error => ({ state: 'rejected', message: error?.message || String(error) }),
          );
          return { scan, detect };
