/**
 * Unit tests for the Monodesign design hook.
...
function mkTmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'monodesign-hook-'));
}
...
describe('readConfig()', () => {
  ...
  it('parses hook runtime and legacy hook detector filters', () => {
    fs.mkdirSync(path.join(cwd, '.monodesign'), { recursive: true });
...
describe('ensureHookGitExcludes()', () => {
  ...
  it('adds hook runtime files to local git info exclude, not tracked .gitignore', () => {
    ...
    assert.match(exclude, /\.monodesign\/hook\.cache\.json/);
    assert.match(exclude, /\.monodesign\/hook\.pending\.json/);
    assert.match(exclude, /\.monodesign\/config\.local\.json/);

    const second = ensureHookGitExcludes(cwd);
    assert.equal(second.changed, false);
    const rewritten = fs.readFileSync(path.join(cwd, '.git', 'info', 'exclude'), 'utf-8');
    assert.equal((rewritten.match(/monodesign-hook-ignore-start/g) || []).length, 1);
  });
});
...
describe('hook-admin.mjs', () => {
  ...
  it('ignore-value --local writes private config and status reports local ignores', () => {
    ...
    assert.match(status, /local file:\s+\.monodesign\/config\.local\.json/);
    assert.match(status, /ignoreValues:\s+overused-font=inter/);
  });

  it('a /monodesign hooks edit preserves sibling hook fields (consent, quiet)', () => {
    fs.mkdirSync(path.join(cwd, '.monodesign'), { recursive: true });
  ...
  it('hooks on accepts declined consent and installs missing provider manifests', () => {
    ...
    for (const provider of ['.claude', '.agents', '.cursor', '.github']) {
      fs.mkdirSync(path.join(cwd, provider, 'skills', 'monodesign', 'scripts'), { recursive: true });
    }
    ...
    fs.writeFileSync(path.join(cwd, '.claude', 'settings.local.json'), JSON.stringify({
      hooks: {
        PostToolUse: [
          ...
          { matcher: 'Edit', hooks: [{ type: 'command', command: 'node ".claude/skills/monodesign/scripts/hook.mjs"' }] },
        ],
      },
    }));
...
describe('renderTemplate()', () => {
  it('starts with the versioned envelope and caps to maxFindings', () => {
    ...
    assert.match(text, /\.\.\. and 7 more \(see \/monodesign audit\)\./);
  ...
  it('emits a directive footer (imperative + judgment clause + confirmed ignore guidance)', () => {
    ...
    assert.match(text, /inline `monodesign-disable <rule>` comment only when the waiver must travel with a file/);
    assert.match(text, /ignore-value \.\.\. --shared/);
    assert.match(text, /ignore-rule overused-font --all-values/);
    assert.match(text, /\/monodesign hooks ignore-file Card\.tsx/);
    assert.match(text, /ignore-rule <id>/);
    assert.match(text, /\/monodesign audit/);
  });
  ...
  it('shows the exact value-specific command for overused-font findings', () => {
    ...
    assert.match(text, /\/monodesign hooks ignore-value overused-font Roboto --shared/);
    assert.match(text, /ignore-rule overused-font --all-values/);
  });
  ...
  it('shows the exact value-specific command for bounce-easing findings', () => {
    ...
    assert.match(text, /\/monodesign hooks ignore-value bounce-easing bounce-ball --shared/);
  });
  ...
  it('does not suggest ignore-value for rules that cannot be value-filtered', () => {
    ...
    assert.doesNotMatch(text, /\/monodesign hooks ignore-value side-tab Inter/);
  });
...
describe('writeAuditLog()', () => {
  ...
  it('appends NDJSON when MONODESIGN_HOOK_LOG is set', () => {
    ...
    writeAuditLog({ MONODESIGN_HOOK_LOG: log }, { event: 'PostToolUse', emitted: true });
    writeAuditLog({ MONODESIGN_HOOK_LOG: log }, { event: 'PostToolUse', emitted: false });
  ...
  it('is a no-op when MONODESIGN_HOOK_LOG is unset', () => {
  ...
  it('prefers the env var over config hook.auditLog', () => {
    ...
    writeAuditLog({ MONODESIGN_HOOK_LOG: envLog }, { event: 'PostToolUse' }, cwd);
  ...
  it('resolves config auditLog from entry.cwd (the event project root), not the fallback cwd', () => {
    ...
    fs.mkdirSync(path.join(projectDir, '.monodesign'), { recursive: true });
    fs.writeFileSync(path.join(projectDir, '.monodesign', 'config.json'),
...
`);
  ...
  it('emits a clean ack when the file has zero findings', async () => {
    ...
    assert.match(r.stdout, /keep following the project design system and the monodesign skill guidance/);
    assert.equal(r.audit.emitted, true);
  ...
  it('MONODESIGN_HOOK_QUIET=1 suppresses clean and pending acks, keeps findings emission', async () => {
    ...
    const rClean = await runHook({
      ...
      env: { MONODESIGN_HOOK_QUIET: '1' }, cwd, detector: detClean,
    });
    ...
    const rFindings = await runHook({
      ...
      env: { MONODESIGN_HOOK_QUIET: '1' }, cwd, detector: detFindings,
    });
  ...
  it('re-entrancy guard short-circuits when MONODESIGN_HOOK_DEPTH is set', async () => {
    const file = writeFixture('src/Card.tsx', 'noop');
    const det = fakeDetector([finding('side-tab', 1)]);
    const r = await runHook({
      stdinJson: JSON.stringify(eventFor(file)),
      env: { MONODESIGN_HOOK_DEPTH: '1' },
      cwd,
      detector: det,
    });
    assert.equal(r.stdout, '');
  ...
  it('MONODESIGN_HOOK_DISABLED kill switch', async () => {
    const file = writeFixture('src/Card.tsx', 'noop');
    const det = fakeDetector([finding('side-tab', 1)]);
    for (const v of ['1', 'true', 'yes', 'on', 'TRUE']) {
      const r = await runHook({
        stdinJson: JSON.stringify(eventFor(file)),
        env: { MONODESIGN_HOOK_DISABLED: v },
        cwd,
        detector: det,
      });
  ...
  it('adds a non-blocking note when DESIGN.md is newer than the sidecar', async () => {
    ...
    assert.match(r.stdout, /No deterministic design-quality issues found/);
    assert.match(r.stdout, /DESIGN\.md is newer than \.monodesign\/design\.json/);
    assert.match(r.stdout, /\/monodesign document/);
  });
  ...
  it('emits suppressionNotice text on the threshold-crossing edit', async () => {
    ...
    assert.match(r.stdout, /Run \/monodesign audit to revisit/);
  });
  ...
  it('honors an inline monodesign-disable comment so the hook scans the file clean', async () => {
    // The hook runs the same engine as `monomind design detect`, so an in-file
    ...
    const waived = writeFixture('src/Waived.tsx',
      'const css = "font-family: Inter"; // monodesign-disable-line overused-font');
...
describe('runHook() — cache write gating (issues #344, #305)', () => {
  ...
  // `.monodesign/` footprint: skipped files never dirty the cache, and a
  ...
  it('non-UI edit (.md) does not create .monodesign/', async () => {
    ...
    assert.equal(r.audit.skipped, 'extension');
    assert.ok(!fs.existsSync(path.join(cwd, '.monodesign')), '.monodesign should not exist');
  });

  it('clean UI edit in a project with no footprint does not create .monodesign/, still acks', async () => {
  ...
  it('detector-missing path does not create .monodesign/', async () => {
    const file = write('src/Card.tsx', 'noop');
    const r = await runHook({
      stdinJson: JSON.stringify(eventFor(file)),
      env: {}, cwd, detector: {},
  ...
  it('fresh findings create the cache, and dedup works on the next run', async () => {
    ...
    const first = await runHook({ stdinJson: JSON.stringify(eventFor(file)), env: {}, cwd, detector: det });
    assert.match(first.stdout, /Design hook findings requiring review/);
    assert.ok(fs.existsSync(path.join(cwd, '.monodesign', 'hook.cache.json')), 'cache should exist');
  ...
  it('clean UI edit in an opted-in project (existing .monodesign/) still persists editCount', async () => {
    fs.mkdirSync(path.join(cwd, '.monodesign'), { recursive: true });
    const file = write('src/Card.tsx', 'noop');
  ...
  it('umbrella launch keys the cache to the edited file\'s project root', async () => {
    // cwd is the umbrella: no .git / package.json / .monodesign of its own.
    ...
    assert.ok(fs.existsSync(path.join(child, '.monodesign', 'hook.cache.json')), 'cache should land in the child project');
    assert.ok(!fs.existsSync(path.join(cwd, '.monodesign')), 'umbrella root should stay clean');
  });
});
...
describe('suppressionNotice()', () => {
  it('starts with envelope and mentions /monodesign audit', () => {
...
describe('renderCleanAck() / renderPendingAck()', () => {
  it('renderCleanAck stays short and ends with the steer line', () => {
    ...
    assert.match(text, /^\[monodesign@1\] Design hook scanned src\/App\.jsx\. No deterministic design-quality issues found\./);
    assert.match(text, /keep following the project design system and the monodesign skill guidance/);
  ...
  it('renderPendingAck quotes up to 3 known findings and counts the rest', () => {
    ...
    assert.match(text, /^\[monodesign@1\] Design hook scanned src\/SlopCard\.jsx\./);
...
describe('resolveHarness() / normalizeHookEvent()', () => {
  it('routes explicit env and Cursor conversation_id to cursor harness', () => {
    assert.equal(resolveHarness({ MONODESIGN_HOOK_HARNESS: 'cursor' }), 'cursor');
  ...
  it('routes a GitHub Copilot postToolUse event (toolName/toolArgs) to the github harness', () => {
    ...
    assert.equal(resolveHarness({ MONODESIGN_HOOK_HARNESS: 'github' }), 'github');
...
describe('expandScanTargets()', () => {
  ...
  it('does not follow imports from traversal-looking primary targets', () => {
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'monodesign-hook-outside-'));
    try {
...
describe('Cursor hook scripts', () => {
  ...
  it('preToolUse denies proposed writes with detector findings before they land', () => {
    ...
    const out = execFileSync(process.execPath, [path.join('skill', 'scripts', 'hook-before-edit.mjs')], {
      ...
      env: { ...process.env, MONODESIGN_HOOK_LOG: logPath },
      encoding: 'utf-8',
    });
  ...
  it('preToolUse gates configured template extensions (issue #316)', () => {
    ...
    fs.mkdirSync(path.join(cwd, '.monodesign'), { recursive: true });
    fs.writeFileSync(path.join(cwd, '.monodesign', 'config.json'), JSON.stringify({
  ...
  it('preToolUse honors truthy MONODESIGN_HOOK_DISABLED values before stdin parsing', () => {
    const logPath = path.join(cwd, 'hook.ndjson');
    ...
    const out = execFileSync(process.execPath, [path.join('skill', 'scripts', 'hook-before-edit.mjs')], {
      ...
      env: {
        ...process.env,
        MONODESIGN_HOOK_DISABLED: 'true',
        MONODESIGN_HOOK_LOG: logPath,
      },
      encoding: 'utf-8',
    });
...
describe('runHook() — emission enrichment', () => {
  ...
  it('returns emission.kind fresh with findings on new hits', async () => {
    ...
    const r = await runHook({
      ...
      env: { MONODESIGN_HOOK_HARNESS: 'claude' },
      cwd,
