...
describe('buildInsertWrapperLines', () => {
  it('produces an insert scaffold without an original variant', () => {
    ...
    assert.match(joined, /monodesign-variants-start abc12345/);
    assert.match(joined, /data-monodesign-variants="abc12345"/);
    assert.match(joined, /data-monodesign-mode="insert"/);
    assert.match(joined, /Variants: insert below this line/);
    assert.doesNotMatch(joined, /data-monodesign-variant="original"/);
  });
  ...
  it('keeps marker comments inside the wrapper for JSX', () => {
    ...
    const joined = lines.join('\n');
    assert.match(joined, /^\s*<div data-monodesign-variants="jsx12345"/m);
    assert.match(joined, /{\/\* monodesign-variants-start jsx12345 \*\/}/);
    assert.doesNotMatch(joined, /data-monodesign-variant="original"/);
  });
});
...
describe('live-insert CLI integration', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'monodesign-insert-test-'));
  });
...
<body>
  ...
  <section class="features">
    ...
    assert.match(after, /data-monodesign-mode="insert"/);
    assert.match(after, /monodesign-variants-start ins12345/);
    assert.ok(after.indexOf('class="hero"') < after.indexOf('monodesign-variants-start ins12345'));
    assert.ok(after.indexOf('monodesign-variants-start ins12345') < after.indexOf('class="features"'));
    assert.doesNotMatch(after, /data-monodesign-variant="original"/);
  });
  ...
  <section class="cta">CTA</section>
    ...
    assert.ok(after.indexOf('monodesign-variants-start ins99999') < after.indexOf('class="cta"'));
    assert.ok(after.indexOf('class="hero"') < after.indexOf('monodesign-variants-start ins99999'));
  });
  ...
  return (
    ...
    assert.match(after, /data-monodesign-mode="insert"/);
    assert.match(after, /{\/\* monodesign-variants-start jsxins01 \*\/}/);
  });
