...
describe('live browser script parts', () => {
  ...
  it('assembles prelude, session helper, and browser UI in order', () => {
    ...
    const tokenIndex = script.indexOf('window.__MONODESIGN_TOKEN__');
    const portIndex = script.indexOf('window.__MONODESIGN_PORT__');
    const commandPrefixIndex = script.indexOf('window.__MONODESIGN_COMMAND_PREFIX__');
    const vocabIndex = script.indexOf('window.__MONODESIGN_VOCAB__');
    ...
    assert.match(script, /window\.__MONODESIGN_COMMAND_PREFIX__ = "\$"/);
    assert.ok(vocabIndex < sessionIndex);
    assert.ok(sessionIndex < domIndex);
    assert.ok(domIndex < browserIndex);
    assert.match(script, /monodesign live script part: session-state \(live-browser-session\.js\)/);
    assert.match(script, /monodesign live script part: dom-helpers \(live-browser-dom\.js\)/);
    assert.match(script, /monodesign live script part: browser-ui \(live-browser\.js\)/);
  });
});
