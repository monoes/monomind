...
describe('live-browser.js regression guards', () => {
  ...
  it('uses a Svelte-gated painted-ancestor crop proxy for shader capture', () => {
    ...
    assert.match(
      SOURCE,
      /function shouldUseAncestorCropShaderProxy\(el\) \{[\s\S]{0,260}?window\.__MONODESIGN_LIVE_ADAPTER__[\s\S]{0,280}?currentPreviewMode === 'svelte-component' \|\| svelteComponentSession[\s\S]{0,260}?dataset\?\.monodesignPreview === 'svelte-component';/,
  ...
  it('lets page editables keep Enter and arrow keydown events', () => {
    ...
    assert.match(
      ...
      'monodesign inline edit rows must keep their existing Escape-cancel path',
    );
  });
  ...
  it('pick mode preference persists in localStorage', () => {
    assert.match(
      SOURCE,
      /const INTERACTION_PREFS_KEY = 'monodesign-live-interaction';[\s\S]{0,3000}?function saveInteractionPrefs\(\)/,
  ...
  it('detect mode shows an empty result toast once per requested scan', () => {
    ...
    assert.match(
      SOURCE,
      /window\.postMessage\(\{ source: 'monodesign-command', action: 'remove' \}, '\*'\);[\s\S]{0,80}?activeDetectScanId = null;[\s\S]{0,80}?pendingDetectScanId = null;/,
  ...
  it('insert mode UI and generate payload guards', () => {
    ...
    assert.match(SOURCE, /data-monodesign-insert-placeholder/, 'placeholder element must be marked');
    assert.match(
      SOURCE,
      /showHighlight\(el\)[\s\S]{0,120}?data-monodesign-insert-placeholder/,
    ...
    assert.match(SOURCE, /data-monodesign-placeholder-resize/, 'placeholder edge handles on annotation overlay');
