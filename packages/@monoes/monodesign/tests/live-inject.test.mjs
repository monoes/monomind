...
function runInject(cwd, configPath, args) {
  try {
    const out = execFileSync('node', [INJECT, ...args], {
      cwd,
      encoding: 'utf-8',
      env: { ...process.env, MONODESIGN_LIVE_CONFIG: configPath },
...
function runInjectDefault(cwd, args) {
  try {
    const out = execFileSync('node', [INJECT, ...args], {
      cwd,
      encoding: 'utf-8',
      env: { ...process.env, MONODESIGN_LIVE_CONFIG: '' },
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    return JSON.parse(out.trim());
  } catch (err) {
...
describe('live-inject — insert/remove round-trip preserves file bytes', () => {
  ...
  it('reports .monodesign/live/config.json as the default missing config path', () => {
    ...
    assert.equal(result.path, join(realpathSync(tmp), '.monodesign', 'live', 'config.json'));
  });

  it('uses .monodesign/live/config.json without an environment override', () => {
    const original = `<html>
  <body>
    <p>Content</p>
  </body>
...
`;
    ...
    const configDir = join(tmp, '.monodesign', 'live');
...
`;
  ...
  it('round-trips through CSP-meta patch and revert (insert mutates the meta tag, remove restores it)', () => {
    ...
    // and stashes the original directives in `data-monodesign-csp-original`.
...
`;
  ...
  it('preserves the character after an insertAfter anchor with no trailing newline (#227)', () => {
    ...
    assert.ok(
      afterInject.includes('<!-- monodesign-live-end -->\nX</head>'),
