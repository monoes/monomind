import { describe, test, expect, beforeEach, afterEach } from './lib/bun-test-shim.mjs';
...
describe('monodesign ignores CLI', () => {
  let root;
  ...
  function readConfig(name = 'config.json') {
    return JSON.parse(readFileSync(join(root, '.monodesign', name), 'utf-8'));
  }
  ...
  test('rejects broad wildcard value ignores', () => {
    ...
    expect(existsSync(join(root, '.monodesign', 'config.json'))).toBe(false);
  });

  test('removes an existing broad wildcard value ignore', () => {
    mkdirSync(join(root, '.monodesign'), { recursive: true });
    writeFileSync(join(root, '.monodesign', 'config.json'), JSON.stringify({
      detector: {
