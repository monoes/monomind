 ...
 *   3. MONODESIGN_CONTEXT_DIR env var as a power-user escape hatch (only
...
beforeEach(() => {
  scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'monodesign-loadctx-'));
  savedEnv = process.env.MONODESIGN_CONTEXT_DIR;
  delete process.env.MONODESIGN_CONTEXT_DIR;
});
...
describe('resolveContextDir', () => {
  ...
  it('uses MONODESIGN_CONTEXT_DIR as a fallback when defaults are empty (relative path)', () => {
    write('design/PRODUCT.md');
    process.env.MONODESIGN_CONTEXT_DIR = 'design';
    assert.equal(resolveContextDir(scratch), path.join(scratch, 'design'));
  });

  it('uses MONODESIGN_CONTEXT_DIR as a fallback when defaults are empty (absolute path)', () => {
    const elsewhere = fs.mkdtempSync(path.join(os.tmpdir(), 'monodesign-elsewhere-'));
    try {
  ...
  it('default paths win over MONODESIGN_CONTEXT_DIR (lazy escape hatch)', () => {
  ...
  it('ignores empty MONODESIGN_CONTEXT_DIR', () => {
    write('PRODUCT.md');
    process.env.MONODESIGN_CONTEXT_DIR = '   ';
    assert.equal(resolveContextDir(scratch), scratch);
  });
...
describe('loadContext (monorepo project context)', () => {
  ...
  it('does not discover dependency or generated directories as workspace candidates', () => {
    ...
    const res = spawnSync(process.execPath, [SCRIPT_PATH], {
      cwd: scratch,
      encoding: 'utf8',
      env: { ...process.env, MONODESIGN_NO_UPDATE_CHECK: '1' },
    });
...
describe('loadContext (MONODESIGN_CONTEXT_DIR escape hatch)', () => {
  ...
  it('reports a missing override directory as no-context, not as a crash', () => {
    process.env.MONODESIGN_CONTEXT_DIR = 'no/such/dir';
    const ctx = loadContext(scratch);
...
describe('context.mjs CLI', () => {
  it('emits NO_PRODUCT_MD directive when no PRODUCT.md is found', async () => {
    const { spawnSync } = await import('node:child_process');
    const res = spawnSync(process.execPath, [SCRIPT_PATH], { cwd: scratch, encoding: 'utf8', env: { ...process.env, MONODESIGN_NO_UPDATE_CHECK: '1' } });
    assert.equal(res.status, 0);
...
// not surface UPDATE_AVAILABLE.
describe('context.mjs update check (disabled in monomind)', () => {
  it('never emits UPDATE_AVAILABLE, even when a newer version is cached', () => {
    ...
    const cacheFile = path.join(scratch, 'update-check.json');
    fs.writeFileSync(cacheFile, JSON.stringify({ lastCheck: Date.now(), latestVersion: '99.0.0' }));
    const project = path.join(scratch, 'project');
    fs.mkdirSync(project, { recursive: true });
    fs.writeFileSync(path.join(project, 'PRODUCT.md'), '# Acme\n');
    const env = { ...process.env, MONODESIGN_UPDATE_CACHE: cacheFile, MONODESIGN_NO_UPDATE_CHECK: '' };
    const res = spawnSync(process.execPath, [skillScript], { cwd: project, encoding: 'utf8', env });
    assert.equal(res.status, 0);
