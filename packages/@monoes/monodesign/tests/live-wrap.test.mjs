...
describe('findElement', () => {
  ...
  it('skips lines that contain data-monodesign-variant', () => {
    const lines = [
      '<div class="hero" data-monodesign-variant="original">Old</div>',
...
describe('wrapCli integration', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'monodesign-wrap-test-'));
...
<body>
  <div class="hero-section">
    ...
    assert.ok(modified.includes('data-monodesign-variants="test123"'));
    assert.ok(modified.includes('data-monodesign-variant-count="3"'));
    assert.ok(modified.includes('data-monodesign-variant="original"'));
    assert.ok(modified.includes('display: contents'));
    assert.ok(modified.includes('monodesign-variants-start test123'));
    assert.ok(modified.includes('monodesign-variants-end test123'));
    // Original should NOT be hidden (stays visible until variants arrive)
    assert.ok(!modified.includes('data-monodesign-variant="original" style="display: none"'));
  });
  ...
  return (
    ...
    assert.ok(modified.includes('{/* monodesign-variants-start jsx123'));
    assert.ok(modified.includes('data-monodesign-variant-count="2"'));
  });
...
<div id="pricing">
  ...
  <p>Plans start at $10/mo.</p>
    ...
    assert.ok(modified.includes('data-monodesign-variants="id123"'));
...
<div class="after">After</div>`;
    ...
    assert.ok(modified.includes('data-monodesign-variants="pres123"'));
  });
  ...
  <h1>Plain title</h1>
    ...
    assert.equal(result.cssAuthoring.styleTag, '<style data-monodesign-css="SESSION_ID">');
...
<section class="hero-shell">
  <h1>{title}</h1>
    ...
    assert.deepEqual(result.cssSelectorPrefixExamples, [
      ...
      '[data-monodesign-variant="2"]',
      '[data-monodesign-variant="3"]',
    ]);
    assert.equal(result.cssAuthoring.mode, 'astro-global-prefixed');
    assert.equal(result.cssAuthoring.strategy, 'global-prefixed');
    assert.equal(result.cssAuthoring.styleTag, '<style is:inline data-monodesign-css="SESSION_ID">');
    assert.match(result.cssAuthoring.rulePattern, /^\[data-monodesign-variant="N"\]/);
    assert.ok(
...
// .monodesign/live/pending-manual-edits.json from local dev tripped the
// fail-loud check in live-wrap. Clear the buffer around each test.
function clearManualEditsBuffer() {
  try {
    const p = join(process.cwd(), '.monodesign/live/pending-manual-edits.json');
    rmSync(p, { force: true });
  } catch {}
}
...
describe('live-wrap — JSX / TSX correctness', () => {
  ...
  return (
    ...
    assert.ok(modified.includes('monodesign-variants-start wrapA'), 'wrapper was created');
    ...
    const originalMatch = modified.match(/data-monodesign-variant="original"[^>]*>([\s\S]*?)\s*<\/div>/);
...
export default function App() {
  ...
  return (
    ...
    // The outer wrapper <div data-monodesign-variants="..."> appears BEFORE
    // both marker comments — markers are tucked inside.
    const wrapperIdx = modified.indexOf('data-monodesign-variants="frag1"');
    const startMarkerIdx = modified.indexOf('monodesign-variants-start frag1');
    const endMarkerIdx = modified.indexOf('monodesign-variants-end frag1');
  ...
  it('HTML wrapper keeps marker comments OUTSIDE the wrapper <div> (existing layout)', () => {
    ...
    const modified = readFileSync(join(tmp, 'page.html'), 'utf-8');
    const wrapperIdx = modified.indexOf('data-monodesign-variants="htmlFrag"');
    const startMarkerIdx = modified.indexOf('monodesign-variants-start htmlFrag');
  ...
  return (
    ...
    const modified = readFileSync(join(tmp, 'Page.tsx'), 'utf-8');
    const originalMatch = modified.match(/data-monodesign-variant="original"[\s\S]*?<\/div>/);
  ...
  return (
    ...
    assert.ok(modified.includes('data-monodesign-variants="short1"'),
  ...
  <section class="multiline-target">
    ...
    // line (the monodesign-variants-end marker for HTML).
    assert.match(lines[result.endLine - 1], /monodesign-variants-end ml1/,
  ...
  return (
    ...
    assert.ok(modified.includes('data-monodesign-variants="dyn1"'), 'wrapped (first-match fallback)');
  });
