...
function compileProviderBlocks(content, activeTags = []) {
...
describe('live reference authoring contract', () => {
  it('keeps setup guidance focused on inferred target paths', () => {
    ...
    assert.doesNotMatch(skillSrc, /Use this same scripts directory for all Monodesign helper commands/);
  ...
  it('keeps the live prompt focused on the foreground poll loop', () => {
    const liveMd = readFileSync(join(ROOT, 'skill/reference/live.md'), 'utf-8');
    const manualAgentMd = readFileSync(join(ROOT, 'skill/agents/monodesign-manual-edit-applier.md'), 'utf-8');
    ...
    assert.doesNotMatch(liveMd, /MONODESIGN_LIVE_COPY_AGENT|mock/);
    ...
    assert.match(liveMd, /delegate source edits to `monodesign_manual_edit_applier`/);
    ...
    assert.match(manualAgentMd, /codex-name: monodesign_manual_edit_applier/);
  ...
  it('passes cssAuthoring into the LLM E2E agent instead of hard-coding scoped CSS', { skip: !existsSync(llmAgentPath) && 'live-e2e harness not ported' }, () => {
    ...
    assert.doesNotMatch(
      llmAgent,
      /with @scope \(\[data-monodesign-variant=/,
