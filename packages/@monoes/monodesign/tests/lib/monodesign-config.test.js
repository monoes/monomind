...
} from '../../cli/lib/monodesign-config.mjs';
...
describe('cli/lib/monodesign-config', () => {
  ...
  test('writing consent gitignores config.local.json via .git/info/exclude', () => {
    ...
    expect(exclude).toContain('.monodesign/config.local.json');
    ...
    expect((again.match(/monodesign-config-ignore-start/g) || []).length).toBe(1);
  ...
  test('writeDetectionConfig local ignores do not create an implicit design-system override', () => {
    ...
    expect(readFileSync(join(root, '.git', 'info', 'exclude'), 'utf-8')).toContain('.monodesign/config.local.json');
  });
