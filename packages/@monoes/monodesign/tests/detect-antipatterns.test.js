...
const __TESTDIR__ = path.dirname(fileURLToPath(import.meta.url));
const FIXTURES = path.join(__TESTDIR__, 'fixtures', 'antipatterns');
const SCRIPT = path.join(__TESTDIR__, '..', 'cli', 'engine', 'detect-antipatterns.mjs');
const BENCH_SCRIPT = path.join(__TESTDIR__, '..', 'scripts', 'benchmark-detector.mjs');
...
function writeStaticFixture(files) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'monodesign-static-'));
...
describe('CLI', () => {
  ...
  test('local DESIGN.md enables design-system rules by default and --no-design-system disables them', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'monodesign-cli-design-system-'));
    try {
...
`);
  ...
  test('--scope filters CLI output to a design domain', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'monodesign-cli-scope-'));
    try {
      fs.writeFileSync(path.join(dir, 'DESIGN.md'), `---
typography:
  body:
...
`);
  ...
  test('detector designSystem.enabled=false disables CLI design-system rules', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'monodesign-cli-design-disabled-'));
    try {
      fs.mkdirSync(path.join(dir, '.monodesign'), { recursive: true });
      fs.writeFileSync(path.join(dir, '.monodesign', 'config.json'), JSON.stringify({
...
`);
  ...
  test('respects .monodesign config ignoreFiles like the hook', async () => {
    await withStaticFixture({
      '.monodesign/config.json': JSON.stringify({
  ...
  test('respects .monodesign config ignoreRules like the hook', async () => {
    await withStaticFixture({
      '.monodesign/config.json': JSON.stringify({
  ...
  test('respects .monodesign config ignoreValues like the hook', async () => {
    await withStaticFixture({
      '.monodesign/config.json': JSON.stringify({
        detector: {
          ignoreValues: [
