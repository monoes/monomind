...
function mkTmp() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'monodesign-design-system-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('loadDesignSystemForCwd()', () => {
  it('loads DESIGN.md plus .monodesign/design.json and marks stale sidecars', () => {
    const cwd = mkTmp();
    fs.mkdirSync(path.join(cwd, '.monodesign'), { recursive: true });
    const designMd = path.join(cwd, 'DESIGN.md');
    const sidecarJson = path.join(cwd, '.monodesign', 'design.json');
