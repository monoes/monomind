...
- **`.monodesign/live/config.json`** (live mode): pre-configured so `{{command_prefix}}monodesign live` boots straight into variant mode with no first-time detour.
