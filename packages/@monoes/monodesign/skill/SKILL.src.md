...
The same engine is available as `node {{scripts_path}}/detect.mjs`. It reads HTML/CSS locally — no network, web-only (skip for native platforms). Full catalog with detection rules and remediation: [reference/antipatterns-catalog.md](reference/antipatterns-catalog.md).
...
Setup (context gathering, register) is already loaded by then; sub-commands don't re-invoke `{{command_prefix}}monodesign`.
...
**Pin** creates a standalone shortcut so `{{command_prefix}}<command>` invokes `{{command_prefix}}monodesign <command>` directly. **Unpin** removes it. The script writes to every harness directory present in the project.
...
`{{command_prefix}}monodesign hooks <on|off|status|ignore-rule|ignore-file|ignore-value|reset>` manages the design detector hook for this project. The hook auto-runs the detector after direct UI file edits and surfaces findings as system reminders. Full flow is in [reference/hooks.md](reference/hooks.md); load it when the user invokes `{{command_prefix}}monodesign hooks` with any argument.
