 ...
 *
 * Provides runtime feature flag management for v1 integration,
 * enabling gradual rollout, A/B testing, and environment-specific
 * feature enablement.
 *
 * @module v1/integration/feature-flags
...
export class FeatureFlagManager extends EventEmitter {
  ...
  private loadFromEnvironment(): void {
    for (const flag of Object.keys(FEATURE_FLAG_DEFINITIONS)) {
      const envVar = `MONOBRAIN_${flag.replace(/([A-Z])/g, '_$1').toUpperCase()}`;
