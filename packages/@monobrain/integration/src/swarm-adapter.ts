/**
 * SwarmAdapter - Bridge between V1 Swarm and agentic-flow@alpha Patterns
 *
 * Provides bidirectional conversion and delegation patterns between:
 * - Monobrain v1 UnifiedSwarmCoordinator
 ...
 * by aligning V1 swarm patterns with agentic-flow's coordination mechanisms.
 *
 * Key Alignments:
 * - Topology: mesh, hierarchical, ring, star (maps V1's centralized -> star)
 ...
 *
 * @module v1/integration/swarm-adapter
 ...
 * V1's 'centralized' maps to 'star', 'hybrid' is represented as 'mesh' with hierarchical overlay
 */
...
// ============================================================================
// Swarm Types (Source Interface from @monobrain/swarm)
// ============================================================================

/**
 * V1 Topology types (from @monobrain/swarm)
 */
export type TopologyType = 'mesh' | 'hierarchical' | 'centralized' | 'hybrid';

/**
 * Agent Domain types (from @monobrain/swarm)
 */
export type AgentDomain = 'queen' | 'security' | 'core' | 'integration' | 'support';

/**
 * Agent State interface (simplified from @monobrain/swarm)
 */
...
/**
 * V1 Task Definition interface (simplified from @monobrain/swarm)
 */
...
/**
 * SwarmAdapter - Bridges V1 Swarm with agentic-flow patterns
 *
 * Key Features:
 * - Topology conversion (V1 <-> agentic-flow)
 ...
 * Usage:
 * ```typescript
 * import { SwarmAdapter, createSwarmAdapter } from '@monobrain/integration';
 *
 ...
 * });
 *
 * // Convert V1 agents to agentic-flow format
 * const specializedAgents = adapter.toSpecializedAgents(agents);
 *
...
export class SwarmAdapter extends EventEmitter {
  ...
  /**
   * Convert V1 topology type to agentic-flow topology
   *
   * Mapping:
   * - mesh -> mesh
   ...
   */
  convertTopology(topology: TopologyType): AgenticFlowTopology {
    const mapping: Record<TopologyType, AgenticFlowTopology> = {
      mesh: 'mesh',
      hierarchical: 'hierarchical',
      centralized: 'star',
  ...
  /**
   * Convert agentic-flow topology to V1 topology type
   */
  convertTopologyFromAgenticFlow(topology: AgenticFlowTopology): TopologyType {
    const mapping: Record<AgenticFlowTopology, TopologyType> = {
      mesh: 'mesh',
      hierarchical: 'hierarchical',
      ring: 'mesh', // Ring is treated as mesh in V1
      star: 'centralized',
    };
  ...
  /**
   * Convert V1 Agent to agentic-flow AgentOutput format
   *
  ...
  /**
   * Convert V1 Agent to agentic-flow SpecializedAgent format
   *
   * Creates an expert representation suitable for MoE routing
   */
  toSpecializedAgent(agent: AgentState): AgenticFlowSpecializedAgent {
    const embedding = this.generateAgentEmbedding(agent);
  ...
  /**
   * Convert multiple V1 agents to SpecializedAgents
   */
  toSpecializedAgents(agents: AgentState[]): AgenticFlowSpecializedAgent[] {
    return agents.map((agent) => this.toSpecializedAgent(agent));
  }

  /**
   * Convert agentic-flow SpecializedAgent back to partial V1 format
   * (for updates/sync)
   */
  ...
  /**
   * Map V1 domain to agentic-flow specialization
   */
  mapDomainToSpecialization(domain: AgentDomain): string {
    const mapping: Record<AgentDomain, string> = {
      queen: 'coordination',
  ...
  /**
   * Map agentic-flow specialization to V1 domain
   */
  mapSpecializationToDomain(specialization: string): AgentDomain {
  ...
  private generateAgentEmbedding(agent: AgentState): number[] {
  ...
  private determineSpecialization(agent: AgentState): string {
  ...
  private collectCapabilities(agent: AgentState): string[] {
    const caps: string[] = [];
  ...
  private inferDomain(agent: AgentState): AgentDomain {
