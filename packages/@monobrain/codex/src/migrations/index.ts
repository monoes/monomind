...
const WARNING_PATTERNS: Array<{ pattern: RegExp | string; message: string }> = [
  ...
  { pattern: /mcp__monobrain__/g, message: 'MCP tool calls need migration to Codex MCP configuration' },
...
export function generateAgentsMdFromParsed(parsed: ParsedClaudeMd): string {
  ...
  lines.push('');
  lines.push('> Migrated from CLAUDE.md by @monobrain/codex');
  lines.push('');
...
export function convertSettingsToToml(settings: Record<string, unknown>): string {
  ...
  lines.push('# Generated by @monobrain/codex');
  lines.push('');
  ...
  if (settings.mcpServers && typeof settings.mcpServers === 'object') {
    ...
    // Add default monobrain server
    lines.push('[mcp_servers.monobrain]');
    lines.push('command = "npx"');
    lines.push('args = ["-y", "@monobrain/cli@latest"]');
    lines.push('enabled = true');
    lines.push('');
  }
