/**
 * @monobrain/codex - AGENTS.md Generator
 *
...
function generateMinimal(options: AgentsMdOptions): string {
  const {
    projectName,
    description = 'A Monobrain powered project',
...
### Storing Patterns
\`\`\`bash
npx @monobrain/cli memory store \\
  --key "pattern-name" \\
...
### Searching Memory
\`\`\`bash
npx @monobrain/cli memory search \\
  --query "search terms" \\
  --namespace patterns
\`\`\`
...
Monobrain exposes tools via Model Context Protocol:
...
Monobrain uses hooks for lifecycle automation:
...
npx @monobrain/cli hooks pre-task \\
  --description "implementing authentication"

# After completing a task
npx @monobrain/cli hooks post-task \\
  --task-id "task-123" \\
  --success true

# Route a task to agents
npx @monobrain/cli hooks route \\
...
\`\`\`bash
# List workers
npx @monobrain/cli hooks worker list

# Trigger specific worker
npx @monobrain/cli hooks worker dispatch --trigger audit

# Check worker status
npx @monobrain/cli hooks worker status
\`\`\`
...
\`\`\`bash
# Set log level
export MONOBRAIN_LOG_LEVEL=debug

# Enable verbose mode
npx @monobrain/cli --verbose <command>
\`\`\`

### Health Checks
\`\`\`bash
# Run diagnostics
npx @monobrain/cli doctor --fix

# Check system status
npx @monobrain/cli status
\`\`\`
`;
...
npx @monobrain/cli logs --type agent-actions --last 1h

# Export audit log
npx @monobrain/cli logs export --format json --output audit.json
\`\`\`
...
\`\`\`bash
# Check current role
npx @monobrain/cli claims list

# Request elevated permissions
npx @monobrain/cli claims request --permission deploy:production
\`\`\`
...
1. **Contain** - Immediately stop affected agents
   \`\`\`bash
   npx @monobrain/cli agent stop --all --force
   \`\`\`
...
2. **Decide** - Roll back if safe, or forward-fix
   \`\`\`bash
   # Rollback
   npx @monobrain/cli deployment rollback --env production

   # Or forward-fix
   npx @monobrain/cli workflow run hotfix
   \`\`\`
...
\`\`\`bash
# Restore from backup
npx @monobrain/cli memory restore --snapshot latest

# Restore specific checkpoint
npx @monobrain/cli session restore --checkpoint <id>
\`\`\`
...
- [ ] Run \`npx @monobrain/cli doctor\` to verify setup
