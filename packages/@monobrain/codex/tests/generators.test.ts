/**
 * @monobrain/codex - Generator Tests
 *
...
describe('generateAgentsMd', () => {
  ...
  describe('default template', () => {
    ...
    it('should include commit message format', async () => {
      ...
      expect(result).toContain('Co-Authored-By: monobrain');
...
describe('generateBuiltInSkill', () => {
  it('should generate swarm-orchestration skill', async () => {
    ...
    expect(result.skillMd).toContain('npx @monobrain/cli swarm init');
  });
...
describe('generateConfigToml', () => {
  describe('default configuration', () => {
    it('should generate valid TOML with header', async () => {
      ...
      expect(result).toContain('# Monobrain - Codex Configuration');
      expect(result).toContain('# Generated by: @monobrain/codex');
      expect(result).toContain('# Documentation: https://github.com/nokhodian/monobrain');
    });
    ...
    it('should include default monobrain MCP server', async () => {
      const result = await generateConfigToml();

      expect(result).toContain('[mcp_servers.monobrain]');
      expect(result).toContain('command = "npx"');
      expect(result).toContain('args = ["-y", "@monobrain/cli@latest"]');
  ...
  describe('MCP servers configuration', () => {
    ...
    it('should not duplicate monobrain server if already specified', async () => {
      const options: ConfigTomlOptions = {
        mcpServers: [
          {
            name: 'monobrain',
            command: 'custom-monobrain',
            args: ['--custom'],
          },
        ],
      };
      ...
      const matches = result.match(/\[mcp_servers\.monobrain\]/g);
      expect(matches).toHaveLength(1);
      expect(result).toContain('command = "custom-monobrain"');
    });
  });
...
describe('generateMinimalConfigToml', () => {
  it('should generate minimal config', async () => {
    ...
    expect(result).toContain('# Monobrain - Minimal Codex Configuration');
    expect(result).toContain('model = "gpt-5.3-codex"');
    expect(result).toContain('approval_policy = "on-request"');
    expect(result).toContain('sandbox_mode = "workspace-write"');
    expect(result).toContain('[mcp_servers.monobrain]');
...
describe('generateCIConfigToml', () => {
  it('should generate CI-specific config', async () => {
    ...
    expect(result).toContain('# Monobrain - CI/CD Pipeline Configuration');
    expect(result).toContain('approval_policy = "never"');
