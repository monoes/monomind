/**
 * @monobrain/codex - Migration Tests
 *
...
const SAMPLE_CLAUDE_MD = `# Monobrain Project
...
describe('convertSettingsToToml', () => {
  ...
  it('should convert MCP servers', () => {
    ...
    expect(result).toContain('args = ["-y", "@monobrain/cli"]');
    expect(result).toContain('[mcp_servers.custom-server]');
    expect(result).toContain('command = "node"');
  });

  it('should add default monobrain server when no mcpServers', () => {
    const settings = {
      model: 'gpt-4',
    };
    ...
    // The implementation adds a default monobrain server when none specified
    expect(result).toContain('[mcp_servers.monobrain]');
    expect(result).toContain('command = "npx"');
  });
