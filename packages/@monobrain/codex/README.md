...
<p align="center">
  <strong>OpenAI Codex CLI Adapter for Monobrain V1</strong><br/>
  <em>Self-learning multi-agent orchestration following the <a href="https://agentics.org">Agentics Foundation</a> standard</em>
</p>

<p align="center">
  <a href="https://www.npmjs.com/package/@monobrain/codex"><img src="https://img.shields.io/npm/v/@monobrain/codex?label=npm&color=blue" alt="npm version"></a>
  <a href="https://github.com/nokhodian/monobrain"><img src="https://img.shields.io/badge/license-MIT-green" alt="license"></a>
...
Transform OpenAI Codex CLI into a **self-improving AI development system**. While Codex executes code, monobrain orchestrates, coordinates, and **learns from every interaction**.

| Traditional Codex   | With Monobrain                  |
| ------------------- | ------------------------------- |
...
npx monobrain@alpha init --codex

# Full setup with all 137+ skills
npx monobrain@alpha init --codex --full

# Dual mode (both Claude Code and Codex)
npx monobrain@alpha init --dual
```
...
| `.monobrain/data/memory.db` | Vector memory database                 |
...
npx monobrain@alpha init --codex --minimal
...
| --------------------------- | ------------------------------ | --------------------------------------------------- |
| V1 Security Overhaul        | `$v1-security-overhaul`        | Complete security architecture with CVE remediation |
| V1 Memory Unification       | `$v1-memory-unification`       | Unify 6+ memory systems into AgentDB with HNSW      |
| V1 Integration Deep         | `$v1-integration-deep`         | Deep agentic-flow@alpha integration (ADR-001)       |
| V1 Performance Optimization | `$v1-performance-optimization` | Achieve 2.49x-7.47x speedup targets                 |
| V1 Swarm Coordination       | `$v1-swarm-coordination`       | 15-agent hierarchical mesh coordination             |
| V1 DDD Architecture         | `$v1-ddd-architecture`         | Domain-Driven Design architecture                   |
| V1 Core Implementation      | `$v1-core-implementation`      | Core module implementation                          |
| V1 MCP Optimization         | `$v1-mcp-optimization`         | MCP server optimization and transport               |
| V1 CLI Modernization        | `$v1-cli-modernization`        | CLI modernization and hooks enhancement             |
...
Both platforms share the same `.monobrain/` runtime:
...
### CLI Commands (NEW in V1.0.0-alpha.8)

The `@monobrain/codex` package now includes built-in dual-mode orchestration:
...
npx monobrain-codex dual run --template feature --task "Add user authentication"
...
npx monobrain-codex dual status
```
...
```typescript
import { DualModeOrchestrator, CollaborationTemplates } from "@monobrain/codex";
...
// Listen to events
orchestrator.on("worker:started", ({ id, role }) =>
...
MONOBRAIN_MEMORY_PATH=./.monobrain/data
...
```typescript
import { CodexInitializer } from "@monobrain/codex";
...
```typescript
import { initializeCodexProject } from "@monobrain/codex";
...
```typescript
import { CodexInitializer, initializeCodexProject } from "@monobrain/codex";
...
```typescript
import { migrate } from "@monobrain/codex";
...
5. **Register MCP**: `codex mcp add monobrain -- npx monobrain mcp start`
...
# Re-register
codex mcp remove monobrain
codex mcp add monobrain -- npx monobrain mcp start

# Test connection
npx monobrain mcp test
```
...
npx monobrain memory list

# Manually add a test pattern
npx monobrain memory store --key "test" --value "test pattern" --namespace patterns
```
...
# Rebuild skills
npx monobrain@alpha init --codex --force
```
...
```bash
# Check HNSW index
npx monobrain memory stats

# Rebuild index
npx monobrain memory optimize --rebuild-index
```
...
| Package                                                                  | Description                              |
| ------------------------------------------------------------------------ | ---------------------------------------- |
| [@monobrain/cli](https://www.npmjs.com/package/@monobrain/cli)           | Main CLI (26 commands, 140+ subcommands) |
| [monobrain](https://www.npmjs.com/package/monobrain)                     | Umbrella package                         |
| [@monobrain/memory](https://www.npmjs.com/package/@monobrain/memory)     | AgentDB with HNSW vector search          |
| [@monobrain/security](https://www.npmjs.com/package/@monobrain/security) | Security module                          |
