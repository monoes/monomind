 ...
 *
 * @module v1/testing/v2-compat/api-compat.test
 */
...
/**
 * Mock V1 module registry for testing
 */
interface MockModuleRegistry {
  ...
  checkTypeCompatibility: Mock<(v2Type: string, currentType: string) => boolean>;
}
...
/**
 * V1 class implementations mapping
 */
const CURRENT_CLASSES: Record<string, MockClass> = {
  'UnifiedSwarmCoordinator': {
    name: 'UnifiedSwarmCoordinator',
    currentName: 'UnifiedSwarmCoordinator',
    methods: [
  ...
  'UnifiedMemoryService': {
    ...
    currentName: 'UnifiedMemoryService',
    methods: [
  ...
  'AgentLifecycleService': {
    ...
    currentName: 'AgentLifecycleService',
    methods: [
  ...
  'TaskExecutionService': {
    ...
    currentName: 'TaskExecutionService',
    methods: [
...
function createMockModuleRegistry(): MockModuleRegistry {
  return {
    getClass: vi.fn().mockImplementation((name: string) => {
      const currentName = CLASS_ALIASES[name] || name;
      return CURRENT_CLASSES[currentName] || null;
    }),
    getClasses: vi.fn().mockReturnValue(Object.keys(CURRENT_CLASSES)),
    ...
    getMethodSignature: vi.fn().mockImplementation((className: string, methodName: string) => {
      const currentName = CLASS_ALIASES[className] || className;
      const classInfo = CURRENT_CLASSES[currentName];
      if (!classInfo) return null;

      const method = classInfo.methods.find(m => m.name === methodName);
      return method?.signature || null;
    }),
    checkTypeCompatibility: vi.fn().mockImplementation((v2Type: string, currentType: string) => {
      // Simplified type compatibility check
      if (v2Type === currentType) return true;
      if (v2Type === 'any') return true;
      if (currentType.includes(v2Type)) return true;
      return false;
    }),
  };
}
...
describe('V2 API Compatibility', () => {
  ...
  describe('Import Path Aliases', () => {
    it.each(Object.entries(IMPORT_ALIASES))('should resolve V2 import "%s" to current "%s"', (v2Path, currentPath) => {
      const resolved = mockRegistry.resolveImport(v2Path);

      expect(resolved).toBe(currentPath);
    });

    it('should resolve monobrain/hive-mind to @monobrain/swarm', () => {
      const resolved = mockRegistry.resolveImport('monobrain/hive-mind');

      expect(resolved).toBe('@monobrain/swarm');
    });

    it('should resolve monobrain/memory to @monobrain/memory', () => {
      const resolved = mockRegistry.resolveImport('monobrain/memory');

      expect(resolved).toBe('@monobrain/memory');
    });
  ...
  describe('Class Name Aliases', () => {
    it.each(Object.entries(CLASS_ALIASES))('should map V2 class "%s" to current "%s"', (v2Name, currentName) => {
      const classInfo = mockRegistry.getClass(v2Name);

      expect(classInfo).not.toBeNull();
      expect(classInfo?.currentName).toBe(currentName);
    });
  ...
  describe('Backward Compatible Usage Patterns', () => {
    it('should support V2 HiveMind instantiation pattern', () => {
      ...
      // equivalent: const hive = new UnifiedSwarmCoordinator(config);
      const classInfo = mockRegistry.getClass('HiveMind');
    ...
    it('should support V2 MemoryManager query pattern', () => {
      ...
      // equivalent: const results = await memory.search('search term');
      const classInfo = mockRegistry.getClass('MemoryManager');
    ...
    it('should support V2 agent spawn pattern', () => {
      ...
      // equivalent: const agent = await lifecycle.spawn({ agentType: 'coder' });
      const classInfo = mockRegistry.getClass('AgentManager');
...
describe('API Coverage', () => {
  ...
  it('should have V1 equivalents for all interfaces', () => {
    for (const iface of V2_API_INTERFACES) {
      expect(iface.currentEquivalent).toBeDefined();
      expect(iface.currentEquivalent).not.toBe('');
    }
  });
