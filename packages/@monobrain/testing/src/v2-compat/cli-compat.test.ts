 ...
 *
 * Tests all 25 V2 CLI commands work via compatibility layer or native V1 equivalents.
 * Verifies flag compatibility and output format compatibility.
 *
 * @module v1/testing/v2-compat/cli-compat.test
 */
...
function createMockCLIExecutor(): MockCLIExecutor {
  ...
  return {
    execute: vi.fn().mockImplementation(async (command: string, args: string[]) => {
      // Simulate V1 CLI behavior with V2 command translation
...
describe('CLI Command Coverage', () => {
  ...
  it('should have V1 equivalents for all non-deprecated commands', () => {
    const nonDeprecated = V2_CLI_COMMANDS.filter(c => !c.deprecated);

    for (const cmd of nonDeprecated) {
      expect(cmd.currentEquivalent).toBeDefined();
      expect(cmd.currentEquivalent).not.toBe('');
    }
  });
