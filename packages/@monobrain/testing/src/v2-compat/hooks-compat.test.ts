 ...
 *
 * @module v1/testing/v2-compat/hooks-compat.test
 */
...
function createMockHooksSystem(): MockHooksSystem {
  ...
  const registeredHooks = new Set(currentHooks);
  ...
  return {
    ...
    getHookInfo: vi.fn().mockImplementation((name: string) => {
      return V2_HOOKS.find(h => h.name === name || h.currentEquivalent === name) || null;
    }),
...
describe('V2 Hooks Compatibility', () => {
  ...
  describe('Edit Hooks', () => {
    ...
    it.each(editHooks)('should support V2 hook: $name', async (hook: V2Hook) => {
      ...
      const result = await mockHooks.trigger(hook.currentEquivalent || hook.name, params);
  ...
  describe('Command Hooks', () => {
    ...
    it('should handle pre-bash as pre-command', async () => {
      // pre-bash should map to pre-command in V1
      const info = mockHooks.getHookInfo('pre-bash');

      expect(info?.currentEquivalent).toBe('pre-command');
    });
  });
...
describe('Hooks Coverage', () => {
  ...
  it('should have V1 equivalents for all hooks', () => {
    for (const hook of V2_HOOKS) {
      expect(hook.currentEquivalent).toBeDefined();
      expect(hook.currentEquivalent).not.toBe('');
    }
  });
