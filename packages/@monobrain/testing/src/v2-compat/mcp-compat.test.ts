 ...
 *
 * @module v1/testing/v2-compat/mcp-compat.test
 */
...
function createMockMCPClient(): MockMCPClient {
  ...
  return {
    callTool: vi.fn().mockImplementation(async (name: string, params: Record<string, unknown>) => {
      const currentName = TOOL_NAME_MAPPING[name] || name;

      if (!currentTools.includes(currentName)) {
        throw new Error(`Tool not found: ${name} (translated: ${currentName})`);
      }
      ...
      return responses[currentName] || { success: true };
    }),
...
describe('V2 MCP Tool Compatibility', () => {
  ...
  describe('Agent Tools', () => {
    ...
    it('should translate dispatch_agent to agent/spawn', () => {
      const currentName = mockMCP.translateToolName('dispatch_agent');

      expect(currentName).toBe('agent/spawn');
    });

    it('should translate agents/spawn to agent/spawn', () => {
      const currentName = mockMCP.translateToolName('agents/spawn');

      expect(currentName).toBe('agent/spawn');
    });

    it('should translate agents/info to agent/status', () => {
      const currentName = mockMCP.translateToolName('agents/info');

      expect(currentName).toBe('agent/status');
    });
    ...
    it('should translate agent parameters correctly', () => {
      ...
      const currentParams = mockMCP.translateParams('dispatch_agent', v2Params);

      expect(currentParams.agentType).toBe('coder');
      expect(currentParams.id).toBe('my-coder');
      expect(currentParams.priority).toBe('high');
    });
  ...
  describe('Swarm Tools', () => {
    ...
    it('should translate swarm_status to swarm/status', () => {
      const currentName = mockMCP.translateToolName('swarm_status');

      expect(currentName).toBe('swarm/status');
    });

    it('should translate swarm/get-comprehensive-status to swarm/status', () => {
      const currentName = mockMCP.translateToolName('swarm/get-comprehensive-status');
    ...
    it('should translate swarm init parameters', () => {
      ...
      const currentParams = mockMCP.translateParams('mcp__ruv-swarm__swarm_init', v2Params);

      expect(currentParams.topology).toBe('mesh');
      expect(currentParams.maxAgents).toBe(10);
      expect(currentParams.config).toHaveProperty('consensusMechanism');
    });
  ...
  describe('Memory Tools', () => {
    ...
    it('should translate memory/query to memory/search', () => {
      const currentName = mockMCP.translateToolName('memory/query');

      expect(currentName).toBe('memory/search');
    });
    ...
    it('should translate memory query parameters', () => {
      ...
      const currentParams = mockMCP.translateParams('memory/query', v2Params);

      expect(currentParams.query).toBe('test query');
      expect(currentParams.searchType).toBe('hybrid');
      expect(currentParams.limit).toBe(20);
    });
  ...
  describe('Config Tools', () => {
    it('should translate config/get to config/load', () => {
      const currentName = mockMCP.translateToolName('config/get');

      expect(currentName).toBe('config/load');
    });

    it('should translate config/update to config/save', () => {
      const currentName = mockMCP.translateToolName('config/update');

      expect(currentName).toBe('config/save');
    });
  ...
  describe('Neural/Learning Tools', () => {
    it('should translate neural_status to hooks/metrics', () => {
      const currentName = mockMCP.translateToolName('mcp__ruv-swarm__neural_status');

      expect(currentName).toBe('hooks/metrics');
    });

    it('should translate neural_train to hooks/pretrain', () => {
      const currentName = mockMCP.translateToolName('mcp__ruv-swarm__neural_train');

      expect(currentName).toBe('hooks/pretrain');
    });
  ...
  describe('Full MCP Validation', () => {
    ...
    it('should identify V1 equivalents', async () => {
      const result = await validator.validateMCPTools();

      for (const check of result.checks.filter(c => c.name.startsWith('MCP Tool:'))) {
        if (check.passed && check.details) {
          expect(check.details.currentEquivalent).toBeDefined();
        }
      }
    });
    ...
    it('should report breaking changes correctly', async () => {
      ...
      // Most tools should have V1 equivalents
...
describe('MCP Tool Coverage', () => {
  ...
  it('should have V1 equivalents for most tools', () => {
    const withEquivalent = V2_MCP_TOOLS.filter(t => t.currentEquivalent);
