 ...
 *
 * @module v1/testing/v2-compat/report-generator
 */
...
function generateEnhancedMarkdown(report: FullValidationReport, options: ReportOptions): string {
  ...
  lines.push(`> **Version**: ${report.currentVersion}`);
  ...
  if (allBreaking.length === 0) {
    lines.push('No breaking changes detected. V2 code should work with V1 using the compatibility layer.');
    lines.push('');
  } else {
    ...
    for (const check of allBreaking.slice(0, 50)) {
      const issue = check.currentBehavior === 'Not available' ? 'Removed' : 'Changed';
  ...
  lines.push("// In your configuration");
  ...
  lines.push('');
  lines.push('| Feature | V2 Status | V1 Status | Compatibility |');
  lines.push('|---------|-----------|-----------|---------------|');
  ...
  lines.push('## Appendix');
  lines.push('');
  lines.push('### A. V2 to V1 Tool Name Mapping');
  lines.push('');
  lines.push('| V2 Tool Name | V1 Tool Name |');
  lines.push('|--------------|--------------|');
  ...
  lines.push('### B. V2 to V1 Import Aliases');
  lines.push('');
  lines.push('| V2 Import | V1 Import |');
  lines.push('|-----------|-----------|');
  lines.push('| monobrain/hive-mind | @monobrain/swarm |');
  lines.push('| monobrain/swarm | @monobrain/swarm |');
  lines.push('| monobrain/memory | @monobrain/memory |');
  lines.push('| monobrain/agents | @monobrain/agent-lifecycle |');
  lines.push('| monobrain/tasks | @monobrain/task-execution |');
  lines.push('');

  lines.push('### C. V2 to V1 Class Aliases');
  lines.push('');
  lines.push('| V2 Class | V1 Class |');
  lines.push('|----------|----------|');
  ...
  lines.push('*For more information, see [v1/docs/v1-migration/BACKWARD-COMPATIBILITY.md](../v1-migration/BACKWARD-COMPATIBILITY.md)*');
...
function generateDetailedSection(title: string, result: ValidationResult, expectedCount: number): string[] {
  ...
  if (itemChecks.length > 0) {
    lines.push('| Item | Status | V1 Equivalent |');
    lines.push('|------|--------|---------------|');

    for (const check of itemChecks.slice(0, 40)) {
      const status = check.passed ? 'OK' : (check.breaking ? 'BREAKING' : 'WARNING');
      const currentName = check.details?.currentEquivalent as string || check.migrationPath?.replace('Use "', '').replace('" instead', '') || '-';
      const itemName = check.name.replace(/^(CLI|MCP Tool|Hook|API Class|API Method): /, '');
      lines.push(`| ${itemName} | ${status} | ${currentName} |`);
    }
...
function generateExamplesSection(): string[] {
  ...
  lines.push('npx monobrain hive-mind status');
  lines.push('');
  lines.push('# V1 (recommended)');
  lines.push('npx @monobrain/cli swarm init');
  lines.push('npx @monobrain/cli swarm status');
  lines.push('```');
  lines.push('');
  ...
  lines.push('});');
  lines.push('');
  lines.push('// tool call (with compatibility layer)');
  lines.push("const agent = await mcp.callTool('dispatch_agent', {");
  lines.push("  type: 'coder',");
  lines.push("  name: 'my-agent',");
  lines.push('  priority: 8');
  ...
  lines.push("import { MemoryManager } from 'monobrain/memory';");
  lines.push('');
  lines.push('// imports with aliases');
  lines.push("import { UnifiedSwarmCoordinator as HiveMind } from '@monobrain/swarm';");
  lines.push("import { UnifiedMemoryService as MemoryManager } from '@monobrain/memory';");
  lines.push('');
...
function generateMigrationScriptsSection(): string[] {
  ...
  lines.push('');
  lines.push('```bash');
  lines.push('# Run the V1 migration tool');
  lines.push('npx @monobrain/cli migrate --from v2 --to v1');
  lines.push('');
  lines.push('# Migrate configuration');
  lines.push('npx @monobrain/cli migrate config --input .monobrain/config.yaml');
  lines.push('');
  lines.push('# Migrate memory database');
  lines.push('npx @monobrain/cli migrate memory --input .monobrain/memory.db');
  lines.push('```');
  lines.push('');
