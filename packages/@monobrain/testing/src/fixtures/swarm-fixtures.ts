/**
 * @monobrain/testing - Swarm Fixtures
 *
 ...
 *
 * Based on ADR-003 (Single Coordination Engine) and V1 swarm specifications.
 */
import { vi, type Mock } from 'vitest';
import type { AgentType, AgentInstance, MockAgent, createMockAgent } from './agent-fixtures.js';
...
export const swarmConfigs: Record<string, SwarmConfig> = {
  defaultConfig: {
    ...
    maxAgents: 15,
    name: 'V1 Default Swarm',
    description: 'Standard V1 15-agent hierarchical-mesh swarm',
    coordination: {
...
export function createSwarmConfig(
  base: keyof typeof swarmConfigs = 'defaultConfig',
...
export const invalidSwarmConfigs = {
  zeroAgents: createSwarmConfig('defaultConfig', { maxAgents: 0 }),

  negativeHeartbeat: createSwarmConfig('defaultConfig', {
    coordination: {
      consensusProtocol: 'raft',
  ...
  invalidTopology: {
    ...swarmConfigs.defaultConfig,
    topology: 'invalid-topology' as SwarmTopology,
  },

  invalidProtocol: createSwarmConfig('defaultConfig', {
    coordination: {
  ...
  zeroMessageSize: createSwarmConfig('defaultConfig', {
    communication: {
      protocol: 'quic',
      maxMessageSize: 0,
      retryAttempts: 3,
