 ...
 *
 * @module v1/testing/regression/performance-baseline
 */
...
export class PerformanceBaseline {
  ...
  private async simulateStartup(): Promise<void> {
    ...
    await import('@monobrain/shared');
    await import('@monobrain/memory');
  }
  ...
  private async benchmarkEventThroughput(): Promise<number> {
    const { EventBus } = await import('@monobrain/shared');
    const eventBus = new EventBus();

    let count = 0;
    eventBus.subscribe('agent:spawned', () => { count++; });

    const { createAgentSpawnedEvent } = await import('@monobrain/shared');
