 ...
 *
 * @module v1/testing/regression/integration-regression
 */
...
export class IntegrationRegressionSuite {
  ...
  private registerDefaultTests(): void {
    ...
    this.tests.push({
      ...
      timeout: 5000,
      run: async () => {
        const { UnifiedMemoryService, HybridBackend } = await import('@monobrain/memory');
    ...
    this.tests.push({
      ...
      timeout: 10000,
      run: async () => {
        const { HybridBackend, UnifiedMemoryService } = await import('@monobrain/memory');
    ...
    this.tests.push({
      ...
      timeout: 5000,
      run: async () => {
        const { EventBus, createAgentSpawnedEvent } = await import('@monobrain/shared');

        const eventBus = new EventBus();
        let received = false;

        // Use a valid event type from the V1 system
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const { HookRegistry, HookPriority } = await import('@monobrain/shared');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          ...
          const shared = await import('@monobrain/shared');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const memory = await import('@monobrain/memory');
          return (
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const swarm = await import('@monobrain/swarm');
