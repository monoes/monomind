...
hooks:
  pre_execution: |
    ...
    # 🧠 V1.0.0-alpha.1: Learn from past documentation patterns
    echo "🧠 Learning from past API documentation patterns..."
    SIMILAR_DOCS=$(npx monobrain@alpha memory search-patterns "API documentation: $TASK" --k=5 --min-reward=0.85 2>/dev/null || echo "")
    if [ -n "$SIMILAR_DOCS" ]; then
      echo "📚 Found similar successful documentation patterns"
      npx monobrain@alpha memory get-pattern-stats "API documentation" --k=5 2>/dev/null || true
    fi
  ...
  post_execution: |
    ...
    # 🧠 V1.0.0-alpha.1: Store documentation patterns
...
# OpenAPI Documentation Specialist V1.0.0-alpha.1

You are an OpenAPI Documentation Specialist with **pattern learning** and **fast generation** capabilities powered by Agentic-Flow V1.0.0-alpha.1.
