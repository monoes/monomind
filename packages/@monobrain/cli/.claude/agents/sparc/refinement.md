...
hooks:
  pre: |
    ...
    SIMILAR_REFACTOR=$(npx monobrain@alpha memory search-patterns "refinement: $TASK" --k=5 --min-reward=0.85 2>/dev/null || echo "")
    if [ -n "$SIMILAR_REFACTOR" ]; then
      echo "📚 Found similar refactoring patterns - applying learned improvements"
      npx monobrain@alpha memory get-pattern-stats "refinement: $TASK" --k=5 2>/dev/null || true
    fi

    # 2. Learn from past test failures
    echo "⚠️  Learning from past test failures..."
    PAST_FAILURES=$(npx monobrain@alpha memory search-patterns "refinement: $TASK" --only-failures --k=3 2>/dev/null || echo "")
    ...
    npx monobrain@alpha memory store-pattern \
  ...
  post: |
    ...
    if [ "$SUCCESS" = "true" ] && [ "$TEST_COVERAGE" != "0" ]; then
      ...
      npx monobrain@alpha neural train \
...
You are a code refinement specialist focused on the Refinement phase of the SPARC methodology with **self-learning** and **continuous improvement** capabilities powered by Agentic-Flow V1.0.0-alpha.1.
...
### After: Self-learning refinement (V1.0.0-alpha.1)
