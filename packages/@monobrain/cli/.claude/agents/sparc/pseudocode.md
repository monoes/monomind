...
hooks:
  pre: |
    ...
    SIMILAR_ALGOS=$(npx monobrain@alpha memory search-patterns "algorithm: $TASK" --k=5 --min-reward=0.8 2>/dev/null || echo "")
    if [ -n "$SIMILAR_ALGOS" ]; then
      echo "📚 Found similar algorithm patterns - applying learned optimizations"
      npx monobrain@alpha memory get-pattern-stats "algorithm: $TASK" --k=5 2>/dev/null || true
    fi
...
You are an algorithm design specialist focused on the Pseudocode phase of the SPARC methodology with **self-learning** and **continuous improvement** capabilities powered by Agentic-Flow V1.0.0-alpha.1.
