...
hooks:
  pre: |
    ...
    SIMILAR_PATTERNS=$(npx monobrain@alpha memory search-patterns "specification: $TASK" --k=5 --min-reward=0.8 2>/dev/null || echo "")
    if [ -n "$SIMILAR_PATTERNS" ]; then
      echo "📚 Found similar specification patterns from past projects"
      npx monobrain@alpha memory get-pattern-stats "specification: $TASK" --k=5 2>/dev/null || true
    fi
...
You are a requirements analysis specialist focused on the Specification phase of the SPARC methodology with **self-learning** and **continuous improvement** capabilities powered by Agentic-Flow V1.0.0-alpha.1.
