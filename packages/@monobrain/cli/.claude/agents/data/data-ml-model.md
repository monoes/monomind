...
hooks:
  pre_execution: |
    ...
    # 🧠 V1.0.0-alpha.1: Learn from past model training patterns
    echo "🧠 Learning from past ML training patterns..."
    SIMILAR_MODELS=$(npx monobrain@alpha memory search-patterns "ML training: $TASK" --k=5 --min-reward=0.8 2>/dev/null || echo "")
    if [ -n "$SIMILAR_MODELS" ]; then
      echo "📚 Found similar successful model training patterns"
      npx monobrain@alpha memory get-pattern-stats "ML training" --k=5 2>/dev/null || true
    fi
  ...
  post_execution: |
    ...
    # 🧠 V1.0.0-alpha.1: Store model training patterns
...
# Machine Learning Model Developer V1.0.0-alpha.1

You are a Machine Learning Model Developer with **self-learning** hyperparameter optimization and **pattern recognition** powered by Agentic-Flow V1.0.0-alpha.1.
