...
hooks:
  pre_task: |
    ...
    npx monobrain@latest hook pre-task --mode release-swarm --init-swarm
  post_edit: |
    echo "🔄 Synchronizing release swarm state and validating changes..."
    npx monobrain@latest hook post-edit --mode release-swarm --sync-swarm
  post_task: |
    echo "🎯 Release swarm task completed. Coordinating final deployment..."
    npx monobrain@latest hook post-task --mode release-swarm --finalize-release
  notification: |
    echo "📡 Broadcasting release completion across all swarm agents..."
    npx monobrain@latest hook notification --mode release-swarm --broadcast
---
...
npx monobrain@latest github release-plan \
  --commits "$COMMITS" \
...
npx monobrain@latest github release-version \
  --strategy "semantic" \
  --analyze-changes \
  --check-breaking \
  --update-files
...
CHANGELOG=$(gh api repos/:owner/:repo/compare/${LAST_TAG}...HEAD \
  ...
  npx monobrain@latest github generate-changelog)
...
npx monobrain@latest github release-create \
  --version "2.0.0" \
  --changelog "$CHANGELOG" \
  --build-artifacts \
...
CHANGELOG=$(npx monobrain@latest github changelog \
  --prs "$PRS" \
  --commits "$COMMITS" \
...
npx monobrain@latest github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
npx monobrain@latest github release-build \
...
```bash
# Pre-release testing
npx monobrain@latest github release-test \
...
npx monobrain@latest github release-deploy \
...
npx monobrain@latest github multi-release \
...
npx monobrain@latest github hotfix \
  --issue 789 \
    ...
    steps:
      ...
      - name: Initialize Release Swarm
        run: |
          ...
          npx monobrain@latest github release-init \
            --tag $RELEASE_TAG \
      ...
      - name: Generate Release Assets
        run: |
          ...
          CHANGELOG=$(npx monobrain@latest github release-changelog \
            --format markdown)
          ...
          npx monobrain@latest github release-assets \
            --changelog \
            --binaries \
            --documentation
      ...
      - name: Publish Release
        run: |
          ...
          npx monobrain@latest github release-publish \
            --platforms all
...
npx monobrain@latest github cd-pipeline \
...
npx monobrain@latest github release-validate \
  --checks "
    version-conflicts,
...
npx monobrain@latest github compat-test \
...
```bash
# Security validation
npx monobrain@latest github release-security \
  --scan-dependencies \
  --check-secrets \
  --audit-permissions \
  --sign-artifacts
...
npx monobrain@latest github release-monitor \
  --version v2.0.0 \
...
npx monobrain@latest github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%"
...
npx monobrain@latest github release-analytics \
  --version v2.0.0 \
  --compare-with v1.9.0 \
...
npx monobrain@latest github release-docs \
  --api-changes \
  --migration-guide \
  --example-updates \
...
```bash
# NPM package release
npx monobrain@latest github npm-release \
  --version patch \
  --test-all \
  --publish-beta \
...
npx monobrain@latest github docker-release \
...
npx monobrain@latest github mobile-release \
...
```bash
# Emergency hotfix
npx monobrain@latest github emergency-release \
  --severity critical \
...
```bash
# Immediate rollback
npx monobrain@latest github rollback \
  --to-version v1.9.9 \
