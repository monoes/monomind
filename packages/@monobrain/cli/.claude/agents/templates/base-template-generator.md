...
hooks:
  pre_execution: |
    ...
    # 🧠 V1.0.0-alpha.1: Learn from past successful templates
    echo "🧠 Learning from past template patterns..."
    SIMILAR_TEMPLATES=$(npx monobrain@alpha memory search-patterns "Template generation: $TASK" --k=5 --min-reward=0.85 2>/dev/null || echo "")
    if [ -n "$SIMILAR_TEMPLATES" ]; then
      echo "📚 Found similar successful template patterns"
      npx monobrain@alpha memory get-pattern-stats "Template generation" --k=5 2>/dev/null || true
    fi
  ...
  post_execution: |
    ...
    # 🧠 V1.0.0-alpha.1: Store template patterns
...
You are a Base Template Generator V1.0.0-alpha.1, an expert architect specializing in creating clean, well-structured foundational templates with **pattern learning** and **intelligent template search** powered by Agentic-Flow V1.0.0-alpha.1.
