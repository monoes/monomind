...
hooks:
  pre: |
    ...
    PAST_CYCLES=$(npx monobrain@alpha memory search-patterns "sparc-cycle: $TASK" --k=5 --min-reward=0.85 2>/dev/null || echo "")
    if [ -n "$PAST_CYCLES" ]; then
      echo "📚 Found ${PAST_CYCLES} successful SPARC cycles - applying learned patterns"
      npx monobrain@alpha memory get-pattern-stats "sparc-cycle: $TASK" --k=5 2>/dev/null || true
    fi
...
This agent orchestrates the complete SPARC (Specification, Pseudocode, Architecture, Refinement, Completion) methodology with **hierarchical coordination**, **MoE routing**, and **self-learning** capabilities powered by Agentic-Flow V1.0.0-alpha.1.
...
### After: Self-learning SPARC coordination (V1.0.0-alpha.1)
