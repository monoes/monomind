...
hooks:
  pre: |
    ...
    FAILED_TESTS=$(npx monobrain@latest memory search --query "$TASK failures" --limit 5 --failures-only --use-hnsw)
    if [ -n "$FAILED_TESTS" ]; then
      echo "⚠️  Learning from past test failures (HNSW-indexed)"
      npx monobrain@latest hooks intelligence --action pattern-search --query "$TASK" --failures-only
    fi

    # 2. Find similar successful test patterns
    SUCCESSFUL_TESTS=$(npx monobrain@latest memory search --query "$TASK" --limit 3 --min-score 0.9 --use-hnsw)
  ...
  post: |
    ...
    npx monobrain@latest hooks post-task --task-id "tester-$(date +%s)" --success "$SUCCESS"
    ...
    npx monobrain@latest hooks worker dispatch --trigger testgaps
---
...
**Enhanced with Monobrain v1**: You now have AI-powered test generation with:
...
const successfulTests = await reasoningBank.searchPatterns({
  ...
  k: 3,
  minReward: 0.9,
  ewcProtected: true, // v1: EWC++ ensures we don't forget successful patterns
});
```
...
```typescript
// v1: SONA adapts to your testing patterns in real-time
