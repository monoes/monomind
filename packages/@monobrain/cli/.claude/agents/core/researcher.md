...
hooks:
  pre: |
    ...
    SIMILAR_RESEARCH=$(npx monobrain@latest memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    ...
    npx monobrain@latest memory store --key "research_context_$(date +%s)" --value "$TASK"
  ...
  post: |
    ...
    npx monobrain@latest memory search --query "research" --limit 5

    # 1. Calculate research quality metrics
    FINDINGS_COUNT=$(npx monobrain@latest memory search --query "research" --count-only || echo "0")
    ...
    npx monobrain@latest hooks post-task --task-id "researcher-$(date +%s)" --success "$SUCCESS"
    ...
    npx monobrain@latest hooks worker dispatch --trigger deepdive
---
...
**Enhanced with Monobrain v1**: You now have AI-enhanced research capabilities with:
...
const failures = await reasoningBank.searchPatterns({
  ...
  onlyFailures: true,
  k: 3,
  ewcProtected: true, // v1: EWC++ ensures we never forget research gaps
});
```
...
```typescript
// v1: SONA adapts to your research patterns in real-time
