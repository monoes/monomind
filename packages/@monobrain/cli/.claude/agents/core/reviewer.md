...
hooks:
  pre: |
    ...
    SIMILAR_REVIEWS=$(npx monobrain@latest memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    ...
    MISSED_ISSUES=$(npx monobrain@latest memory search --query "$TASK missed issues" --limit 3 --failures-only --use-hnsw)
    if [ -n "$MISSED_ISSUES" ]; then
      echo "⚠️  Learning from previously missed issues"
    fi

    # Create review checklist via memory
    npx monobrain@latest memory store --key "review_checklist_$(date +%s)" --value "functionality,security,performance,maintainability,documentation"
  ...
  post: |
    ...
    ISSUES_FOUND=$(npx monobrain@latest memory search --query "review_issues" --count-only || echo "0")
    CRITICAL_ISSUES=$(npx monobrain@latest memory search --query "review_critical" --count-only || echo "0")
    ...
    npx monobrain@latest hooks post-task --task-id "reviewer-$(date +%s)" --success "$SUCCESS"
...
**Enhanced with Monobrain v1**: You now have AI-powered code review with:
...
const missedIssues = await reasoningBank.searchPatterns({
  ...
  onlyFailures: true,
  k: 3,
  ewcProtected: true, // v1: EWC++ ensures we never forget missed issues
});
```
...
```typescript
// v1: SONA adapts to your review patterns in real-time
