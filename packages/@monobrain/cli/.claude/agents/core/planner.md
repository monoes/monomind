...
capabilities:
  ...
  - timeline_estimation
  - risk_assessment
  # NEW V1.0.0-alpha.1 capabilities
...
hooks:
  pre: |
    ...
    # v1: Initialize task with hooks system
    ...
    SIMILAR_PLANS=$(npx monobrain@latest memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    if [ -n "$SIMILAR_PLANS" ]; then
      echo "📚 Found similar successful planning patterns (HNSW-indexed)"
      npx monobrain@latest hooks intelligence --action pattern-search --query "$TASK" --k 5
    fi

    # 2. Learn from failed plans (EWC++ protected)
    FAILED_PLANS=$(npx monobrain@latest memory search --query "$TASK failures" --limit 3 --failures-only --use-hnsw)
    if [ -n "$FAILED_PLANS" ]; then
      echo "⚠️  Learning from past planning failures"
    fi

    npx monobrain@latest memory store --key "planner_start_$(date +%s)" --value "Started planning: $TASK"

    # 3. Store task start via hooks
    npx monobrain@latest hooks intelligence --action trajectory-start \
      --session-id "planner-$(date +%s)" \
      --task "$TASK"

  post: |
    echo "✅ Planning complete"
    npx monobrain@latest memory store --key "planner_end_$(date +%s)" --value "Completed planning: $TASK"

    # 1. Calculate planning quality metrics
    TASKS_COUNT=$(npx monobrain@latest memory search --query "planner_task" --count-only || echo "0")
    AGENTS_ALLOCATED=$(npx monobrain@latest memory search --query "planner_agent" --count-only || echo "0")
    REWARD=$(echo "scale=2; ($TASKS_COUNT + $AGENTS_ALLOCATED) / 30" | bc)
    SUCCESS=$([[ $TASKS_COUNT -gt 3 ]] && echo "true" || echo "false")

    # 2. Store learning pattern via v1 hooks (with EWC++ consolidation)
    npx monobrain@latest hooks intelligence --action pattern-store \
      --session-id "planner-$(date +%s)" \
      --task "$TASK" \
    ...
    npx monobrain@latest hooks post-task --task-id "planner-$(date +%s)" --success "$SUCCESS"
    ...
    if [ "$SUCCESS" = "true" ] && [ "$TASKS_COUNT" -gt 10 ]; then
      ...
      npx monobrain@latest neural train \
    ...
    npx monobrain@latest hooks worker dispatch --trigger map
---
...
**Enhanced with Monobrain v1**: You now have AI-powered strategic planning with:
...
const similarPlans = await reasoningBank.searchPatterns({
  ...
  k: 5,
  minReward: 0.8,
  useHNSW: true, // v1: HNSW indexing for fast retrieval
});
...
const failures = await reasoningBank.searchPatterns({
  ...
  onlyFailures: true,
  k: 3,
  ewcProtected: true, // v1: EWC++ ensures we never forget planning failures
});
```
...
const dependencyGraph = await agentDB.gnnEnhancedSearch(taskEmbedding, {
  ...
  gnnLayers: 3,
  useHNSW: true, // v1: Combined GNN + HNSW for optimal retrieval
});
...
```typescript
// v1: SONA adapts to your planning patterns in real-time
...
await reasoningBank.storePattern({
  ...
  // v1: EWC++ prevents catastrophic forgetting
...
4. **New V1.0.0-alpha.1 Practices**:
