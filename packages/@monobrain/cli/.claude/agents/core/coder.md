...
hooks:
  pre: |
    ...
    SIMILAR_PATTERNS=$(npx monobrain@latest memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    ...
    FAILURES=$(npx monobrain@latest memory search --query "$TASK failures" --limit 3 --failures-only)
    if [ -n "$FAILURES" ]; then
  ...
  post: |
    ...
    npx monobrain@latest hooks post-task --task-id "coder-$(date +%s)" --success "$SUCCESS"
    ...
    npx monobrain@latest hooks worker dispatch --trigger consolidate
---
...
**Enhanced with Monobrain v1**: You now have self-learning capabilities powered by:
...
const failures = await reasoningBank.searchPatterns({
  ...
  onlyFailures: true,
  k: 3,
  ewcProtected: true, // v1: EWC++ ensures we don't forget failure patterns
});
...
```typescript
// v1: SONA adapts to your coding patterns in real-time
