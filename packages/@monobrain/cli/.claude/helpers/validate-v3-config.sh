#!/bin/bash
# v1 Configuration Validation Script
# Ensures all v1 development dependencies and configurations are properly set up

set -e

echo "🔍 Monobrain V1 Configuration Validation"
echo "==========================================="
echo ""
...
required_files=(
  ...
  ".claude/helpers/update-v1-progress.sh"
  ".monobrain/metrics/v1-progress.json"
  ".monobrain/metrics/performance.json"
  ".monobrain/security/audit-status.json"
  "package.json"
)
    ...
    case "$file" in
      ...
      ".monobrain/metrics/v1-progress.json")
        if jq empty "$file" 2>/dev/null; then
          log_success "v1 progress JSON is valid"
          ...
          log_error "Invalid JSON in v1-progress.json"
        fi
        ;;
    esac
  else
...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  if [ "$current_branch" = "v1" ]; then
    log_success "On v1 development branch"
  else
    log_warning "Not on v1 branch (current: $current_branch)"
  fi
else
...
if [ $ERRORS -eq 0 ] && [ $WARNINGS -eq 0 ]; then
  log_success "All checks passed! v1 development environment is ready."
  exit 0
elif [ $ERRORS -eq 0 ]; then
  echo -e "${YELLOW}⚠️  $WARNINGS warnings found, but no critical errors.${RESET}"
  log_info "v1 development can proceed with minor issues to address."
  exit 0
else
  ...
  log_error "Please fix critical errors before proceeding with v1 development."
  exit 1
fi