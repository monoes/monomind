#!/bin/bash
# Monobrain V1 - Security Scanner Worker
...
scan_secrets() {
  ...
  for pattern in "${patterns[@]}"; do
    local count=$(grep -riE "$pattern" "$PROJECT_ROOT/src" "$PROJECT_ROOT/v1" 2>/dev/null | grep -v node_modules | grep -v ".git" | wc -l | tr -d '[:space:]')
    count=${count:-0}
...
scan_vulnerabilities() {
  ...
  local sql_count=$(grep -rE "execute\s*\(" "$PROJECT_ROOT/src" "$PROJECT_ROOT/v1" 2>/dev/null | grep -v node_modules | grep -v ".test." | wc -l | tr -d '[:space:]')
  vulns=$((vulns + ${sql_count:-0}))

  # Command injection patterns
  local cmd_count=$(grep -rE "exec\s*\(|spawn\s*\(" "$PROJECT_ROOT/src" "$PROJECT_ROOT/v1" 2>/dev/null | grep -v node_modules | grep -v ".test." | wc -l | tr -d '[:space:]')
  vulns=$((vulns + ${cmd_count:-0}))

  # Unsafe eval
  local eval_count=$(grep -rE "\beval\s*\(" "$PROJECT_ROOT/src" "$PROJECT_ROOT/v1" 2>/dev/null | grep -v node_modules | wc -l | tr -d '[:space:]')
