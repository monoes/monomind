#!/bin/bash
# v1 Quick Status - Compact development status overview
...
echo -e "${PURPLE}⚡ Monobrain V1 Quick Status${RESET}"
...
if [ -f ".monobrain/metrics/v1-progress.json" ]; then
  DOMAINS=$(jq -r '.domains.completed // 0' ".monobrain/metrics/v1-progress.json" 2>/dev/null || echo "0")
  AGENTS=$(jq -r '.swarm.activeAgents // 0' ".monobrain/metrics/v1-progress.json" 2>/dev/null || echo "0")
  DDD_PROGRESS=$(jq -r '.ddd.progress // 0' ".monobrain/metrics/v1-progress.json" 2>/dev/null || echo "0")
fi

if [ -f ".monobrain/security/audit-status.json" ]; then
  CVES_FIXED=$(jq -r '.cvesFixed // 0' ".monobrain/security/audit-status.json" 2>/dev/null || echo "0")
fi

if [ -f ".monobrain/metrics/performance.json" ]; then
  SPEEDUP=$(jq -r '.flashAttention.speedup // "1.0x"' ".monobrain/metrics/performance.json" 2>/dev/null || echo "1.0x")
  MEMORY=$(jq -r '.memory.reduction // "0%"' ".monobrain/metrics/performance.json" 2>/dev/null || echo "0%")
fi
