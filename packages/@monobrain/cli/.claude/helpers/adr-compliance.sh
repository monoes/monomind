#!/bin/bash
# Monobrain V1 - ADR Compliance Checker Worker
...
check_adr_001() {
  ...
  local imports=$(grep -r "from.*agentic-flow\|require.*agentic-flow" "$PROJECT_ROOT/v1" "$PROJECT_ROOT/src" 2>/dev/null | grep -v node_modules | wc -l)
...
check_adr_002() {
  ...
  [ -d "$PROJECT_ROOT/v1" ] || [ -d "$PROJECT_ROOT/src/domains" ] && score=$((score + 30))

  # Check for bounded contexts
  local contexts=$(find "$PROJECT_ROOT/v1" "$PROJECT_ROOT/src" -type d -name "domain" 2>/dev/null | wc -l)
  [ "$contexts" -gt 0 ] && score=$((score + 35))

  # Check for anti-corruption layers
  local acl=$(grep -r "AntiCorruption\|Adapter\|Port" "$PROJECT_ROOT/v1" "$PROJECT_ROOT/src" 2>/dev/null | grep -v node_modules | wc -l)
...
check_adr_003() {
  ...
  grep -rq "SwarmCoordinator\|UnifiedCoordinator" "$PROJECT_ROOT/v1" "$PROJECT_ROOT/src" 2>/dev/null && score=$((score + 50))

  # Check for no duplicate coordinators
  local coordinators=$(grep -r "class.*Coordinator" "$PROJECT_ROOT/v1" "$PROJECT_ROOT/src" 2>/dev/null | grep -v node_modules | grep -v ".test." | wc -l)
...
check_adr_005() {
  ...
  [ -d "$PROJECT_ROOT/packages/@monobrain/mcp" ] && score=$((score + 40))

  # Check for MCP tools
  local tools=$(grep -r "tool.*name\|registerTool" "$PROJECT_ROOT/v1" 2>/dev/null | wc -l)
  [ "$tools" -gt 5 ] && score=$((score + 30))

  # Check for MCP schemas
  grep -rq "schema\|jsonSchema" "$PROJECT_ROOT/packages/@monobrain/mcp" 2>/dev/null && score=$((score + 30))
...
check_adr_008() {
  ...
  local jest_refs=$(grep -r "from.*jest\|jest\." "$PROJECT_ROOT/v1" "$PROJECT_ROOT/src" 2>/dev/null | grep -v node_modules | grep -v "vitest" | wc -l)
