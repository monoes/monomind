...
List all active agents in the Monobrain system with filtering options.

## Usage

```bash
npx monobrain agent list [options]
npx monobrain agent ls [options]  # Alias
```
...
npx monobrain agent list --all

# Filter by type
npx monobrain agent list -t coder

# Filter by status
npx monobrain agent list -s idle

# JSON output for scripting
npx monobrain agent list --format json

# Combined filters
npx monobrain agent list -t researcher -s active
```
...
- v1: `security-architect`, `memory-specialist`, `performance-engineer`
