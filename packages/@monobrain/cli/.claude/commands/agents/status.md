...
```bash
npx monobrain agent status <agent-id>
npx monobrain agent status --id <agent-id>
```
...
```bash
# Get status by ID
npx monobrain agent status coder-lx7m9k2

# Using --id flag
npx monobrain agent status --id researcher-abc123

# JSON output
npx monobrain agent status coder-lx7m9k2 --format json
```
...
- `npx monobrain agent metrics` - Aggregate metrics
- `npx monobrain agent health` - Health check
- `npx monobrain agent logs <id>` - Activity logs
