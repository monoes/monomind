...
Display comprehensive performance metrics for agents including v1 performance gains.

## Usage

```bash
npx monobrain agent metrics [agent-id] [options]
```
...
npx monobrain agent metrics coder-lx7m9k2

# Last hour
npx monobrain agent metrics -p 1h

# Last 7 days
npx monobrain agent metrics --period 7d

# JSON output
npx monobrain agent metrics --format json
```
...
- `npx monobrain agent status` - Individual agent metrics
- `npx monobrain performance benchmark` - Full performance suite
- `npx monobrain status` - System-wide status
