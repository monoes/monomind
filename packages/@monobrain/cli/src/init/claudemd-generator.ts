...
Bash("npx @monobrain/cli@latest swarm init --topology hierarchical --max-agents 8 --strategy specialized")
...
\`\`\`bash
npx @monobrain/cli@latest memory search --query "[task keywords]" --namespace patterns
npx @monobrain/cli@latest hooks route --task "[task description]"
\`\`\`

### After Completing Any Task Successfully
\`\`\`bash
npx @monobrain/cli@latest memory store --namespace patterns --key "[pattern-name]" --value "[what worked]"
npx @monobrain/cli@latest hooks post-task --task-id "[id]" --success true --store-results true
\`\`\`
...
npx @monobrain/cli@latest memory store --key "pattern-auth" --value "JWT with refresh" --namespace patterns
...
- Run \`npx @monobrain/cli@latest security scan\` after security-related changes`;
}
...
npx @monobrain/cli@latest security audit --report
npx @monobrain/cli@latest security cve --check
\`\`\`
...
npx @monobrain/cli@latest performance profile --target "[component]"
npx @monobrain/cli@latest performance metrics --format table
\`\`\`
