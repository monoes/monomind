 ...
 *
 * Creates a single binary containing kernel, runtime, Monobrain CLI, models/keys,
...
const MONOBRAIN_COMMANDS = 'init agent swarm memory mcp task session config status start workflow hooks hive-mind daemon neural security performance providers plugins deployment embeddings claims migrate process doctor completions'.split(' ');
...
type SectionId = 'kernel' | 'runtime' | 'monobrain' | 'models' | 'data' | 'verify';
...
export class RvfaBuilder {
  ...
  constructor(options: BuildOptions) {
    this.opts = {
      ...
      monobrainVersion: options.monobrainVersion || detectMonobrainVersion(),
  ...
  async build(): Promise<BuildResult> {
    ...
    const stages: { id: SectionId; raw: Buffer; label: string }[] = [
      ...
      { id: 'monobrain',   raw: this.buildMonobrainSection(),   label: 'Monobrain CLI' },
  ...
  private buildKernelSection(): Buffer {
    return jsonBuf({
      ...
      init: '/sbin/init -> monobrain-init (PID 1)',
  ...
  private buildMonobrainSection(): Buffer {
    let packageMeta: Record<string, unknown> | null = null;
    try {
      const raw = execSync('npm pack monobrain@latest --dry-run --json 2>/dev/null', { encoding: 'utf-8', timeout: 15_000 });
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && parsed.length > 0) packageMeta = parsed[0];
    } catch { /* manifest-only fallback */ }

    return jsonBuf({
      type: 'monobrain', version: this.opts.monobrainVersion,
      package: packageMeta ?? { name: 'monobrain', version: this.opts.monobrainVersion },
      commands: MONOBRAIN_COMMANDS, commandCount: MONOBRAIN_COMMANDS.length,
  ...
  private buildVerifySection(): Buffer {
    ...
    if (existsSync(scriptPath)) {
      ...
      script = Buffer.from([
        '#!/bin/sh', 'set -e', 'MONOBRAIN_CMD="${MONOBRAIN_CMD:-monobrain}"',
        'echo "Running basic verification..."',
        '$MONOBRAIN_CMD --version && echo "  OK: CLI" || echo "  FAIL: CLI"',
        '$MONOBRAIN_CMD doctor && echo "  OK: Doctor" || echo "  FAIL: Doctor"',
  ...
  private buildHeaderPartial(): Partial<RvfaHeader> {
    ...
    const boot: RvfaBootConfig = {
      entrypoint: '/opt/monobrain/bin/cli.js',
      args: ['--profile', this.opts.profile],
      env: { NODE_ENV: 'production', MONOBRAIN_MEMORY_BACKEND: 'hybrid', MONOBRAIN_LOG_LEVEL: 'info' },
    ...
    return {
      magic: 'RVFA', version: 1, name: 'monobrain-appliance', appVersion: this.opts.monobrainVersion,
...
function detectMonobrainVersion(): string {
  try {
