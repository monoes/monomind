...
export class RvfaRunner {
  ...
  /**
   * Run natively via Node.js: extract Monobrain section to a temp dir,
  ...
  async runNative(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const monobrain = tryExtract(this.reader, 'monobrain');
      if (!monobrain) return fail('RVFA appliance does not contain a "monobrain" section');

      const entryFile = join(workDir, 'monobrain-bundle.js');
      await writeFile(entryFile, monobrain);
  ...
  async runContainer(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const monobrain = tryExtract(this.reader, 'monobrain');
      if (!monobrain) return fail('RVFA appliance does not contain a "monobrain" section');
      await writeFile(join(workDir, 'monobrain-bundle.js'), monobrain);
      ...
      const dockerfile = [
        `FROM ${baseImage}`, 'WORKDIR /app', 'COPY monobrain-bundle.js .',
        data ? 'COPY data.bin .' : '', `CMD ["node", "monobrain-bundle.js"${cmdArgs}]`,
      ].filter(Boolean).join('\n');
