...
async function initCodexAction(
  ...
  try {
    ...
    // Try multiple resolution strategies for the @monobrain/codex package
    // Use a variable to prevent TypeScript from statically resolving the optional module
    const codexModuleId = '@monobrain/codex';
    const resolutionStrategies = [
      ...
      async () => {
        const projectPath = path.join(ctx.cwd, 'node_modules', '@monobrain', 'codex', 'dist', 'index.js');
      ...
      async () => {
        ...
        const codexPath = path.join(globalPath, '@monobrain', 'codex', 'dist', 'index.js');
    ...
    if (!CodexInitializer) {
      throw new Error('Cannot find module @monobrain/codex');
    }
    ...
    if (errorMessage.includes('Cannot find module') || errorMessage.includes('@monobrain/codex')) {
      output.printError('The @monobrain/codex package is not installed.');
      output.printInfo('Install it with: npm install @monobrain/codex');
      output.writeln();
