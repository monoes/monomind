...
export class CLI {
  ...
  private async initSubsystems(): Promise<void> {
    ...
    try {
      ...
      // Fallback: well-known sibling path on this machine (safe — wrapped in try/catch)
      const defaultExtraPath = '/Users/morteza/Desktop/tools/agency-agents';
      const { existsSync } = await import('fs');
      if (extraPaths.length === 0 && existsSync(defaultExtraPath)) {
        extraPaths.push(defaultExtraPath);
      }
