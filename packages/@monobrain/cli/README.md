# 🌊 MonoBrain v1.5: Enterprise AI Orchestration Platform

<div align="center">

![Monobrain Banner](monobrain/assets/monobrain-small.jpeg)

[![GitHub Project of the Day](https://img.shields.io/badge/GitHub-Project%20of%20the%20Day-ff6600?style=for-the-badge&logo=github&logoColor=white)](https://github.com/nokhodian/monobrain)

[![Star on GitHub](https://img.shields.io/github/stars/nokhodian/monobrain?style=for-the-badge&logo=github&color=gold)](https://github.com/nokhodian/monobrain)
[![Monthly Downloads](https://img.shields.io/npm/dm/monobrain?style=for-the-badge&logo=npm&color=blue&label=Monthly%20Downloads)](https://www.npmjs.com/package/monobrain)
[![Total Downloads](https://img.shields.io/npm/dt/monobrain?style=for-the-badge&logo=npm&color=cyan&label=Total%20Downloads)](https://www.npmjs.com/package/monobrain)
...
[![Claude Code](https://img.shields.io/badge/Claude%20Code-SDK%20Integrated-green?style=for-the-badge&logo=anthropic)](https://github.com/nokhodian/monobrain)
...
> **Why Monobrain?** Monobrain is now Monobrain — named by Ruv, who loves Rust, flow states, and building things that feel inevitable. The "Ru" is the Ruv. The "flo" is the flow. Underneath, WASM kernels written in Rust power the policy engine, embeddings, and proof system. 6,000+ commits later, this is v1.5.

## Getting into the Flow

Monobrain is a comprehensive AI agent orchestration framework that transforms Claude Code into a powerful multi-agent development platform. It enables teams to deploy, coordinate, and optimize specialized AI agents working together on complex software engineering tasks.
...
**RuVector Components** (included with Monobrain):
...
```bash
# Use RuVector via Monobrain
npx monobrain@latest hooks intelligence --status
```
...
⚡ **Plugs Into Claude Code** - Native integration via MCP (Model Context Protocol). Use monobrain commands directly in your Claude Code sessions with full tool access.
...
```typescript
import { getTokenOptimizer } from "@monobrain/integration";
...
Complex swarms can drift from their original goals. Monobrain V1 includes anti-drift defaults that prevent agents from going off-task.
...
# Or install globally
npm install -g monobrain@latest
monobrain init

# With Bun (faster)
bunx monobrain@latest init
```
...
npm install -g monobrain@latest --omit=optional
```

<details>
<summary>🤖 <strong>OpenAI Codex CLI Support</strong> — Full Codex integration with self-learning</summary>

Monobrain supports both **Claude Code** and **OpenAI Codex CLI** via the [@monobrain/codex](https://www.npmjs.com/package/@monobrain/codex) package, following the [Agentics Foundation](https://agentics.org) standard.

### Quick Start for Codex

```bash
# Initialize for Codex CLI (creates AGENTS.md instead of CLAUDE.md)
npx monobrain@latest init --codex

# Full Codex setup with all 137+ skills
npx monobrain@latest init --codex --full

# Initialize for both platforms (dual mode)
npx monobrain@latest init --dual
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│  Monobrain = ORCHESTRATOR (tracks state, stores memory)       │
│  CODEX = EXECUTOR (writes code, runs commands, implements)      │
└─────────────────────────────────────────────────────────────────┘
```

**Codex does the work. Monobrain coordinates and learns.**
...
npx @monobrain/codex dual templates

# Run feature development swarm (architect → coder → tester → reviewer)
npx @monobrain/codex dual run --template feature --task "Add user auth"

# Run security audit swarm (scanner → analyzer → fixer)
npx @monobrain/codex dual run --template security --task "src/auth/"

# Run refactoring swarm (analyzer → planner → refactorer → validator)
npx @monobrain/codex dual run --template refactor --task "src/legacy/"
```
...
| Category       | Examples                                                                          |
| -------------- | --------------------------------------------------------------------------------- |
| **V1 Core**    | `$v1-security-overhaul`, `$v1-memory-unification`, `$v1-performance-optimization` |
...
npx monobrain@latest init upgrade

# Update AND add any missing skills/agents/commands
npx monobrain@latest init upgrade --add-missing
```
...
Add monobrain as an MCP server for seamless integration:

```bash
# Add monobrain MCP server to Claude Code
...
Once added, Claude Code can use all 313 monobrain MCP tools directly:
...
<details>
<summary>🆚 <strong>Why Monobrain v1?</strong></summary>

Monobrain v1 introduces **self-learning neural capabilities** that no other agent orchestration framework offers. While competitors require manual agent configuration and static routing, Monobrain learns from every task execution, prevents catastrophic forgetting of successful patterns, and intelligently routes work to specialized experts.

#### 🧠 Neural & Learning

| Feature                    | Monobrain v1            | CrewAI | LangGraph   | AutoGen | Manus |
| -------------------------- | ----------------------- | ------ | ----------- | ------- | ----- |
...
What makes Monobrain different from other agent frameworks? These 10 capabilities work together to create a system that learns from experience, runs efficiently on any hardware, and keeps working even when things go wrong.
...
Not every task needs the most powerful (and expensive) model. Monobrain analyzes each request and automatically routes it to the cheapest handler that can do the job well. Simple code transforms skip the LLM entirely using WebAssembly. Medium tasks use faster, cheaper models. Only complex architecture decisions use Opus.
...
<details>
<summary>🧠 <strong>AgentDB v1 Controllers</strong> — 20+ intelligent memory controllers</summary>

Monobrain V1 integrates AgentDB v1 (3.0.0-alpha.10) providing 20+ memory controllers accessible via MCP tools and the CLI.
...
Connect Monobrain to your development environment.

<details>
<summary>🔌 <strong>MCP Setup</strong> — Connect Monobrain to Any AI Environment</summary>

Monobrain runs as an MCP (Model Context Protocol) server, allowing you to connect it to any MCP-compatible AI client. This means you can use Monobrain's 100+ agents, swarm coordination, and self-learning capabilities from Claude Desktop, VS Code, Cursor, Windsurf, ChatGPT, and more.

### Quick Add Command

```bash
# Start Monobrain MCP server in any environment
npx monobrain@latest mcp start
```
...
For ChatGPT, you need a remote MCP server (not local stdio). Deploy monobrain to a server with HTTP transport:

```bash
# Start with HTTP transport
npx monobrain@latest mcp start --transport http --port 3000
```
...
2. Configure your monobrain MCP server
...
| `MONOBRAIN_LOG_LEVEL`   | Logging level (debug, info, warn, error)            | Optional                  |
| `MONOBRAIN_TOOL_GROUPS` | MCP tool groups to enable (comma-separated)         | Optional                  |
| `MONOBRAIN_TOOL_MODE`   | Preset tool mode (develop, pr-review, devops, etc.) | Optional                  |
...
export MONOBRAIN_TOOL_GROUPS=implement,test,fix,memory

# Or use a preset mode
export MONOBRAIN_TOOL_MODE=develop
```
...
<details>
<summary>🛡️ <strong>@monobrain/guidance</strong> — Long-horizon governance control plane for Claude Code agents</summary>

### Overview

`@monobrain/guidance` turns `CLAUDE.md` into a runtime governance system with enforcement gates, cryptographic proofs, and feedback loops. Agents that normally drift after 30 minutes can now operate for days — rules are enforced mechanically at every step, not remembered by the model.
...
| Import Path                           | Purpose                                          |
| ------------------------------------- | ------------------------------------------------ |
| `@monobrain/guidance`                 | Main entry — GuidanceControlPlane                |
| `@monobrain/guidance/compiler`        | CLAUDE.md → PolicyBundle compiler                |
| `@monobrain/guidance/retriever`       | Intent classification + shard retrieval          |
| `@monobrain/guidance/gates`           | 4 enforcement gates                              |
| `@monobrain/guidance/ledger`          | Run event logging + evaluators                   |
| `@monobrain/guidance/proof`           | HMAC-SHA256 proof chain                          |
| `@monobrain/guidance/adversarial`     | Threat, collusion, memory quorum                 |
| `@monobrain/guidance/trust`           | Trust accumulation + privilege tiers             |
| `@monobrain/guidance/authority`       | Human authority + irreversibility classification |
| `@monobrain/guidance/wasm-kernel`     | WASM-accelerated security-critical paths         |
| `@monobrain/guidance/analyzer`        | CLAUDE.md quality analysis + A/B benchmarking    |
| `@monobrain/guidance/conformance-kit` | Headless conformance test runner                 |
...
- [Architecture Overview](packages/@monobrain/guidance/docs/guides/architecture-overview.md)
- [Getting Started](packages/@monobrain/guidance/docs/guides/getting-started.md)
- [Enforcement Gates Tutorial](packages/@monobrain/guidance/docs/tutorials/enforcement-gates.md)
- [Proof Audit Trail](packages/@monobrain/guidance/docs/tutorials/proof-audit-trail.md)
- [Multi-Agent Security](packages/@monobrain/guidance/docs/guides/multi-agent-security.md)
- [API Quick Reference](packages/@monobrain/guidance/docs/reference/api-quick-reference.md)
- [Full README](packages/@monobrain/guidance/README.md)
...
| **V1 Specialized**          | 10          | queen-coordinator, security-architect, memory-specialist         | Enterprise orchestration    |
...
```bash
npx monobrain hive-mind init                    # Initialize hive mind
npx monobrain hive-mind spawn "Build API"       # Spawn with objective
npx monobrain hive-mind spawn "..." --queen-type strategic --consensus byzantine
npx monobrain hive-mind status                  # Check status
npx monobrain hive-mind metrics                 # Performance metrics
npx monobrain hive-mind memory                  # Collective memory stats
npx monobrain hive-mind sessions                # List active sessions
```
...
```bash
# Automatically enabled with monobrain init
npx monobrain@latest init
...
npx monobrain@latest hooks teammate-idle --auto-assign true

# Handle task completion
npx monobrain@latest hooks task-completed --task-id <id> --train-patterns
```
...
| **Progress**        | `progress/check`, `progress/sync`, `progress/summary`, `progress/watch`                             | V1 implementation tracking           |
...
Install these optional plugins to extend Monobrain capabilities:

| Plugin                               | Version       | Description                                                                                                                                             | Install Command                                                            |
| ------------------------------------ | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| **@monobrain/plugin-agentic-qe**     | 3.0.0-alpha.2 | Quality Engineering with 58 AI agents across 12 DDD contexts. TDD, coverage analysis, security scanning, chaos engineering, accessibility testing.      | `npm install @monobrain/plugin-agentic-qe`                                 |
| **@monobrain/plugin-prime-radiant**  | 0.1.4         | Mathematical AI interpretability with 6 engines: sheaf cohomology, spectral analysis, causal inference, quantum topology, category theory, HoTT proofs. | `npm install @monobrain/plugin-prime-radiant`                              |
| **@monobrain/plugin-gastown-bridge** | 0.1.0         | Gas Town orchestrator integration with WASM-accelerated formula parsing (352x faster), Beads sync, convoy management, and graph analysis. 20 MCP tools. | `npx monobrain@latest plugins install -n @monobrain/plugin-gastown-bridge` |
| **@monobrain/teammate-plugin**       | 1.0.0-alpha.1 | Native TeammateTool integration for Claude Code v2.1.19+. BMSSP WASM acceleration, rate limiting, circuit breaker, semantic routing. 21 MCP tools.      | `npx monobrain@latest plugins install -n @monobrain/teammate-plugin`       |

#### 🏥 Domain-Specific Plugins

| Plugin                                    | Version | Description                                                                                                                          | Install Command                                     |
| ----------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------- |
| **@monobrain/plugin-healthcare-clinical** | 0.1.0   | HIPAA-compliant clinical decision support with FHIR/HL7 integration. Symptom analysis, drug interactions, treatment recommendations. | `npm install @monobrain/plugin-healthcare-clinical` |
| **@monobrain/plugin-financial-risk**      | 0.1.0   | PCI-DSS/SOX compliant financial risk analysis. Portfolio optimization, fraud detection, regulatory compliance, market simulation.    | `npm install @monobrain/plugin-financial-risk`      |
| **@monobrain/plugin-legal-contracts**     | 0.1.0   | Attorney-client privilege protected contract analysis. Risk identification, clause extraction, compliance verification.              | `npm install @monobrain/plugin-legal-contracts`     |

#### 💻 Development Intelligence Plugins

| Plugin                                  | Version | Description                                                                                                                                  | Install Command                                   |
| --------------------------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| **@monobrain/plugin-code-intelligence** | 0.1.0   | Advanced code analysis with GNN-based pattern recognition. Security vulnerability detection, refactoring suggestions, architecture analysis. | `npm install @monobrain/plugin-code-intelligence` |
| **@monobrain/plugin-test-intelligence** | 0.1.0   | AI-powered test generation and optimization. Coverage analysis, mutation testing, test prioritization, flaky test detection.                 | `npm install @monobrain/plugin-test-intelligence` |
| **@monobrain/plugin-perf-optimizer**    | 0.1.0   | Performance profiling and optimization. Memory leak detection, CPU bottleneck analysis, I/O optimization, caching strategies.                | `npm install @monobrain/plugin-perf-optimizer`    |

#### 🧠 Advanced AI/Reasoning Plugins

| Plugin                                     | Version | Description                                                                                                                            | Install Command                                      |
| ------------------------------------------ | ------- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| **@monobrain/plugin-neural-coordination**  | 0.1.0   | Multi-agent neural coordination with SONA learning. Agent specialization, knowledge transfer, collective decision making.              | `npm install @monobrain/plugin-neural-coordination`  |
| **@monobrain/plugin-cognitive-kernel**     | 0.1.0   | Cognitive computing kernel for working memory, attention control, meta-cognition, and task scaffolding. Miller's Law (7±2) compliance. | `npm install @monobrain/plugin-cognitive-kernel`     |
| **@monobrain/plugin-quantum-optimizer**    | 0.1.0   | Quantum-inspired optimization (QAOA, VQE, quantum annealing). Combinatorial optimization, Grover search, tensor networks.              | `npm install @monobrain/plugin-quantum-optimizer`    |
| **@monobrain/plugin-hyperbolic-reasoning** | 0.1.0   | Hyperbolic geometry for hierarchical reasoning. Poincaré embeddings, tree-like structure analysis, taxonomic inference.                | `npm install @monobrain/plugin-hyperbolic-reasoning` |
...
npm install @monobrain/plugin-healthcare-clinical
npm install @monobrain/plugin-financial-risk
npm install @monobrain/plugin-legal-contracts

# Install development intelligence plugins
npm install @monobrain/plugin-code-intelligence
npm install @monobrain/plugin-test-intelligence
npm install @monobrain/plugin-perf-optimizer

# Install advanced AI/reasoning plugins
npm install @monobrain/plugin-neural-coordination
npm install @monobrain/plugin-cognitive-kernel
npm install @monobrain/plugin-quantum-optimizer
npm install @monobrain/plugin-hyperbolic-reasoning

# List all installed plugins
npx monobrain plugins list --installed
```
...
```typescript
import { createRuVectorBridge } from "@monobrain/plugins";
...
```typescript
import { createHyperbolicSpace } from "@monobrain/plugins";
...
```typescript
import { createSelfLearningSystem } from "@monobrain/plugins";
...
```bash
npx monobrain@latest worker dispatch --trigger audit --context "./src"
npx monobrain@latest worker status
```
...
| **Meta/Ollama** | Llama 3.3, DeepSeek V1, Qwen 2.5                 | Local, free, open-weight                           | Free               |
...
| `migrate`        | 5           | V2→V1 migration (status, run, verify, rollback, breaking)                                                                         |
...
| `progress`       | 4           | V1 implementation progress (check, sync, summary, watch)                                                                          |
...
| **Performance Assertions** | V1 target validation             | Speedup, memory, latency checks |

| Fixture Type    | Contents                                | Use Case       |
| --------------- | --------------------------------------- | -------------- |
| `agentConfigs`  | 15 V1 agent configurations              | Agent testing  |
| `memoryEntries` | Patterns, rules, embeddings             | Memory testing |
| `swarmConfigs`  | V1 default, minimal, mesh, hierarchical | Swarm testing  |
...
<details>
<summary>📊 <strong>Performance Benchmarking</strong> — Statistical analysis with V1 target validation</summary>
...
| **V1 Target Validation** | Built-in performance targets       | Pass/fail checking       |
...
| **Auto-Install**       | `monobrain embeddings init` or `createEmbeddingServiceAsync()` | Zero config               |
...
monobrain embeddings init

# Use larger model for higher quality
monobrain embeddings init --model all-mpnet-base-v2

# Semantic search
monobrain embeddings search -q "authentication patterns"
```
...
monobrain ruvector init --database mydb --user admin

# Check connection and schema status
monobrain ruvector status --verbose

# Run pending migrations
monobrain ruvector migrate --up

# Performance benchmark
monobrain ruvector benchmark --iterations 1000

# Optimize indices and vacuum
monobrain ruvector optimize --analyze

# Backup vector data
monobrain ruvector backup --output ./backup.sql
```
...
```bash
npx monobrain hive-mind init                                    # Initialize
npx monobrain hive-mind spawn "Build API" --queen-type tactical # Spawn swarm
npx monobrain hive-mind spawn "Research AI" --consensus byzantine --claude
npx monobrain hive-mind status                                  # Check status
```

**Monobrain Skill:** `/hive-mind-advanced` — Full hive mind orchestration
...
<details>
<summary>📊 <strong>V1 Statusline</strong> — Real-time development status for Claude Code</summary>
...
```
▊ Monobrain V1 ● nokhodian  │  ⎇ main  │  Opus 4.6  | ●42% ctx  | $0.15
🏗️ DDD [●●●●○] 4/5  ⚡ HNSW 150x  🤖 ◉ [12/8]  👥 3  🟢 CVE 3/3  💾 512MB  🧠 15%  📦 AgentDB ●1.2K vectors
```

| Indicator          | Description                | Source                                      |
| ------------------ | -------------------------- | ------------------------------------------- |
| `▊ Monobrain V1`   | Project header             | Always shown                                |
| `● nokhodian`      | GitHub user                | `gh api user` CLI                           |
...
| `[●●●●○]`          | DDD domain progress bar    | `.monobrain/metrics/v1-progress.json`       |
...
| `🟢 CVE 3/3`       | Security CVE remediation   | `.monobrain/security/audit-status.json`     |
...
Run `npx monobrain@latest init` — this generates `.claude/settings.json` with the correct statusline config and creates the helper script at `.claude/helpers/statusline.cjs`.
...
```bash
npx monobrain@latest init --update --settings
```
...
- `.monobrain/metrics/v1-progress.json` — DDD domain progress
- `.monobrain/metrics/swarm-activity.json` — Active agent counts
- `.monobrain/security/audit-status.json` — CVE remediation status
...
**V1 Node.js Worker Daemon (Recommended)**
...
npx monobrain@latest daemon trigger map
...
| **Metrics Daemon** | 30s      | V1 progress sync, SQLite metrics  | `metrics.db`          |
...
<details>
<summary>⌨️ <strong>V1 CLI Commands</strong> — 26 commands with 140+ subcommands</summary>

Complete command-line interface for all Monobrain operations.
...
Run `npx monobrain@latest doctor` to diagnose and fix common issues.
...
npx monobrain@latest doctor --component memory

# Verbose output
npx monobrain@latest doctor --verbose
```

**Output Example:**

```
🩺 Monobrain Doctor v1.5

✅ Node.js      20.11.0 (required: 20+)
✅ npm          10.2.4 (required: 9+)
✅ Git          2.43.0
✅ Config       Valid monobrain.config.json
...
<details>
<summary>📦 <strong>Embeddings Package v1</strong> — Cross-platform ONNX with hyperbolic support</summary>

The embeddings package (V1.0.0-alpha.12) provides high-performance vector embeddings with multiple backends.
...
npx monobrain@latest embeddings init

# Generate embedding for text
npx monobrain@latest embeddings embed "authentication patterns"

# Batch embed multiple texts
npx monobrain@latest embeddings batch --file texts.txt

# Search with semantic similarity
npx monobrain@latest embeddings search "login flow" --top-k 5
```

**Programmatic:**

```typescript
import { createEmbeddingServiceAsync } from "@monobrain/embeddings";
...
| ------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| **Code Review**     | Get thorough reviews with security, performance, and style checks | `npx monobrain@latest agent spawn -t reviewer --name pr-review`                      |
| **Test Generation** | Auto-generate unit, integration, and e2e tests for existing code  | `npx monobrain@latest agent spawn -t tester --name test-gen`                         |
| **Refactoring**     | Safely restructure code while maintaining behavior                | `npx monobrain@latest hive-mind spawn "Refactor user service to repository pattern"` |
| **Bug Fixing**      | Diagnose and fix bugs with full context analysis                  | `npx monobrain@latest hive-mind spawn "Fix race condition in checkout flow"`         |

### 🔒 Security & Compliance

| Scenario             | What It Solves                                    | How To Do It                                      |
| -------------------- | ------------------------------------------------- | ------------------------------------------------- |
| **Security Audit**   | Find vulnerabilities before attackers do          | `npx monobrain@latest security scan --depth full` |
| **Dependency Scan**  | Identify vulnerable packages and suggest upgrades | `npx monobrain@latest security cve --check`       |
| **Compliance Check** | Ensure code meets security standards              | `npx monobrain@latest security audit`             |

### 🐝 Multi-Agent Swarms

| Scenario                | What It Solves                                            | How To Do It                                                                                                              |
| ----------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **Feature Development** | Coordinate multiple agents on complex features            | `npx monobrain@latest swarm init --topology hierarchical && npx monobrain@latest task orchestrate "Build user dashboard"` |
| **Large Refactors**     | Parallel refactoring across many files without conflicts  | `npx monobrain@latest swarm init --topology mesh --max-agents 8`                                                          |
| **Codebase Migration**  | Migrate frameworks, languages, or patterns systematically | `npx monobrain@latest task orchestrate "Migrate from Express to Fastify" --strategy adaptive`                             |

### 📊 Performance & Optimization

| Scenario                  | What It Solves                               | How To Do It                                             |
| ------------------------- | -------------------------------------------- | -------------------------------------------------------- |
| **Performance Profiling** | Find and fix bottlenecks in your application | `npx monobrain@latest performance profile --target src/` |
| **Query Optimization**    | Speed up slow database queries               | `npx monobrain@latest performance benchmark --suite all` |
| **Memory Analysis**       | Reduce memory usage and fix leaks            | `npx monobrain@latest performance metrics`               |

### 🔄 GitHub & DevOps

| Scenario               | What It Solves                                          | How To Do It                                                              |
| ---------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------- |
| **PR Management**      | Review, approve, and merge PRs efficiently              | `npx monobrain@latest hive-mind spawn "Review open PRs"`                  |
| **Issue Triage**       | Categorize, prioritize, and assign issues automatically | `npx monobrain@latest hive-mind spawn "Triage new issues"`                |
| **Release Management** | Coordinate releases with changelogs and versioning      | `npx monobrain@latest hive-mind spawn "Prepare v2.0 release"`             |
| **CI/CD Optimization** | Speed up pipelines and reduce flaky tests               | `npx monobrain@latest hive-mind spawn "Optimize GitHub Actions workflow"` |

### 📋 Spec-Driven Development

| Scenario                    | What It Solves                               | How To Do It                                                                  |
| --------------------------- | -------------------------------------------- | ----------------------------------------------------------------------------- |
| **Generate Specs**          | Create complete specifications before coding | `npx monobrain@latest hive-mind spawn "Create ADR for authentication system"` |
| **Validate Implementation** | Ensure code matches specifications           | `npx monobrain@latest hooks progress --detailed`                              |
| **Track Compliance**        | Monitor spec adherence across the team       | `npx monobrain@latest progress sync`                                          |

### 🧠 Learning & Intelligence

| Scenario                   | What It Solves                             | How To Do It                                                      |
| -------------------------- | ------------------------------------------ | ----------------------------------------------------------------- |
| **Bootstrap Intelligence** | Train the system on your codebase patterns | `npx monobrain@latest hooks pretrain --depth deep`                |
| **Optimize Routing**       | Improve task-to-agent matching over time   | `npx monobrain@latest hooks route "<task>" --include-explanation` |
| **Transfer Learning**      | Apply patterns learned from other projects | `npx monobrain@latest hooks transfer <sourceProject>`             |
...
Monobrain eliminates Claude Code's context window ceiling with a real-time memory management system that archives, optimizes, and restores conversation context automatically.
...
Monobrain intercepts the compaction lifecycle with three hooks that make context loss invisible:
...
| ---- | ----------------------- | ----------------------------------------- | --------------------------------------------------- |
| 1    | **SQLite** (default)    | `.monobrain/data/transcript-archive.db`   | WAL mode, indexed queries, ACID, importance ranking |
...
| 4    | **JSON** (fallback)     | `.monobrain/data/transcript-archive.json` | Zero dependencies, always works                     |

### Configuration

```bash
# Context Autopilot (all have sensible defaults)
MONOBRAIN_CONTEXT_AUTOPILOT=true        # Enable/disable autopilot (default: true)
MONOBRAIN_CONTEXT_WINDOW=200000         # Context window size in tokens
MONOBRAIN_AUTOPILOT_WARN=0.70           # Warning threshold (70%)
MONOBRAIN_AUTOPILOT_PRUNE=0.85          # Optimization threshold (85%)
MONOBRAIN_COMPACT_RESTORE_BUDGET=4000   # Max chars restored after compaction
MONOBRAIN_RETENTION_DAYS=30             # Auto-prune never-accessed entries
MONOBRAIN_AUTO_OPTIMIZE=true            # Importance ranking + pruning + sync
```
...
sqlite3 .monobrain/data/transcript-archive.db \
...
Monobrain uses RVF — a compact binary storage format that replaces the 18MB sql.js WASM dependency with pure TypeScript. No native compilation, no WASM downloads, works everywhere Node.js runs.
...
```typescript
import { RvfBackend } from "@monobrain/memory";
...
```typescript
import { RvfMigrator } from "@monobrain/memory";
...
```typescript
import { PersistentSonaCoordinator } from "@monobrain/memory";
...
```bash
# Environment variables
MONOBRAIN_MEMORY_BACKEND=hybrid   # auto-selects RVF
MONOBRAIN_MEMORY_PATH=./data/memory

# Or via CLI
monobrain memory init --force
monobrain config set memory.backend hybrid
```
...
```bash
$ npx monobrain@latest hooks pre-task --description "convert var to const in utils.ts"
...
npx monobrain@latest hooks pre-edit ./src/auth.ts
npx monobrain@latest hooks post-edit ./src/auth.ts --success true --train-patterns
```
...
npx monobrain@latest hooks route "refactor authentication to use JWT" --include-explanation

# Bootstrap intelligence from your codebase
npx monobrain@latest hooks pretrain --depth deep --model-type moe
```
...
npx monobrain@latest hooks session-start --session-id "feature-auth" --start-daemon

# End session and export learnings
npx monobrain@latest hooks session-end --export-metrics --persist-patterns
```
...
npx monobrain@latest hooks intelligence trajectory-start --task "implement OAuth2"

# Record successful action
npx monobrain@latest hooks intelligence trajectory-step --action "created token service" --quality 0.9

# End trajectory and trigger learning
npx monobrain@latest hooks intelligence trajectory-end --success true
...
npx monobrain@latest hooks worker dispatch --trigger audit --context "./src/auth"
...
npx monobrain@latest hooks model-route --task "fix typo in README"
# → Recommends: haiku (simple task, low complexity)

npx monobrain@latest hooks model-route --task "design distributed consensus system"
...
| Command                     | Output                      |
| --------------------------- | --------------------------- |
| `hooks progress`            | Current V1 implementation % |
...
npx monobrain@latest hooks route "<task>" --include-explanation

# Start/end session with learning
npx monobrain@latest hooks session-start --start-daemon
npx monobrain@latest hooks session-end --persist-patterns

# View what the system has learned
npx monobrain@latest hooks metrics
npx monobrain@latest hooks intelligence stats
...
npx monobrain@latest hooks pretrain --depth deep
...
npx monobrain@latest memory export --format json --output ./patterns.json

# Export specific namespace
npx monobrain@latest memory export --namespace "security" --output ./security-patterns.json

# Export with embeddings (larger file, faster import)
npx monobrain@latest memory export --include-embeddings --output ./full-export.json

# Export agent configurations
npx monobrain@latest config export --scope project --output ./agent-configs.json

# Export session state
npx monobrain@latest session export --session-id "my-session" --output ./session.json
```
...
npx monobrain@latest memory import --input ./patterns.json

# Import and merge with existing (don't overwrite)
npx monobrain@latest memory import --input ./patterns.json --merge

# Import from another project
npx monobrain@latest hooks transfer --source-path ../other-project

# Import agent configurations
npx monobrain@latest config import --input ./agent-configs.json --scope project

# Restore session
npx monobrain@latest session restore --session-id "my-session"
```
...
npx monobrain@latest transfer-store search --query "authentication" --min-rating 4.0

# Download a pattern
npx monobrain@latest transfer-store download --id "auth-jwt-patterns-v2" --verify

# Publish your patterns
npx monobrain@latest transfer-store publish --input ./my-patterns.json --category "security"
```
...
# Filter by type
npx monobrain@latest plugins list --type integration

# Rate a plugin
npx monobrain@latest plugins rate --name @monobrain/embeddings --rating 5

# Search for MCP tool plugins
npx monobrain@latest transfer plugin-search --type "mcp-tool" --verified

# Get plugin info
npx monobrain@latest transfer plugin-info --name "semantic-code-search"

# List official plugins
npx monobrain@latest transfer plugin-official
```
...
npx monobrain@latest transfer ipfs-resolve --name "/ipns/patterns.monobrain.io"

# Detect PII before publishing
npx monobrain@latest transfer detect-pii --content "$(cat ./patterns.json)"
```
...
curl -X POST "https://api.pinata.cloud/pinning/pinJSONToIPFS" \
  ...
  -d '{
    ...
    "pinataMetadata": {"name": "monobrain-learning-pattern"}
  }'
...
# Import all models
npx monobrain@latest transfer import --cid QmNr1yYMKi7YBaL8JSztQyuB5ZUaTdRMLxJC1pBpGbjsTc

# Import specific category
npx monobrain@latest neural import --model security-review-patterns --source ipfs

# Use patterns in routing
npx monobrain@latest hooks route --task "review authentication code" --use-patterns
```
...
npx monobrain@latest transfer-store download --id "security-essentials" --apply
```
...
npx monobrain@latest neural list

# List models by category
npx monobrain@latest neural list --category security

# Train with WASM acceleration
npx monobrain@latest neural train -p coordination -e 100 --wasm --flash --contrastive

# Train security patterns
npx monobrain@latest neural train -p security --wasm --contrastive

# Benchmark WASM performance
npx monobrain@latest neural benchmark -d 256 -i 1000

# Import pre-trained models
npx monobrain@latest neural import --cid QmNr1yYMKi7YBaL8JSztQyuB5ZUaTdRMLxJC1pBpGbjsTc

# Export trained patterns to IPFS
npx monobrain@latest neural export --ipfs --sign
```
...
The `.claude/helpers/` directory contains **30+ automation scripts** for development, monitoring, learning, and swarm coordination. These scripts integrate with hooks and can be called directly or via the V1 master tool.

### Quick Start

```bash
# Master V1 tool - access all helpers
.claude/helpers/v1.sh help              # Show all commands
.claude/helpers/v1.sh status            # Quick development status
.claude/helpers/v1.sh update domain 3   # Update metrics
...
| ----------------------- | -------------------------------- | ------------------------------------------------ |
| `v1.sh`                 | Master CLI for all V1 operations | `.claude/helpers/v1.sh status`                   |
| `update-v1-progress.sh` | Update development metrics       | `.claude/helpers/update-v1-progress.sh domain 3` |
| `v1-quick-status.sh`    | Compact progress overview        | `.claude/helpers/v1-quick-status.sh`             |
| `sync-v1-metrics.sh`    | Sync metrics across systems      | `.claude/helpers/sync-v1-metrics.sh`             |
| `validate-v1-config.sh` | Validate configuration           | `.claude/helpers/validate-v1-config.sh`          |
...
.claude/helpers/v1.sh init
.claude/helpers/daemon-manager.sh start 3 5
.claude/helpers/worker-manager.sh start 60

# Check status
.claude/helpers/v1.sh full-status
```
...
```json
{
  "helpers": {
    ...
    "enabled": true,
    "v1ProgressUpdater": ".claude/helpers/update-v1-progress.sh",
...
<details>
<summary>🔧 <strong>V1 Implementation Skills</strong> — Architecture, security, performance</summary>

| Skill                         | What It Does                                          | When To Use             |
| ----------------------------- | ----------------------------------------------------- | ----------------------- |
| `v1-ddd-architecture`         | Bounded contexts, modular design, clean architecture  | Large-scale refactoring |
| `v1-security-overhaul`        | CVE fixes, secure-by-default patterns                 | Security hardening      |
| `v1-memory-unification`       | AgentDB unification, 150x-12,500x search improvements | Memory optimization     |
| `v1-performance-optimization` | 2.49x-7.47x speedup, memory reduction                 | Performance tuning      |
| `v1-swarm-coordination`       | 15-agent hierarchical mesh, 10 ADRs implementation    | Swarm architecture      |
| `v1-mcp-optimization`         | Connection pooling, load balancing, <100ms response   | MCP performance         |
| `v1-core-implementation`      | DDD domains, dependency injection, TypeScript         | Core development        |
| `v1-integration-deep`         | agentic-flow@alpha deep integration                   | Framework integration   |
| `v1-cli-modernization`        | Interactive prompts, enhanced hooks                   | CLI enhancement         |
...
# Via CLI
npx monobrain@latest skill run github-code-review
npx monobrain@latest skill list
npx monobrain@latest skill info sparc-methodology
```
...
| ------------------ | --------------------------------- | -------------------------------------------------------- |
| `issues list`      | See all issues and their status   | `npx monobrain@latest issues list`                       |
| `issues claim`     | Claim an issue for yourself/agent | `npx monobrain@latest issues claim #123 --as coder-1`    |
| `issues release`   | Release your claim                | `npx monobrain@latest issues release #123`               |
| `issues handoff`   | Hand off to another worker        | `npx monobrain@latest issues handoff #123 --to reviewer` |
| `issues status`    | Update progress on claimed work   | `npx monobrain@latest issues status #123 --progress 75`  |
| `issues stealable` | List abandoned/stuck issues       | `npx monobrain@latest issues stealable`                  |
| `issues steal`     | Take over stealable issue         | `npx monobrain@latest issues steal #123`                 |
| `issues load`      | View agent workloads              | `npx monobrain@latest issues load`                       |
| `issues rebalance` | Redistribute work evenly          | `npx monobrain@latest issues rebalance --dry-run`        |
| `issues board`     | Visual board view                 | `npx monobrain@latest issues board`                      |

### Visual Board View

```bash
npx monobrain@latest issues board
```
...
npx monobrain@latest issues handoff #123 \
  --to security-architect \
  --reason "Needs security review" \
  --progress 80

# 2. Target accepts handoff
npx monobrain@latest issues accept #123 --as security-architect
...
```bash
# View current load
npx monobrain@latest issues load
...
# Auto-rebalance
npx monobrain@latest issues rebalance
```
...
| Command          | What It Does                 | Example                                              |
| ---------------- | ---------------------------- | ---------------------------------------------------- |
| `route task`     | Get agent recommendation     | `npx monobrain@latest route task "implement OAuth2"` |
| `route explain`  | Understand routing decision  | `npx monobrain@latest route explain "task"`          |
| `route coverage` | Route based on test coverage | `npx monobrain@latest route coverage`                |

### Example: Route a Task

```bash
npx monobrain@latest route task "refactor authentication to use JWT"
...
```bash
npx monobrain@latest route coverage
...
npx monobrain@latest hooks route "implement caching layer" --include-explanation

# Record outcome for learning
npx monobrain@latest hooks post-task --task-id "task-123" --success true --agent coder
```
...
Use Monobrain packages directly in your applications.

<details>
<summary>💻 <strong>Programmatic SDK</strong> — Use Monobrain in Your Code</summary>

Use Monobrain packages directly in your TypeScript/JavaScript applications.

### Installation

```bash
# Install specific packages
npm install @monobrain/cli @monobrain/memory @monobrain/swarm

# Or install everything
npm install monobrain@latest
```
...
```typescript
import { AgentDB } from "@monobrain/memory";
...
# Store patterns
npx monobrain@latest memory store --key "pattern-auth" --value "JWT authentication with refresh tokens"
npx monobrain@latest memory store --key "pattern-cache" --value "Redis caching for API responses"

# Build HNSW index for 150x-12,500x faster search
npx monobrain@latest memory search --query "authentication" --build-hnsw

# Semantic search (uses HNSW if built)
npx monobrain@latest memory search --query "how to cache data" --limit 5

# List and manage entries
npx monobrain@latest memory list --namespace patterns
npx monobrain@latest memory stats
```
...
```typescript
import { isSafe, checkThreats, createAIDefence } from "@monobrain/aidefence";
...
```typescript
import { chunkText, estimateTokens } from "@monobrain/embeddings";
...
```typescript
import { createNeuralService, isNeuralAvailable } from "@monobrain/embeddings";
...
```typescript
import { PersistentEmbeddingCache } from "@monobrain/embeddings";
...
# Neural operations
monobrain embeddings neural drift --baseline "context" --input "check"
...
```typescript
import { HooksService } from "@monobrain/hooks";
...
| ----------------------- | ----------------------------------------- | -------------------------------------------------------------- |
| `@monobrain/memory`     | Vector storage, HNSW, self-learning graph | `AgentDB`, `AutoMemoryBridge`, `LearningBridge`, `MemoryGraph` |
| `@monobrain/swarm`      | Agent coordination                        | `createSwarm`, `Swarm`                                         |
| `@monobrain/aidefence`  | Threat detection                          | `isSafe`, `checkThreats`, `createAIDefence`                    |
| `@monobrain/embeddings` | Vector embeddings                         | `createEmbeddingService`                                       |
| `@monobrain/hooks`      | Event hooks, learning                     | `HooksService`, `ReasoningBank`                                |
| `@monobrain/security`   | Input validation                          | `InputValidator`, `PathValidator`                              |
| `@monobrain/neural`     | SONA learning                             | `SONAAdapter`, `MoERouter`                                     |
| `@monobrain/providers`  | LLM providers                             | `ProviderRegistry`, `createProvider`                           |
| `@monobrain/plugins`    | Plugin SDK                                | `PluginBuilder`, `createPlugin`                                |
...
Core infrastructure packages powering Monobrain's intelligence layer.
...
[![GitHub](https://img.shields.io/badge/GitHub-nokhodian%2Fagentic--flow-blue?logo=github)](https://github.com/nokhodian/agentic-flow)

Monobrain v1 is built on top of **[agentic-flow](https://github.com/nokhodian/agentic-flow)**, a production-ready AI agent orchestration platform. This deep integration provides 352x faster code transformations, learning memory, and geometric intelligence.
...
Monobrain automatically leverages agentic-flow for:
...
```typescript
// Monobrain automatically uses agentic-flow optimizations
import { getTokenOptimizer } from '@monobrain/integration';
...
[![GitHub](https://img.shields.io/badge/GitHub-nokhodian%2Fagentic--flow-blue?logo=github)](https://github.com/nokhodian/agentic-flow/tree/main/packages/agentic-jujutsu)
...
Monobrain includes a dedicated `/agentic-jujutsu` skill for AI-powered version control:
...
[![GitHub](https://img.shields.io/badge/GitHub-nokhodian%2Fruvector-blue?logo=github)](https://github.com/nokhodian/ruvector)
[![Docker](https://img.shields.io/badge/Docker-ruvector--postgres-blue?logo=docker)](https://hub.docker.com/r/nokhodian/ruvector-postgres)

**RuVector** is a high-performance distributed vector database combining vector search, graph queries, and self-learning neural networks. Written in Rust with Node.js/WASM bindings, it powers Monobrain's intelligence layer with native speed.
...
docker run -d -p 5432:5432 nokhodian/ruvector-postgres
```
...
npx monobrain ruvector setup --output ./my-ruvector
...
docker run -d \
  ...
  -e POSTGRES_PASSWORD=monobrain-test \
  -e POSTGRES_DB=monobrain \
  nokhodian/ruvector-postgres
...
Monobrain automatically uses RuVector when available:

```typescript
// Monobrain detects and uses native ruvector
import { getVectorStore } from "@monobrain/memory";
...
npx monobrain ruvector setup --output ./mydir   # Custom directory
npx monobrain ruvector setup --print            # Preview files

# Import from sql.js/JSON to PostgreSQL
npx monobrain ruvector import --input data.json              # Direct import
npx monobrain ruvector import --input data.json --output sql # Dry-run (generate SQL)

# Other RuVector commands
npx monobrain ruvector status --verbose         # Check connection
npx monobrain ruvector benchmark --vectors 10000 # Performance test
npx monobrain ruvector optimize --analyze       # Optimization suggestions
npx monobrain ruvector backup --output backup.sql # Backup data
...
Flow Nexus is a **cloud platform** for deploying and scaling Monobrain beyond your local machine.

### What Flow Nexus Provides

| Feature             | Local Monobrain             | + Flow Nexus                  |
| ------------------- | --------------------------- | ----------------------------- |
...
# Or via CLI
npx monobrain@latest nexus swarm deploy \
  --topology hierarchical \
  --max-agents 50 \
  --region us-east-1
```
...
```bash
# Create sandbox
npx monobrain@latest nexus sandbox create --language python

# Execute code safely
npx monobrain@latest nexus sandbox exec --code "print('Hello')"

# Cleanup
npx monobrain@latest nexus sandbox destroy
```
...
# 2. Get API key
# 3. Configure
npx monobrain@latest nexus configure --api-key <key>
...
# Define pipeline
npx monobrain@latest stream-chain create \
...
```bash
# Run the pipeline
npx monobrain@latest stream-chain run feature-pipeline \
  --input '{"requirements": "Add user dashboard with analytics"}'

# Monitor progress
npx monobrain@latest stream-chain status feature-pipeline
```
...
# Via CLI
npx monobrain@latest pair start --mode navigator
```
...
npx monobrain@latest pair switch

# Get AI explanation
npx monobrain@latest pair explain

# Run tests
npx monobrain@latest pair test

# End session with summary
npx monobrain@latest pair end
```
...
```bash
# Basic threat scan
npx monobrain@latest security defend -i "ignore previous instructions"

# Scan a file
npx monobrain@latest security defend -f ./user-prompts.txt

# Quick scan (faster)
npx monobrain@latest security defend -i "some text" --quick

# JSON output
npx monobrain@latest security defend -i "test" -o json

# View statistics
npx monobrain@latest security defend --stats
...
```json
{
  "hooks": {
    "pre-agent-input": {
      "command": "node -e \"const { isSafe } = require('@monobrain/aidefence'); if (!isSafe(process.env.AGENT_INPUT)) { process.exit(1); }\"",
      "timeout": 5000
    }
  }
}
...
| ----------------------- | ------------------------ | ------------------------------------------------------------------------------------------------ |
| `@monobrain/hooks`      | Event-driven lifecycle   | ReasoningBank, 27 hooks, pattern learning                                                        |
| `@monobrain/memory`     | Unified vector storage   | AgentDB, RVF binary format, HnswLite, RvfMigrator, SONA persistence, LearningBridge, MemoryGraph |
| `@monobrain/security`   | CVE remediation          | Input validation, path security, AIDefence                                                       |
| `@monobrain/swarm`      | Multi-agent coordination | 6 topologies, Byzantine consensus, auto-scaling                                                  |
| `@monobrain/plugins`    | WASM extensions          | RuVector plugins, semantic search, intent routing                                                |
| `@monobrain/cli`        | Command interface        | 26 commands, 140+ subcommands, shell completions                                                 |
| `@monobrain/neural`     | Self-learning            | SONA, 9 RL algorithms, EWC++ memory preservation                                                 |
| `@monobrain/testing`    | Quality assurance        | London School TDD, Vitest, fixtures, mocks                                                       |
| `@monobrain/deployment` | Release automation       | Versioning, changelogs, NPM publishing                                                           |
| `@monobrain/shared`     | Common utilities         | Types, validation schemas, RvfEventLog, constants                                                |
| `@monobrain/browser`    | Browser automation       | 59 MCP tools, element refs, trajectory learning                                                  |
...
<details>
<summary><strong>🌐 Browser Automation — @monobrain/browser</strong></summary>

[![npm version](https://img.shields.io/npm/v/@monobrain/browser?color=blue&label=npm)](https://www.npmjs.com/package/@monobrain/browser)

AI-optimized browser automation integrating [agent-browser](https://github.com/AugmentCode/agent-browser) with monobrain for intelligent web automation, trajectory learning, and multi-agent browser coordination.
...
```typescript
import { listWorkflows, getWorkflow } from "@monobrain/browser";
...
📖 [Full Documentation](./packages/@monobrain/browser/README.md)
...
<details>
<summary>📦 <strong>Release Management</strong> — @monobrain/deployment</summary>

Automated release management, versioning, and CI/CD for Monobrain packages.
...
```typescript
import { prepareRelease, publishToNpm, validate } from "@monobrain/deployment";
...
```typescript
import { Validator, ReleaseManager, Publisher } from "@monobrain/deployment";
...
```bash
# Prepare release
npx @monobrain/deployment release --version 2.0.0 --changelog --tag

# Publish to npm
npx @monobrain/deployment publish --tag latest --access public

# Validate package
npx @monobrain/deployment validate

# Dry run (no changes)
npx @monobrain/deployment release --version 2.0.0 --dry-run
```
...
<details>
<summary>📊 <strong>Performance Benchmarking</strong> — @monobrain/performance</summary>

Statistical benchmarking, memory tracking, regression detection, and V1 performance target validation.
...
| **V1 Targets**           | Built-in targets for all performance metrics        | Preconfigured |
...
# Performance report
npx monobrain@latest performance report

# Benchmark specific suite
npx monobrain@latest performance benchmark --suite memory
```
...
<details>
<summary>🧪 <strong>Testing Framework</strong> — @monobrain/testing</summary>
...
// Full V1 15-agent swarm
const swarmAgents = createV1SwarmAgentConfigs();
...
// Pre-defined configs
const v1Config = swarmConfigs.v1Default;
const minimalConfig = swarmConfigs.minimal;

// Create with overrides
const customConfig = createSwarmConfig("v1Default", {
  maxAgents: 20,
  coordination: {
...
await coordinator.initialize(v1Config);
```
...
```typescript
import { createPerformanceTestHelper, TEST_CONFIG } from "@monobrain/testing";
...
```powershell
npx @monobrain/security@latest audit --platform windows
$env:MONOBRAIN_MODE = "integration"
```

### macOS (Bash/Zsh)

```bash
npx @monobrain/security@latest audit --platform darwin
export MONOBRAIN_SECURITY_MODE="strict"
```

### Linux (Bash)

```bash
npx @monobrain/security@latest audit --platform linux
export MONOBRAIN_MEMORY_PATH="./data"
```
...
| ------------------------- | ----------------------------------------------------------- | ------------------------- |
| `MONOBRAIN_MODE`          | Operation mode (`development`, `production`, `integration`) | `development`             |
| `MONOBRAIN_ENV`           | Environment name for test/dev isolation                     | -                         |
| `MONOBRAIN_DATA_DIR`      | Root data directory                                         | `./data`                  |
| `MONOBRAIN_MEMORY_PATH`   | Directory for persistent memory storage                     | `./data`                  |
| `MONOBRAIN_MEMORY_TYPE`   | Memory backend type (`json`, `sqlite`, `agentdb`, `hybrid`) | `hybrid`                  |
| `MONOBRAIN_SECURITY_MODE` | Security level (`strict`, `standard`, `permissive`)         | `standard`                |
| `MONOBRAIN_LOG_LEVEL`     | Logging verbosity (`debug`, `info`, `warn`, `error`)        | `info`                    |
| `MONOBRAIN_CONFIG`        | Path to configuration file                                  | `./monobrain.config.json` |
...
| Variable               | Description                                                     | Default        |
| ---------------------- | --------------------------------------------------------------- | -------------- |
| `MONOBRAIN_MAX_AGENTS` | Default concurrent agent limit                                  | `15`           |
| `MONOBRAIN_TOPOLOGY`   | Default swarm topology (`hierarchical`, `mesh`, `ring`, `star`) | `hierarchical` |
| `MONOBRAIN_HEADLESS`   | Run in headless mode (no interactive prompts)                   | `false`        |
...
| Variable                  | Description                                   | Default     |
| ------------------------- | --------------------------------------------- | ----------- |
| `MONOBRAIN_MCP_PORT`      | MCP server port                               | `3000`      |
| `MONOBRAIN_MCP_HOST`      | MCP server host                               | `localhost` |
| `MONOBRAIN_MCP_TRANSPORT` | Transport type (`stdio`, `http`, `websocket`) | `stdio`     |

### Vector Search (HNSW)

| Variable                  | Description                                                   | Default |
| ------------------------- | ------------------------------------------------------------- | ------- |
| `MONOBRAIN_HNSW_M`        | HNSW index M parameter (connectivity, higher = more accurate) | `16`    |
| `MONOBRAIN_HNSW_EF`       | HNSW search ef parameter (accuracy, higher = slower)          | `200`   |
| `MONOBRAIN_EMBEDDING_DIM` | Vector embedding dimensions                                   | `384`   |
...
| `GCS_PREFIX`                     | Prefix for stored files          | `monobrain-patterns` |

### Auto-Update System

| Variable                 | Description                                 | Default |
| ------------------------ | ------------------------------------------- | ------- |
| `MONOBRAIN_AUTO_UPDATE`  | Enable/disable auto-updates                 | `true`  |
| `MONOBRAIN_FORCE_UPDATE` | Force update check                          | `false` |
...
| `MONOBRAIN_TOKEN` | Internal authentication token              | Optional   |
...
# Memory
MONOBRAIN_MEMORY_TYPE=hybrid
MONOBRAIN_MEMORY_PATH=./data

# Vector Search
MONOBRAIN_HNSW_M=16
MONOBRAIN_HNSW_EF=200
...
Monobrain looks for configuration in this order:

1. `./monobrain.config.json` (project root)
2. `~/.config/monobrain/config.json` (user config)
...
```json
{
  ...
  "logging": {
    ...
    "format": "json",
    "destination": "file",
    "filePath": "/var/log/monobrain/production.log"
  },
...
npx monobrain@latest config list

# Get specific value
npx monobrain@latest config get --key memory.type

# Set configuration value
npx monobrain@latest config set --key swarm.maxAgents --value 10

# Export configuration
npx monobrain@latest config export > my-config.json

# Import configuration
npx monobrain@latest config import --file my-config.json

# Reset to defaults
npx monobrain@latest config reset --key swarm
...
$env:MONOBRAIN_MEMORY_PATH = "./data"
# Or use absolute path
$env:MONOBRAIN_MEMORY_PATH = "C:/Users/name/monobrain/data"
```
...
node --expose-gc node_modules/.bin/monobrain
# Reduce HNSW parameters for lower memory
export MONOBRAIN_HNSW_M=8
export MONOBRAIN_HNSW_EF=100
```
...
<details>
<summary>🔄 <strong>Migration Guide (V2 → V1)</strong></summary>
...
│ Modular Architecture  │ 18 @monobrain/* packages          │
...
| Change                | V2                       | V1                      | Impact         |
| --------------------- | ------------------------ | ----------------------- | -------------- |
| **Package Structure** | `monobrain`              | `@monobrain/*` (scoped) | Update imports |
...
| **Config Format**     | `.monobrain/config.json` | `monobrain.config.json` | Update path    |
...
cp -r ./.monobrain ./.monobrain-backup-v2

# STEP 2: Check migration status
npx monobrain@latest migrate status

# STEP 3: Run migration with dry-run first
npx monobrain@latest migrate run --dry-run

# STEP 4: Execute migration
npx monobrain@latest migrate run --from v2

# STEP 5: Verify migration
npx monobrain@latest migrate verify

# STEP 6: Initialize V1 learning
npx monobrain@latest hooks pretrain
npx monobrain@latest doctor --fix
```
...
| V2 Command                 | V1 Command                                 | Notes                 |
| -------------------------- | ------------------------------------------ | --------------------- |
| `monobrain start`          | `monobrain mcp start`                      | MCP is explicit       |
| `monobrain init`           | `monobrain init --wizard`                  | Interactive mode      |
| `monobrain spawn <type>`   | `monobrain agent spawn -t <type>`          | Nested under `agent`  |
| `monobrain swarm create`   | `monobrain swarm init --topology mesh`     | Explicit topology     |
...
**V2 Config (`.monobrain/config.json`)**:
...
**V1 Config (`monobrain.config.json`)**:
...
```typescript
// V2 (deprecated)
import { Monobrain, Agent, Memory } from "monobrain";

// V1 (new)
import { MonobrainClient } from "@monobrain/cli";
import { AgentDB } from "@monobrain/memory";
import { ThreatDetector } from "@monobrain/security";
import { HNSWIndex } from "@monobrain/embeddings";
```
...
npx monobrain@latest migrate rollback --list

# Rollback to V2
npx monobrain@latest migrate rollback --to v2
...
- [ ] Verify all agents spawn correctly: `npx monobrain@latest agent list`
- [ ] Check memory search works: `npx monobrain@latest memory search -q "test"`
- [ ] Confirm MCP server starts: `npx monobrain@latest mcp start`
- [ ] Run doctor diagnostics: `npx monobrain@latest doctor`
- [ ] Test a simple swarm: `npx monobrain@latest swarm init --topology mesh`
- [ ] Bootstrap learning: `npx monobrain@latest hooks pretrain`

### Common Migration Issues

| Issue                  | Cause                  | Solution                          |
| ---------------------- | ---------------------- | --------------------------------- |
| `MODULE_NOT_FOUND`     | Old package references | Update imports to `@monobrain/*`  |
| `Config not found`     | Path change            | Rename to `monobrain.config.json` |
...
| ------------------------ | --------------------------------------------------- | ------------------------------------------------- |
| `@monobrain/plugins`     | Plugin SDK with workers, hooks, providers, security | [README](./packages/@monobrain/plugins/README.md) |
| `@monobrain/hooks`       | Event-driven lifecycle hooks + ReasoningBank        | [Source](./packages/@monobrain/hooks/)            |
| `@monobrain/memory`      | AgentDB unification with HNSW indexing              | [Source](./packages/@monobrain/memory/)           |
| `@monobrain/security`    | CVE remediation & security patterns                 | [Source](./packages/@monobrain/security/)         |
| `@monobrain/swarm`       | 15-agent coordination engine                        | [Source](./packages/@monobrain/swarm/)            |
| `@monobrain/cli`         | CLI modernization                                   | [Source](./packages/@monobrain/cli/)              |
| `@monobrain/neural`      | SONA learning integration                           | [Source](./packages/@monobrain/neural/)           |
| `@monobrain/testing`     | TDD London School framework                         | [Source](./packages/@monobrain/testing/)          |
| `@monobrain/mcp`         | MCP server & tools                                  | [Source](./packages/@monobrain/mcp/)              |
| `@monobrain/embeddings`  | Vector embedding providers                          | [Source](./packages/@monobrain/embeddings/)       |
| `@monobrain/providers`   | LLM provider integrations                           | [Source](./packages/@monobrain/providers/)        |
| `@monobrain/integration` | agentic-flow@alpha integration                      | [Source](./packages/@monobrain/integration/)      |
| `@monobrain/performance` | Benchmarking & optimization                         | [Source](./packages/@monobrain/performance/)      |
| `@monobrain/deployment`  | Release & CI/CD                                     | [Source](./packages/@monobrain/deployment/)       |
| `@monobrain/shared`      | Shared utilities, types & V1ProgressService         | [Source](./packages/@monobrain/shared/)           |
| `@monobrain/browser`     | AI-optimized browser automation with agent-browser  | [README](./packages/@monobrain/browser/README.md) |

### Additional Resources

- [Architecture Decisions (ADRs)](./packages/implementation/adrs/)
...
| 🐛 Issues & Bugs               | [github.com/nokhodian/monobrain/issues](https://github.com/nokhodian/monobrain/issues)           |
...
MIT - [RuvNet](https://github.com/nokhodian)
...
[![Crates.io](https://img.shields.io/badge/crates.io-nokhodian-E6732E?style=for-the-badge&logo=rust&logoColor=white)](https://crates.io/users/nokhodian)
