#!/bin/bash
# Monobrain V1 - Auto-sync Metrics from Actual Implementation
# Scans the v1 codebase and updates metrics to reflect reality
...
count_modules() {
    ...
    if [ -d "$v1_DIR/@monobrain" ]; then
        for dir in "$v1_DIR/@monobrain"/*/; do
            if [ -d "$dir" ]; then
                name=$(basename "$dir")
                modules+=("$name")
                ((count++))
...
calculate_module_progress() {
    local module="$1"
    local module_dir="$v1_DIR/@monobrain/$module"
...
check_security_status() {
    local cves_fixed=0
    local security_dir="$v1_DIR/@monobrain/security/src"
...
count_total_lines() {
    find "$v1_DIR" -name "*.ts" -type f -exec cat {} \; 2>/dev/null | wc -l
}

# Count total files
count_total_files() {
    find "$v1_DIR" -name "*.ts" -type f 2>/dev/null | wc -l
}
...
count_domains() {
    ...
    # Map @monobrain modules to DDD domains
    [ -d "$v1_DIR/@monobrain/swarm" ] && ((domains++))      # task-management
    [ -d "$v1_DIR/@monobrain/memory" ] && ((domains++))     # session-management
    [ -d "$v1_DIR/@monobrain/performance" ] && ((domains++)) # health-monitoring
    [ -d "$v1_DIR/@monobrain/cli" ] && ((domains++))        # lifecycle-management
    [ -d "$v1_DIR/@monobrain/integration" ] && ((domains++)) # event-coordination
...
# Main sync function
sync_metrics() {
    log "Scanning v1 implementation..."
    ...
    cat > "$METRICS_DIR/v1-progress.json" << EOF
{
  "domains": {
    "completed": $domains,
    "total": 5,
    "list": [
      {"name": "task-management", "status": "$([ -d "$v1_DIR/@monobrain/swarm" ] && echo "complete" || echo "pending")", "module": "swarm"},
      {"name": "session-management", "status": "$([ -d "$v1_DIR/@monobrain/memory" ] && echo "complete" || echo "pending")", "module": "memory"},
      {"name": "health-monitoring", "status": "$([ -d "$v1_DIR/@monobrain/performance" ] && echo "complete" || echo "pending")", "module": "performance"},
      {"name": "lifecycle-management", "status": "$([ -d "$v1_DIR/@monobrain/cli" ] && echo "complete" || echo "pending")", "module": "cli"},
      {"name": "event-coordination", "status": "$([ -d "$v1_DIR/@monobrain/integration" ] && echo "complete" || echo "pending")", "module": "integration"}
    ]
  },
  "ddd": {
  ...
  "swarm": {
    ...
    "coordination": "$([ -d "$v1_DIR/@monobrain/swarm" ] && echo "ready" || echo "pending")"
  },
  ...
  "criticalVulnerabilities": [
    {
      ...
      "severity": "critical",
      "status": "$([ -f "$v1_DIR/@monobrain/security/src/input-validator.ts" ] && echo "fixed" || echo "pending")",
    ...
    },
    {
      ...
      "severity": "critical",
      "status": "$([ -f "$v1_DIR/@monobrain/security/src/path-validator.ts" ] && echo "fixed" || echo "pending")",
    ...
    },
    {
      ...
      "severity": "critical",
      "status": "$([ -f "$v1_DIR/@monobrain/security/src/safe-executor.ts" ] && echo "fixed" || echo "pending")",
...
EOF
    ...
    echo ""
    echo -e "${GREEN}v1 Implementation Status:${RESET}"
