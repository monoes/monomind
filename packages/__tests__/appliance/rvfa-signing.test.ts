 ...
 * Run: npx tsx --test v1/__tests__/appliance/rvfa-signing.test.ts
 */
...
import {
  ...
  RvfaVerifier,
  type RvfaKeyPair,
} from '../../@monobrain/cli/src/appliance/rvfa-signing.js';
import {
  RvfaWriter,
  RvfaReader,
  createDefaultHeader,
...
function buildTestRvfa(name = 'test-appliance'): Buffer {
  ...
  writer.addSection('monobrain', Buffer.from('monobrain-data'), { compression: 'none' });
  return writer.build();
}
