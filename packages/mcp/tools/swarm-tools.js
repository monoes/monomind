...
// ============================================================================
// ============================================================================
// Task 48: SubgraphComposition — swarm/subgraph/compose
// ============================================================================
const subgraphComposeSchema = z.object({
    agentSlugs: z.array(z.string()).min(1).describe('Agent slugs to compile into a subgraph'),
    name: z.string().optional().describe('Subgraph name'),
    description: z.string().optional().describe('Subgraph description'),
    version: z.string().optional().describe('Subgraph version (default: 1.0.0)'),
});
async function handleSubgraphCompose(input) {
    try {
        const { compile } = await import('@monobrain/hooks/subgraph/subgraph-compiler');
        const compiled = compile(input.agentSlugs, {
            name: input.name,
            description: input.description,
            version: input.version,
        });
        return {
            subgraphId: compiled.id,
            agents: compiled.agents.map((a) => ({ id: a.id, slug: a.agentSlug, role: a.role })),
        ...
        // Inline fallback if @monobrain/hooks is not importable
        const slugs = input.agentSlugs;
        const coordSlug = slugs.find((s) => s.includes('coordinator') || s.includes('architect')) ?? slugs[0];
        return {
            subgraphId: `sg-${Date.now()}`,
            agents: slugs.map((s, i) => ({ id: `node-${i}`, slug: s, role: s === coordSlug ? 'coordinator' : 'specialist' })),
            edgeCount: slugs.length - 1,
            entryNodeId: 'node-0',
            topology: 'star',
        };
    }
...
/**
 * swarm/subgraph/compose tool (Task 48)
 */
export const subgraphComposeTool = {
    name: 'swarm/subgraph/compose',
    description: 'Compile agent slugs into a CompiledSubGraph with auto-detected star topology',
    inputSchema: {
        type: 'object',
        properties: {
            agentSlugs: { type: 'array', items: { type: 'string' }, description: 'Agent slugs to include in subgraph', minItems: 1 },
    ...
    handler: async (input) => {
        const validated = subgraphComposeSchema.parse(input);
        return handleSubgraphCompose(validated);
    },
    category: 'swarm',
    tags: ['swarm', 'subgraph', 'composition'],
    version: '1.0.0',
};
