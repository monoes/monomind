 ...
 * Maps V2 underscore-based tool names to V1 slash-based implementations.
 *
...
export const swarmInitTool: MCPTool = {
  ...
  handler: async (input: Record<string, unknown>, context) => {
    ...
    return initSwarmTool.handler(currentInput, context);
  },
  category: 'v2-compat',
...
export const neuralPatternsTool: MCPTool = {
  ...
  handler: async (input: Record<string, unknown>, context) => {
    const metrics = await metricsTool.handler({ period: '24h', includeCurrent: true }, context) as Record<string, unknown>;
...
/**
 * Map V2 tool name to V1 equivalent
 */
export function mapV2ToCurrentToolName(v2Name: string): string {
