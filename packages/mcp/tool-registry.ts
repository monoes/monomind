...
export class ToolRegistry extends EventEmitter {
  ...
  // Task 38: ToolRetry — set once when retry-executor.js is available (avoids per-call import)
  _retryExecutor: { wrapHandler: (h: (i: Record<string, unknown>) => Promise<unknown>, p: unknown, n: string) => (i: Record<string, unknown>) => Promise<unknown> } | null = null;
  ...
  constructor(private readonly logger: ILogger) {
    super();
    // Initialize retry executor lazily — no import attempt per call
    import('./retry-executor.js').then(({ RetryExecutor }) => {
      this._retryExecutor = new RetryExecutor({ dlqDataDir: '.monobrain/dlq' });
    }).catch(() => { /* retry-executor.js not yet compiled */ });
  }
  ...
  async execute(
    ...
    try {
      ...
      // Task 38: ToolRetry — lazy singleton, initialized once when retry-executor.js is compiled
      let result: unknown;
      if (this._retryExecutor) {
        const retryPolicy = {
          maxAttempts: 3, initialDelayMs: 200, backoffMultiplier: 2,
          maxDelayMs: 2000, jitterMs: 100,
          retryableErrors: ['ECONNRESET', 'ETIMEDOUT', 'ENOTFOUND'],
        };
        result = await this._retryExecutor.wrapHandler(
          (inp: Record<string, unknown>) => metadata.tool.handler(inp, execContext),
          retryPolicy,
          name,
        )(input);
      } else {
        result = await metadata.tool.handler(input, execContext);
      }
