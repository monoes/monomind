...
- MCP server v1 with enhanced tools
- CLI v1 (MCP-first)
...
1. **Parallel Development:** v2 maintenance and v1 development run in parallel
...
**Goal:** Establish v1 architecture foundation

**Deliverables:**

- [ ] src-v1/ directory structure
...
1. Set up v1 development environment
...
```bash
# Create v1 branch
git checkout -b v1-development

# Create directory structure
mkdir -p src-v1/{shared-kernel,infrastructure,agent-lifecycle,task-execution,memory-management,coordination}

# Set up package.json for v1
...
```typescript
// src-v1/shared-kernel/types/
-agent -
  types.ts - // AgentId, AgentType, AgentStatus
  task -
  types.ts - // TaskId, TaskType, TaskStatus
  memory -
  types.ts - // MemoryEntry, MemoryQuery
  event -
  types.ts - // DomainEvent base classes
  // src-v1/shared-kernel/events/
  agent -
  events.ts - // AgentSpawned, AgentTerminated
  task -
  events.ts - // TaskCreated, TaskCompleted
  system -
  events.ts - // SystemStarted, SystemShutdown
  // src-v1/shared-kernel/errors/
  domain -
...
```typescript
// src-v1/infrastructure/event-bus/event-bus.ts
...
// Tests
-event - bus.test.ts - event - handler.test.ts;
```

**Logging Service (Day 2-3)**

```typescript
// src-v1/infrastructure/logging/logger.ts
...
```typescript
// src-v1/infrastructure/persistence/
-connection -
...
1. Set up CI/CD pipeline for v1
...
# Generate API docs
npx typedoc --out docs/api src-v1/
...
````markdown
# docs/v1/guides/migration-from-v2.md

## Overview

Step-by-step guide for migrating from v2 to v1
...
```typescript
// src-v1/agent-lifecycle/domain/models/agent.ts
...
```typescript
// src-v1/agent-lifecycle/domain/interfaces/agent-repository.ts
...
```typescript
// src-v1/agent-lifecycle/application/services/agent-lifecycle-service.ts
...
```typescript
// src-v1/agent-lifecycle/infrastructure/repositories/agent-repository.ts
...
```typescript
// src-v1/agent-lifecycle/api/cli/spawn-agent-command.ts
...
```typescript
// src-v1/agent-lifecycle/api/mcp/agent-tools.ts
...
// Run benchmarks
npm run benchmark
npm run benchmark:compare -- --base=v2 --compare=v1
```

**Target Performance:**
| Operation | v2 | v1 Target | Improvement |
|-----------|-----|-----------|-------------|
