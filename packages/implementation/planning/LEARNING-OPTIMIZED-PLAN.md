...
```typescript
// src/v1/hooks/learning-integration.ts
import {
  // agentic-flow hooks
  beginTaskTrajectory,
...
const reflexionTools = ["reflexion_store", "reflexion_retrieve"];

// Skill Library (2 tools)
const skillTools = ["skill_create", "skill_search"];

// Causal Memory (3 tools)
const causalTools = ["causal_add_edge", "causal_query", "learner_discover"];

// Recall with Provenance (1 tool)
const recallTools = ["recall_with_certificate"];
```

### 3.2 MCP Tool Coordination

```typescript
// src/packages/mcp/learning-coordinator.ts
...
```typescript
// src/v1/learning/nightly-config.ts
...
```typescript
// src/v1/learning/nightly-pipeline.ts
...
```typescript
// src/v1/learning/memory-config.ts
...
_Optimized Learning Plan - V1.0.0_
_Packages: agentic-flow@2.0.1-alpha.50, agentdb@2.0.0-alpha.3.1_
