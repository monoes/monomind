...
```typescript
// src/v1/core/secure-foundation.ts
  ...
  agenticFlow: z
    .object({
      ...
      sona: z
        ...
        .default("balanced"),
      attention: z
        .enum(["flash", "multi-head", "linear", "hyperbolic", "moe"])
        .default("flash"),
...
```typescript
// src/v1/speed/flash-attention-coordinator.ts
import {
...
| Operation      | v2 Current | v1 Target | Method           |
| -------------- | ---------- | --------- | ---------------- |
...
```typescript
// src/v1/learning/sona-manager.ts
...
export class SONAManager {
  ...
  async learn(context: LearningContext): Promise<void> {
    ...
    await this.adapter.storePattern(context.operation, context.result, reward, {
      algorithm: "ppo",
    });
...
```typescript
// src/v1/learning/learning-hooks.ts
...
```typescript
// src/v1/learning/reasoningbank-bridge.ts
...
```typescript
// src/v1/compatibility/v2-api.ts
...
export class SwarmCoordinator {
  private v1Flash: FlashCoordinator;
  private v1Sona: SONAManager;
  private eventEmitter = new EventEmitter();

  constructor(config?: V2Config) {
    // v1 enhancements transparent to v2 callers
    this.v1Flash = new FlashCoordinator();
    this.v1Sona = new SONAManager({ profile: "balanced" });
  }

  async initialize(): Promise<void> {
    await Promise.all([this.v1Flash.initialize(), this.v1Sona.initialize()]);
  }

  // v2 method signature preserved exactly
  async spawnAgent(profile: AgentProfile): Promise<string> {
    // Internally uses v1 with learning
    const agentId = await this.v1Flash.spawnAgent(profile.type, {
      learning: true, // v1 enhancement
      ...profile,
    });
  ...
  async assignTask(task: Task): Promise<void> {
    await this.v1Flash.executeTask(task);
...
export class AgentManager {
  private v1: EnhancedAgentManager;

  // All v2 methods work exactly as before
  async createAgent(template: string): Promise<string> {
    return this.v1.createAgentWithLearning(template);
  }

  async getAgent(id: string): Promise<Agent | undefined> {
    return this.v1.getAgent(id);
  }

  async terminateAgent(id: string): Promise<void> {
    return this.v1.terminateAgent(id);
  }
}

// v2 MemoryManager - fully preserved
export class MemoryManager {
  private v1: HybridMemorySystem;

  async store(key: string, value: any): Promise<void> {
    return this.v1.store(key, value);
  }

  async retrieve(key: string): Promise<any> {
    return this.v1.retrieve(key);
  }

  async search(query: string): Promise<any[]> {
    return this.v1.search(query);
  }
}
```
...
```typescript
// src/v1/compatibility/config-migration.ts
export async function autoMigrateConfig(): Promise<void> {
  const configPath = ".claude/config.json";

  if (!(await fileExists(configPath))) {
    // Fresh install - create v1 config
    await createDefaultV1Config(configPath);
    return;
  }
  ...
  // Save v1 config
  await saveConfig(configPath, v1Config);

  console.log("✅ Configuration migrated to v1");
}
```
...
```typescript
// src/v1/init/init-controller.ts
...
export class InitController {
  ...
  private async initAgenticFlow(): Promise<InitStepResult> {
    ...
    await Promise.all([this.flash.initialize(), this.sona.initialize()]);
...
export abstract class ModeInitializer {
  ...
  protected async loadModePatterns(): Promise<void> {
    const patterns = await this.sona.searchPatterns(`mode:${this.modeName}`, {
      k: 100,
    });

    console.log(
      `Loaded ${patterns.length} learned patterns for ${this.modeName} mode`,
    );
  }
...
export class StandardModeInitializer extends ModeInitializer {
  ...
  get requiredAgents() {
    return ["coder", "tester", "reviewer", "researcher", "planner"];
  }
  get requiredSkills() {
    return ["swarm-orchestration", "pair-programming"];
  }
...
export class SparcModeInitializer extends ModeInitializer {
  ...
  get requiredSkills() {
    return ["sparc-methodology", "tdd-london-swarm"];
  }
...
export class HiveMindModeInitializer extends ModeInitializer {
  ...
  get requiredSkills() {
    return ["hive-mind-advanced", "collective-intelligence"];
  }
...
export class EnterpriseModeInitializer extends ModeInitializer {
  ...
  get requiredAgents() {
    return [
      ...StandardModeInitializer.prototype.requiredAgents,
  ...
  return new Command("init")
    ...
    .action(async (options) => {
      ...
      if (result.success) {
        ...
        console.log(
          `   - Flash Attention: ${result.capabilities.flashAttention ? "✓" : "✗"}`,
        );
        console.log(
          `   - SONA Learning: ${result.capabilities.sonaLearning ? "✓" : "✗"}`,
        );
        console.log(
          `   - Reflexion: ${result.capabilities.reflexion ? "✓" : "✗"}`,
        );
        console.log(
          `   - v2 Compatible: ${result.capabilities.v2Compatible ? "✓" : "✗"}`,
        );
      } else {
...
_Optimized Plan Version: 1.0.0_
