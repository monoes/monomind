...
export class SqlJsBackend implements IMemoryBackend {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new MemoryBackendError("Failed to initialize sql.js backend", {
        error,
      });
    }
  }
...
export async function createDatabase(dbPath, options = {}) {
  ...
  throw new Error(
    "No SQLite provider available (tried better-sqlite3 and sql.js)",
  );
}
...
export class DatabaseManager implements IDatabaseProvider {
  ...
  private async initializeSQLiteWithRecovery(): Promise<IDatabaseProvider> {
    try {
      ...
      if (
        error instanceof NativeModuleError ||
        isNativeModuleVersionError(error)
      ) {
        console.warn(
          "\n" +
            (error instanceof NativeModuleError
              ? error.message
              : getNativeModuleRecoveryMessage(error)),
        );
        console.warn(
          "   Attempting fallback to sql.js (cross-platform SQLite)...\n",
        );
        ...
        try {
          ...
          console.warn(
            "   sql.js not available, falling back to JSON storage.\n",
          );
        }
      }
