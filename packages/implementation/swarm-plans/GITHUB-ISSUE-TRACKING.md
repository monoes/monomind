...
This document defines the **GitHub issue tracking strategy** for the 15-agent swarm implementation of Monobrain v1. Agent #1 (Queen Coordinator) manages all issue lifecycle with frequent reply updates from all agents.
...
```
GitHub Project: Monobrain v1 Implementation
├── Milestone: V1.0.0-alpha.1 (Week 1-2)
...
_Reply generated by Agent #X (monobrain v1 swarm)_
```
...
// Create all v1 issues
async function initializeV1Issues() {
  // Create milestones
  const alpha1 = await createMilestone("V1.0.0-alpha.1", "Week 1-2");
  const alpha5 = await createMilestone("V1.0.0-alpha.5", "Week 3-6");
  const beta1 = await createMilestone("V1.0.0-beta.1", "Week 7-10");
  const release = await createMilestone("V1.0.0", "Week 11-14");
  ...
  switch (context.type) {
    ...
    case "daily":
      return DAILY_TEMPLATE.replace(
        ...
        .replace(
          "{{progress}}",
          formatMilestoneProgress(context.data.milestone),
        );
  }
}
```
  ...
  status-update:
    ...
    steps:
      ...
      - name: Collect agent status
        ...
        run: |
          # Query swarm status
          STATUS=$(npx monobrain swarm status --json)
  ...
  daily-summary:
    ...
    steps:
      ...
      - name: Generate daily summary
        run: npx monobrain swarm summary --daily > summary.md
