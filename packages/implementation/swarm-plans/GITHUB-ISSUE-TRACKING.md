...
This document defines the **GitHub issue tracking strategy** for the 15-agent swarm implementation of Monomind v1. Agent #1 (Queen Coordinator) manages all issue lifecycle with frequent reply updates from all agents.
...
```
GitHub Project: Monomind v1 Implementation
...
_Reply generated by Agent #X (monomind v1 swarm)_
```
  ...
  status-update:
    ...
    steps:
      ...
      - name: Collect agent status
        ...
        run: |
          # Query swarm status
          STATUS=$(npx monomind swarm status --json)
  ...
  daily-summary:
    ...
    steps:
      ...
      - name: Generate daily summary
        run: npx monomind swarm summary --daily > summary.md
