...
| **P2 — Medium** | AgentDB bridge unavailable | #1399 | CLI bundles `@monomind/memory@alpha.11` (missing `ControllerRegistry`); runtime patch targets v1.x paths |
...
| **P2 — Medium** | Hive-mind uses native tools instead of MCP | #1422 | No tool preference enforcement; Claude defaults to native tools over Monomind MCP |
...
File: `packages/@monomind/cli/src/daemon/headless-worker-executor.ts`
...
1. On `daemon start`, check `$PROJECT/.monomind/daemon.pid`
...
File: `packages/@monomind/cli/src/commands/swarm.ts` (line ~645)
...
File: `packages/@monomind/cli/src/commands/memory.ts` (init handler)
...
1. Update `@monomind/cli` dependency on `@monomind/memory` to `>=3.0.0-alpha.12`
...
Audit all `type: 'array'` properties and add appropriate `items` schema. This is partially addressed in PR #73 (monomind repo).

**4.2 — Enforce Monomind MCP tool preference in hive-mind**

Options:
- **A**: Add `--allowedTools` constraint when spawning hive-mind sessions to prefer Monomind MCP tools
- **B**: Add system prompt injection that instructs Claude to use Monomind MCP tools for orchestration
...
2. Integration test: `npx monomind daemon start` → workers execute successfully
3. Integration test: `npx monomind memory init` → process exits cleanly
