...
**Date**: 2026-03-25
**Author**: RuvNet
**Version**: v1.5.42 → v1.5.43
**Tracking**: GitHub Issues #1395, #1423, #1425, #1428, #1431, #1399, #1404, #1422

## Context

Community-reported issues have identified 6 critical/high-severity bugs and 2 moderate issues in v1.5.42 that collectively degrade core functionality — swarm execution, headless workers, memory initialization, AgentDB bridge, MCP schema validation, and hive-mind tool routing. Multiple reporters confirm that the swarm orchestration engine registers agents but never dispatches work, headless workers fail 100% of the time, and accumulated daemon processes can cause kernel panic on macOS.

This ADR documents all findings, root causes, and the remediation plan for v1.5.43.
...
| **P2 — Medium** | AgentDB bridge unavailable | #1399 | CLI bundles `@monobrain/memory@alpha.11` (missing `ControllerRegistry`); runtime patch targets v1.x paths |
...
| **P2 — Medium** | Hive-mind uses native tools instead of MCP | #1422 | No tool preference enforcement; Claude defaults to native tools over Monobrain MCP |

## Decision

Address all issues in a single v1.5.43 release, prioritized by severity and dependency order.

### Phase 1: Headless Worker Lifecycle (P0) — #1395, #1431

**1.1 — Fix stdin pipe (one-line change)**

File: `packages/@monobrain/cli/src/daemon/headless-worker-executor.ts`
...
1. On `daemon start`, check `$PROJECT/.monobrain/daemon.pid`
...
File: `packages/@monobrain/cli/src/commands/swarm.ts` (line ~645)
...
File: `packages/@monobrain/cli/src/commands/memory.ts` (init handler)
...
1. Update `@monobrain/cli` dependency on `@monobrain/memory` to `>=3.0.0-alpha.12`
...
Audit all `type: 'array'` properties and add appropriate `items` schema. This is partially addressed in PR #73 (monobrain repo).

**4.2 — Enforce Monobrain MCP tool preference in hive-mind**

Options:
- **A**: Add `--allowedTools` constraint when spawning hive-mind sessions to prefer Monobrain MCP tools
- **B**: Add system prompt injection that instructs Claude to use Monobrain MCP tools for orchestration
...
2. Integration test: `npx monobrain daemon start` → workers execute successfully
3. Integration test: `npx monobrain memory init` → process exits cleanly
...
- ADR-066 — Prior audit remediation (v1.5.24)
