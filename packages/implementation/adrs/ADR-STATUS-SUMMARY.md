...
**V1 Version:** 3.0.0-alpha.117 (CLI) / 3.0.0-alpha.76 (umbrella)
...
| progress         | 4     | V1 progress tracking       |
...
| `v1alpha` (cli)     | 3.0.0-alpha.117 |
| `alpha` (cli)       | 3.0.0-alpha.117 |
| `latest` (wrapper)  | 3.0.0-alpha.76  |
| `v1alpha` (wrapper) | 3.0.0-alpha.76  |
...
```typescript
output.writeln(
  output.warning("⚠ No real CVE database configured. Showing example data."),
);
output.writeln(
  output.dim(
...
console.warn(
  `⚠ [Discovery] OFFLINE MODE - Could not resolve IPNS: ${ipnsName}`,
);
```
...
V1 now implements **171 MCP tools** with full V2 backward compatibility:

| Category              | V2 Status | V1 Status  | Tools    |
| --------------------- | --------- | ---------- | -------- |
...
These are enhancements, not blockers for V1 production readiness.

---

**Document Maintained By:** Architecture Team
**Status:** ✅ V1 All ADRs Complete (22/22) - **BETA READY**
