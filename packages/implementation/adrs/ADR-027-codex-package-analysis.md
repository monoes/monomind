...
1. **MCP Server Mode**
   ...
   - Monobrain can connect to Codex as MCP client
...
4. **Session Interop**
   ...
   - Monobrain uses session persistence
...
3. **Generate `config.toml`** with:
   - MCP server configuration for monobrain
   - Skill enablement
...
2. **Use `.monobrain/` as shared runtime**
...
```toml
# Monobrain as MCP server for Codex
...
| Variable                             | Purpose                                         | Monobrain Use Case      |
| ------------------------------------ | ----------------------------------------------- | ----------------------- |
...
## Monobrain Integration Opportunities
...
6. **Dynamic Tools**
   Register monobrain tools at runtime via the MCP protocol.
...
Monobrain could use similar patterns for swarm state management.
...
The package architecture is similar to Claude Code's approach, making it straightforward to create a compatible Codex integration in monobrain.

## @monobrain/codex Package

Based on this analysis, we've created the `@monobrain/codex` package as the first step in the coflow rebranding initiative.

### Package Location

```
packages/@monobrain/codex/
├── package.json
├── tsconfig.json
└── src/
...
npx @monobrain/codex init --template default
...
npx @monobrain/codex templates

# List built-in skills
npx @monobrain/codex skills
```

### Future: coflow Umbrella

This package is the first step in transitioning from `monobrain` to `coflow`:
