...
This document defines the Domain-Driven Design (DDD) architecture for integrating OpenAI Codex support into monobrain via the `@monobrain/codex` package. The design follows the existing V1 architecture patterns while introducing new bounded contexts for Codex-specific functionality.
...
- **Compatibility**: Maintains `monobrain` branding during transition
...
class InitializationApplicationService {
  ...
  async initializeDualMode(options: DualModeInitOptions): Promise<DualModeResult> {
    ...
    return {
      claude: claudeResult,
      codex: codexResult,
      syncConfigPath: path.join(options.projectPath, '.monobrain', 'platform-sync.yaml')
    };
  }
}
```
...
```typescript
// packages/@monobrain/cli/src/commands/init.ts
...
The design maintains consistency with V1's existing DDD patterns while introducing the flexibility needed to support multiple agentic coding platforms.
