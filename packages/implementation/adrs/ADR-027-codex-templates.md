...
> **Branding Note**: This package is published as `@monobrain/codex` and is the first step in transitioning to the `coflow` brand. The future umbrella package will be `npm/npx coflow`.
...
| Property | Value |
|----------|-------|
| Package Name | `@monobrain/codex` |
| Location | `packages/@monobrain/codex/` |
| Future Umbrella | `coflow` |
| CLI Command | `npx @monobrain/codex init` |
| Integration | Works with `@monobrain/cli` via `--codex` flag |
...
### Storing Patterns
```bash
npx monobrain@latest memory store \
  --key "pattern-name" \
...
### Searching Memory
```bash
npx monobrain@latest memory search \
  --query "search terms" \
  --namespace patterns
```
...
Monobrain exposes tools via MCP:
...
Monobrain uses hooks for automation:
...
### Example
```bash
npx monobrain@latest hooks pre-task \
...
### Before Starting
```bash
npx monobrain@latest memory search --query "[task keywords]"
```
...
```bash
npx monobrain@latest memory retrieve \
  --key "unique-key" \
  --namespace patterns
```
...
```bash
npx monobrain@latest hooks route --task "specification: [requirements]"
```
...
```bash
npx monobrain@latest hooks route --task "pseudocode: [algorithm design]"
```
...
```bash
npx monobrain@latest hooks route --task "architecture: [system design]"
```
...
```bash
npx monobrain@latest hooks route --task "refinement: [improvements]"
```
...
```bash
npx monobrain@latest hooks route --task "completion: [validation]"
```
...
### 1. Input Validation
```bash
npx monobrain@latest security scan --check input-validation
```

### 2. Path Security
```bash
npx monobrain@latest security scan --check path-traversal
```

### 3. Command Injection
```bash
npx monobrain@latest security scan --check command-injection
```
...
```toml
# Monobrain V1 - Codex Configuration
# Generated by: monobrain init --codex
# Documentation: https://github.com/nokhodian/monobrain
...
```toml
# Monobrain V1 - Minimal Codex Configuration
...
```toml
# Monobrain V1 - CI/CD Pipeline Configuration
...
dependencies:
  tools:
    - type: "mcp"
      value: "monobrain"
      description: "Monobrain MCP server for swarm coordination"
      transport: "stdio"
      command: "npx"
      args: ["-y", "@monobrain/cli@latest"]
```
...
```typescript
// Usage in monobrain CLI or standalone
// Package: @monobrain/codex (first step toward coflow rebranding)
...
import {
  ...
  generateConfigToml,
  CodexInitializer
} from '@monobrain/codex';

// Or via the CLI
// npx @monobrain/codex init
// npx @monobrain/codex generate-skill --name my-skill
...
const config = await generateConfigToml({
  ...
  mcpServers: [
    { name: 'monobrain', command: 'npx', args: ['-y', '@monobrain/cli@latest'] }
  ],
  skills: [
...
The `@monobrain/codex` package provides the following commands:
...
npx @monobrain/codex init --template minimal

# Generate a new skill
npx @monobrain/codex generate-skill --name custom-skill

# Migrate from Claude Code to Codex
npx @monobrain/codex migrate --from claude.md

# Validate AGENTS.md and skills
npx @monobrain/codex validate

# Generate dual-platform setup (Claude Code + Codex)
npx @monobrain/codex init --dual
```

## Integration with @monobrain/cli
