...
export function createSwarmHub(eventBus?: IEventBus): ISwarmHub {
  console.warn(
    "[DEPRECATION] createSwarmHub() is deprecated. Use createUnifiedSwarmCoordinator() instead.",
  );
...
```typescript
/**
 * V1 Swarm Hub - DUPLICATE FILE (DEPRECATED)
 *
...
- **V1.0.0-alpha**: Deprecation warnings added (current)
- **V1.0.0-beta**: SwarmHub marked as legacy
- **v1.1.0**: Consider removing SwarmHub (6 months after stable)
...
1. **Remove SwarmHub**: After v1.1.0 (6+ months post-stable)
...
4. Plan SwarmHub removal for v1.1.0+
