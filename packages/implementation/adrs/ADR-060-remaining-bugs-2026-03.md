...
ADR-059 triaged 30 open issues and 11 were fixed in v1.5.3 (PR #1297, #1298). This ADR covers the 19 remaining open issues plus 5 additional issues discovered during the fix cycle that were not in the original triage. Total: 24 open items.
...
- **Status:** ✅ **FIXED** in v1.5.6/v1.5.7. Platform-aware spawn: `windowsHide: true` + `shell: true` on Windows, no `detached` on Windows. Verified 10/10 tests pass on Windows.
...
- **Status:** ✅ **FIXED** in PR #1300 (v1.5.4). Added `readStdin()` async function with `!process.stdin.isTTY` detection, JSON parsing, and merged `hookInput` into prompt resolution. **Updated** in v1.5.7: replaced `for await` with event-based + 500ms timeout to prevent hanging when Claude Code leaves stdin in an ambiguous state.
...
- **Effort:** Medium.
- **Status:** ✅ **FIXED** in v1.5.6. Added SIGHUP handler (`process.on('SIGHUP', () => {})`) in foreground daemon mode. PID file now written after `child.unref()` + 100ms delay to prevent race condition.

### 4. auto-memory-hook.mjs fails to resolve @monobrain/memory (#1287)
...
- **Effort:** Small.
- **Status:** ✅ **FIXED** in PR #1300 (v1.5.4). Replaced `loadMemoryPackage()` with 4-strategy resolution: local dev path, `createRequire`, ESM import, walk-up search. Template in `helpers-generator.ts` also updated.
...
- **Status:** ✅ **FIXED** in v1.5.6. Added 4 missing hook matchers: `PostToolUse:Bash` (post-command tracking), `PreToolUse:Write|Edit|MultiEdit` (pre-edit validation), `SubagentEnd` (agent completion metrics), `Notification` (event logging). Total: 8 hook event types with 12 matchers.

### 6. recordFeedback() exposed by AgentDB v1 has zero callers (#1209)
...
- **Status:** ✅ **FIXED** in PR #1300 (v1.5.4). Wired `bridgeRecordFeedback()` into post-edit handler in `hooks-tools.ts`, completing the JUDGE step.
...
- **Effort:** Small.
- **Status:** ✅ **FIXED** in PR #1300 (v1.5.4). Added `memoryGraph: true` to controllers config in `memory-bridge.ts` `getRegistry()`.
...
- **Effort:** Small.
- **Status:** ✅ **FIXED** in PR #1300 (v1.5.4). Registered `workflow_run` in `workflow-tools.ts`, `task_assign` in `task-tools.ts`, `task_summary` and `mcp_status` in `system-tools.ts`.
...
- **Effort:** Trivial.
- **Status:** ✅ **FIXED** in PR #1300 (v1.5.4). Added `.unref()` to 6 `setInterval` calls across `mcp-server.ts`, `output.ts`, `worker-queue.ts`, `container-worker-pool.ts`.

### 10. Zero swarms always: `monobrain spawn hive-mind --claude` (#1279)
...
- **Effort:** Small.
- **Status:** ✅ **FIXED** in PR #1300 (v1.5.4). `getStatus()` in `mcp-server.ts` now detects stdio mode via `!process.stdin.isTTY`, env var, or options. CLI displays "Running (stdio mode)".
...
- **Effort:** Trivial.
- **Status:** ✅ **FIXED** in PR #1300 (v1.5.4). Changed `df -h` to `df -Ph` for POSIX single-line output; added NaN guard for percentage parsing.
...
- **Impact:** Some users get ECOMPROMISED errors on `npx monobrain`. Related to the removed preinstall script.
- **Fix:** Document cache clear workaround: `npm cache clean --force`. The preinstall removal in v1.5.3 should prevent new occurrences.
- **Effort:** Trivial — documentation only.

### 17. AgentDB v2 → v1 upgrade (#1207)
...
- **Status:** ✅ **CLOSED** in v1.5.4 cycle. All four stubs closed as housekeeping.
...
### 23. ADR-058: Self-Contained monobrain.rvf Appliance (#1245)
...
| --------- | ------ | ------ | --------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| **P0**    | 1      | 1      | 0         | Windows platform support (**fixed** v1.5.6)                                                                                          |
...
1. ~~**Immediately** fix #1211 (hook stdin)~~ — ✅ Done in v1.5.4, refined in v1.5.7 (timeout fix).
2. ~~**Sprint 1** targets the learning loop: stdin → feedback → memory graph → memory resolution.~~ — ✅ Done (5/5 items fixed in v1.5.4).
3. ~~**Sprint 2** targets platform parity (Windows, macOS) and configuration completeness.~~ — ✅ Done (5/5 items fixed across v1.5.4–v1.5.6). Windows verified 10/10 tests pass.
...
| —   | Daemon branding            | "Worker Daemon" → "MonoBrain Daemon" in 3 status displays                               |

### V1.0.0 (2026-03-05) — Branding Sweep

All "Monobrain V1" → "MonoBrain V1" across 30+ files (CLI source, helpers, statusline).

### v1.5.4 (2026-03-05) — Sprint 1: Intelligence Pipeline

Fixes delivered in PR #1300, merged to `main` on 2026-03-05.

### Issues Fixed in v1.5.4 (10)
...
- ✅ Published v1.5.4: all 3 npm packages, all dist-tags verified
...
- ~~Sprint 1 fixes should ship as **v1.5.4** (patch — learning pipeline).~~ ✅ Shipped as v1.5.4 (PR #1300).
- ~~Sprint 2 remaining fixes (#1282, #1283, #1291) should ship as **v1.6.0**.~~ ✅ Shipped as V1.0.0 (branding), v1.5.6 (platform+settings), v1.5.7 (stdin timeout).
- Sprint 3 fixes should ship as **v1.7.0** (minor — neural features). 4 items remain: #1243, #1227, #1217, #1279.
