...
- Package: `@monobrain/codex` (npm)
- Future umbrella: `coflow` (npm/npx coflow)
- Current umbrella: `monobrain` (maintained for compatibility)
...
We will create a **parallel Codex integration** in monobrain that:
...
5. **Maps monobrain concepts** to Codex equivalents
6. **Supports dual-mode** projects (both Claude Code and Codex)

### Architecture

```
monobrain init --codex
...
└── .monobrain/                # Runtime (shared between both)
    ├── config.yaml
    └── data/
```
...
| monobrain Concept | Claude Code Output | Codex Output |
|---------------------|-------------------|--------------|
| Project instructions | `CLAUDE.md` | `AGENTS.md` |
...
```bash
npx monobrain@latest memory store --key "[pattern]" --value "[learned]"
```
```
...
1. Create `@monobrain/codex` package in `packages/@monobrain/codex/`
2. Implement AGENTS.md generator
3. Implement SKILL.md generator
4. Implement config.toml generator
5. Set up npm publishing for `@monobrain/codex`
...
5. Wire up `@monobrain/codex` as dependency
...
4. Publish skills as part of `@monobrain/codex`

### Phase 4: Automation Integration (Week 5)
1. Map monobrain hooks to Codex Automations
...
3. Maintain `monobrain` as alias for compatibility
...
### AGENTS.md Sections (Monobrain Template)
...
| Security | From @monobrain/security patterns |
| Performance | From V1 performance targets |
...
These features were discovered through binary analysis and can be leveraged for deep monobrain integration.
...
**Integration opportunity**: Map to monobrain swarm coordination.
...
**Integration opportunity**: Register monobrain tools dynamically.
...
```toml
# Monobrain V1 - Codex Configuration (Enhanced)
