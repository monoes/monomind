...
export class NeuralLearningSystem {
   ...
   * Attempts to start the real @ruvector/sona WASM engine; falls back to
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      await engine.initialize(); // loads WASM, throws if @ruvector/sona unavailable
      this.sonaEngine = engine;
    } catch {
      // @ruvector/sona not installed or failed to init — JS SONAManager fallback active
   ...
   * available, the real @ruvector/sona WASM engine.
   */
  async completeTask(trajectoryId: string, quality?: number): Promise<void> {
    ...
    if (trajectory) {
      ...
      if (memory) {
        const extracted = this.patternLearner.extractPattern(trajectory, memory);
        if (extracted) {
          this.sona.storePattern({
            name: extracted.name,
            ...
            qualityHistory: extracted.qualityHistory,
            evolutionHistory: extracted.evolutionHistory,
          });
        }
      }
  ...
  async cleanup(): Promise<void> {
    ...
    // M5: shut down ReasoningBank to close any open AgentDB SQLite handle
    if (typeof (this.reasoningBank as any).shutdown === 'function') {
      await (this.reasoningBank as any).shutdown();
    }
