...
export class ReasoningBank extends EventEmitter {
    ...
    constructor(config = {}) {
        ...
        this.embeddingService = new FallbackEmbeddingService(this.config.dimensions, this.config.useMockEmbeddings ?? false);
    }
    /**
    ...
    async storeInMemory(pattern, type) {
        if (!this.memoryBackend)
            return;
        try {
...
class FallbackEmbeddingService {
    ...
    cache = new Map();
    skipExternalCall;
    constructor(dimensions = 384, skipExternalCall = false) {
        this.dimensions = dimensions;
 ...
 */
export async function onPostTask(bridge, result) {
