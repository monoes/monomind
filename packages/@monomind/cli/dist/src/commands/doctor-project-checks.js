...
function routingFeedbackFallback() {
    try {
        ...
        const flags = records.map(r => r.intelligenceFeedback).filter((v) => typeof v === 'boolean');
...
export async function checkMonoesIntegration() {
    try {
        ...
        if (line.startsWith('routing accuracy (last 100): no outcome data yet')) {
            ...
            if (fb)
                return { name: 'Routing Learning', status: fb.degenerate ? 'warn' : 'pass', message: fb.message };
        }
