...
export async function handler(input, context) {
    ...
    try {
        ...
        const checkResults = await performChecks(validatedInput.includeChecks, validatedInput.projectPath, validatedInput.criteria);
        // Assess risks
        ...
        // Determine verdict
        const verdict = determineVerdict(blockers, warnings, confidence, validatedInput.strictMode);
        // Determine sign-offs required
        const signOffRequired = determineSignOffs(validatedInput.releaseType, riskAssessment.overallRisk);
        // Generate release notes
        const releaseNotes = await generateReleaseNotes(validatedInput.releaseType, validatedInput.compareToRelease, bridge);
        // Build result
        const result = {
            success: true,
...
async function performChecks(includeChecks, projectPath, customCriteria) {
    const results = [];
    // Quality gates check
...
function assessRisks(checkResults, releaseType) {
    const factors = [];
    ...
    const overallRisk = riskScore >= 3 ? 'critical' : riskScore >= 2 ? 'high' : riskScore >= 1 ? 'medium' : 'low';
...
function identifyBlockersAndWarnings(checkResults, riskAssessment) {
    const blockers = [];
    const warnings = [];
...
function calculateConfidence(checkResults, strictMode) {
...
function determineVerdict(blockers, warnings, confidence, strictMode) {
...
function determineSignOffs(releaseType, riskLevel) {
    const signOffs = [];
    ...
    if (releaseType === 'major') {
        signOffs.push({ role: 'Product Owner', status: 'pending' }, { role: 'Engineering Manager', status: 'pending' });
    }
...
async function generateReleaseNotes(releaseType, compareToRelease, bridge) {
    ...
    return {
        version: versionMap[releaseType] || '2.4.0',
...
//# sourceMappingURL=assess-readiness.js.map