...
export async function handler(input, context) {
    ...
    try {
        ...
        const chaosResult = await simulateChaos(validatedInput, injection, validatedInput.dryRun);
...
function captureMetricSnapshot(phase, degradation = 0) {
    const baseMetrics = {
        cpu: 35,
        memory: 60,
        networkLatency: 15,
...
function prepareInjection(input) {
...
function determineAffectedComponents(target, failureType) {
...
async function simulateChaos(input, injection, dryRun) {
    const events = [];
...
function calculateDegradation(failureType, intensity) {
...
function calculateRecoveryTime(failureType, degradation) {
    const baseRecovery = {
        'network-latency': 2,
...
function simulateDurationEvents(duration, degradation) {
    const events = [];
    // Add mid-point event
    if (duration > 10) {
        events.push({
...
function assessImpact(chaosResult, target) {
    const degradation = chaosResult.degradation;
    const severity = degradation >= 90
        ? 'critical'
        : degradation >= 70
            ? 'high'
            : degradation >= 40
...
function generateRecommendations(chaosResult, impact) {
...
//# sourceMappingURL=chaos-inject.js.map