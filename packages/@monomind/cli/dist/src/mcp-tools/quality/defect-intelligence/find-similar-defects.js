...
export async function handler(input, context) {
    ...
    try {
        ...
        const rawMatches = await performSimilaritySearch(validatedInput.query, validatedInput.searchScope, validatedInput.maxResults * 2, // Get more to filter
        bridge);
        // Filter by minimum similarity
        const filteredMatches = rawMatches.filter((m) => m.similarity >= validatedInput.minSimilarity);
...
async function performSimilaritySearch(query, scope, maxResults, bridge) {
    const matches = [];
    // Build search query
    const searchQuery = [
        query.description,
...
function createMatchFromPattern(pattern, baseSimilarity) {
    return {
...
function generateSimulatedMatches(query, maxResults) {
...
function getResolutionForCategory(category) {
    const resolutions = {
...
function analyzeMatchReasons(query, defect) {
    const reasons = [];
    // Semantic similarity
    reasons.push({
        type: 'semantic',
...
function groupMatches(matches, groupBy) {
    ...
    return Array.from(groups.entries()).map(([name, matchList]) => ({
        ...
        avgSimilarity: Math.round((matchList.reduce((sum, m) => sum + m.similarity, 0) / matchList.length) * 100) / 100,
...
function detectPatterns(matches) {
    ...
    const unresolvedMatches = matches.filter((m) => m.defect.status === 'open' || m.defect.status === 'in-progress');
...
function generateInsights(matches, patterns) {
    const insights = [];
...
//# sourceMappingURL=find-similar-defects.js.map