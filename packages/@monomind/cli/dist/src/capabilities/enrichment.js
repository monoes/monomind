...
export class EnrichmentPipeline {
    ...
    markFailed(filePath, tier) {
        ...
        if (this.state[filePath][tier] === 'done')
            return;
        this.state[filePath][tier] = 'failed';
    }
    markSkipped(filePath, tier) {
        this.ensureEntry(filePath);
        if (this.state[filePath][tier] === 'done')
            return;
     ...
     */
    async runTier(tier, files, monomindDir) {
        if (this._paused || files.length === 0)
            return;
        if (!this.manager)
            return;
        ...
        for (const module of modules) {
            ...
            try {
                if (tier === 't2') {
                    if (!module.enrich) {
                        for (const file of files)
                            this.markSkipped(file.path, tier);
                        continue;
                    }
                ...
                for (const file of files)
                    this.markDone(file.path, tier);
            }
            catch {
                for (const file of files)
                    this.markFailed(file.path, tier);
            }
            if (monomindDir)
                this.saveState(monomindDir);
        }
    }
    saveState(monomindDir) {
