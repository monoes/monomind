export async function getCookies(client, sessionId) {
    const result = await client.send('Network.getAllCookies', {}, sessionId);
    return result.cookies ?? [];
}
export async function setCookies(client, sessionId, cookies) {
    await client.send('Network.setCookies', { cookies }, sessionId);
}
export async function clearCookies(client, sessionId) {
    await client.send('Network.clearBrowserCookies', {}, sessionId);
}
export async function setExtraHeaders(client, sessionId, headers) {
    await client.send('Network.setExtraHTTPHeaders', { headers }, sessionId);
}
export async function enableInterception(client, sessionId) {
...
export async function setLocalStorage(client, sessionId, data) {
