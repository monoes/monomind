...
export class WorkerDaemon extends EventEmitter {
    ...
    persistHeadlessResult(workerType, result) {
        try {
            ...
            const parsed = result.parsedOutput && typeof result.parsedOutput === 'object' && !Array.isArray(result.parsedOutput)
                ? result.parsedOutput
                : {};
            ...
            this.log('warn', `Failed to persist headless ${workerType} result to metrics: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
