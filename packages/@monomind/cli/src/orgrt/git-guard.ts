...
/** The operator's own excludes file: an explicit core.excludesFile, else git's
 *  default `$XDG_CONFIG_HOME/git/ignore`. Git honours exactly one, so pointing
 *  core.excludesFile at the guard's file would silently drop theirs — its
 *  contents are copied into ours instead (a snapshot taken at session start;
 *  later edits to their file only reach the next session). */
function operatorExcludes(baseEnv: NodeJS.ProcessEnv): { path: string; body: string } | undefined {
  const explicit = (() => {
    const count = Number.parseInt(baseEnv.GIT_CONFIG_COUNT ?? '', 10);
    for (let i = 0; Number.isInteger(count) && i < count; i++)
      if (baseEnv[`GIT_CONFIG_KEY_${i}`]?.toLowerCase() === 'core.excludesfile')
        return baseEnv[`GIT_CONFIG_VALUE_${i}`];
    try {
      return execFileSync('git', ['config', '--get', 'core.excludesFile'], {
        encoding: 'utf8',
        stdio: ['ignore', 'pipe', 'ignore'],
        timeout: 10_000,
      }).trim();
  ...
  const xdg = baseEnv.XDG_CONFIG_HOME ?? join(home, '.config');
  const path = explicit || join(xdg, 'git', 'ignore');
  const expanded = path.startsWith('~/') ? join(home, path.slice(2)) : path;
  try {
    return { path: expanded, body: readFileSync(expanded, 'utf8') };
  } catch {
    return undefined;
  }
}

function excludesFile(dir: string, baseEnv: NodeJS.ProcessEnv): string {
  const operator = operatorExcludes(baseEnv);
  const path = join(dir, 'excludes');
  writeFileSync(
    path,
    [
      '# Generated by monomind (#258) for a role session. Regenerated at every',
      '# session start; edits are overwritten.',
      ...(operator
        ? [
            `# Copied from the operator's own excludes file (${operator.path}):`,
            operator.body.trimEnd(),
            '',
          ]
        : []),
      '# Zero-byte placeholders the OS sandbox creates in the role working dir:',
      ...SANDBOX_PLACEHOLDERS.map((p) => `/${p}`),
      '',
    ].join('\n'),
  );
  return path;
...
export function prepareGitGuard(opts: {
  ...
  protectedGitDirs: string[];
  baseEnv?: NodeJS.ProcessEnv;
  /** Hide the placeholder files the OS sandbox creates in the role's cwd.
   *  Only meaningful when the sandbox actually runs. */
  excludeSandboxPlaceholders?: boolean;
}): GitGuard | undefined {
  ...
  const env: Record<string, string> = {
    ...gitConfigEnv(
      [
        ...
        ...(opts.excludeSandboxPlaceholders
          ? ([['core.excludesFile', excludesFile(dir, opts.baseEnv ?? process.env)]] as Array<
              [string, string]
            >)
          : []),
