...
export async function replayFrom(
  ...
  const events = readFileSync(busFile, 'utf8')
    .split('\n')
    .filter(Boolean)
    .map((l) => {
      try {
  ...
  bus.emit({
    type: 'status',
    msg: `replay started from ${run} (${events.length} events replayed)`,
  });
...
export async function resumeOrg(daemon: OrgDaemon, name: string): Promise<RunningOrg | null> {
  ...
  try {
    ...
    console.error(
      'resumeOrg failed: invalid JSON in runtime.json for',
      name,
  ...
  if (!rt?.run || !rt?.checkpoint) {
    console.error(
      'resumeOrg failed: invalid runtime state for',
      name,
      'status:',
      rt?.status,
      'run:',
  ...
  if (rt.checkpoint && isCheckpointExpired(rt.checkpoint)) {
    console.error(
      'resumeOrg failed: checkpoint expired for',
      name,
      'updated:',
      rt.checkpoint.updated,
    );
    return null;
  }
