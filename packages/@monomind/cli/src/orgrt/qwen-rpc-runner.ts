...
interface QwenRpcContentBlock {
  type: string;
  text?: string;
  thinking?: string;
}
interface QwenRpcMessage {
...
export class QwenRpcAgentRunner implements AgentRunner {
  constructor(
    private qwenBin?: string,
    private spawnFn: SpawnQwenRpc = defaultSpawnQwenRpc,
  ) {}
      ...
      for await (const p of args.prompt) {
        ...
        try {
          for (;;) {
            ...
            for (;;) {
              ...
              if (kind === 'result') {
                ...
                if (ev.subtype === 'error') {
                  ...
                  resultError =
                    typeof err === 'string' ? err : (err?.message ?? 'qwen result: error');
                }
                break;
              }
            ...
            if (resultError) {
              throw new Error(
                `QwenRpcAgentRunner: qwen turn failed: ${resultError}` +
                  (stderrTail ? `\nstderr: ${stderrTail.slice(-500)}` : ''),
              );
            }
