...
import { FenceConfigSchema, type FenceConfig } from './types.js';
import type { Decision } from './policy.js';
import type { OrgBus } from './bus.js';

export interface FenceInstance {
  detect(input: string): Promise<{ safe: boolean; threats: { type: string; confidence: number }[]; overallRisk: number }>;
  scanOutput(output: string, originalPrompt?: string): Promise<{ safe: boolean; leakageFound: boolean }>;
  getContextState(): { escalationState: string };
  addAllowlistRule(rule: { id: string; pattern: RegExp | string; types: string[]; context?: string; reason?: string; source?: string }): void;
}
...
export async function createFenceForRole(config: FenceConfig): Promise<FenceInstance | null> {
  try {
    const mod = await import('monofence-ai') as { createMonoDefence: (cfg: Record<string, unknown>) => FenceInstance };
...
export async function scanInput(
  fence: FenceInstance, input: string, abortThreshold: number,
): Promise<Decision> {
  ...
  if (!result.safe && result.overallRisk >= abortThreshold) {
    ...
    return { behavior: 'deny', message: `[fence] threat detected: ${top?.type ?? 'unknown'} (risk ${result.overallRisk.toFixed(2)})` };
  }
  if (ctx.escalationState === 'attack') {
    return { behavior: 'deny', message: `[fence] session in attack escalation state — input blocked` };
  }
  return { behavior: 'allow', updatedInput: {} };
}

export async function scanMessage(
  fence: FenceInstance, body: string, abortThreshold: number, bus: OrgBus, from: string,
): Promise<boolean> {
  const result = await fence.detect(body);
  if (!result.safe && result.overallRisk >= abortThreshold) {
    const top = result.threats[0];
    bus.emit({
      type: 'audit', from, reason: 'fence-message',
