...
import { ORG_DIR, type DecisionGate } from './types.js';
...
export function readGates(root: string, org: string): { gates: DecisionGate[] } {
  try { return JSON.parse(readFileSync(gatesPath(root, org), 'utf8')); } catch { return { gates: [] }; }
}
...
function withGatesLock<T>(daemon: OrgDaemon, org: string, fn: () => Promise<T>): Promise<T> {
  ...
  daemon.gatesLocks.set(org, next.catch(() => { /* slot stays usable for the next caller */ }));
  return next;
}

export async function createGate(daemon: OrgDaemon, org: string, role: string, name: string, description: string): Promise<string> {
...
export async function resolveGate(daemon: OrgDaemon, org: string, gateId: string, approved: boolean, resolution?: string, resolvedBy?: string): Promise<{ ok: true } | { ok: false; error: string }> {
  return withGatesLock(daemon, org, async () => {
    const data = readGates(daemon.root, org);
...
export function listGates(daemon: OrgDaemon, org: string, status?: 'pending' | 'approved' | 'rejected'): DecisionGate[] {
  const data = readGates(daemon.root, org);
...
export function dagCreateTask(daemon: OrgDaemon, org: string, role: string, title: string, assignee: string, deps: string[]): string {
  const running = daemon.orgs.get(org);
...
export function recordDecision(daemon: OrgDaemon, org: string, role: string, decision: {
