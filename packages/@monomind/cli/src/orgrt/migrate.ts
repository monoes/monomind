...
export function checkOrgStructure(def: Pick<OrgDef, 'roles' | 'schedule'>): string[] {
  ...
  if (roots.length > 1)
    errors.push(
      `multiple root roles (${roots.map((r) => r.id).join(', ')}) — exactly one may have reports_to: null`,
    );
...
export function migrateOrgConfig(raw: Record<string, unknown>): {
  ...
  if (loop && typeof loop.poll_interval_minutes === 'number' && def.schedule == null) {
    ...
    notes.push(
      `loop.poll_interval_minutes=${loop.poll_interval_minutes} → schedule "${def.schedule}"`,
    );
  }
...
export function migrateOrgFile(
  cfgPath: string,
  backupPath: string,
): {
  ...
  if (structuralErrors.length) {
    throw new Error(
      `migrated config still fails structural validation: ${structuralErrors.join('; ')}`,
    );
  }
