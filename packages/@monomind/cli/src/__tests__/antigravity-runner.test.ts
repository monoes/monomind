 ...
 * matching the schema from https://antigravity.google/docs/cli/headless.
 *
 * agy event schema differs from both kimi (role-based) and codex (item-based):
 *   - `init` carries conversation_id
 *   - `step_update` with step_type 'agent_response' streams text_delta per-token
 *   - `result` carries conversation_id, status, usage
 */
...
describe('AntigravityAgentRunner', () => {
  ...
  it('builds correct argv: agy -p <prompt> --output-format stream-json', async () => {
    vi.mocked(cp.spawn).mockReturnValue(makeMockChild([
      JSON.stringify({ type: 'init', conversation_id: 'c1' }),
      JSON.stringify({ type: 'result', conversation_id: 'c1', status: 'SUCCESS', usage: { input_tokens: 10, output_tokens: 5 } }),
    ]));
  ...
  it('captures conversation_id from init event', async () => {
    vi.mocked(cp.spawn).mockReturnValue(makeMockChild([
      JSON.stringify({ type: 'init', conversation_id: 'test-conv-123' }),
      JSON.stringify({ type: 'result', conversation_id: 'test-conv-123', status: 'SUCCESS', usage: { input_tokens: 10, output_tokens: 5 } }),
    ]));
  ...
  it('accumulates agent_response text_delta into one message', async () => {
    vi.mocked(cp.spawn).mockReturnValue(makeMockChild([
      JSON.stringify({ type: 'init', conversation_id: 'c1' }),
      JSON.stringify({ type: 'step_update', step_type: 'agent_response', state: 'ACTIVE', text_delta: 'Hello' }),
      JSON.stringify({ type: 'step_update', step_type: 'agent_response', state: 'ACTIVE', text_delta: ' world' }),
      JSON.stringify({ type: 'step_update', step_type: 'agent_response', state: 'DONE' }),
      JSON.stringify({ type: 'result', conversation_id: 'c1', status: 'SUCCESS', usage: { input_tokens: 10, output_tokens: 5 } }),
    ]));
  ...
  it('extracts usage from result event', async () => {
    vi.mocked(cp.spawn).mockReturnValue(makeMockChild([
      JSON.stringify({ type: 'init', conversation_id: 'c1' }),
      JSON.stringify({ type: 'result', conversation_id: 'c1', status: 'SUCCESS', usage: { input_tokens: 1500, output_tokens: 320, thinking_tokens: 50, cache_read_tokens: 200, total_tokens: 2070 } }),
    ]));
  ...
  it('surfaces error from non-SUCCESS result status', async () => {
    vi.mocked(cp.spawn).mockReturnValue(makeMockChild([
      JSON.stringify({ type: 'init', conversation_id: 'c1' }),
      JSON.stringify({ type: 'result', conversation_id: 'c1', status: 'ERROR', error: 'model not found' }),
    ]));
  ...
  it('uses --conversation flag when conversationId provided', async () => {
    vi.mocked(cp.spawn).mockReturnValue(makeMockChild([
      JSON.stringify({ type: 'init', conversation_id: 'existing-conv' }),
      JSON.stringify({ type: 'result', conversation_id: 'existing-conv', status: 'SUCCESS', usage: { input_tokens: 0, output_tokens: 0 } }),
    ]));
  ...
  it('falls back to result.response when no text_delta streamed', async () => {
    ...
    vi.mocked(cp.spawn).mockReturnValue(makeMockChild([
      JSON.stringify({ type: 'init', conversation_id: 'c1' }),
      JSON.stringify({ type: 'result', conversation_id: 'c1', status: 'SUCCESS', response: 'Final response text', usage: { input_tokens: 10, output_tokens: 5 } }),
    ]));
  ...
  it('strips tool_call fences from streamed text (no tools registered → error result → clean second turn)', async () => {
    vi.mocked(cp.spawn)
      .mockReturnValueOnce(makeMockChild([
        JSON.stringify({ type: 'init', conversation_id: 'c1' }),
        JSON.stringify({ type: 'step_update', step_type: 'agent_response', state: 'ACTIVE', text_delta: 'Sending...\n\n```tool_call\n{"name": "org_send", "arguments": {"to": "boss"}}\n```' }),
        JSON.stringify({ type: 'result', conversation_id: 'c1', status: 'SUCCESS', usage: { input_tokens: 10, output_tokens: 5 } }),
      ]))
      .mockReturnValueOnce(makeMockChild([
        JSON.stringify({ type: 'init', conversation_id: 'c1' }),
        JSON.stringify({ type: 'step_update', step_type: 'agent_response', state: 'ACTIVE', text_delta: 'Done.' }),
        JSON.stringify({ type: 'result', conversation_id: 'c1', status: 'SUCCESS', usage: { input_tokens: 10, output_tokens: 5 } }),
      ]));
