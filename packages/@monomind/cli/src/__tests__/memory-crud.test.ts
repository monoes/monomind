...
describe('memory-crud commands (sql.js fallback path)', () => {
  ...
  describe('store', () => {
    it('requires a key', async () => {
      const result = await run(storeCommand, ctx({ flags: { _: [], value: 'hello' } }));
      expect(result.success).toBe(false);
      expect(result.exitCode).toBe(1);
    });

    it('requires a value when non-interactive', async () => {
      const result = await run(storeCommand, ctx({ flags: { _: [], key: 'k1' } }));
      expect(result.success).toBe(false);
      expect(result.exitCode).toBe(1);
    });
    ...
    it('stores a value under a key+namespace, findable by retrieve', async () => {
      const result = await run(
        storeCommand,
        ctx({
          flags: { _: [], key: 'api/auth', value: 'JWT implementation', namespace: 'patterns' },
        }),
      );
    ...
    it.skipIf(process.platform === 'win32')(
      'persists .swarm/memory.db with owner-only (0600) permissions',
      async () => {
        const result = await run(
          storeCommand,
    ...
    it('stores without an explicit namespace, defaulting to "default"', async () => {
      ...
      const got = await run(retrieveCommand, ctx({ flags: { _: [], key: 'no-ns-key' } }));
      expect(got.success).toBe(true);
    ...
    it('stores tags and reflects them on the stored entry', async () => {
      const result = await run(
        storeCommand,
        ctx({
          flags: { _: [], key: 'tagged', value: 'v', namespace: 'default', tags: 'bugfix,auth' },
        }),
      );
      expect(result.success).toBe(true);
      expect((result.data as any).tags).toEqual(['bugfix', 'auth']);

      const got = await run(retrieveCommand, ctx({ flags: { _: [], key: 'tagged' } }));
    ...
    it('upsert=false leaves the original value on a duplicate key (sql.js fallback does a plain INSERT)', async () => {
      ...
      const first = await run(storeCommand, ctx({ flags: { _: [], key: 'dup', value: 'first' } }));
    ...
    it(
      ...
      async () => {
        ...
        const updated = await run(
          storeCommand,
          ctx({
            flags: { _: [], key: 'up-key', value: 'updated', namespace: 'up-ns', upsert: true },
          }),
        );
    ...
    it('returns an empty (not crashed) result set for a query that matches nothing', async () => {
      await run(
        ...
        ctx({
          flags: {
            _: [],
            key: 'unrelated',
            value: 'completely unrelated content about oceans',
            namespace: 'search-ns',
          },
        }),
      );
      const result = await run(
        ...
        ctx({
          flags: {
            _: [],
            query: 'zzz_definitely_not_present_zzz',
            namespace: 'search-ns',
            threshold: 0.9,
          },
        }),
    ...
    it(
      ...
      async () => {
        await run(
          storeCommand,
          ctx({
            flags: { _: [], key: 'resurrect-key', value: 'original', namespace: 'resurrect-ns' },
          }),
        );
        const del = await run(
          deleteCommand,
