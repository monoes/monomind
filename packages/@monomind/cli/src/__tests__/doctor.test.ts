...
function resultData(result: CommandResult | undefined): {
  ...
  return (result as CommandResult).data as {
    passed: number;
    warnings: number;
    failed: number;
    results: HealthCheck[];
...
describe('doctorCommand', () => {
  ...
  it('returns a failure result for an unknown --component', async () => {
    const result = await doctorCommand.action?.(
      makeCtx({ flags: { _: [], component: 'not-a-real-component' } }),
    );
  ...
  it('runs the full default check set against a bare project and returns a well-formed CommandResult', async () => {
    ...
    expect(data.results.length).toBe(34 + JEV_ROW);
  ...
  it('--fix applies the real local Helper Files fix and re-checks it in place', async () => {
    ...
    const before = await doctorCommand.action?.(
      makeCtx({ flags: { _: [], component: 'helpers' } }),
    );
