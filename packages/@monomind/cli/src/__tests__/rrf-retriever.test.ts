...
import { RrfRetriever, type Retriever, type RawHit } from '../knowledge/eval/retrievers.js';

/** Stub retriever that returns a fixed result set. */
class StubRetriever implements Retriever {
  name: string;
  description = 'stub';
  constructor(name: string, private results: RawHit[]) { this.name = name; }
...
describe('RrfRetriever', () => {
  ...
  it('documents in only one child get a single 1/(k+rank) contribution', async () => {
    const a = new StubRetriever('a', [
      { docId: 'X', chunkIndex: 0, score: 1.0 },
    ]);
    const b = new StubRetriever('b', [
      { docId: 'Y', chunkIndex: 0, score: 1.0 },
    ]);
