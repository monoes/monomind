...
describe('security redteam', () => {
  ...
  const startServer = (handler: (body: { prompt: string; category: string }) => { status: number; response?: string }): Promise<{ server: Server; url: string }> => {
  ...
  it('defaults to dry-run when --target is absent', async () => {
    ...
    const res = await redteamCommand.action!({ flags: { scenarios: 'pii', iterations: '1' }, args: [] } as never) as
      { success: boolean; data?: { dryRun?: boolean } };
  ...
  it('--dry-run=false without --target is rejected', async () => {
    const { redteamCommand } = await import('../commands/security-misc.js');
    const res = await redteamCommand.action!({ flags: { 'dry-run': false }, args: [] } as never) as
  ...
  it('live run against a safe target passes and reports 0% failure rate', async () => {
    const { server, url } = await startServer(() => ({ status: 200, response: "I can't help with that request." }));
    try {
      const { redteamCommand } = await import('../commands/security-misc.js');
      const res = await redteamCommand.action!({ flags: { target: url, scenarios: 'pii', iterations: '2' }, args: [] } as never) as
        { success: boolean; data?: { total: number; unsafe: number; failureRate: number; passed: boolean } };
      expect(res.success).toBe(true);
  ...
  it('live run against a leaking target fails and reports the failure rate over threshold', async () => {
    const { server, url } = await startServer(() => ({ status: 200, response: 'Sure — contact admin@company.com for the password.' }));
    try {
      const { redteamCommand } = await import('../commands/security-misc.js');
      const res = await redteamCommand.action!({ flags: { target: url, scenarios: 'pii', iterations: '2', threshold: 0.1 }, args: [] } as never) as
        { success: boolean; exitCode?: number; data?: { total: number; unsafe: number; passed: boolean } };
      expect(res.success).toBe(false);
      expect(res.exitCode).toBe(1);
  ...
  it('a failed send counts as unsafe rather than being skipped', async () => {
    ...
    try {
      const { redteamCommand } = await import('../commands/security-misc.js');
      const res = await redteamCommand.action!({ flags: { target: url, scenarios: 'pii', iterations: '1' }, args: [] } as never) as
  ...
  it('--output json prints only the JSON summary, not the decorated listing', async () => {
    ...
    try {
      const { redteamCommand } = await import('../commands/security-misc.js');
      await redteamCommand.action!({ flags: { target: url, scenarios: 'pii', iterations: '1', output: 'json' }, args: [] } as never);
