...
describe('GET /api/org/:name/budgets — dual usage-event shape handling', () => {
  ...
  it('counts both org:usage (forwarded) and agent:usage (legacy) events toward the budget total', async () => {
    ...
    writeFileSync(join(runsDir, 'run1.jsonl'), events.map((e) => JSON.stringify(e)).join('\n') + '\n');

    const req = { method: 'GET', url: `/api/org/myorg/budgets?dir=${encodeURIComponent(cwd)}` } as any;
    const res = makeRes();
    const handled = await handleOrgRoutes(req, res, '/api/org/myorg/budgets', null, { projectDir: cwd });
