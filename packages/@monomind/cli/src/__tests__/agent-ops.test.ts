...
function seedAgentStore(
  dir: string,
  agents: Array<{ agentId: string; agentType: string; status: string; taskCount?: number }>,
) {
...
describe('metricsCommand', () => {
  ...
  it('reflects agents spawned via the real agent_spawn flow', async () => {
    ...
    (await spawnCommand.action?.(
      makeCtx({ flags: { type: 'coder', name: 'real-agent', _: [] } }),
    )) as CommandResult;
...
describe('poolCommand', () => {
  ...
  it('reflects real agents spawned into the store (via agent_pool status, sourced from the same store.json)', async () => {
    (await spawnCommand.action?.(
      makeCtx({ flags: { type: 'coder', name: 'p1', _: [] } }),
    )) as CommandResult;
    (await spawnCommand.action?.(
      makeCtx({ flags: { type: 'tester', name: 'p2', _: [] } }),
    )) as CommandResult;
...
describe('healthCommand', () => {
  ...
  it('classifies real spawned agents as healthy (fresh agents default to health=1.0)', async () => {
    (await spawnCommand.action?.(
      makeCtx({ flags: { type: 'coder', name: 'h1', _: [] } }),
    )) as CommandResult;
    (await spawnCommand.action?.(
      makeCtx({ flags: { type: 'tester', name: 'h2', _: [] } }),
    )) as CommandResult;
  ...
  it('supports looking up a single agent by ID via args[0]', async () => {
    const spawned = (await spawnCommand.action?.(
      makeCtx({ flags: { type: 'coder', name: 'solo-h', _: [] } }),
    )) as CommandResult;
