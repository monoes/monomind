...
export async function loadEmbeddingModel(options?: {
  ...
  try {
    ...
    if (embPkg && typeof (embPkg as any).createEmbeddingService === 'function') {
      try {
        const service = (embPkg as any).createEmbeddingService({
        ...
        if (probeVec && dims > 1) {
          ...
          embeddingModelState = {
            ...
            model: async (text: string) =>
              Array.from((await service.embed(String(text))).embedding as ArrayLike<number>),
            tokenizer: null,
            dimensions: dims,
          };
          return {
