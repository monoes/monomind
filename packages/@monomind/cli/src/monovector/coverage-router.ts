...
export interface CoverageRouteResult {
  ...
  testTypes: string[];
  targetFiles: string[];
  gaps: Array<{ file: string; currentCoverage: number; gap: number }>;
}
 ...
 */
export function readCoverage(cwd: string = getProjectCwd()): CoverageData {
...
function pct(covered: number | undefined, total: number | undefined, fallback = 100): number {
...
function parseLcov(raw: string, source: string): CoverageData {
  ...
  for (const line of raw.split('\n')) {
    ...
    else if (t.startsWith('FNF:')) fnf = Number(t.slice(4)) || 0;
    else if (t.startsWith('FNH:')) fnh = Number(t.slice(4)) || 0;
    else if (t === 'end_of_record') flush();
  }
  flush();
...
function suggestTestTypes(entry: CoverageEntry): string[] {
  const types: string[] = [];
  if (entry.lines < 80 || entry.statements < 80) types.push('unit');
  if (entry.branches < 80) types.push('edge-case');
  if (entry.functions < 80) types.push('unit');
  if (entry.lines < 50) types.push('integration');
...
function estimateEffort(gap: number): number {
...
export async function coverageGaps(
  ...
  for (const g of gaps) (byAgent[g.suggestedAgent] ??= []).push(g.file);
...
export async function coverageRoute(
  ...
  if (below.length === 0) action = 'skip';
  else if (overall < threshold - 20) action = 'prioritize';
  else if (overall < threshold) action = 'add-tests';
  else action = 'review-coverage';
