/**
 * MonoVector LLM WASM Integration
 *
 * Wraps @monoes/ruvllm-wasm for browser-native LLM inference utilities.
 ...
 *
 * @module @monomind/cli/monovector/ruvllm-wasm
 */
...
/**
 * Check if @monoes/ruvllm-wasm is installed and loadable.
 */
export async function isRuvllmWasmAvailable(): Promise<boolean> {
  try {
    const mod = await import('@monoes/ruvllm-wasm');
...
export async function initRuvllmWasm(): Promise<void> {
  ...
  try {
    ...
    const wasmPath = require_.resolve('@monoes/ruvllm-wasm/ruvllm_wasm_bg.wasm');
    const wasmBytes = readFileSync(wasmPath);
    // MUST use object form — initSync(bytes) is deprecated
    mod.initSync({ module: wasmBytes });
    _wasmReady = true;
  } catch (err) {
    throw new Error(`Failed to initialize @monoes/ruvllm-wasm: ${err}`);
  }
}
