/**
 * MonoVector Agent WASM Integration
 *
 * Wraps @monoes/rvagent-wasm for sandboxed AI agent execution.
 ...
 *
 * @module @monomind/cli/monovector/agent-wasm
 */
...
/**
 * Check if @monoes/rvagent-wasm is installed and loadable.
 */
export async function isAgentWasmAvailable(): Promise<boolean> {
  try {
    const mod = await import('@monoes/rvagent-wasm');
...
export async function initAgentWasm(): Promise<void> {
  ...
  try {
    ...
    const wasmPath = require_.resolve('@monoes/rvagent-wasm/rvagent_wasm_bg.wasm');
    const wasmBytes = readFileSync(wasmPath);
    mod.initSync({ module: wasmBytes });
    _wasmReady = true;
  } catch (err) {
    throw new Error(`Failed to initialize @monoes/rvagent-wasm: ${err}`);
  }
}
