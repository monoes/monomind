...
export const monographVisualizeTool: MCPTool = {
  ...
  description:
    'Render the knowledge graph as HTML (default), SVG, or JSON. Writes output to a file under .monomind/visualize/ and returns the file path (output can be multi-MB at the max node count, too large to return inline).',
  inputSchema: {
    type: 'object',
    properties: {
  ...
  handler: async (input) => {
    ...
    try {
      ...
      const limit =
        Number.isFinite(rawMaxNodes) && rawMaxNodes > 0
          ? Math.min(Math.floor(rawMaxNodes), MAX_EXPORT_NODES)
          : 500;
      const nodes = db
        .prepare(`SELECT ${NODE_RENDER_COLUMNS} FROM nodes LIMIT ?`)
        .all(limit) as any[];
      ...
      const rendered =
        fmt === 'json'
          ? toJson(nodes as any, edges as any)
          : fmt === 'svg'
            ? toSvg(nodes as any, edges as any)
      ...
      return text(
        `Visualization written to ${outPath} (${nodes.length} nodes, ${edges.length} edges)${ext === 'html' ? '\nNote: HTML uses the vis-network CDN script and requires network access to render.' : ''}`,
      );
    } finally {
      closeDb(db);
    }
...
export const monographExportTool: MCPTool = {
  ...
  description:
    'Export the knowledge graph in various formats: obsidian, canvas, cypher, graphml, svg, json.',
  inputSchema: {
    type: 'object',
    properties: {
      format: {
        type: 'string',
        description: 'Format: obsidian, canvas, cypher, graphml, svg, json',
      },
  ...
  handler: async (input) => {
    const { openDb, closeDb, toJson, toSvg, toGraphml, toCypher, toObsidian, toCanvas } =
    ...
    try {
      ...
      const requestedOut =
        (input.outputPath as string | undefined) ?? join(getProjectCwd(), '.monomind', 'export');
...
export const monographSnapshotTool: MCPTool = {
  ...
  description:
    'Save current graph state to a named JSON snapshot. Use with monograph_diff to compare before/after changes.',
  inputSchema: {
    type: 'object',
    properties: {
      name: {
        type: 'string',
        description: 'Snapshot name (default: ISO timestamp). Used as the filename.',
      },
    },
  },
  handler: async (input) => {
    ...
    try {
      ...
      const rawName =
        (input.name as string | undefined) ?? new Date().toISOString().replace(/[:.]/g, '-');
...
export const monographDiffTool: MCPTool = {
  name: 'monograph_diff',
  description:
    'Compare two named graph snapshots (saved via monograph_snapshot). Omit "after" to diff the "before" snapshot against the live graph.',
  inputSchema: {
    type: 'object',
    properties: {
      before: {
        type: 'string',
        description: 'Name of the before snapshot (without .json extension)',
      },
      after: {
        type: 'string',
        description: 'Name of the after snapshot, or omit to compare against the live graph',
      },
    },
    required: ['before'],
  },
  handler: async (input) => {
    ...
    const { readFileSync, existsSync, statSync: statSyncSnap } = await import('node:fs');
    const { resolve: resolvePath } = await import('node:path');
    ...
    const indexNodes = (nodes: Array<NodeRef & { id?: string }>) => {
      for (const n of nodes) {
        if (n.id) nodeById.set(n.id as string, n);
      }
    };
    ...
    const lines = [
      ...
      section(
        'New nodes',
        diff.newNodes.map((n) => `  + ${formatNode(n)}`),
      ),
      section(
        'Removed nodes',
        diff.removedNodes.map((n) => `  - ${formatNode(n)}`),
      ),
      section(
        'New edges',
        diff.newEdges.map(
          (e) =>
            `  + ${resolveEdgeEnd(e.sourceId)} --[${e.relation}]--> ${resolveEdgeEnd(e.targetId)}`,
        ),
      ),
      section(
        'Removed edges',
...
export const monographReportTool: MCPTool = {
  ...
  handler: async (input) => {
    ...
    try {
      ...
      const rawOutPath =
        (input.path as string | undefined) ?? join(getProjectCwd(), '.monomind', 'GRAPH_REPORT.md');
...
export const monographWikiTool: MCPTool = {
  name: 'monograph_wiki',
  description:
    'Retrieve LLM-generated wiki pages for code communities. Returns one page by communityId or all pages if no filter provided.',
  inputSchema: {
    type: 'object',
    properties: {
      communityId: {
        type: 'string',
        description: 'Community ID to retrieve (omit to list all pages)',
      },
    },
  },
  handler: async (input) => {
    ...
    const db = openDb(getDbPath());
    try {
      const result = getWikiToolResult(db, {
        communityId: input.communityId as string | undefined,
      });
      if (result.pages.length === 0) {
        return text(
          'No wiki pages found. Run monograph_wiki_build to generate community wiki pages.',
        );
      }
...
export const monographWikiBuildTool: MCPTool = {
  ...
  description:
    'Generate wiki pages for code communities using Claude Code CLI (no API key needed — reuses Claude Code auth).',
  inputSchema: {
    type: 'object',
    properties: {
      communityId: {
        type: 'string',
        description: 'Only generate for this community ID (omit for all communities)',
      },
      force: {
        type: 'boolean',
        description: 'Regenerate even if page already exists (default false)',
      },
      model: {
        type: 'string',
  ...
  handler: async (input) => {
    ...
    try {
      ...
      return text(
        `Wiki build complete: ${parts.join(', ') || 'nothing to do'}. Use monograph_wiki to read the pages.`,
      );
    } finally {
      closeDb(db);
    }
...
export const monographServeTool: MCPTool = {
  ...
  description:
    'Start a web UI server that visualizes the knowledge graph interactively using Sigma.js. Returns the URL where the dashboard is accessible.',
  inputSchema: {
    type: 'object',
    properties: {
      ...
      open: {
        type: 'boolean',
        description: 'Open the URL in the default browser after starting (default false)',
      },
    },
  },
  handler: async (input) => {
    ...
    return text(
      `Monograph web UI ${result.status === 'already_running' ? 'already running' : 'started'} at ${result.url}`,
    );
  },
};
