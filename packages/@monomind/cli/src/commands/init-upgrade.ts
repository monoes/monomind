...
export const upgradeCommand: Command = {
  ...
  options: [
    ...
    },
    {
      name: 'all',
      description:
        'Upgrade all known monomind projects on this machine (scans ~/Desktop, ~/projects, etc.)',
      type: 'boolean',
      default: false,
    },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (upgradeAll) {
      ...
      output.writeln(
        output.dim('Scanning ~/Desktop, ~/projects, ~/code… (this may take a moment)'),
      );
      output.writeln();
      const projects = findMonomindProjects();
      if (projects.length === 0) {
        output.printInfo(
          'No monomind projects found. Install monomind in a project first: npx monomind init',
        );
      ...
      try {
        ...
        if (fs.existsSync(ctrlPath) && fs.statSync(ctrlPath).size <= 4096) {
          ...
          if (
            typeof ctrlCfg.url === 'string' &&
            /^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?(\/|$)/.test(ctrlCfg.url)
          ) {
      ...
      const emitUpgradeProgress = async (
        projDir: string,
        ...
        try {
          ...
          const req = http.request({
            ...
            port: parseInt(url.port || '4242', 10),
            path: url.pathname,
            method: 'POST',
            headers: {
      ...
      for (const projDir of projects) {
        ...
        const spinner = output.createSpinner({
          text: `[${projIdx}/${projects.length}] ${projDir}`,
        });
        spinner.start();
        try {
      ...
      output.writeln();
      output.printInfo(
        `Done: ${succeeded} upgraded, ${failed} failed out of ${projects.length} projects.`,
      );
    ...
    if (addMissing && upgradeSettings) {
      output.writeln(
        output.dim('Updates helpers, settings, and adds any missing skills/agents/commands'),
      );
    } else if (addMissing) {
    ...
    const spinnerText = upgradeSettings
      ...
      : addMissing
        ? 'Upgrading and adding missing assets...'
        : 'Upgrading...';
    ...
    try {
      ...
      spinner.fail('Upgrade failed');
      output.printError(
        `Failed to upgrade: ${error instanceof Error ? error.message : String(error)}`,
      );
      return { success: false, exitCode: 1 };
    }
  },
