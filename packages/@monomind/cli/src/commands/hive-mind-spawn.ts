  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const result = await callMCPTool<{
        success: boolean;
        spawned: number;
        workers: Array<{ agentId: string; role: string; joinedAt: string }>;
        totalWorkers: number;
        hiveStatus: string;
        hiveId?: string;
        message: string;
        error?: string;
      }>('hive-mind_spawn', { count, role, agentType, prefix });
      ...
      if (launchClaude) {
        ...
        try {
          const statusResult = await callMCPTool<{ hiveId?: string }>('hive-mind_status', { includeWorkers: false });
