...
const listSubcommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const builtinActions = [
      ...
      {
        id: 'linkedin:send_connection',
        platform: 'linkedin',
        name: 'Send Connection Request',
        source: 'built-in',
      },
      {
        id: 'linkedin:publish_post',
        platform: 'linkedin',
        name: 'Publish Post',
        source: 'built-in',
      },
      ...
      {
        id: 'instagram:comment_post',
        platform: 'instagram',
        name: 'Comment on Post',
        source: 'built-in',
      },
      {
        id: 'instagram:follow_user',
        platform: 'instagram',
        name: 'Follow User',
        source: 'built-in',
      },
    ...
    const customActions = await Promise.all(
...
const runSubcommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printError(
        `Action not found: ${actionId}. Run "monomind browse action list" to see available actions.`,
      );
      return { success: false, exitCode: 1 };
    }
        ...
        switch (step.type) {
          ...
          case 'find':
            for (const sel of (step.selectors as string[]) ?? []) {
              ...
              if (found) {
                refs.set(step.as as string, found);
                break;
              }
            }
          ...
          case 'wait':
            if (step.condition === 'network_idle')
              await waitFor(client, sessionId, {
                load: 'networkidle',
                timeout: step.timeout as number,
              });
            else if (step.condition === 'selector')
              await waitFor(client, sessionId, {
                selector: step.selector as string,
                timeout: step.timeout as number,
              });
            else if (step.condition === 'duration')
              await new Promise((r) => setTimeout(r, (step.timeout as number) ?? 1000));
            output.writeln(output.dim(`  wait -> ${step.condition}`));
            break;
          case 'extract': {
            const ref = refs.get(step.target as string);
            if (!ref) throw new Error(`Element "${step.target}" not found`);
            const val = await evaluateJs(
              client,
              sessionId,
