...
export const listCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printError(
        `Failed to list: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      return { success: false, exitCode: 1 };
    }
  },
...
export const editCommand: Command = {
  ...
  options: [
    ...
    { name: 'value', description: 'New value/content', type: 'string' },
    {
      name: 'source',
      short: 's',
      description: 'Source to edit: sqlite, palace, knowledge',
      type: 'string',
      default: 'sqlite',
      choices: ['sqlite', 'palace', 'knowledge'],
    },
    { name: 'id', description: 'Entry ID (palace/knowledge)', type: 'string' },
  ],
  examples: [
    {
      command: 'monomind memory edit -k "pattern/auth" --value "updated content"',
      description: 'Edit SQLite-backed entry',
    },
    {
      command: 'monomind memory edit --source palace --id "abc123" --value "new content"',
      description: 'Edit Memory Palace drawer',
    },
    {
      command: 'monomind memory edit --source knowledge --id "chunk-42" --value "updated"',
      description: 'Edit knowledge chunk',
    },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (source === 'sqlite' || source === 'lancedb') {
      ...
      if (!key) {
        output.printError('Key is required for sqlite edit. Use --key or -k');
        return { success: false, exitCode: 1 };
      }
...
export const templatesCommand: Command = {
  ...
  options: [
    {
      name: 'type',
      short: 't',
      description: 'Template type: user, feedback, project, reference',
      type: 'string',
  ...
  examples: [
    ...
    {
      command: 'monomind memory templates -t feedback',
      description: 'Show feedback template only',
    },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    for (const key of keys) {
      ...
      if (!t) {
        output.printError(
          `Unknown type: "${key}". Valid types: user, feedback, project, reference`,
        );
        return { success: false, exitCode: 1 };
      }
    ...
    output.writeln(
      output.dim(
        'Save with: monomind memory store -k "<name>" --value "<content>" --namespace auto-memory',
      ),
    );
    return { success: true };
  },
