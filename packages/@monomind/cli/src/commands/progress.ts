...
interface ProgressResult {
  overall?: number;
  progress?: number;
  cli?: ProgressCategory;
  mcp?: ProgressCategory;
  hooks?: ProgressCategory;
  packages?: ProgressCategory;
  ddd?: ProgressCategory;
  codebase?: ProgressCodebase;
...
const checkCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      spinner.start();
      const result = (await callMCPTool('progress_check', { detailed })) as ProgressResult;
      spinner.stop();
      ...
      if (detailed && result.cli) {
        ...
        if (result.codebase) {
          output.writeln(
            output.dim(
              `Codebase: ${result.codebase.totalFiles} files, ${result.codebase.totalLines.toLocaleString()} lines`,
            ),
          );
        }
...
const syncCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      spinner.start();
      const result = (await callMCPTool('progress_sync', {})) as ProgressResult;
      spinner.stop();
      if (ctx.flags.format === 'json') {
        output.printJson(result);
        return { success: true, data: result };
...
const summaryCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      spinner.start();
      const result = (await callMCPTool('progress_summary', {})) as ProgressResult;
      spinner.stop();
      if (ctx.flags.format === 'json') {
        output.printJson(result);
        return { success: true, data: result };
...
const watchCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult | void> => {
    ...
    const check = async (): Promise<void> => {
      try {
        const result = (await callMCPTool('progress_check', {})) as ProgressResult;
        const currentProgress = result.overall ?? result.progress ?? 0;
        if (currentProgress !== lastProgress) {
          output.writeln(
            `${output.warning('→')} Progress changed: ${lastProgress}% → ${output.success(currentProgress + '%')}`,
          );
          lastProgress = currentProgress;
        } else {
          process.stdout.write(
            `\r${progressBar(currentProgress, 20)} ${output.dim(new Date().toLocaleTimeString())}`,
          );
        }
      } catch (error) {
        output.printError(
