...
const neuralCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const dimensionRaw = parseInt(ctx.flags.dimension as string || '384', 10);
    const dimension = Number.isFinite(dimensionRaw) ? Math.max(1, Math.min(dimensionRaw, 4096)) : 384;
    const numVectorsRaw = parseInt(ctx.flags.vectors as string || '1000', 10);
...
const regressionCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const suiteDirRaw = ctx.flags.suite as string || '.monomind/benchmarks/definitions';
