...
// PID file payload persisted to disk
...
// Internal daemon state tracked across an action invocation
...
// Background worker descriptor used by the workers subcommand
...
const workersCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const type = ctx.flags?.type as string | undefined;
