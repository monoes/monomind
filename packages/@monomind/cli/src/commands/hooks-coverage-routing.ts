...
export const coverageRouteCommand: Command = {
  ...
  examples: [
    {
      command: 'monomind hooks coverage-route -t "fix bug in auth"',
      description: 'Route with coverage awareness',
    },
    {
      command: 'monomind hooks coverage-route -t "add tests" --threshold 90',
      description: 'Route with custom threshold',
    },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (diskCoverage.found) {
      ...
      if (diskCoverage.summary.overallLineCoverage < threshold) {
        suggestions.push(
          `Overall line coverage (${diskCoverage.summary.overallLineCoverage.toFixed(1)}%) is below ${threshold}% threshold`,
        );
      }
