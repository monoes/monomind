...
interface RvfaBuildResult {
  sections?: Array<{ id: string; size: number }>;
  totalSize: number;
}

interface RvfaChecksumResult {
  section: string;
  valid: boolean;
}

interface RvfaExtractResult {
  id: string;
  size: number;
  path: string;
}

interface RvfaRunResult {
  port?: number;
  pid?: number;
}
...
async function loadModule<T = unknown>(path: string, exportName: string, label: string): Promise<T | null> {
  try {
...
const buildCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const RvfaBuilder = await loadModule<new (opts: {
      profile: string;
      outputPath: string;
      arch: string;
      models: string[];
      apiKeysPath?: string;
    }) => { build(): Promise<RvfaBuildResult> }>('../appliance/rvfa-builder.js', 'RvfaBuilder', 'builder');
...
const inspectCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const RvfaReader = await loadModule<new (file: string) => { parse(): Promise<RvfaHeader> }>(
...
const verifyCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const RvfaReader = await loadModule<new (file: string) => {
      parse(): Promise<RvfaHeader>;
      verifyChecksums(): Promise<RvfaChecksumResult[]>;
...
const extractCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const RvfaReader = await loadModule<new (file: string) => {
      parse(): Promise<RvfaHeader>;
      extractSection(id: string, dest: string): Promise<RvfaExtractResult>;
      extractAll(dest: string): Promise<RvfaExtractResult[]>;
    }>('../appliance/rvfa-format.js', 'RvfaReader', 'format');
    if (!RvfaReader) return { success: false, exitCode: 1 };
    if (!(await requireFile(file))) return { success: false, exitCode: 1 };
...
const runCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const RvfaRunner = await loadModule<new (opts: {
      file: string;
      mode: string;
      isolation: string;
    }) => { boot(): Promise<RvfaRunResult> }>('../appliance/rvfa-runner.js', 'RvfaRunner', 'runner');
