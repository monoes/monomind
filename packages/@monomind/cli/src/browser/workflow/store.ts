...
import type { RunRecord, WorkflowDef } from './types.js';
...
export async function readWorkflow(filePath: string): Promise<WorkflowDef> {
  ...
  try {
    ...
    throw new WorkflowStoreError(
      `Invalid workflow JSON in ${filePath}: ${(err as Error).message}`,
      err,
    );
  }
}
...
export async function listRuns(workflowId?: string): Promise<RunRecord[]> {
  ...
  const query = workflowId
    ? db.prepare(
        `SELECT * FROM browse_runs WHERE workflow_id = ? ORDER BY started_at DESC LIMIT 50`,
      )
...
> {
  ...
  const query = db.prepare(
    `SELECT id, platform, username, last_used_at FROM browse_sessions ORDER BY last_used_at DESC`,
  );
...
export async function getSessionCookies(
  ...
  const stmt = db.prepare(
    `SELECT cookies FROM browse_sessions WHERE platform = ? AND username = ? LIMIT 1`,
  );
