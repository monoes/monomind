...
function ensureFmKey(fm: string, key: string, value: string): string {
  ...
  // otherwise at the top. When description is a YAML block scalar
  // (`description: |` / `>`), its value continues on the following
  // indented lines — skip past all of them so the new key lands after the
  // block instead of inside it (inserting mid-block corrupts both the
  // description and the new key's parsed value).
