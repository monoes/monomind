...
import { hasUnmarkedGeneratedBody, mergeGeneratedBlock } from './shared.js';
import type { InitResult } from './types.js';

/**
 * The banner monomind stamps on its own generated instructions. Used to tell
 * this file's auto-generated body apart from a user's own text when deciding
 * whether appending a managed block would duplicate it (GH #278).
 */
export const SHARED_INSTRUCTIONS_SIGNATURE = '> Auto-generated by `monomind init`.';
...
${SHARED_INSTRUCTIONS_SIGNATURE} Edit freely — this file is prepended to every agent prompt.
...
export function writeSharedInstructions(cwd: string, force: boolean, result: InitResult): void {
  ...
  try {
    ...
    // This file is monomind's own output, so a pre-marker release left its
    // body here unwrapped. Appending the block next to it produced two copies
    // of the same auto-generated instructions — see GH #278. Memory seeding
    // below still runs; only the redundant write is skipped.
    if (
      ...
      result.skipped.push(
        '.agents/shared_instructions.md (already holds a monomind-generated body — not duplicated)',
      );
    } else {
