...
/**
 * The header monomind stamps on its own generated CLAUDE.md body. Used to
 * recognise that body when it sits unwrapped in a file, so a managed block is
 * not appended beside it (GH #278).
 */
export const CLAUDE_MD_SIGNATURE = '# Claude Code Configuration - Monomind';
...
export function generateClaudeMd(options: InitOptions, template?: ClaudeMdTemplate): string {
  ...
  const header = `${CLAUDE_MD_SIGNATURE}\n`;
