...
if (req.method === 'GET' && /^\/api\/orgs\/[a-z0-9][a-z0-9_-]{0,63}$/i.test(url)) {
  try {
    ...
    if (!fs.existsSync(f)) { res.writeHead(404); res.end('{"error":"not found"}'); return true; }
    res.writeHead(200, { 'Content-Type': 'application/json', ...(corsOrigin ? { 'Access-Control-Allow-Origin': corsOrigin } : {}) });
...
if (req.method === 'GET' && /^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}$/i.test(url)) {
  try {
    ...
    if (!fs.existsSync(configFile)) { res.writeHead(404); res.end('{"error":"org not found"}'); return true; }
    ...
    const _loopRunning = (() => {
      try {
        ...
        const orgRunsDir = path.join(ctx._getGitMonomindDir(d) || path.join(d, '.monomind'), 'orgs', orgName, 'runs');
    ...
    const running = !fs.existsSync(stopFile) && (_runstateAlive || ctx.activeOrgRuns.has(orgName) || _loopRunning);
...
if (req.method === 'GET' && /^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/activity$/i.test(url)) {
  try {
    ...
    if (orgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(orgName)) { res.writeHead(400); res.end('[]'); return true; }
...
if (req.method === 'GET' && /^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/projects$/i.test(url)) {
  try {
    ...
    if (!fs.existsSync(projFile)) { res.writeHead(200, { 'Content-Type': 'application/json', ...(corsOrigin ? { 'Access-Control-Allow-Origin': corsOrigin } : {}) }); res.end('[]'); return true; }
...
if (req.method === 'GET' && /^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/agent\/[a-z0-9][a-z0-9_-]{0,63}$/i.test(url)) {
  try {
    ...
    if (orgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(orgName)) { res.writeHead(400); res.end('{}'); return true; }
    if (roleId.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(roleId)) { res.writeHead(400); res.end('{}'); return true; }
    ...
    const orgFile = path.join(d, '.monomind', 'orgs', `${orgName}.json`);
    if (!fs.existsSync(orgFile)) { res.writeHead(404); res.end('{}'); return true; }
    const config = JSON.parse(fs.readFileSync(orgFile, 'utf8'));
    const role = (config.roles || []).find(r => r.id === roleId);
    if (!role) { res.writeHead(404); res.end('{}'); return true; }
...
if (req.method === 'POST' && /^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/agent\/[a-z0-9][a-z0-9_-]{0,63}\/avatar$/i.test(url)) {
  ...
  try {
    ...
    const orgName = decodeURIComponent(parts[3]);
    const roleId = decodeURIComponent(parts[5]);
    if (orgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(orgName)) { res.writeHead(400); res.end('{"ok":false,"error":"Invalid org name"}'); return true; }
    if (roleId.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(roleId)) { res.writeHead(400); res.end('{"ok":false,"error":"Invalid role id"}'); return true; }
    ...
    if (avatarDataUrl != null && (typeof avatarDataUrl !== 'string' || avatarDataUrl.length > 2_000_000 || !/^data:image\/(png|jpe?g|webp|gif);base64,[A-Za-z0-9+/=]+$/.test(avatarDataUrl))) {
      res.writeHead(400); res.end('{"ok":false,"error":"Invalid or oversized avatarDataUrl"}'); return true;
    }
    ...
    const orgFile = path.join(d, '.monomind', 'orgs', `${orgName}.json`);
    if (!fs.existsSync(orgFile)) { res.writeHead(404); res.end('{"ok":false,"error":"org not found"}'); return true; }
    const config = JSON.parse(fs.readFileSync(orgFile, 'utf8'));
    const role = (config.roles || []).find(r => r.id === roleId);
    if (!role) { res.writeHead(404); res.end('{"ok":false,"error":"role not found"}'); return true; }
...
if (req.method === 'GET' && /^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/search(\?.*)?$/i.test(url)) {
  try {
    ...
    if (!q || q.length < 2) { res.writeHead(200, { 'Content-Type': 'application/json', ...(corsOrigin ? { 'Access-Control-Allow-Origin': corsOrigin } : {}) }); res.end('{"hits":[]}'); return true; }
...
if (req.method === 'GET' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/health$/i)) {
  try {
    ...
    const base = path.join(path.resolve(_healthQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs');
...
if (req.method === 'GET' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/environments$/i)) {
  try {
    ...
    const envsPath = path.join(path.resolve(_envsQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs', `${orgName}-environments.json`);
...
if (req.method === 'GET' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/workspaces$/i)) {
  try {
    ...
    const base = path.join(path.resolve(_wsQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs');
...
if (req.method === 'GET' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/invites(\?.*)?$/i)) {
  try {
    ...
    const base = path.join(path.resolve(_invitesQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs');
...
if (req.method === 'GET' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/plugins$/i)) {
  try {
    ...
    const base = path.join(path.resolve(_pluginsQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind');
    let plugins = [];
    try {
...
if (req.method === 'GET' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/my-issues$/i)) {
  try {
    ...
    const base = path.join(path.resolve(_myIssuesQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs');
    let payload = { issues: [] };
    try {
...
if (req.method === 'GET' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/approvals(\?.*)?$/i)) {
  try {
    ...
    const base = path.join(path.resolve(_approvalsQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs');
    const readJsonSafe = (f) => { try { return JSON.parse(fs.readFileSync(f, 'utf8')); } catch(_) { return null; } };
...
if (req.method === 'GET' && url === '/api/questions') {
  try {
    const _qDir = new URL(req.url, 'http://localhost').searchParams.get('dir') || ctx.projectDir || process.cwd();
...
if (req.method === 'POST' && url === '/api/questions/answer') {
  ...
  try {
    ...
    if (!hostUrl) {
      ...
      ctx.appendToFile(path.join(projDir, 'data', 'mastermind-events.jsonl'), JSON.stringify(queuedEvent) + '\n').catch(() => {});
    ...
    if (fwd.ok && fwdData.ok) {
      ...
      ctx.appendToFile(path.join(path.resolve(dir || ctx.projectDir || process.cwd()), 'data', 'mastermind-events.jsonl'), JSON.stringify(event) + '\n').catch(() => {});
...
if (req.method === 'POST' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/approvals\/[^/]+$/i)) {
  ...
  try {
    ...
    if (orgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(orgName)) { res.writeHead(400); res.end('Invalid org name'); return true; }
    if (!approvalId) { res.writeHead(400); res.end('{"error":"approval id required"}'); return true; }
    const parsed = JSON.parse(body);
    const action = parsed.action;
    if (!['approve', 'reject', 'revision_requested'].includes(action)) {
      res.writeHead(400); res.end('{"error":"action must be approve, reject, or revision_requested"}'); return true;
    }
    const _postApprovalsQs = new URL(req.url, 'http://localhost').searchParams;
    const base = path.join(path.resolve(_postApprovalsQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs');
    ...
    if (idx === -1) { res.writeHead(404); res.end('{"error":"approval not found"}'); return true; }
    ...
    ctx.appendToFile(path.join(path.resolve(_postApprovalsQs.get('dir') || ctx.projectDir || process.cwd()), 'data', 'mastermind-events.jsonl'), JSON.stringify(event) + '\n').catch(() => {});
    ctx.broadcastMm(event);
    res.writeHead(200, { 'Content-Type': 'application/json', ...(corsOrigin ? { 'Access-Control-Allow-Origin': corsOrigin } : {}) });
...
if (req.method === 'GET' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/secrets$/i)) {
  try {
    ...
    const base = path.join(path.resolve(_secretsQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs');
...
if (req.method === 'GET' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/budgets$/i)) {
  try {
    ...
    const base = path.join(path.resolve(_budgetsQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs');
...
if (req.method === 'GET' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/join-requests(\?.*)?$/i)) {
  try {
    ...
    const joinFile = path.join(path.resolve(_joinQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs', `${orgName}-join-requests.json`);
    let requests = [];
    try {
...
if (req.method === 'POST' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/goals$/i)) {
  ...
  try {
    ...
    if (orgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(orgName)) { res.writeHead(400); res.end('Invalid org name'); return true; }
    const parsed = JSON.parse(body);
    if (!parsed || !Array.isArray(parsed.goals)) { res.writeHead(400); res.end('{"error":"goals array required"}'); return true; }
    const _postGoalsQs = new URL(req.url, 'http://localhost').searchParams;
    const goalsFile = path.join(path.resolve(_postGoalsQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs', `${orgName}-goals.json`);
...
if (req.method === 'POST' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/routines$/i)) {
  ...
  try {
    ...
    if (orgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(orgName)) { res.writeHead(400); res.end('Invalid org name'); return true; }
    const parsed = JSON.parse(body);
    if (!parsed || !Array.isArray(parsed.routines)) { res.writeHead(400); res.end('{"error":"routines array required"}'); return true; }
    const _postRoutinesQs = new URL(req.url, 'http://localhost').searchParams;
    const routinesFile = path.join(path.resolve(_postRoutinesQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs', `${orgName}-routines.json`);
...
if (req.method === 'GET' && url.match(/^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/files$/i)) {
  try {
    const orgName = decodeURIComponent(url.split('/')[3]);
    if (orgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(orgName)) { res.writeHead(400); res.end('{"error":"Invalid org name"}'); return true; }
...
if (req.method === 'GET' && url === '/api/file-content') {
  try {
    ...
    if (!rawPath) { res.writeHead(400); res.end('Missing path'); return true; }
    ...
    if (!fs.existsSync(resolved)) { res.writeHead(404); res.end('Not found'); return true; }
    const stat = fs.statSync(resolved);
    if (!stat.isFile()) { res.writeHead(400); res.end('Not a file'); return true; }
    if (stat.size > 524288) { res.writeHead(413); res.end('File too large'); return true; }
...
if (req.method === 'DELETE' && url.match(/^\/api\/orgs\/[a-z0-9][a-z0-9_-]{0,63}(\?.*)?$/i)) {
  try {
    ...
    const orgsDir = path.join(path.resolve(_delOrgQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs');
    const configFile = path.join(orgsDir, `${orgName}.json`);
    const v1ConfigFile = path.join(orgsDir, `${orgName}.v1.json`);
    if (!fs.existsSync(configFile) && !fs.existsSync(v1ConfigFile)) { res.writeHead(404); res.end('{"error":"org not found"}'); return true; }
    ...
    try { const lpf = path.join(path.resolve(ctx.projectDir || process.cwd()), '.monomind', 'loops', `${orgName}.md`); if (fs.existsSync(lpf)) fs.unlinkSync(lpf); } catch(_) {}
    // Emit org:delete event
    const deleteEvent = { type: 'org:delete', org: orgName, ts: Date.now() };
    ctx.appendToFile(path.join(ctx.projectDir || process.cwd(), 'data', 'mastermind-events.jsonl'), JSON.stringify(deleteEvent) + '\n').catch(() => {});
...
if (req.method === 'POST' && url.match(/^\/api\/orgs\/[a-z0-9][a-z0-9_-]{0,63}\/stop$/i)) {
  try {
    ...
    ctx.appendToFile(path.join(dataDir, 'mastermind-events.jsonl'), JSON.stringify(stopEvent) + '\n').catch(() => {});
...
if (req.method === 'POST' && url.match(/^\/api\/orgs\/[a-z0-9][a-z0-9_-]{0,63}\/copy$/i)) {
  ...
  try {
    const orgName = decodeURIComponent(url.split('/')[3]);
    if (orgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(orgName)) { res.writeHead(400); res.end(JSON.stringify({ error: 'Invalid org name' })); return true; }
    let payload = {};
    try { payload = JSON.parse(body); } catch(_) {}
    const destination = payload.destination ? String(payload.destination).trim() : '';
    if (!destination) { res.writeHead(400); res.end(JSON.stringify({ error: 'destination is required' })); return true; }
    if (!path.isAbsolute(destination)) { res.writeHead(400); res.end(JSON.stringify({ error: 'destination must be an absolute path' })); return true; }
    const _copyOrgQs = new URL(req.url, 'http://localhost').searchParams;
    const srcOrgsDir = path.join(path.resolve(_copyOrgQs.get('dir') || ctx.projectDir || process.cwd()), '.monomind', 'orgs');
    const srcFile = path.join(srcOrgsDir, `${orgName}.json`);
    if (!fs.existsSync(srcFile)) { res.writeHead(404); res.end(JSON.stringify({ error: 'org not found' })); return true; }
...
if (req.method === 'GET' && /^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/runs(\?.*)?$/i.test(url)) {
  try {
    ...
    if (_rOrgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(_rOrgName)) { res.writeHead(400); res.end('{"error":"Invalid org name"}'); return true; }
...
if (req.method === 'GET' && /^\/api\/org\/[a-z0-9][a-z0-9_-]{0,63}\/runs\/[a-z0-9][a-z0-9_-]{0,79}(\?.*)?$/i.test(url)) {
  try {
    ...
    if (_rvOrgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(_rvOrgName) ||
        _rvRunId.length > 80  || !/^[a-z0-9][a-z0-9_-]*$/i.test(_rvRunId)) { res.writeHead(400); res.end('{"error":"Invalid org or run id"}'); return true; }
    ...
    if (_rvRunId === `threads-${_rvOrgName}`) {
      ...
      res.writeHead(404); res.end('{"error":"threads file not found"}'); return true;
    }
    ...
    if (!_rvFile) { res.writeHead(404); res.end('{"error":"run not found"}'); return true; }
...
if (req.method === 'GET' && /^\/api\/org\/[^/]+\/artifact/.test(url)) {
  try {
    ...
    if (!_rawPath) { res.writeHead(400); res.end(JSON.stringify({ error: 'path required' })); return true; }
    ...
    if (!_safe) { res.writeHead(403); res.end(JSON.stringify({ error: 'path not allowed' })); return true; }
    if (!fs.existsSync(_filePath)) { res.writeHead(404); res.end(JSON.stringify({ error: 'file not found' })); return true; }
    ...
    if (_size > 2 * 1024 * 1024) { res.writeHead(413); res.end(JSON.stringify({ error: 'file too large', size: _size })); return true; }
...
if (req.method === 'GET' && url === '/api/mastermind-stream') {
  ...
  const ka = setInterval(() => { try { res.write(': ping\n\n'); } catch (_) { clearInterval(ka); ctx.removeMmClient(res); } }, 20000);
  req.on('close', () => { ctx.removeMmClient(res); clearInterval(ka); });
  return true;
}
...
if (req.method === 'GET' && url.match(/^\/api\/mastermind\/session\/[^/]+\/trace$/)) {
  try {
    ...
    const sessFile = path.join(ctx.projectDir || process.cwd(), 'data', 'sessions', `${sid}.json`);
    let s = null;
    ...
    if (!s) { res.writeHead(404); res.end('Session not found'); return true; }
...
if (req.method === 'GET' && url === '/api/status') {
  try {
    ...
    const orgRuns = {};
    ctx.activeOrgRuns.forEach((runId, org) => { orgRuns[org] = runId; });
...
if (req.method === 'GET' && /^\/api\/orgs\/[^/]+\/runs\/current$/.test(url)) {
  try {
    ...
    if (!orgName || orgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(orgName)) {
      res.writeHead(400); res.end('{"error":"invalid org name"}'); return true;
    }
    ...
    if (!runFile) { res.writeHead(404); res.end('{"events":[],"runId":null}'); return true; }
    ...
    res.writeHead(200, { 'Content-Type': 'application/json', ...(corsOrigin ? { 'Access-Control-Allow-Origin': corsOrigin } : {}) });
    res.end(JSON.stringify({ runId: detectedRunId, events, active: ctx.activeOrgRuns.has(orgName) }));
  } catch(err) {
...
if (req.method === 'POST' && /^\/api\/orgs\/[a-z0-9][a-z0-9_-]{0,63}\/mark-complete$/i.test(url)) {
  ...
  if (_mcOrgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(_mcOrgName)) {
    res.writeHead(400, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: 'Invalid org name' })); return true;
  }
  ...
  if (!_mcRunId) {
    res.writeHead(404, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: 'No active run for org: ' + _mcOrgName })); return true;
  }
  ...
  try {
    ...
    if (fs.existsSync(_mcRunFile)) await ctx.appendToFile(_mcRunFile, JSON.stringify(_mcEvent) + '\n');
...
if (req.method === 'GET' && /^\/api\/orgs\/[a-z0-9][a-z0-9_-]{0,63}\/runs\/current\/stream$/i.test(url)) {
  ...
  if (_stOrgName.length > 64 || !/^[a-z0-9][a-z0-9_-]*$/i.test(_stOrgName)) {
    res.writeHead(400); res.end('Invalid org name'); return true;
  }
  ...
  try {
    ...
    if (!_stReplayedViaSqlite) {
      ...
      const _stRunId = ctx.activeOrgRuns.get(_stOrgName) || ctx._getActiveRunId(_stOrgName, _stRoot);
      if (_stRunId) {
  ...
  req.on('close', () => {
    ...
    if (_stClients) { _stClients.delete(res); if (_stClients.size === 0) ctx.runStreamClients.delete(_stOrgName); }
  });
  return true;
}
...
if (req.method === 'POST' && /^\/api\/orgs\/[a-z0-9][a-z0-9_-]{0,63}\/chat$/i.test(url)) {
  ...
  try {
    ...
    if (!_chText) { res.writeHead(400, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ error: 'text is required' })); return true; }
    ...
    const _chRunId = ctx.activeOrgRuns.get(_chOrgName) || ctx._getActiveRunId(_chOrgName, _chRoot);
    ...
    if (_chRunId) {
      ...
      if (fs.existsSync(_chRunFile)) await ctx.appendToFile(_chRunFile, JSON.stringify(_chEvent) + '\n');
    }
