...
export function checkCorpusHealth(stats: CorpusStats): CorpusHealthResult {
  const warnings: string[] = [];

  if (stats.wordCount < CORPUS_WARN_MIN_WORDS) {
    warnings.push(
      `Corpus too small: ${stats.wordCount.toLocaleString()} words (min ${CORPUS_WARN_MIN_WORDS.toLocaleString()}). ` +
  ...
  if (stats.wordCount > CORPUS_WARN_MAX_WORDS) {
    warnings.push(
      `Corpus too large: ${stats.wordCount.toLocaleString()} words (max ${CORPUS_WARN_MAX_WORDS.toLocaleString()}). ` +
  ...
  if (stats.fileCount > CORPUS_WARN_MAX_FILES) {
    warnings.push(
      `Too many files: ${stats.fileCount} (max ${CORPUS_WARN_MAX_FILES}). ` +
