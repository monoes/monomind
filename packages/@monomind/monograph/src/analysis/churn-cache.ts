...
export interface CachedChurnResult {
  files: Map<
    string,
    { commits: number; linesAdded: number; linesDeleted: number; weightedCommits: number }
  >;
  shallowClone: boolean;
  authorPool: string[];
  cacheHit: boolean;
...
function saveChurnCache(cacheDir: string, sha: string, gitAfter: string, cache: ChurnCache): void {
  ...
  writeFileSync(
    tmp,
    JSON.stringify({ ...cache, version: CHURN_CACHE_VERSION, lastIndexedSha: sha, gitAfter }),
  );
...
function mergeChurnCaches(base: ChurnCache, delta: ChurnCache): ChurnCache {
  ...
  for (const df of delta.files) {
    const remapped = df.events.map((e) => ({
      ...e,
      authorIdx: e.authorIdx !== null ? (authorMap.get(e.authorIdx) ?? null) : null,
    }));
...
export function analyzeChurnCached(
  root: string,
  gitAfter: string,
  cacheDir: string,
  noCache = false,
): CachedChurnResult | null {
