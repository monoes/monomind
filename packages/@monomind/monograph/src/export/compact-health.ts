...
export function buildHealthCompactLines(report: CompactHealthReport): string[] {
  ...
  for (const f of report.findings ?? []) {
    const crapSuffix =
      f.crap !== undefined
        ...
        : '';
    lines.push(
      `high-complexity:${rel(f.path, root)}:${f.line}:${f.name}:cyclomatic=${f.cyclomatic},cognitive=${f.cognitive},severity=${f.severity}${crapSuffix}`,
    );
  }

  for (const h of report.hotspots ?? []) {
    lines.push(
      `hotspot:${rel(h.path, root)}:score=${h.score.toFixed(1)},commits=${h.commits},churn=${h.linesAdded + h.linesDeleted},density=${h.complexityDensity.toFixed(2)},fan_in=${h.fanIn},trend=${h.trend}`,
    );
  }
  ...
  if (report.healthTrend) {
    ...
    for (const m of report.healthTrend.metrics) {
      ...
      lines.push(
        `trend:${m.name}:previous=${m.previous.toFixed(1)},current=${m.current.toFixed(1)},delta=${sign}${m.delta.toFixed(1)},direction=${m.direction}`,
      );
    }
  }
  return lines;
...
export function buildDuplicationCompactLines(report: CompactDuplicationReport): string[] {
  ...
  for (let i = 0; i < report.cloneGroups.length; i++) {
    ...
    for (const inst of group.instances) {
      ...
      lines.push(
        `clone-group-${i + 1}:${rel(inst.file, root)}:${inst.startLine}-${inst.endLine}${tokens ? `:${tokens}` : ''}`,
      );
    }
  }
  return lines;
