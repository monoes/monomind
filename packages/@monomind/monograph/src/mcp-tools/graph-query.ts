...
export function queryGraph(db: Database.Database, input: GraphQueryInput): GraphQueryResult {
  ...
  const seedRows = db
    .prepare(`SELECT * FROM nodes WHERE name LIKE ? OR label LIKE ? LIMIT 20`)
  ...
  if (!truncated) {
    ...
    const frontier: Array<{ id: string; depth: number }> = seeds.map((s) => ({
      id: s.id,
      depth: 0,
    }));
