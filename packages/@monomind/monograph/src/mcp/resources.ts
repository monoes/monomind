...
export const monographResources: MCPResource[] = [
  ...
  },
  {
    ...
    mimeType: 'application/json',
    handler: async (_uri) => ({
      content: JSON.stringify({ note: 'Process detection available in Sub-project 3' }),
    }),
  },
  {
    ...
    handler: async (_uri) => {
      ...
      try {
        const labels = db
          .prepare(
            'SELECT DISTINCT label, COUNT(*) as count FROM nodes GROUP BY label ORDER BY count DESC',
          )
          .all();
        const relations = db
          .prepare(
            'SELECT DISTINCT relation, COUNT(*) as count FROM edges GROUP BY relation ORDER BY count DESC',
          )
          .all();
        return {
          content: JSON.stringify({ nodeLabels: labels, edgeRelations: relations }, null, 2),
        };
      } finally {
        closeDb(db);
      }
...
export const monographTools: MCPToolDef[] = [
  {
    ...
    description:
      'List all registered typed property definitions in the node property registry, including their types, cardinalities, view contexts, and queryability.',
  ...
  },
  {
    ...
    description:
      'Query nodes by a typed metadata property (layer, ua_type, complexity, tags, etc). Returns nodes whose JSON properties contain the given value for the named property.',
    inputSchema: {
      type: 'object',
      properties: {
        ident: {
          type: 'string',
          description: 'Property identifier (e.g. layer, ua_type, complexity, tags, language)',
        },
