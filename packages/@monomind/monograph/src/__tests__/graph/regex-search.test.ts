...
function insertNode(
  ...
  id: string,
  name = id,
  opts: { filePath?: string; language?: string; label?: string } = {},
) {
  db.prepare(
...
function insertEdge(
  ...
  db.prepare(
    `INSERT INTO edges (id, source_id, target_id, relation, confidence, confidence_score, reason)
     VALUES (?, ?, ?, ?, 'EXTRACTED', 1.0, ?)`,
  ).run(id, src, tgt, relation, reason);
}
