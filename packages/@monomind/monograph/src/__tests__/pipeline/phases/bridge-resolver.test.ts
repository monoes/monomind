...
function edgeRows(dbPath: string): {
  source_id: string;
  target_id: string;
  relation: string;
  confidence: string;
  ...
  try {
    return db
      .prepare(`SELECT source_id, target_id, relation, confidence, reason FROM edges`)
      .all() as never[];
  } finally {
    db.close();
  }
...
`,
    ...
    const bridgeEdge = edges.find(
      (e) => e.relation === 'CALLS' && e.confidence === 'INFERRED' && isBridgeEdge(e),
    );
...
`,
    ...
    const jsFn = db
      .prepare(`SELECT id FROM nodes WHERE name = 'SendMessage' AND label = 'Function'`)
