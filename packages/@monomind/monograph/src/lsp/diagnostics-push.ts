...
export function pushImportDiagnostics(
  map: DiagnosticMap,
  ...
  for (const r of results)
    singleLineDiag(
      ...
      r.filePath,
      r.line,
      `Cannot resolve import '${r.specifier}'`,
      'unresolved-import',
      'error',
    );
}
...
export function pushDepDiagnostics(map: DiagnosticMap, results: UnusedDepFinding[]): void {
  const msg = (r: UnusedDepFinding) =>
    r.kind === 'unused'
      ? `Package '${r.name}' is listed but not imported`
      : `Package '${r.name}' is imported but not in dependencies`;
  for (const r of results)
    singleLineDiag(
      map,
      'package.json',
      0,
      msg(r),
      r.kind === 'unused' ? 'unused-dep' : 'unlisted-dep',
    );
}
...
export function pushMemberDiagnostics(map: DiagnosticMap, results: UnusedMemberFinding[]): void {
  for (const r of results)
    singleLineDiag(
      ...
      r.filePath,
      r.line,
      `Unused member '${r.className}.${r.member}'`,
      'unused-member',
    );
}

export function pushCircularDepDiagnostics(
  map: DiagnosticMap,
  results: CircularDepFinding[],
): void {
  for (const r of results) {
    ...
    if (first)
      singleLineDiag(
        ...
        first,
        0,
        `Circular dependency involving ${r.files.length} files`,
        'circular-dep',
        'warning',
      );
  }
...
export function pushBoundaryViolationDiagnostics(
  map: DiagnosticMap,
  results: BoundaryViolFinding[],
): void {
  for (const r of results)
    singleLineDiag(
      map,
      r.fromFile,
      r.line,
      `Boundary violation: imports from restricted zone (rule: ${r.rule})`,
      'boundary-violation',
      'error',
    );
}

export function pushDuplicateExportDiagnostics(
  map: DiagnosticMap,
  results: DupeExportFinding[],
): void {
  for (const r of results)
    singleLineDiag(
      map,
      r.filePath,
      r.line,
      `Duplicate export '${r.symbol}' — exported from multiple files`,
      'duplicate-export',
    );
}

export function pushDuplicationDiagnostics(
  map: DiagnosticMap,
  results: DuplicationFinding[],
): void {
  for (const r of results)
    singleLineDiag(
      map,
      r.filePath,
      r.startLine,
      `Code duplication (group ${r.groupId}, lines ${r.startLine}-${r.endLine})`,
      'duplication',
      'information',
    );
}

export function pushStaleSuppressionDiagnostics(
  map: DiagnosticMap,
  results: StaleSuppressionFinding[],
): void {
  for (const r of results)
    singleLineDiag(
      map,
      r.filePath,
      r.line,
      `Stale suppression comment for '${r.code}' — no matching finding`,
      'stale-suppression',
      'hint',
    );
}
