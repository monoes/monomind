...
type LineKind = 'function' | 'class' | 'block' | 'comment' | 'import' | 'other';
...
function classifyLine(line: string): LineKind {
  ...
  if (IMPORT_RE.test(trimmed)) return 'import';
  if (FUNCTION_RE.test(trimmed)) return 'function';
...
function groupLines(sourceLines: string[]): LineGroup[] {
...
function splitGroupIntoChunks(
  ...
  while (start < lines.length) {
    ...
    for (let j = end - 1; j >= start; j--) {
      if (overlapTokens + lineTokens[j] > overlap) break;
...
export function chunkSource(source: string, options?: ChunkOptions): Chunk[] {
