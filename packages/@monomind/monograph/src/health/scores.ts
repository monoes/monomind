...
export function exceededThresholdFromBools(
  cyclomatic: boolean,
  cognitive: boolean,
  crap: boolean,
): ExceededThreshold {
...
export function computeFindingSeverity(opts: FindingSeverityOpts): FindingSeverity {
  ...
  const cogSev: FindingSeverity =
    cognitive >= cognitiveCritical ? 'critical' : cognitive >= cognitiveHigh ? 'high' : 'moderate';
  const cycSev: FindingSeverity =
    cyclomatic >= cyclomaticCritical
      ? 'critical'
      : cyclomatic >= cyclomaticHigh
        ? 'high'
        : 'moderate';
  const crapSev: FindingSeverity =
    crap === undefined
      ? 'moderate'
      : crap >= DEFAULT_CRAP_CRITICAL
        ? 'critical'
        : crap >= DEFAULT_CRAP_HIGH
          ? 'high'
          : 'moderate';
