...
export const RECOMMENDATION_CATEGORIES: Record<ExtendedRecommendationCategory, CategoryMeta> = {
  UrgentChurnComplexity: {
    label: 'Urgent: High churn + complexity',
    compactLabel: 'urgent',
    priority: 1,
  },
  BreakCircularDependency: {
    label: 'Break circular dependency',
    compactLabel: 'circular',
    priority: 2,
  },
  ...
  ExtractComplexFunctions: {
    label: 'Extract complex functions',
    compactLabel: 'complex',
    priority: 5,
  },
  ExtractDependencies: {
    label: 'Extract/decouple dependencies',
    compactLabel: 'decouple',
    priority: 6,
  },
  ...
  ReduceFanOut: {
    label: 'Reduce fan-out (too many imports)',
    compactLabel: 'fan-out',
    priority: 9,
  },
...
export function computeTargetThresholds(sample: MetricSample): TargetThresholds {
  ...
  return {
    ...
    fanOutP90: Math.max(FLOOR.fanOutP90, percentile(fo, 0.9)),
    complexityP90: Math.max(FLOOR.complexityP90, percentile(cc, 0.9)),
    locP90: Math.max(FLOOR.locP90, percentile(lc, 0.9)),
