...
export function computeHealthScore(vs: VitalSignsInput, _totalFiles: number): HealthScore {
  const p: HealthScorePenalties = {
    deadCode: Math.min(15, vs.deadCodePct * 0.15),
    duplication: Math.min(15, vs.duplicationPct * 0.15),
    complexityHigh: Math.min(10, vs.complexityHighPct * 0.1),
    complexityCritical: Math.min(10, vs.complexityCriticalPct * 0.2),
    crapHigh: Math.min(10, vs.crapHighPct * 0.1),
    crapCritical: Math.min(10, vs.crapCriticalPct * 0.2),
    hotspotDensity: Math.min(10, vs.hotspotDensity * 0.1),
    couplingConcentration: Math.min(5, vs.couplingHighPct * 0.05),
    busFactor: Math.min(10, vs.busFactorRisk * 10),
    largeFunctions: Math.min(5, vs.largeFunctionsPct * 0.1),
