...
const KNOWN_REGRESSION_FIELDS = new Set(['baseline', 'tolerance', 'failOnRegression']);
...
function validateBoundaries(boundaries: unknown, errors: ValidationError[]): void {
  if (!boundaries || typeof boundaries !== 'object' || Array.isArray(boundaries)) {
    errors.push({
      field: 'boundaries',
      message: 'must be an object mapping zone names to arrays of allowed zones',
    });
    return;
  }
  for (const [zone, allowed] of Object.entries(boundaries as Record<string, unknown>)) {
    if (!Array.isArray(allowed) || !allowed.every((v) => typeof v === 'string')) {
      errors.push({
        field: `boundaries.${zone}`,
        message: 'must be an array of zone name strings',
      });
    }
  }
}
...
export function validateConfig(configPath: string): ValidationResult {
  ...
  if (!existsSync(configPath)) {
    return {
      valid: false,
      errors: [{ field: '', message: `Config file not found: ${configPath}` }],
      warnings,
    };
  }
  ...
  if ('health' in parsed && parsed.health && typeof parsed.health === 'object') {
    ...
    if (
      'minScore' in health &&
      (typeof health.minScore !== 'number' || health.minScore < 0 || health.minScore > 100)
    ) {
