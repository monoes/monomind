...
function truncHint(remaining: number, total: number): string {
  if (remaining > SCOPING_THRESHOLD || total > SCOPING_THRESHOLD) {
    return dim(
      `... and ${remaining} more — try --workspace <name> or --changed-since main to scope`,
    );
  }
...
function sectionHeader(
  ...
  const bullet =
    level === 'error' ? col(RED, '●') : level === 'warn' ? col(YELLOW, '●') : col(CYAN, '●');
...
export function formatUnusedFiles(
  files: FallowUnusedFile[],
...
export function formatUnusedExports(
  ...
  if (hiddenFiles > 0) {
    ...
    lines.push(
      `  ${dim(`... and ${hiddenItems} more in ${hiddenFiles} file${hiddenFiles === 1 ? '' : 's'}`)}`,
    );
  }
  lines.push('');
  return lines;
...
export function formatUnusedDeps(deps: FallowUnusedDependency[]): string[] {
  ...
  for (const d of deps) {
    const workspaces =
      d.usedInWorkspaces.length > 0 ? dim(` (imported in ${d.usedInWorkspaces.join(', ')})`) : '';
...
export function formatUnusedMembers(
  members: FallowUnusedMember[],
...
export function buildCheckHumanLines(
  results: FallowAnalysisResults,
  opts: HumanCheckOptions = {},
): string[] {
  const lines: string[] = [];
  ...
  if (hasStructure) {
    ...
    if (results.boundaryViolations.length > 0) {
      ...
      for (const v of results.boundaryViolations.slice(0, MAX_FLAT)) {
        lines.push(
          `  ${v.fromPath}:${v.line} ${dim('→')} ${v.toPath} ${dim(`(${v.fromZone} → ${v.toZone})`)}`,
        );
      }
      lines.push('');
    }
