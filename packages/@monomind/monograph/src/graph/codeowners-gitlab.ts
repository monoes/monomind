...
export function singleFileOwnership(
  entries: CodeownersEntry[],
  filePath: string,
): { section: string; owners: string[]; ownerCount: number } {
...
export function sectionAndOwnersOf(
  entries: CodeownersEntry[],
  filePath: string,
): { section: string; owners: string[] } {
  const { owners, section } = matchOwners(entries, filePath);
