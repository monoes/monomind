...
export function computeRiskProfile(db: MonographDb): RiskProfileReport {
  ...
  const p50loc = percentile(sortedLocs, 0.5);
  const p90loc = percentile(sortedLocs, 0.9);
...
export function formatRiskProfile(report: RiskProfileReport): string {
  const { functionSizeDistribution, paramCountDistribution, riskSummary, p50loc, p90loc, p95loc } =
    report;
  ...
  for (const bin of functionSizeDistribution) {
    ...
    lines.push(
      `  ${bin.label.padEnd(15)}  ${String(bin.count).padStart(5)}  (${String(bin.percentage).padStart(5)}%)  ${bar}`,
    );
  }
  ...
  if (riskSummary.highRisk > 0) {
    lines.push('');
    lines.push(
      `high_risk_note: ${riskSummary.highRisk} function(s) have both loc>100 AND params>5 — prime refactor candidates`,
    );
  }
