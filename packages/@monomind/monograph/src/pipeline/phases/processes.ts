...
export interface ProcessesOutput {
  processResult: {
    ...
    memberships: Map<string, string>; // nodeId → processId
    stats: { totalProcesses: number; totalSteps: number };
  };
}
...
function bfsCallChain(startNodeId: string, db: Database.Database, maxDepth = 8): Set<string> {
  ...
  while (queue.length > 0) {
    ...
    const callees = calleesStmt.all(item.id) as Array<{ target_id: string }>;
...
function scoreEntryPoints(db: Database.Database): ScoredNode[] {
  const symbolRows = db
    .prepare(
      `SELECT id, name, file_path, is_exported, language
       FROM nodes
  ...
  for (const row of symbolRows) {
    ...
    if (ENTRY_FILENAMES.has(filename)) score += 2;
...
export const processesPhase: PipelinePhase<ProcessesOutput> = {
  ...
  async execute(ctx, _deps) {
    ...
    const symbolCountRow = db
      .prepare(
        `SELECT COUNT(*) as cnt FROM nodes WHERE label NOT IN ('File', 'Folder', 'Community', 'Process', 'Document')`,
      )
      .get() as { cnt: number };
    ...
    const processNodes: MonographNode[] = [];
    const processEdges: MonographEdge[] = [];
