...
export const docsParsePhase: PipelinePhase<DocsParseOutput> = {
  ...
  async execute(ctx, deps) {
    ...
    const docFiles = fileNodes.filter(
      (n) => n.filePath && DOC_EXTENSIONS.has(extname(n.filePath).toLowerCase()),
    );
    ...
    for (const fileNode of docFiles) {
      ...
      if (headings.length === 0) {
        const title =
          ...
          basename(fileNode.filePath!).replace(/\.[^.]+$/, '');
    ...
    for (const ref of deferredRefs as Array<{
      sourceId: string;
      fileId: string;
      target: string;
      isWiki: boolean;
      ...
      if (ref.isWiki) {
        ...
        target =
          fileByRel.get(ref.target.replace(/^\.\//, '')) ??
          fileByName.get(
            basename(ref.target)
              .replace(/\.mdx?$/, '')
              .toLowerCase(),
    ...
    ctx.onProgress?.({
      phase: 'docs-parse',
      message: `Parsed ${docFiles.length} doc files → ${sectionNodes.length} sections`,
    });
