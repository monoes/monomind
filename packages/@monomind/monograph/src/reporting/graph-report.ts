...
function queryCommunities(db: MonographDb): CommunityStat[] {
  ...
  try {
    const labels = db.prepare(`SELECT id, label FROM communities`).all() as {
      id: number;
      label: string | null;
    }[];
...
export function buildMarkdownWithQuestions(
  ...
  for (const q of capped) {
    if (q.type === 'bridge_node') {
      lines.push(
        `- **bridge_node**: \`${q.node.name}\` bridges community ${q.commA} and ${q.commB}`,
      );
    } else if (q.type === 'ambiguous_edge') {
      lines.push(
        `- **ambiguous_edge**: \`${q.edge.sourceId}\` → \`${q.edge.targetId}\` — ${q.reason}`,
      );
    } else if (q.type === 'verify_inferred') {
      lines.push(
        `- **verify_inferred**: \`${q.edge.sourceId}\` → \`${q.edge.targetId}\` (inferred from ${q.inferredFrom})`,
      );
      ...
      lines.push(
        `- **low_cohesion**: community ${q.community.id} (cohesion: ${q.community.cohesionScore.toFixed(2)})`,
      );
    }
  }
  lines.push('');
 ...
 */
export function generateGraphReportFromDb(db: MonographDb, outputPath: string): GraphReportResult {
