...
function parseByKind(kind, content) {
    switch (kind) {
        case 'dockerfile':
...
function parseDockerComposeEntries(content) {
    const entries = [];
    for (const line of content.split('\n')) {
        const t = line.trim();
        if (!t.startsWith('command:') && !t.startsWith('entrypoint:'))
            continue;
...
function parseTomlYamlCommandEntries(content) {
    const entries = [];
    for (const line of content.split('\n')) {
        const t = line.trim();
        if (!/^(command|entrypoint)\s*[=:]/.test(t))
            continue;
        for (const token of t.split(/\s+/)) {
            if (JSTS.test(token))
                entries.push(token);
...
function probeDir(dir, seen, results) {
...
export function detectInfraFiles(projectRoot) {
    const results = [];
    const seen = new Set();
...
export function parseDockerfileEntries(content) {
    const entries = [];
    for (const line of content.split('\n')) {
        ...
        if (jsonMatch) {
            try {
                const parts = JSON.parse(`[${jsonMatch[1]}]`);
                for (const part of parts) {
                    if (typeof part === 'string' && JSTS.test(part))
                        entries.push(part);
                }
            }
            catch {
...
export function parseProcfileEntries(content) {
    const entries = [];
    for (const line of content.split('\n')) {
        const trimmed = line.trim();
...
//# sourceMappingURL=infrastructure-entries.js.map