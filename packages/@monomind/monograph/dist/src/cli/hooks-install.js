import { existsSync, writeFileSync, chmodSync, readdirSync, readFileSync, unlinkSync, } from 'fs';
...
function buildHookBlock(hookName) {
    const verb = hookName === 'post-merge' || hookName === 'post-checkout'
 ...
 */
export function installGitHooks(repoPath, hooks) {
 ...
 */
export function uninstallGitHooks(repoPath, hooks) {
    const hooksDir = getHooksDir(repoPath);
    for (const hook of hooks) {
        if (!/^[\w-]+$/.test(hook)) {
            throw new Error(`Invalid hook name: "${hook}" — must be alphanumeric with hyphens only`);
        ...
        if (!original.includes(HOOK_MARKER_START))
            continue;
 ...
 */
export function listInstalledHooks(repoPath) {
    const hooksDir = getHooksDir(repoPath);
    if (!existsSync(hooksDir))
        return [];
    const installed = [];
