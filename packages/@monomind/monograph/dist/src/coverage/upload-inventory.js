...
export function parseGitRemoteToProjectId(url) {
    const match = url.match(/[:/]([^/]+\/[^/]+?)(?:\.git)?$/);
    return match ? match[1].replace('/', '_') : 'unknown';
}
/** Extract function inventory from a source text (heuristic, no full AST). */
export function extractFunctionInventory(source, filePath) {
    const results = [];
...
export async function uploadInventory(args, functions) {
...
//# sourceMappingURL=upload-inventory.js.map