...
export function checkStaleness(db, repoPath) {
    ...
    let staleSince = null;
    try {
        const firstCommit = execSync(`git log --format="%ai" ${indexedCommitFull}..HEAD --reverse --max-count=1`, { cwd: repoPath, encoding: 'utf8' }).trim();
