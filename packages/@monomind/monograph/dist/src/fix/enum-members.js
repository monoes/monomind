...
export function declaresExportedEnum(line) {
    ...
    if (!line.match(/\bexport\b/))
        return null;
    return { name };
}
export function findEnumDeclarationRange(lines, enumName) {
...
export function isEnumBodyEmpty(lines, range) {
    const body = lines.slice(range[0] + 1, range[1]).join('\n');
    return body.replace(/[,\s]/g, '').length === 0;
}
export function removeEnumMember(source, memberName, enumName) {
...
export function fixEnumMembers(fixes) {
...
//# sourceMappingURL=enum-members.js.map