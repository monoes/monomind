export async function runFix(results, opts, appliers = {}) {
    ...
    if (total === 0) {
        if (opts.output === 'json')
            console.log(JSON.stringify({ dryRun: opts.dryRun, fixes: [], total_fixed: 0 }));
        else if (!opts.quiet)
            console.error('No issues to fix.');
        return { exitCode: 0 };
    }
    const fixes = [];
    let hadError = false;
    ...
    if (opts.output === 'json') {
        ...
        if (opts.dryRun)
            console.error('Dry run complete. No files were modified.');
        else
            console.error(`Fixed ${fixes.filter(f => f.applied).length} issue(s).`);
    }
