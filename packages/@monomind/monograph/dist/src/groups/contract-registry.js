 ...
 */
export function extractHttpContracts(db, repoName) {
 ...
 */
export function buildContractLinks(contracts) {
 ...
 */
export function saveContractRegistry(registryPath, links, contracts) {
    ...
    `);
        const insertContract = db.prepare(`INSERT INTO contracts (method, path, handler_name, handler_file, repo)
       VALUES (?, ?, ?, ?, ?)`);
        const insertLink = db.prepare(`INSERT INTO links (path, method, producer_repo, consumer_repos)
       VALUES (?, ?, ?, ?)`);
 ...
 */
export function loadContractRegistry(registryPath) {
    ...
    try {
        ...
        const links = linkRows.map((r) => ({
            ...
            consumerRepos: JSON.parse(r.consumer_repos),
        }));
