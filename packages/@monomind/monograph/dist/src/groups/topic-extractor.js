...
function detectBroker(source) {
    if (/kafka/i.test(source) || /kafkajs/i.test(source))
        return 'kafka';
    if (/amqplib|rabbitmq|channel\./i.test(source))
        return 'rabbitmq';
    if (/SQSClient|aws-sdk.*SQS/i.test(source))
        return 'sqs';
    if (/@google-cloud\/pubsub/i.test(source))
        return 'pubsub';
    return 'unknown';
}
export function extractTopicContracts(source, filePath) {
    const results = [];
