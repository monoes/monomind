...
function parseFrontmatter(text) {
    ...
    if (!fmMatch)
        return { meta: {}, body: text, lineOffset: 0 };
    ...
    for (const line of yaml.split('\n')) {
        ...
        if (itemMatch && currentKey && Array.isArray(meta[currentKey])) {
            meta[currentKey].push(itemMatch[1].trim().replace(/^["']|["']$/g, ''));
            continue;
        }
...
// ── Heading extraction ────────────────────────────────────────────────────────
function extractHeadings(body, lineOffset) {
    const lines = body.split('\n');
    const positions = [];
    for (let i = 0; i < lines.length; i++) {
        const m = lines[i].match(/^(#{1,6})\s+(.+?)(?:\s+#+)?$/);
        if (m)
            positions.push({ level: m[1].length, title: m[2].trim(), idx: i });
    }
...
function extractLinks(text) {
    ...
    for (const m of text.matchAll(/\[[^\]]+\]\(([^)]+\.mdx?(?:#[^)]*)?)\)/g)) {
        ...
        if (!href.startsWith('http'))
            links.push({ target: href, isWiki: false });
    }
    return links;
}
// ── Tag extraction ────────────────────────────────────────────────────────────
function extractTags(text, meta) {
    ...
    for (const key of ['tags', 'tag', 'categories', 'category']) {
        const v = meta[key];
        if (Array.isArray(v))
            v.forEach(t => typeof t === 'string' && tags.add(t.toLowerCase()));
...
// ── Phase ─────────────────────────────────────────────────────────────────────
export const docsParsePhase = {
    name: 'docs-parse',
    deps: ['structure'],
    async execute(ctx, deps) {
        // Respect codeOnly flag
        if (ctx.options.codeOnly)
            return { sectionNodes: [], docFiles: 0 };
        ...
        if (docFiles.length === 0)
            return { sectionNodes: [], docFiles: 0 };
        const fileByRel = new Map(docFiles.map(n => [n.filePath, n]));
        const fileByName = new Map(docFiles.map(n => [basename(n.filePath).replace(/\.mdx?$/, '').toLowerCase(), n]));
        const sectionNodes = [];
        ...
        for (const fileNode of docFiles) {
            ...
            try {
                ...
                if (st.size > ctx.options.maxFileSizeBytes)
                    continue;
            ...
            if (headings.length === 0) {
                const title = meta['title'] ?? basename(fileNode.filePath).replace(/\.[^.]+$/, '');
                ...
                for (const lnk of extractLinks(body))
                    deferredRefs.push({ sourceId: sectionId, fileId: fileNode.id, ...lnk });
                continue;
            }
            ...
            for (const h of headings) {
                ...
                for (const lnk of extractLinks(h.content))
                    deferredRefs.push({ sourceId: sectionId, fileId: fileNode.id, ...lnk });
            }
            ...
            if (preamble && firstSectionId) {
                ...
                for (const lnk of extractLinks(preamble))
                    deferredRefs.push({ sourceId: firstSectionId, fileId: fileNode.id, ...lnk });
            }
        ...
        for (const ref of deferredRefs) {
            ...
            if (ref.isWiki) {
                ...
                if (!target)
                    target = fileByName.get(normTarget);
            }
            else {
        ...
        if (conceptNodes.size > 0)
            insertNodes(ctx.db, [...conceptNodes.values()]);
...
function wireTagEdges(sourceId, tags, conceptNodes, edges) {
