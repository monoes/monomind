 ...
 */
export function extractMiddlewareChain(source, handlerName) {
    ...
    const isNextMiddleware = /export\s+(?:default\s+)?(?:async\s+)?function\s+middleware\b/.test(snippet) &&
 ...
 */
function extractNestedWrappers(snippet, handlerName) {
    const wrappers = [];
 ...
 */
function extractArrayMiddleware(snippet, handlerName) {
    // Match: something.method('path', ...args, handler  — capture everything before handler
    const re = new RegExp(`\\.(?:get|post|put|delete|patch|use)\\s*\\(\\s*['"][^'"]*['"]\\s*,\\s*([^)]+?)\\s*\\b${escapeRe(handlerName)}\\b`, 'i');
    const m = re.exec(snippet);
    if (!m)
        return [];
    const before = m[1];
...
//# sourceMappingURL=middleware-extractor.js.map