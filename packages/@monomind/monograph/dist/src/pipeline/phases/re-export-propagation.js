 ...
 */
export function propagateReExports(db) {
    // Collect all RE_EXPORTS edges (barrel file → re-exported file), keyed by File node IDs
    const reExportEdges = db.prepare(`SELECT source_id, target_id FROM edges WHERE relation = 'RE_EXPORTS'`).all();
    ...
    for (const e of reExportEdges) {
        if (!barrelExports.has(e.source_id))
            barrelExports.set(e.source_id, new Set());
    ...
    const reachedFilePaths = new Set(db.prepare(`
    ...
    const fileIdToPath = new Map(db.prepare(`SELECT id, file_path FROM nodes WHERE label = 'File' AND file_path IS NOT NULL`).all().map(r => [r.id, r.file_path]));
    // Map file_path → File node ID for inserting synthetic IMPORTS edges
    const filePathToId = new Map();
    for (const [id, path] of fileIdToPath)
        filePathToId.set(path, id);
  ...
  `);
    const addPropagated = db.transaction((toAdd) => {
...
export const reExportPropagationPhase = {
...
//# sourceMappingURL=re-export-propagation.js.map