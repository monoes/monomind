...
// ── Detectors ─────────────────────────────────────────────────────────────────
function detectTypeOrmEntities(source, filePath, language, entities, entityNodes, fieldNodes, edges) {
...
function detectPrismaEntities(source, filePath, language, entities, entityNodes, fieldNodes, edges) {
    ...
    while ((m = modelRe.exec(source)) !== null) {
        ...
        while ((fm = fieldRe.exec(body)) !== null) {
            ...
            if (['model', 'enum', 'type', 'datasource', 'generator'].includes(fieldName))
                continue;
...
function detectMongooseEntities(source, filePath, language, entities, entityNodes, fieldNodes, edges) {
...
function detectSequelizeEntities(source, filePath, language, entities, entityNodes, fieldNodes, edges) {
    // Track entity names we've already registered to avoid duplicates.
    const registeredNames = new Set();
    function registerEntity(entityName) {
        const entityNodeId = makeId('entity', entityName + filePath);
        if (registeredNames.has(entityName))
            return entityNodeId;
    ...
    while ((im = initRe.exec(source)) !== null) {
        ...
        if (!block)
            continue;
        if (!modelClasses.has(candidateName) && !block.includes('DataTypes'))
            continue;
        ...
        while ((fm = fieldRe.exec(block)) !== null) {
            ...
            if (seen.has(fieldName))
                continue;
            if (['type', 'allowNull', 'defaultValue', 'primaryKey', 'unique', 'references'].includes(fieldName))
                continue;
            seen.add(fieldName);
...
function detectSQLAlchemyEntities(source, filePath, language, entities, entityNodes, fieldNodes, edges) {
...
// ── Helpers ───────────────────────────────────────────────────────────────────
function addField(entityName, fieldName, fieldType, filePath, language, entityNodeId, fields, fieldNodes, edges) {
 ...
 */
function extractBraceBlock(source, openPos, maxLen) {
