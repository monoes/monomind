 ...
 */
export function extractWildcardBindings(source) {
    const results = [];
 ...
 */
export function extractWildcardMemberAccesses(source, alias) {
    const results = [];
 ...
 */
function resolveSpecifierPath(sourceFilePath, moduleSpecifier) {
    if (!sourceFilePath)
        return null;
 ...
 */
function specifierMatchesFilePath(resolvedPath, candidateFilePath) {
    // Strip extension from candidate for comparison
    const candidateBase = candidateFilePath.replace(/\.[^./\\]+$/, '');
    if (candidateBase === resolvedPath)
        return true;
    // Handle implicit /index
    if (candidateBase === resolvedPath + '/index')
        return true;
    return false;
}
/**
 ...
 */
export function synthesizeWildcardImports(sourceFileId, source, nodes, edges) {
    ...
    for (const binding of bindings) {
        ...
        for (const access of accesses) {
            const candidateNodes = nameIndex.get(access.member) ?? nameIndex.get(access.member.toLowerCase()) ?? [];
            // Filter candidates to only those belonging to the target module
            let filtered;
            if (resolvedPath !== null) {
                // Strong filter: match by resolved absolute path
                filtered = candidateNodes.filter(n => n.filePath != null && specifierMatchesFilePath(resolvedPath, n.filePath));
            }
            else {
                // Fallback: match by basename (mirrors cross-file.ts resolution)
                filtered = candidateNodes.filter(n => n.filePath != null &&
...
//# sourceMappingURL=wildcard-synthesis.js.map