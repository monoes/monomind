...
export const llmExtractPhase = {
    ...
    async execute(ctx) {
      ...
      `)
        ...
        const existingConcepts = new Set(ctx.db.prepare(`SELECT id FROM nodes WHERE label = 'Concept'`).all()
            .map(r => r.id));
        for (const section of sections) {
            const props = JSON.parse(section.properties);
            ...
            for (const triple of triples) {
                ...
                if (!node_1 || !node_2 || !relation)
                    continue;
                const rel = relation.toUpperCase();
                if (!ALLOWED_RELATIONS.has(rel))
                    continue;
                ...
                for (const [cId, cName] of [[c1Id, n1], [c2Id, n2]]) {
                ...
                if (!seenEdges.has(e2Id)) {
                    ...
                    allEdges.push({
                        ...
                        relation: rel, confidence: 'INFERRED', confidenceScore: CONFIDENCE_SCORE.INFERRED,
                        weight: 1,
                    });
                }
            }
