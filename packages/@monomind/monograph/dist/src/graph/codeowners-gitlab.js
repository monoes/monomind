...
export function parseCodeownersWithSections(content) {
    const entries = [];
    let currentSection;
    let sectionDefaults = [];
...
export function matchOwners(entries, filePath) {
    ...
    for (let i = entries.length - 1; i >= 0; i--) {
        ...
        if (pathMatchesPattern(filePath, entry.pattern)) {
            if (entry.negated)
                return { owners: [], section: entry.section };
...
function pathMatchesPattern(filePath, pattern) {
    ...
    if (pat === '**/*')
        return true;
    if (pat.startsWith('**'))
        return norm.endsWith(pat.slice(2));
    if (pat.startsWith('/'))
        return norm === pat.slice(1) || norm.startsWith(pat.slice(1) + '/');
    if (pat.endsWith('/'))
...
export function sectionOf(entries, filePath) {
    return matchOwners(entries, filePath).section ?? NO_SECTION_LABEL;
}
export function sectionAndOwnersOf(entries, filePath) {
    const { owners, section } = matchOwners(entries, filePath);
