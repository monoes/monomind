...
export function computeMaintainabilityIndex(db) {
  ...
  `).all();
    ...
    for (const node of nodes) {
        ...
        const inDegreeRow = db.prepare('SELECT COUNT(*) as c FROM edges WHERE target_id = ?').get(node.id);
        const outDegreeRow = db.prepare('SELECT COUNT(*) as c FROM edges WHERE source_id = ?').get(node.id);
        ...
        db.prepare('UPDATE nodes SET properties = ? WHERE id = ?').run(JSON.stringify(props), node.id);
        results.push({
            nodeId: node.id,
            name: node.name,
