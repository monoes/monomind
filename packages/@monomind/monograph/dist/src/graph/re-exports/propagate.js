export function propagateStarReExport(modules, edges, edgesByTarget, barrelId, barrelIdx, sourceIdx, entryStarTargets) {
    ...
    if (changed && entryStarTargets.has(barrelId)) {
        ...
        for (const ei of upstreamEdgeIndices) {
            ...
            if (edge.importedName === undefined) {
                propagateStarReExport(modules, edges, edgesByTarget, modules[edge.targetIdx].fileId, edge.targetIdx, barrelIdx, entryStarTargets);
            }
        }
    }
    return changed;
}
export function propagateNamedReExport(modules, barrelId, barrelIdx, sourceIdx, importedName, exportedName, existingRefs) {
    const source = modules[sourceIdx];
    const barrel = modules[barrelIdx];
    ...
    for (const refId of existingRefs) {
        ...
        if (refIdx !== -1) {
            ...
            const alreadyRefs = refMod.references.some(r => r.name === exportedName && r.fromFile === barrel.filePath);
            if (!alreadyRefs) {
