 ...
 */
export function pruneDanglingEdges(db) {
 ...
 */
export function pruneOrphanNodes(db) {
    const result = db.prepare(`
    DELETE FROM nodes
    WHERE label != 'File'
 ...
 */
export function pruneStaleFileNodes(db, existingPaths) {
    const staleRows = db.prepare(`SELECT id, file_path FROM nodes WHERE label = 'File' AND file_path IS NOT NULL`).all();
    let removed = 0;
 ...
 */
export function pruneAll(db, existingPaths) {
...
export function formatPruneResult(result) {
    ...
    if (total === 0)
        return 'Prune pass: nothing to remove — graph is clean.';
    return [
