...
export const monographResources = [
    {
    ...
    },
    {
        ...
        handler: async (input) => {
            ...
            try {
                ...
                if (results.length === 0)
                    return text('No nodes found matching the given property value.');
                const lines = results.map(r => `[${r.label}] ${r.name}  ${r.filePath ?? ''}  (${ident}=${JSON.stringify(r.propertyValue)})`);
