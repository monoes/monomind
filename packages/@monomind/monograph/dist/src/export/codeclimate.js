...
function makeIssue(checkName, description, categories, severity, filePath, line, repoRoot) {
 ...
 */
export function exportCodeClimate(db, repoRoot) {
    const issues = [];
  ...
  `).all(top10pct);
    for (const row of godNodes) {
        if (row.in_degree === 0)
            continue;
        issues.push(makeIssue('monograph/god-node', `God node: "${row.name}" has ${row.in_degree} incoming dependencies.`, ['Complexity'], 'major', row.file_path, row.line ?? 1, repoRoot));
    }
    // ── Unreachable files ──────────────────────────────────────────────────────
  ...
  `).all();
    for (const row of unreachable) {
        issues.push(makeIssue('monograph/unreachable-file', `Unreachable file: "${row.name}" is not reachable from any entry point.`, ['Duplication'], 'minor', row.file_path, 1, repoRoot));
    }
  ...
  `).all();
    for (const row of dupes) {
        issues.push(makeIssue('monograph/duplicate', `Duplicate: "${row.src_name}" is structurally similar to another file.`, ['Duplication'], 'minor', row.src_path, row.src_line ?? 1, repoRoot));
    }
    return issues;
}
...
export function exportHealthCodeClimate(findings) {
...
export function exportDuplicationCodeClimate(findings) {
    return findings.map(f => ({
        type: 'issue',
