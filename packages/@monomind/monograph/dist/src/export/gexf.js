 ...
 */
export function toGexf(nodes, edges) {
    const nodeXml = nodes
        .map(n => `    <node id="${esc(n.id)}" label="${esc(n.name)}">` +
        `<attvalues>` +
        ...
        .join('\n');
    const edgeXml = edges
        .map(e => `    <edge id="${esc(e.id)}" source="${esc(e.sourceId)}" target="${esc(e.targetId)}" label="${esc(e.relation)}" weight="${e.confidenceScore}"/>`)
        .join('\n');
