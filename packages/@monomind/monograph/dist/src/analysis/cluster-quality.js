...
function nodeSilhouette(nodeId, community, communityMembers, adj) {
    ...
    const a = sameMembers.reduce((sum, nid) => sum + (neighbors.has(nid) ? 0 : 1), 0) /
        sameMembers.length;
    // b(i): minimum mean distance to any other community
    let b = Infinity;
    for (const [cid, members] of communityMembers) {
        if (cid === community || members.length === 0)
            continue;
        const meanDist = members.reduce((sum, nid) => sum + (neighbors.has(nid) ? 0 : 1), 0) /
            members.length;
        if (meanDist < b)
            b = meanDist;
    }
    if (!isFinite(b))
        return 0; // only one community exists
 ...
 */
export function modularityScore(memberships, edges) {
    if (edges.length === 0 || memberships.size === 0)
        return 0;
    const adj = buildAdjacency(edges);
    ...
    const communityDegreeSum = new Map();
    const intraCommunityEdges = new Map();
...
//# sourceMappingURL=cluster-quality.js.map