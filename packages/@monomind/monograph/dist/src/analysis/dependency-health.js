 ...
 */
export function formatDependencyHealth(result) {
    ...
    const advice = [];
    if (details.cyclePenalty > 0.1)
        advice.push('reduce circular imports (cyclePenalty high)');
    if (details.deadCodeRatio > 0.1)
        advice.push('remove unreachable exports (deadCodeRatio high)');
    if (details.fanSkew > 0.25)
        advice.push('break up high-fan-out modules (fanSkew high)');
