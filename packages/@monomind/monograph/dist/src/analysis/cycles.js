 ...
 */
function findCyclesInSCC(scc, adjacency, maxCycles) {
...
export function detectCycles(db) {
    // Load all File nodes
    const nodeRows = db.prepare(`SELECT id, file_path, community_id FROM nodes WHERE label = 'File'`).all();
    const nodeMap = new Map();
    for (const row of nodeRows) {
        nodeMap.set(row.id, row);
    }
    // Load all IMPORTS and RE_EXPORTS edges between File nodes
    const edgeRows = db.prepare(`SELECT e.source_id, e.target_id, e.relation
     FROM edges e
    ...
    const edgeRelationMap = new Map(); // "src\0tgt" -> relation
    for (const row of nodeRows) {
    ...
    const cycles = [];
    const filesInCyclesSet = new Set();
