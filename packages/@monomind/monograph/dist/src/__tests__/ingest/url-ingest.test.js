...
describe('ingestUrl', () => {
    it('returns IngestResult with id, type, and url', async () => {
        const mockFetch = vi.fn().mockResolvedValue({
            ...
            headers: { get: (_h) => 'text/html' },
        });
        const result = await ingestUrl('https://example.com/page', { fetch: mockFetch });
    ...
    it('blocks oversized responses', async () => {
        ...
        const mockFetch = vi.fn().mockResolvedValue({
            ok: true,
            text: async () => bigBody,
            headers: { get: (h) => h === 'content-length' ? String(bigBody.length) : 'text/html' },
        });
        await expect(ingestUrl('https://example.com/big', { fetch: mockFetch, maxBytes: 10 * 1024 * 1024 })).rejects.toThrow(/too large/i);
    });
});
