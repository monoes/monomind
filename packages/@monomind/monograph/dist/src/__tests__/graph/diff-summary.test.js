...
function edge(sourceId, targetId) {
    return { id: `${sourceId}->${targetId}`, sourceId, targetId, relation: 'imports' };
}
describe('GraphDiff.summary', () => {
    it('includes summary string in diff result', () => {
        const before = { nodes: [node('a'), node('b')], edges: [], capturedAt: '' };
        const after = { nodes: [node('a'), node('b'), node('c')], edges: [edge('a', 'c')], capturedAt: '' };
        const diff = diffSnapshots(before, after);
        expect(typeof diff.summary).toBe('string');
    });
    it('reports new nodes in summary', () => {
        const before = { nodes: [node('a')], edges: [], capturedAt: '' };
        const after = { nodes: [node('a'), node('b'), node('c')], edges: [], capturedAt: '' };
        const diff = diffSnapshots(before, after);
        expect(diff.summary).toMatch(/2 new node/);
    });
    it('reports removed nodes in summary', () => {
        const before = { nodes: [node('a'), node('b')], edges: [], capturedAt: '' };
        const after = { nodes: [node('a')], edges: [], capturedAt: '' };
        const diff = diffSnapshots(before, after);
        expect(diff.summary).toMatch(/1 node removed/);
    });
    it('reports new edges in summary', () => {
        const before = { nodes: [node('a'), node('b')], edges: [], capturedAt: '' };
        const after = { nodes: [node('a'), node('b')], edges: [edge('a', 'b')], capturedAt: '' };
        const diff = diffSnapshots(before, after);
        expect(diff.summary).toMatch(/1 new edge/);
    });
    it('reports removed edges in summary', () => {
        const before = { nodes: [node('a'), node('b')], edges: [edge('a', 'b')], capturedAt: '' };
        const after = { nodes: [node('a'), node('b')], edges: [], capturedAt: '' };
        const diff = diffSnapshots(before, after);
        expect(diff.summary).toMatch(/1 edge removed/);
    });
    it('returns "no changes" when graph is unchanged', () => {
        const snap = { nodes: [node('a')], edges: [], capturedAt: '' };
