...
function makeEdge(id, src, tgt, overrides = {}) {
    return {
        id,
        sourceId: src,
        targetId: tgt,
...
describe('mergeGraphIntoDb', () => {
    ...
    it('skips existing nodes with default strategy', () => {
        ...
        const row = db.prepare('SELECT name FROM nodes WHERE id = ?').get('a');
