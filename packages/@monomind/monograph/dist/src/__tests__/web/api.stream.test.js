...
describe('streamGraph', () => {
    it('calls onRecord for each node and edge', async () => {
        ...
        expect(records.some((r) => r.type === 'node')).toBe(true);
        expect(records.some((r) => r.type === 'edge')).toBe(true);
    });
    it('each record has a type field', async () => {
        ...
        for (const r of records) {
            expect(['node', 'edge']).toContain(r.type);
        }
    });
});
//# sourceMappingURL=api.stream.test.js.map