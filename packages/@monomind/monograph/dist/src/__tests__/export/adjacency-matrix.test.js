...
import { buildAdjacencyMatrix, buildAdjacencyMatrixFromDb, adjacencyMatrixToCsv, } from '../../export/adjacency-matrix.js';
...
describe('buildAdjacencyMatrix', () => {
    ...
    it('counts multi-edges between the same pair', () => {
        const nodes = [makeNode('a'), makeNode('b')];
        const e1 = { ...makeEdge('a', 'b'), id: 'e1' };
        const e2 = { ...makeEdge('a', 'b'), id: 'e2' };
...
describe('adjacencyMatrixToCsv', () => {
    ...
    it('escapes double quotes in node names', () => {
        const node = { id: 'x', label: 'Function', name: 'say "hello"', normLabel: 'say hello', isExported: false };
        const am = buildAdjacencyMatrix([node], []);
        const csv = adjacencyMatrixToCsv(am);
        expect(csv).toContain('""hello""');
    });
});
//# sourceMappingURL=adjacency-matrix.test.js.map