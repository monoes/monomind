...
export function getCommunitiesResource(db) {
    ...
    const communityRows = db
        .prepare(`SELECT n.community_id, c.label, COUNT(*) AS member_count
       FROM nodes n
       ...
       ORDER BY member_count DESC`)
        .all();
    const topMembersQuery = db.prepare(`SELECT name, label, file_path
     FROM nodes
     WHERE community_id = ?
     ORDER BY name
     LIMIT 5`);
    const communities = communityRows.map((row) => {
        const memberRows = topMembersQuery.all(row.community_id);
...
//# sourceMappingURL=communities-resource.js.map