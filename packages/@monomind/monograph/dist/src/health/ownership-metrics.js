...
export function filterSuggestedReviewers(contributors, topContributor, maxStaleDays = 90) {
    return contributors
        .filter(c => c.identifier !== topContributor.identifier && c.staleDays <= maxStaleDays)
        .slice(0, 3);
}
export function formatOwnershipMetrics(m) {
    const lines = [
    ...
    if (m.declaredOwner)
        lines.push(`Owner: ${m.declaredOwner}`);
    if (m.unowned)
        lines.push('(unowned — no CODEOWNERS rule matched)');
    if (m.drift)
        lines.push(`Drift: ${m.driftReason ?? 'unknown reason'}`);
    return lines.join('\n');
}
//# sourceMappingURL=ownership-metrics.js.map