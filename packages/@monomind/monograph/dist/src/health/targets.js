...
export function computeThresholds(files) {
    if (files.length === 0) {
...
function computePriority(file, thresholds) {
...
function effortForFile(file, thresholds) {
    const lines = file.lineCount ?? 0;
    if (lines >= 500 || file.fanIn >= thresholds.fanInP95)
        return 'High';
    if (lines < 100 && file.fanIn < thresholds.fanInP25)
        return 'Low';
    return 'Medium';
}
function effortNumeric(effort) {
    switch (effort) {
        case 'Low': return 1;
...
function matchRule(file, thresholds) {
    const churn = file.churnScore ?? 0;
    const complexity = file.complexity ?? 0;
    const deadCode = file.deadCodeRatio ?? 0;
    const coverage = file.coveragePct ?? 100;
    if (churn >= 50 && complexity > 0.5)
        return 'UrgentChurnComplexity';
    if (file.inCycle && file.fanIn >= 5)
        return 'BreakCircularDependency';
    if (complexity > 0.3 && file.fanIn >= thresholds.fanInP95)
        return 'SplitHighImpact';
    if (deadCode > 0.3)
        return 'RemoveDeadCode';
    if (complexity > 0.3)
        return 'ExtractComplexFunctions';
    if (file.fanOut > thresholds.fanOutP95)
        return 'ExtractDependencies';
    if (coverage < 50)
        return 'AddTestCoverage';
    if (file.inCycle)
        return 'BreakCircularDependency';
    return null;
}
export function computeRefactoringTargets(files, thresholds, maxTargets = 20) {
    const results = [];
