...
export function extractSymbols(tree, source, repoPath, config, ext) {
    const nodes = [];
    const edges = [];
    const parseErrors = [];
    const language = config.name;
    function nodeId(name, filePath, extra) {
...
function isNodeExported(node, _source) {
...
function handleImport(node, fileNodeId, source, config, edges, repoPath) {
...
function handleInheritance(node, nodeId, edges, _repoPath, _config, _source) {
    for (let i = 0; i < node.childCount; i++) {
