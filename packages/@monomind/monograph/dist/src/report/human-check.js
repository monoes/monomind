...
const CYAN = '\x1b[36m';
function col(c, t) { return `${c}${t}${RESET}`; }
function dim(t) { return col(DIM, t); }
function bold(t) { return col(BOLD, t); }
const MAX_FLAT = 10;
const MAX_GROUPED_FILES = 10;
const MAX_ITEMS_PER_FILE = 5;
const SCOPING_THRESHOLD = 500;
function truncHint(remaining, total) {
...
function sectionHeader(title, count, level = 'error') {
...
export function formatUnusedFiles(files, opts = {}) {
    if (files.length === 0)
        return [];
    const max = opts.maxFlatItems ?? opts.top ?? MAX_FLAT;
    const total = files.length;
    const lines = [sectionHeader('Unused files', total)];
...
export function formatUnusedExports(exports, opts = {}) {
    if (exports.length === 0)
        return [];
    ...
    const lines = [sectionHeader('Unused exports', total)];
...
export function formatUnusedDeps(deps) {
    if (deps.length === 0)
        return [];
    const lines = [sectionHeader('Unused dependencies', deps.length)];
    for (const d of deps) {
...
export function formatUnusedMembers(members, title = 'Unused class members') {
    if (members.length === 0)
        return [];
    const total = members.length;
    const lines = [sectionHeader(title, total)];
...
export function formatUnresolvedImports(imports) {
    if (imports.length === 0)
        return [];
    const total = imports.length;
    const lines = [sectionHeader('Unresolved imports', total)];
...
export function buildCheckHumanLines(results, opts = {}) {
    const lines = [];
    ...
    const hasUnusedCode = results.unusedFiles.length > 0 ||
        filteredExports.length > 0 ||
        filteredTypes.length > 0 ||
        results.unusedEnumMembers.length > 0 ||
        results.unusedClassMembers.length > 0;
    const hasDeps = results.unusedDependencies.length > 0 ||
    ...
    const hasStructure = results.duplicateExports.length > 0 ||
    ...
    if (hasDeps) {
        ...
        if (results.unusedDevDependencies.length > 0) {
            ...
            if (devLines.length > 0)
                devLines[0] = sectionHeader('Unused devDependencies', results.unusedDevDependencies.length);
            lines.push(...devLines);
        }
...
//# sourceMappingURL=human-check.js.map