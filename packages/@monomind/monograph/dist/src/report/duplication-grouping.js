...
export function defaultOwnerResolver(filePath) {
    const norm = filePath.replace(/\\/g, '/');
    const parts = norm.split('/');
    return parts.length > 1 ? parts[parts.length - 2] : '.';
}
export function buildDuplicationGrouping(groups, resolver = defaultOwnerResolver) {
    ...
    for (const group of groups) {
        for (const inst of group.instances) {
            ...
            if (!entry.filePaths.includes(inst.filePath))
                entry.filePaths.push(inst.filePath);
...
export function formatDuplicationGrouping(grouping) {
...
//# sourceMappingURL=duplication-grouping.js.map