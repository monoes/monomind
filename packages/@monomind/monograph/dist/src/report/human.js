...
export function buildDeadCodeHumanLines(findings, root = '') {
    if (findings.length === 0)
        return [col(GREEN, '✓ No dead code found.')];
    const lines = [col(BOLD, `Found ${findings.length} dead code issue(s):`)];
    const MAX = 50;
    ...
    if (findings.length > MAX)
        lines.push(col(DIM, `  ... and ${findings.length - MAX} more`));
    return lines;
}
export function buildHealthHumanLines(findings, root = '') {
    if (findings.length === 0)
        return [col(GREEN, '✓ All functions within complexity thresholds.')];
    const lines = [col(BOLD, `Found ${findings.length} complex function(s):`)];
    const MAX = 30;
    for (const f of findings.slice(0, MAX)) {
        const rel = f.filePath.replace(root, '').replace(/^\//, '');
...
export function buildDuplicationHumanLines(groups, root = '') {
    if (groups.length === 0)
        return [col(GREEN, '✓ No code duplication detected.')];
    const lines = [col(BOLD, `Found ${groups.length} clone group(s):`)];
...
export function buildExportTraceHumanLines(trace) {
    const lines = [col(BOLD, `Export trace: ${col(CYAN, trace.exportName)} in ${DIM}${trace.filePath}${RESET}`)];
...
export function buildFileTraceHumanLines(trace) {
    const lines = [col(BOLD, `File trace: ${DIM}${trace.filePath}${RESET}`)];
...
export function buildDependencyTraceHumanLines(trace) {
    const lines = [col(BOLD, `Dependency trace: ${col(CYAN, trace.packageName)}`)];
