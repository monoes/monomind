...
function vitalStatus(value, warnThreshold, critThreshold, higherIsBetter = false) {
    if (higherIsBetter) {
...
function formatVitalLine(label, status, detail) {
    const icon = statusIcon(status);
    const suffix = detail ? ` (${detail})` : '';
    return `  ${icon} ${label}: ${status}${suffix}`;
}
export function formatVitalSignsSection(vitals) {
    const lines = [];
    lines.push('\n── Vital Signs ──');
    const complexityStatus = vitalStatus(vitals.avgCyclomatic, 1.5, 5.5);
    lines.push(formatVitalLine('Avg Complexity', complexityStatus, complexityStatus !== 'OK' ? `avg ${vitals.avgCyclomatic.toFixed(1)}, threshold 1.5` : undefined));
    const p90Status = vitalStatus(vitals.p90Cyclomatic, 10, 20);
    lines.push(formatVitalLine('P90 Complexity', p90Status, p90Status !== 'OK' ? `p90 ${vitals.p90Cyclomatic}, threshold 10` : undefined));
    const maintStatus = vitalStatus(vitals.maintainabilityAvg, 70, 40, true);
    lines.push(formatVitalLine('Maintainability', maintStatus, maintStatus !== 'OK' ? `avg ${vitals.maintainabilityAvg.toFixed(1)}, threshold 70` : undefined));
    ...
    lines.push(formatVitalLine('Dead Exports', deadExportStatus, `${vitals.deadExportPct.toFixed(1)}%`));
    const hotspotStatus = vitals.hotspotCount > 10 ? 'CRITICAL' : vitals.hotspotCount > 3 ? 'WARNING' : 'OK';
    lines.push(formatVitalLine('Hotspots', hotspotStatus, `${vitals.hotspotCount} hotspot${vitals.hotspotCount !== 1 ? 's' : ''}`));
    ...
    lines.push(formatVitalLine('High Coupling', couplingStatus, `${vitals.couplingHighPct.toFixed(1)}%`));
    return lines;
}
function computeHotspotScore(score) {
    let s = 0;
    if (score.complexityDensity !== undefined)
        s += score.complexityDensity * 30;
    if (score.crapMax !== undefined)
        s += Math.min(score.crapMax / 100, 1) * 25;
    if (score.deadCodeRatio !== undefined)
        s += score.deadCodeRatio * 20;
    if (score.fanIn !== undefined)
        s += Math.min(score.fanIn / 50, 1) * 15;
    if (score.maintainabilityIndex !== undefined)
        s += Math.max(0, (100 - score.maintainabilityIndex) / 100) * 10;
    return Math.min(s, 100);
}
export function formatHotspotSection(scores, limit = DEFAULT_HOTSPOT_LIMIT) {
    if (scores.length === 0)
        return [];
    const sorted = [...scores]
...
export function formatCoverageGapSection(gaps, limit = DEFAULT_COVERAGE_GAP_LIMIT) {
    if (gaps.length === 0)
        return [];
...
export function buildHealthHumanLines(scores, vitals, opts = {}) {
    const lines = [];
    lines.push('\n── Health Report ──');
...
//# sourceMappingURL=human-health.js.map