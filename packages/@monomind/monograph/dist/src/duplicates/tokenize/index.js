...
export function tokenizeFile(filePath, source, skipImports) {
    const ext = path.extname(filePath).toLowerCase();
    if (STYLE_EXTS.has(ext))
        return emptyTokens(source);
    if (SFC_EXTS.has(ext))
        return tokenizeSfc(source, false, skipImports);
    if (ext === '.astro')
        return tokenizeAstro(source, false, skipImports);
    if (ext === '.mdx')
        return tokenizeMdx(source, false, skipImports);
    return tokenizeJsTs(filePath, source, false, skipImports);
}
export function tokenizeFileCrossLanguage(filePath, source, stripTypes, skipImports) {
    const ext = path.extname(filePath).toLowerCase();
    if (STYLE_EXTS.has(ext))
        return emptyTokens(source);
    if (SFC_EXTS.has(ext))
        return tokenizeSfc(source, stripTypes, skipImports);
    if (ext === '.astro')
        return tokenizeAstro(source, stripTypes, skipImports);
    if (ext === '.mdx')
        return tokenizeMdx(source, stripTypes, skipImports);
    return tokenizeJsTs(filePath, source, stripTypes, skipImports);
}
export function tokenizeSfc(source, stripTypes, skipImports) {
...
export function tokenizeAstro(source, stripTypes, skipImports) {
...
export function tokenizeMdx(source, stripTypes, skipImports) {
...
export function tokenizeJsTs(_filePath, source, _stripTypes, skipImports) {
    ...
    while ((match = tokenRe.exec(source)) !== null) {
        ...
        if (match[2]) {
            ...
            if (word === 'import' || word === 'export')
                ...
                inImport = false;
            if (skipImports && inImport && IMPORT_KWS.has(word))
                continue;
            if (word === 'true' || word === 'false') {
                ...
                tokens.push({ kind: { kind: 'Keyword', kwType: word.charAt(0).toUpperCase() + word.slice(1) }, span });
            }
            else {
