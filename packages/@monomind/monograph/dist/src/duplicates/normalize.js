export function resolveNormalization(mode) {
    switch (mode) {
        case 'Exact':
...
export function normalizeAndHash(tokens, mode) {
    return normalizeAndHashResolved(tokens, resolveNormalization(mode));
}
export function normalizeAndHashResolved(tokens, norm) {
    return tokens.map((tok, i) => ({
        hash: hashTokenResolved(tok.kind, norm),
        originalIndex: i,
    }));
}
export function hashTokenResolved(kind, norm) {
    if (norm.normalizeIdentifiers && kind.kind === 'Identifier')
        return djb2('__ID__');
    if (norm.normalizeLiterals) {
        ...
        if (kind.kind === 'NumericLiteral')
            return djb2('__NUM__');
        if (kind.kind === 'TemplateLiteral')
            return djb2('__TPL__');
    }
    if (norm.normalizeKeywords && kind.kind === 'Keyword')
        return djb2('__KW__');
