...
const fixturePath = join(__dirname, '../fixtures/sample.rb');
...
describe('Ruby parser', () => {
  ...
  beforeAll(async () => {
    ...
    result = await parseFile(fixturePath, source, 'src/sample.rb');
  });
  ...
  it('extracts IMPORTS edges (skipped if grammar unavailable)', () => {
    if (result.nodes.length === 0) return;
