...
The `@monoes/memory` module provides **universal cross-platform support** through intelligent database provider selection. This ensures your application works seamlessly on Windows, macOS, and Linux without code changes.
...
```typescript
import { createDatabase, getPlatformInfo } from '@monoes/memory';
...
```typescript
import { createDatabase } from '@monoes/memory';
...
```typescript
import { createDatabase, getAvailableProviders } from '@monoes/memory';
...
npm install @monoes/memory --no-optional
