...
The v1 memory module now integrates with **lancedb@2.0.0-alpha.3.4** to provide vector search capabilities with pure-JS HNSW indexing (O(log n) approximate nearest neighbor vs O(n) brute-force).

## Features

### LanceDBBackend

The `LanceDBBackend` class provides:
...
- **Hybrid Integration**: Seamlessly works with `HybridBackend` for combined SQLite + LanceDB queries
...
npm install lancedb@2.0.0-alpha.3.4
...
```typescript
import { LanceDBBackend } from "@monomind/memory";
...
// Semantic queries go to LanceDB (HNSW-indexed)
...
if (backend.isAvailable()) {
  console.log("Using LanceDB with HNSW");
} else {
...
```typescript
import { MemoryMigrator } from "@monomind/memory";
...
```
LanceDB not available. Install lancedb@2.0.0-alpha.3.4 for vector search support.
```

**Solution**: Install lancedb:
...
```bash
# Run LanceDB backend tests
npm test -- lancedb-backend.test.ts
...
- **ADR-006**: Unified Memory Service with LanceDB
- **ADR-009**: Hybrid Memory Backend (SQLite + LanceDB) as default

## Related Documentation

- [LanceDB GitHub](https://github.com/nokhodian/agentic-flow/tree/main/packages/lancedb)
