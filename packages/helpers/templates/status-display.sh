...
get_metrics() {
    ...
    if [ -f "$METRICS_DIR/v1-progress.json" ]; then
        DOMAINS=$(jq -r '.domains.completed // 0' "$METRICS_DIR/v1-progress.json" 2>/dev/null || echo "0")
        AGENTS=$(jq -r '.swarm.activeAgents // 0' "$METRICS_DIR/v1-progress.json" 2>/dev/null || echo "0")
        DDD_PROGRESS=$(jq -r '.ddd.progress // 0' "$METRICS_DIR/v1-progress.json" 2>/dev/null || echo "0")
    fi
